// Vendored from @narumitw/pi-goal@0.4.2 (MIT).
// Keep docs/solutions/thnk-21-pi-goal-source-review-2026-06-21.md updated when changing this file.
...
const goalCompleteTool = defineTool({
  ...
  async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
    ...
    ctx.ui.setStatus(
      STATUS_KEY,
      completedGoal ? formatStatus(activeGoal) : undefined,
    );
    clearActiveGoal(ctx);
...
export default function goal(pi: ExtensionAPI) {
  ...
  pi.on("agent_end", async (event, ctx) => {
    ...
    if (
      ...
      ctx.ui.notify(
        `Goal token budget reached: ${formatBudget(activeGoal)}`,
        "warning",
      );
      return;
    }
    ...
    if (hadPendingContinuation) {
      ...
      if (continuationPending?.goalId === goalId)
...
function pauseGoal(ctx: StatusContext) {
  ...
  if (activeGoal.status !== "active") {
    ctx.ui.notify(
      `Goal is ${activeGoal.status}; only active goals can be paused.`,
      "warning",
    );
    return;
  }
...
async function resumeGoal(pi: ExtensionAPI, ctx: StatusContext) {
  ...
  if (
    ...
    ctx.ui.notify(
      `Goal is ${activeGoal.status}; only paused or budget-limited goals can be resumed.`,
      "warning",
    );
    return;
  }
  ...
  if (activeGoal.status !== "active") {
    ctx.ui.notify(
      `Goal token budget is still reached: ${formatBudget(activeGoal)}`,
      "warning",
    );
    return;
  }
...
function pauseGoalAfterAgentEnd(
  ...
  const reason =
    assistant.stopReason === "aborted" ? "interruption" : "agent error";
  const details = assistant.errorMessage
    ? ` (${truncateNotification(assistant.errorMessage)})`
    : "";
  ctx.ui.notify(
    `Goal paused after ${reason}${details}. Run /goal resume to continue.`,
    "warning",
  );
}
...
function updateGoalUsage(goal: ActiveGoal, ctx: StatusContext) {
  ...
  goal.timeUsedSeconds = Math.max(
    0,
    Math.floor((Date.now() - goal.startedAt) / 1000),
  );
...
function parseObjective(
  ...
  if (objectiveTokens.length === 0) {
    return kind === "edit"
      ? "Usage: /goal edit <goal_to_complete>"
      : "Usage: /goal <goal_to_complete>";
  }
...
async function sendContinuationPrompt(
  ...
  if (!sent && continuationPending?.marker === marker)
    continuationPending = undefined;
  return sent;
}
...
export function formatStatus(goal: ActiveGoal | undefined) {
  ...
  if (goal.status === "budget_limited")
    return `🎯 budget ${formatBudget(goal)}`;
...
function cancelContinuationPending() {
  if (continuationPending)
    rememberCancelledContinuationMarker(continuationPending.marker);
  continuationPending = undefined;
}

function rememberCancelledContinuationMarker(marker: string) {
  cancelledContinuationMarkers.add(marker);
  if (cancelledContinuationMarkers.size <= MAX_CANCELLED_CONTINUATION_PROMPTS)
    return;
...
function markContinuationDelivered(prompt: string) {
  const marker = extractContinuationMarker(prompt);
  if (marker && continuationPending?.marker === marker)
    continuationPending = undefined;
}
...
function persistGoal(goal: ActiveGoal) {
  extensionApi?.appendEntry<GoalStateEntryData>(GOAL_STATE_ENTRY_TYPE, {
    goal,
  });
}
...
function loadGoalFromSession(ctx: StatusContext): ActiveGoal | undefined {
  ...
  return isGoal(data?.goal) && data.goal.status !== "complete"
    ? data.goal
    : undefined;
}
...
function readState(): Record<string, unknown> {
  const stateFile = legacyStateFile();
