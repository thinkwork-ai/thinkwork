/**
 * Plan §005 U4 — Aurora-backed `SessionStore` for Pi.
 *
 * Pi persists agent loop state as a `SessionData` blob keyed by `id`. We
 * map that 1:1 onto the existing `threads` table (see
 * `packages/database-pg/src/schema/threads.ts`):
 *
 *   - Pi `sessionId` ↔ `threads.id` (UUID, gen_random_uuid())
 *   - `SessionData` blob ↔ `threads.session_data` (jsonb, NULL when no Pi
 ...
 * `(thread, tenant)`. Pre-Pi threads coexist with `session_data IS NULL`.
 *
 ...
 *   - Session is 1:1 with the (thread, agent) pair (every Pi session
 ...
 *     base table.
 *   - Single writer per thread (one Pi loop at a time), so
 ...
 *     agent since this loop started. Either way Pi's caller sees a typed
 *     error rather than a silent no-op.
 *
 * Connection model:
 *   - Uses AWS RDS Data API (HTTP-based). No VPC plumbing on the agentcore-
 *     pi Lambda is required — the IAM grant in
 *     `terraform/modules/app/agentcore-pi/main.tf` (`rds-data:*` scoped to
...
import type { SessionStore, SessionData } from "./pi-session-types.js";
...
export interface AuroraSessionStoreOptions {
   ...
   * to express the (thread, tenant, agent) tuple Pi's session is keyed
...
export class AuroraSessionStore implements SessionStore {
    ...
    if (!opts.agentId || typeof opts.agentId !== "string") {
      throw new AuroraSessionStoreError(
        ...
        "AuroraSessionStore: agentId is required and must be a non-empty string. " +
          "Pi's session is keyed on the (thread, tenant, agent) tuple — the " +
    ...
    if (!opts.clusterArn || typeof opts.clusterArn !== "string") {
      throw new AuroraSessionStoreError(
        ...
        "AuroraSessionStore: clusterArn is required and must be a non-empty string. " +
          "Wire DB_CLUSTER_ARN in terraform/modules/app/agentcore-pi/main.tf and " +
    ...
    if (!opts.secretArn || typeof opts.secretArn !== "string") {
      throw new AuroraSessionStoreError(
        ...
        "AuroraSessionStore: secretArn is required and must be a non-empty string. " +
          "Wire DB_SECRET_ARN in terraform/modules/app/agentcore-pi/main.tf and " +
          "thread it through to the construction site.",
      );
    }
  ...
  async save(id: string, data: SessionData): Promise<void> {
    // Note on `updated_at`: Pi saves session state on every agent-loop
    ...
    // UI's "last activity" sort, and a Pi tick isn't a user-facing thread
    ...
    // agent_id mid-flight). Pi treats all three as "you can't write here"
  ...
  async load(id: string): Promise<SessionData | null> {
    ...
    // Treat it the same as "no session yet" so Pi starts fresh.
  ...
  async delete(id: string): Promise<void> {
    ...
    // continues to render. Pi can re-save() if it resumes the thread.
    ...
    // already NULL or that doesn't exist is a successful no-op (Pi's
