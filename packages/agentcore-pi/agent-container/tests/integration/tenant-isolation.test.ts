 ...
 *    `SandboxFactory` from `@thinkwork/pi-aws.agentcoreCodeInterpreter`
 ...
 *    Pi's session.task spawns sub-agents that should inherit the
...
describe("Audit grep — module-level mutable state", () => {
  ...
  it("module-level `new Map()` (empty/dynamic) is forbidden in src/", () => {
    const matches = grepLines(PI_SRC, "new Map");
  ...
  it("module-level `new Set()` (empty/dynamic) is forbidden in src/", () => {
    const matches = grepLines(PI_SRC, "new Set(");
...
function grepLines(rootDir: string, needle: string): GrepMatch[] {
  let output = "";
  const searchCwd = path.dirname(rootDir);
  ...
  try {
    ...
    const repoRelativeRoot = path.relative(REPO_ROOT, rootDir) || ".";
    try {
      ...
      if (ge.code === "ENOENT") {
        throw new Error(
          "verify-tenant-isolation: neither `rg` nor `git` is on PATH; the audit grep test cannot inspect src/.",
        );
      }
      throw gitErr;
    }
  ...
  for (const raw of output.split("\n")) {
    if (!raw) continue;
    // rg prints `<path>:<line-number>:<content>`.
...
describe("test infrastructure self-check", () => {
  ...
  it("PI_SRC contains the trusted-handler MCP module", () => {
    const mcp = readFileSync(path.join(PI_SRC, "mcp.ts"), "utf8");
