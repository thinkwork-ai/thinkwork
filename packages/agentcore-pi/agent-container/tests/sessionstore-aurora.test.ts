 ...
 *
 * Persists Pi's `SessionData` blobs against `threads.session_data` via the
...
import type { SessionData } from "../src/pi-session-types.js";
...
describe("AuroraSessionStore.delete", () => {
  it("clears session_data without removing the thread row", async () => {
    ...
    // renders. A future caller can re-save() if Pi resumes the thread.
...
describe("AuroraSessionStore — interface conformance", () => {
  it("structurally matches Pi's SessionStore interface (save/load/delete)", () => {
    const store = new AuroraSessionStore({
      tenantId: TENANT_A,
      agentId: AGENT_X,
      clusterArn: CLUSTER_ARN,
