 ...
 *      workspace folder (`Spaces/<slug>/` or `User/<slug>/`). A failure at
 *      file k discards the temp dir — nothing is ever half-mounted.
    ...
    register(pi) {
      ...
      const tool: ToolDefinition = {
        ...
        description:
          ...
          "Spaces mount at Spaces/<slug>/ and user folders at User/<slug>/. Fetched files are " +
        ...
        parameters: Type.Object({
          ...
          slug: Type.String({
            description:
              ...
              "(the segment after Spaces/ or User/).",
          }),
        ...
        async execute(_toolCallId, params) {
          ...
          const mountRel = kind === "space" ? `Spaces/${slug}` : `User/${slug}`;
          ...
          try {
            for (const file of files) {
              ...
              // Read-only files; directories stay writable so an idempotent
              // re-fetch (rm + rename) and container cleanup keep working.
              await chmod(stagedPath, 0o444);
              totalBytes += bytes.byteLength;
              baselineFiles.push({
                path: `${mountRel}/${relPath}`,
                bytes,
            ...
            const mountDir = path.join(workspaceDir, mountRel);
