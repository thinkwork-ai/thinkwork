...
function buildRuntimeToolPolicy(
  ...
  const jsonRenderPolicy = jsonRenderAvailable
    ? [
        ...
        "- If approval or review UI uses a component action reference such as `task.review.primaryActionId` or `form.action.submitActionId`, include a matching `durableActions` descriptor with the same id.",
