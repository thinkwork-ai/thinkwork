...
function buildRuntimeToolPolicy(
  ...
  const jsonRenderPolicy = jsonRenderAvailable
    ? [
        "",
        "### Generated Thread UI",
        "- The `emit_json_render_ui` tool is available. Treat generated UI as the DEFAULT presentation for structured content, not an occasional enhancement.",
        "- Before your final response, run a presentation pass. If the answer contains scan-friendly structured results — tabular data, quantitative series/metrics/trends, or homogeneous record sets — emit generated UI by default. Prose is the fallback, not the default.",
        "- Route by shape: row-and-column / tabular data → `table`; quantitative series, distributions, comparisons, or trends over time → `chart` (set `kind` to `area`, `bar`, `line`, or `pie`); homogeneous or repeated record sets such as Work Items/Linear-like issues, approval/review queues, checklists, timelines, deployment evidence, evaluation runs, connector records, and search results → `result.list`.",
        "- Do NOT return a markdown table or a bulleted list of records when `table`, `chart`, or `result.list` would render it — that is the exact case generated UI exists for.",
        ...
        "- Fall back to plain prose only when the content is genuinely narrative, tiny, open-ended, clearer as text, or has no fitting catalog component.",
        ...
        '- When the generated UI presents the data from ONE earlier tool call in this turn (e.g. charting rows an MCP tool returned), pass that call\'s real id as `sourceToolCallId` so the canvas records a refreshable data-source binding. Tool call ids are shaped like `functions.mcp_<server>_<tool>:<n>` (e.g. `functions.mcp_twenty--crm_execute_tool:15`) — use the exact id, not a placeholder. Example call shape: `{ "sourceToolCallId": "functions.mcp_<server>_<tool>:<n>", "spec": { … }, "mobileFallback": { … } }`. If you omit it and a bindable tool call exists, the emit result lists the candidate ids — re-emit with the SAME part id plus the right `sourceToolCallId`. Omit `sourceToolCallId` for UI not derived from a single tool result.',
