...
import {
  drainWikiCompileScope,
  enqueueAndDrainWikiRebuild,
} from "../src/lib/wiki/rebuild-runner.js";
...
function formatCounts(
  label: string,
  c: Awaited<ReturnType<typeof countWikiScope>>,
): void {
  console.log(`\n=== ${label} ===`);
...
async function main(): Promise<void> {
  ...
  if (args.rebuild) {
    ...
    if (
      result.failedJobId ||
      (args.drain && (result.pendingJobs > 0 || result.runningJobs > 0))
    ) {
      process.exitCode = 1;
    }
  } else if (args.drain) {
...
main()
  ...
  .catch((err) => {
    console.error(
      `[wiki-wipe-and-rebuild] fatal: ${(err as Error).stack ?? err}`,
    );
    process.exit(1);
  });
