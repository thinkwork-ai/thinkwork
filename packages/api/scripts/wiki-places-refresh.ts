...
export async function runPlacesRefresh(
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (row.source === "manual") {
      ...
      log(
        `[refresh] skip ${row.id} (source=manual — never overwrite user edits)`,
      );
      continue;
    }
    ...
    try {
      ...
      log(
        `[refresh] updated ${row.id} (google_place_id=${row.google_place_id})`,
      );
    } catch (err) {
      const msg = (err as Error)?.message ?? String(err);
      result.errors += 1;
...
async function listCandidatesFromDb(args: {
  ...
  if (args.staleBefore)
    clauses.push(lt(wikiPlaces.updated_at, args.staleBefore));
...
async function main(): Promise<void> {
  ...
  const result = await runPlacesRefresh({
    ...
    fetchPlaceDetails: (googlePlaceId) =>
      client ? client.fetchPlaceDetails(googlePlaceId) : Promise.resolve(null),
    breakerState: () => (client ? client.breakerState() : { state: "closed" }),
