...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case "--enqueue-compile":
        out.enqueueCompile = true;
        break;
      case "-h":
      case "--help":
        console.log(
...
async function fetchPage(
  ...
  const result = await db.execute(sql`
		...
		LIMIT ${limit}
	`);
  return ((result as unknown as { rows?: JournalRow[] }).rows ??
    []) as JournalRow[];
}
...
main().catch((err) => {
  console.error(
    `[journal-import-resume] fatal: ${(err as Error).stack ?? err}`,
  );
  process.exit(1);
});
