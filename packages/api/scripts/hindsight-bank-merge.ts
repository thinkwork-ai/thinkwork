...
function printCounts(label: string, counts: BankTableCounts): void {
  const active = Object.entries(counts.tables)
    ...
    .join(" ");
  console.log(
    `  ${label}: ${counts.bankId} total=${counts.total}${active ? ` ${active}` : ""}`,
  );
}
...
async function main(): Promise<void> {
  ...
  console.log(
    `[hindsight-bank-merge] generated=${report.generatedAt} apply=${report.apply}`,
  );
...
main()
  ...
  .catch((err) => {
    console.error(
      `[hindsight-bank-merge] fatal: ${(err as Error).stack ?? err}`,
    );
    process.exit(1);
  });
