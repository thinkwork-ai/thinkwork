...
const SHARED_COMPUTER: ComputerRow = {
  id: "computer-shared",
  tenant_id: TENANT_T,
  owner_user_id: null,
};
const A_THREADS: ThreadRow[] = [
  ...
  },
  {
    ...
    user_id: USER_A,
    channel: "chat",
    created_at: new Date("2026-05-08T10:05:00Z"),
  },
];
const B_THREADS: ThreadRow[] = [
  {
    ...
    user_id: USER_B,
    channel: "chat",
    created_at: new Date("2026-05-08T10:10:00Z"),
  },
];
const SHARED_THREADS: ThreadRow[] = [
  {
    id: "t-shared-A",
    tenant_id: TENANT_T,
    ...
    user_id: USER_A,
    channel: "chat",
    created_at: new Date("2026-05-08T10:15:00Z"),
  },
  {
    id: "t-shared-B",
    tenant_id: TENANT_T,
    ...
    user_id: USER_B,
    channel: "chat",
    created_at: new Date("2026-05-08T10:20:00Z"),
  },
];

const mocks = vi.hoisted(() => {
  const dbState = {
    computers: [] as ComputerRow[],
    threads: [] as ThreadRow[],
  };
  // Set per-test: throw to deny, return undefined to allow.
  const accessCheckImpl = {
    current: (async () => undefined) as (
  ...
  const ne = vi.fn((column: unknown, value: unknown) => ({
    _op: "ne",
    column,
    value,
  }));
  ...
  function buildSelect(table: { _name?: string }) {
    ...
    function resolveRows(): unknown[] {
      ...
      let rows = dbState.threads.filter(
        (r) => r.tenant_id === captured.tenant_id,
      );
  ...
  const requireComputerReadAccess = vi.fn(
    async (_ctx: unknown, computer: ComputerRow) => {
...
describe("threads(tenantId, computerId) resolver — multi-user scope", () => {
  beforeEach(() => {
    mocks.dbState.computers = [COMPUTER_A, COMPUTER_B, SHARED_COMPUTER];
    mocks.dbState.threads = [...A_THREADS, ...B_THREADS, ...SHARED_THREADS];
  ...
  it("filters shared Computer threads to the requesting user", async () => {
    const result = await threads_query(
      null,
      { tenantId: TENANT_T, computerId: "computer-shared" },
      cognitoCtx,
    );
    const ids = (result as { id: string }[]).map((r) => r.id);
