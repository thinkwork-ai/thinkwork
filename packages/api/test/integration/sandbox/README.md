...
| Token-leak assertion fails with `ghp_*` match                   | **Regression in the `sandbox-log-scrubber` subscription filter.** Page platform security. |
...
- `sandbox-log-scrubber` subscription filter on the AgentCore log group (Unit 12) — the only redaction layer; without it the token-leak test will fail
