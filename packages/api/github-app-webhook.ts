 ...
 */
async function lookupTenantSlug(
  _githubOwner: string,
  _githubRepo: string,
): Promise<string | null> {
  return null;
...
async function handlePushEvent(
  ...
  if (!bucket || !appId || !privateKey) {
    console.log(
      "Push sync skipped: missing WORKSPACE_BUCKET, GITHUB_APP_ID, or GITHUB_APP_PRIVATE_KEY",
    );
  ...
  if (pushBranch !== defaultBranch) {
    console.log(
      `Push sync skipped: push to ${pushBranch}, not ${defaultBranch}`,
    );
    return { synced: false, filesSynced: 0 };
  }
  ...
  console.log(
    `Syncing ${changedFiles.length} changed files from ${owner}/${repo} to S3`,
  );
