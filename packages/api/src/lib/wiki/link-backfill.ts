...
export async function runLinkBackfill(
  ...
  log(
    `[phase-a] ${candidates.length} parent candidates derived from summaries`,
  );
  ...
  log(`[phase-a] parent-emitter wrote ${parentEmission.linksWritten} links`);
...
export async function runPhaseCPlaceBackfill(
  ...
  log(`[phase-c] summary ${JSON.stringify(result)}`);
  return result;
}
