...
export function deriveParentCandidates(
  ...
  for (const r of records) {
    ...
    const journalId =
      readString(meta, ["journal_id"]) ?? readString(meta, ["journal", "id"]);
    if (journalId) {
...
export function deriveParentCandidatesFromPageSummaries(
  ...
  for (const p of pages) {
    ...
    const city =
      extractCityFromSummary(p.summary) ?? extractCityFromAddress(p.summary);
    if (!city) continue;
...
export function mergeParentCandidates(
  ...
  for (const list of lists) {
    for (const c of list) {
      ...
      const ids = new Set([...existing.sourceRecordIds, ...c.sourceRecordIds]);
