...
export async function runCompileJob(
  ...
  try {
    ...
    try {
      metrics.duplicate_candidates_count = await countDuplicateTitleCandidates({
        tenantId: job.tenant_id,
        ownerId: job.owner_id,
      });
    } catch (canaryErr) {
    ...
    if (!cursorDrained) {
      try {
        ...
        const parentBucket =
          ...
          Math.floor(job.created_at.valueOf() / 1000 / DEDUPE_BUCKET_SECONDS);
...
async function applyPlan(args: ApplyPlanArgs): Promise<string | null> {
  ...
  for (const upd of plan.pageUpdates) {
    ...
    for (const sec of upd.sections) {
      ...
      if (metrics.sections_rewritten >= MAX_SECTIONS_REWRITTEN_PER_JOB) {
      ...
      // sibling records.
      const sectionSources = resolveCitedRecords(recordById, sec.source_refs);

      const writeRes: SectionWriteResult = await writeSection({
        pageType: existing.type,
        pageTitle: existing.title,
        sectionSlug: sec.slug,
        sectionHeading: existingSec?.heading ?? formatHeadingFromSlug(sec.slug),
    ...
    if (sectionsToApply.length > 0) {
      await upsertPage({
        ...
        type: existing.type,
        entity_subtype: upd.entityTypeSlug ?? existing.entity_subtype ?? null,
        slug: existing.slug,
  ...
  for (const np of plan.newPages) {
    ...
    if (merged) {
      ...
      if (merged === "exact") {
        ...
        metrics.fuzzy_dedupe_merges = (metrics.fuzzy_dedupe_merges ?? 0) + 1;
      }
      continue;
    }
    ...
    const createdPage = await upsertPage({
      ...
      type: np.type,
      entity_subtype: np.entityTypeSlug ?? null,
      slug,
      title: np.title,
    ...
    if (args.ontologySnapshot && np.entityTypeSlug) {
      ...
      if (brainResult.pageUpserted) {
        metrics.brain_pages_upserted = (metrics.brain_pages_upserted ?? 0) + 1;
      }
  ...
  for (const pr of capHit ? [] : plan.promotions) {
    ...
    const promotionSections = pr.sections.map((s, i) => {
      const sectionSources = resolveCitedRecords(recordById, s.source_refs);
      return {
        section_slug: s.slug,
        heading: s.heading,
        body_md: linkifyKnownEntities(s.body_md, args.knownPageRefs ?? []),
    ...
    const page = await upsertPage({
      ...
      owner_id: job.owner_id,
      type: pr.type,
      entity_subtype: pr.entityTypeSlug ?? null,
      slug,
      title: pr.title,
      markCompiled: true,
  ...
  if (args.ontologySnapshot && !args.ontologySnapshot.conservative) {
    ...
    const batchMemoryIds = Array.from(
      new Set(affectedPages.flatMap((p) => p.sourceRecordIds)),
    );
...
async function applyAggregationPlan(args: ApplyAggregationArgs): Promise<void> {
  const {
    job,
    records,
    plan,
    ...
    if (titleCollisions.length > 0) {
      ...
      console.warn(
        `[wiki-compiler] cross-type duplicate guard: agg planner proposed ` +
          ...
          titleCollisions.map((p) => `${p.type}/${p.slug}=${p.id}`).join(", ") +
    ...
    const created = await upsertPage({
      ...
      sections: np.sections.map((s, i) => {
        ...
        return {
          section_slug: s.slug,
          heading: s.heading,
          body_md: linkifyKnownEntities(s.body_md, knownPageRefs ?? []),
          position: i + 1,
          sources: sectionSources.map((r) => ({
            kind: "memory_unit" as const,
            ref: r.id,
  ...
  for (const promo of plan.sectionPromotions) {
    ...
    const currentAgg = parentSection.aggregation ?? emptySectionAggregation();
...
function nextFreePosition(sections: Array<{ position: number }>): number {
