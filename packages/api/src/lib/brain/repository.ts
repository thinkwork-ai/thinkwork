...
export async function findOrCreateTenantEntityPage(
  ...
  const aliases = [
    ...new Set([...(input.aliases ?? []), title].map(normalizeAlias)),
  ]
    .filter(Boolean)
    .map((alias) => ({
      page_id: page.id,
  ...
  if (aliases.length > 0) {
    await db
      .insert(tenantEntityPageAliases)
      .values(aliases)
      .onConflictDoNothing();
  }
...
export async function syncTenantEntityPageBodyFromSections(args: {
  pageId: string;
  db?: DbClient;
}): Promise<{ bodyMd: string; sectionCount: number }> {
  const db = args.db ?? defaultDb;
  const sections = await db
    .select({
      heading: tenantEntityPageSections.heading,
      body_md: tenantEntityPageSections.body_md,
      position: tenantEntityPageSections.position,
      status: tenantEntityPageSections.status,
    })
  ...
  const bodyMd = composeTenantEntityPageBody(activeSections);
  await db
  ...
  return { bodyMd, sectionCount: activeSections.length };
}

export function composeTenantEntityPageBody(
  sections: TenantEntitySectionForBody[],
): string {
  return sections
