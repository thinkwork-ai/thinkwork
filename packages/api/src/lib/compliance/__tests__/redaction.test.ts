...
describe("redactPayload", () => {
  ...
  describe("allow-list drops disallowed fields", () => {
    ...
    it("attachment.received drops raw s3_key / filename — only attachmentId reference survives", () => {
      ...
      const result = redactPayload("attachment.received", {
        ...
        s3_key:
          "tenants/acme/threads/t1/attachments/att-uuid-1/financials.xlsx",
    ...
    it("output.artifact_produced retains artifact identifiers; drops content / s3 key", () => {
      const result = redactPayload("output.artifact_produced", {
        ...
        s3_key:
          "tenants/acme/artifact-payloads/message-artifacts/art-uuid-1/content/r1",
        content: "| col | val |\n| --- | --- |\n",
      });
      expect(result.redacted).not.toHaveProperty("s3_key");
  ...
  describe("string sanitization", () => {
    it("truncates strings exceeding 4096 bytes", () => {
      ...
      expect(Buffer.byteLength(emailValue, "utf-8")).toBeLessThanOrEqual(4096);
  ...
  describe("expanded secret-pattern set", () => {
    ...
    it("scrubs OpenAI sk-proj- keys", () => {
      const result = redactPayload("user.invited", {
        ...
        role: "member",
        invitedBy:
          "sk-proj-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
      });
      expect(result.redacted.invitedBy).toBe("<REDACTED:secret>");
    });
  ...
  describe("error paths", () => {
    it("throws on unknown event type", () => {
      expect(() => redactPayload("nonsense.foo" as never, {})).toThrow(
        /no redaction schema for event type/,
      );
    });
  });
  ...
  describe("Phase 6 reservations (R14) — throw on emit attempt", () => {
    ...
    it.each(phase6Types)(
      ...
      (eventType) => {
        ...
        expect(() => redactPayload(eventType, { anything: "value" })).toThrow(
          /Phase 6 reservation/,
        );
      },
    );
