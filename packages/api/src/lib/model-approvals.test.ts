...
const modelRows = [
  {
    ...
    provider: "bedrock",
    displayName: "Tenant Haiku",
    canonicalDisplayName: "Claude Haiku 4.5",
    inputCostPerMillion: "1.0000",
    outputCostPerMillion: "5.0000",
    ...
    pricingDiagnostics: {},
    lastPricedAt: new Date("2026-01-01T00:00:00Z"),
...
describe("model approvals", () => {
  ...
  it("seeds available tenant defaults for new bootstrap users", async () => {
    const { db, inserts } = createMockDb([
      ...
      [
        { ...modelRows[0], modelId: "agent-default" },
        { ...modelRows[1], modelId: "template-default" },
      ],
    ]);
