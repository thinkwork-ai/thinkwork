...
export function toOntologyEntityType(
  ...
  return {
    ...
    systemMap: row.system_map ?? [],
    systemMapVersion: row.system_map_version ?? 0,
    twinFacets: row.twin_facets ?? [],
    twinFacetsVersion: row.twin_facets_version ?? 0,
    pageSections: row.page_sections ?? [],
    pageSectionsVersion: row.page_sections_version ?? 0,
    guidanceNotes: row.guidance_notes,
    lifecycleStatus: enumValue(row.lifecycle_status),
...
export function toOntologyRelationshipType(
  ...
  return {
    ...
    sourceBindingVersion: row.source_binding_version ?? 0,
    guidanceNotes: row.guidance_notes,
    lifecycleStatus: enumValue(row.lifecycle_status),
    approvedAt: iso(row.approved_at),
    deprecatedAt: iso(row.deprecated_at),
