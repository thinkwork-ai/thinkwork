...
export async function checkConcurrencyLimits(
  ...
  const teamRow = (teamResult.rows || [])[0] as
    | { metadata: Record<string, unknown> | null }
    | undefined;
  const metadata = teamRow?.metadata;
  if (!metadata) return { allowed: true };

  const concurrency = metadata.concurrency as
    | Record<string, unknown>
    | undefined;
  if (!concurrency) return { allowed: true };

  const maxPerAgent = concurrency.maxPerAgent as number | undefined;
  const maxConcurrentAgents = concurrency.maxConcurrentAgents as
    | number
    | undefined;
  const maxByStatus = concurrency.maxByStatus as
  ...
  if (maxPerAgent !== undefined) {
    const r = await db.execute(sql`
		...
		`);
    const count =
      ((r.rows || [])[0] as { count: number } | undefined)?.count || 0;
  ...
  if (maxConcurrentAgents !== undefined) {
    ...
    if (count >= maxConcurrentAgents) {
      return {
        allowed: false,
        reason: `global_agent_limit_reached (${maxConcurrentAgents})`,
      };
    }
  }
  ...
  if (maxByStatus) {
    for (const [statusKey, limit] of Object.entries(maxByStatus)) {
      ...
      if (count >= limit) {
        return {
          allowed: false,
          reason: `status_limit_reached: ${statusKey} (${limit})`,
        };
      }
    }
  }
