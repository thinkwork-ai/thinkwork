...
function analystRaw(over: Record<string, unknown> = {}) {
  const envelope = {
    columns: ANALYST_COLUMNS,
    rows: [["acme", 12]],
    row_count: 1,
    truncated: false,
    stats: { n: { nulls: 0, min: 12, max: 12 } },
    result_file: null,
    ...over,
  };
  return {
...
describe("refreshBinding — query (THINK-228 KTD2/R14)", () => {
    ...
    const { deps, rec } = makeDeps({
      callTool: async () => ({
        ...
        raw: analystRaw({
          ...
          row_count: 2,
          stats: { n: { nulls: 1, min: null, max: null } },
          // A staged pointer appears without truncation only in synthetic
          // cases, but the point stands: nullable-key churn must not trip
          // SCHEMA_STALE. truncated stays false so the R14 gate is inert.
          result_file: "s3://bucket/analyst-staging/t/x.csv",
        }),
      }),
    });
    const result = await refreshBinding(analystBinding(), deps);
    expect(result.outcome).toBe("refreshed");
    expect(result.escalate).toBe(false);
    expect(rec.headApplies).toBe(1);
  });

  it("KTD2: a real column-set change escalates SCHEMA_STALE", async () => {
    const { deps, rec } = makeDeps({
      callTool: async () => ({
        isError: false,
        raw: analystRaw({
          columns: [...ANALYST_COLUMNS, { name: "extra", pg_type: "text" }],
          rows: [["acme", 12, "x"]],
        }),
      }),
    });
  ...
  it("R14: a truncated refresh escalates instead of silently applying a partial aggregate", async () => {
    const { deps, rec } = makeDeps({
      callTool: async () => ({
        isError: false,
        raw: analystRaw({
          truncated: true,
          row_count: 50_000,
          result_file: "s3://bucket/analyst-staging/t/big.csv",
        }),
      }),
    });
    const result = await refreshBinding(analystBinding(), deps);
    ...
    expect(rec.headApplies).toBe(0);
    // Descriptor hash itself is stable under the flip — the escalation came
    // from the explicit truncated check, not the hash.
    expect(
      analystColumnsShapeHash(
        analystEnvelopeFromRaw(analystRaw({ truncated: true }))!.columns,
      ),
    ).toBe(analystBinding().resultShapeHash);
  });

  it("broker failure during refresh degrades to FAILED with last-good retained", async () => {
    const { deps, rec } = makeDeps({
      callTool: async () => ({ isError: true, raw: { error: "cap" } }),
    });
    const result = await refreshBinding(analystBinding(), deps);
    expect(result.outcome).toBe("failed");
    expect(result.quality).toBe("bad");
