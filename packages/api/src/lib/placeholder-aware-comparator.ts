...
import { substitute, type PlaceholderValues } from "./placeholder-substitution.js";
...
export interface ClassifyFileInput {
	...
	/**
	 * Values to substitute into the template before comparing. Typically
	 * `{ AGENT_NAME, HUMAN_NAME, ... }` resolved from the agent's current
	 * DB state.
	 */
...
export function classifyAgentFile(
	...
	// Render the template with the agent's values and compare. We use the
	// same `substitute` the composer uses — that way a file the composer
	// would have rendered identically matches, and files the composer
	// would have changed (e.g. because an em-dash rule applies) don't
	// accidentally classify as forks.
	const rendered = substitute(input.values, input.templateContent);
