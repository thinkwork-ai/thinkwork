...
const HINDSIGHT_FALLBACK_PRICING: Record<
  string,
 ...
 */
export async function recordHindsightCost(
  params: RecordHindsightCostParams,
): Promise<void> {
  ...
  try {
    ...
    console.error(
      `[hindsight-cost] failed to record ${params.phase} cost for bank=${params.bankId}:`,
      err,
    );
  }
}
