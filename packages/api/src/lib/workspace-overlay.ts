...
async function loadAgentContext(
	...
	if (agent.human_pair_id) {
		const userRows = (await db
			...
			.limit(1)) as Array<Omit<HumanRow, "profile_title" | "profile_timezone" | "profile_pronouns">>;
		...
		if (user) {
			const profileRows = (await db
				...
				.limit(1)) as Array<
					Pick<HumanRow, "profile_title" | "profile_timezone" | "profile_pronouns">
				>;
...
async function s3Head(bucket: string, key: string): Promise<{ etag: string | null } | null> {
	try {
...
async function peekSource(
	...
	if (fileClass === "pinned") {
		...
		const pinned = agentCtx.pinnedVersions[path];
		if (pinned) {
	...
	for (const [key, source] of [
		[agentKey, fileClass === "pinned" ? "agent-override-pinned" : "agent-override"] as const,
		[templateKey, "template"] as const,
		[defaultsKey, "defaults"] as const,
	]) {
		const head = await s3Head(bucket, key);
		if (head) {
			return { path, source: source as ComposeSource, sha256: etagToSha(head.etag) };
		}
	}
