...
export function candidateLegacyBankIds(
  agent: Pick<AgentBankRow, "agent_id" | "slug" | "name">,
): string[] {
...
export function parseAliasMappings(values: string[]): AliasMapping[] {
  return values.map((value) => {
    ...
    if (!left || !right) {
      throw new Error(
        `Invalid alias mapping "${value}". Expected sourceBank=userId or sourceBank=tenantId:userId.`,
      );
    }
...
export async function runHindsightBankMerge(
  ...
  for (const mapping of mappings) {
    ...
    for (const source of mapping.candidateLegacyBankIds) {
      ...
      if (options.apply) {
        if (
          conflicts.blockingDocuments > 0 ||
          conflicts.blockingMentalModels > 0
        ) {
          throw new Error(
...
async function loadMappings(
  ...
  return [...grouped.values()].map((mapping) => ({
    ...
    candidateLegacyBankIds: uniqueStrings(
      mapping.candidateLegacyBankIds.filter(
        (bankId) => bankId !== mapping.destinationBankId,
      ),
    ),
  }));
}
...
export async function countBankTables(
  db: DbLike,
  bankId: string,
): Promise<BankTableCounts> {
...
async function applyMergePair(
  ...
  const run = async (tx: DbLike) => {
    await tx.execute(
      sql`SELECT pg_advisory_xact_lock(hashtext(${`hindsight-bank-merge:${sourceBankId}:${destinationBankIdValue}`}))`,
    );
    await ensureDestinationBank(tx, sourceBankId, destinationBankIdValue);
    const removedDuplicateMentalModels =
      await deleteIdenticalDuplicateMentalModels(
        tx,
        sourceBankId,
        destinationBankIdValue,
      );
    const mergedEntities = await mergeDuplicateEntities(
      tx,
      sourceBankId,
      destinationBankIdValue,
    );
    const movedRows: Record<string, number> = {};
    movedRows.documents = await copyDocumentsToDestination(
      tx,
      sourceBankId,
      destinationBankIdValue,
    );
    ...
    for (const table of [
      ...
      movedRows[table] = await updateBankId(
        tx,
        table,
        sourceBankId,
        destinationBankIdValue,
    ...
    const removedSourceDocuments = await deleteSourceDocuments(
      tx,
      sourceBankId,
    );
...
async function mergeDuplicateEntities(
  ...
  const result = await db.execute(sql`
	...
	`);
  const pairs = (result.rows || []) as Array<{
    source_id: string;
...
async function mergeEntityPair(
  db: DbLike,
...
async function executeRowCount(
  db: DbLike,
  query: ReturnType<typeof sql>,
): Promise<number> {
  const result = await db.execute(query);
  return Number(
    (result as any).rowCount ?? (result.rows?.[0] as any)?.rowCount ?? 0,
  );
}
