    ...
    async createMessage(input) {
      const [message] = await dbClient
        ...
        .onConflictDoNothing({
          ...
          // Drizzle 0.39's onConflictDoNothing takes the arbiter predicate as
          // `where` and silently DROPS a `targetWhere` key, emitting an
          // ON CONFLICT with no predicate — which Postgres cannot match to
          // the partial unique index uq_messages_tenant_source_event_id
          // (42P10 at runtime, found in deployed U4 dogfood).
          where: sql`${messages.source_event_id} IS NOT NULL`,
        })
