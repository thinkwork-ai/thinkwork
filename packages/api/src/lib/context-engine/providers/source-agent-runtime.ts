...
export async function runSourceAgent(args: {
  ...
  for (let turn = 1; turn <= args.depthCap; turn++) {
    ...
    if (action.value.final) {
      const normalizedResults = normalizeFinalResults(
        action.value.final.results,
      );
    ...
    for (const [index, call] of toolCalls.entries()) {
      ...
      try {
        ...
        const summary =
          result.summary ?? summarizeObservation(result.observation);
        observations.push({
          turn,
          toolCallId,
          tool: tool.name,
...
function buildUserTurnPrompt(args: {
  ...
  return [
    ...
    "",
    "Allowed tools:",
    ...args.tools.map((tool) => `- ${tool.name}: ${tool.description}`),
    "",
...
function parseAction(
  ...
  try {
    ...
    if (!parsed)
      return { ok: false, error: "model response was not an action" };
...
function normalizeToolCalls(action: SourceAgentAction): SourceAgentToolCall[] {
  const calls =
    action.tool_calls ?? (action.tool_call ? [action.tool_call] : []);
...
function assertRuntimeArgs(
  ...
  for (const allowed of allowedTools) {
    if (!toolNames.has(allowed)) {
      throw new Error(
        `allowed source agent tool is not registered: ${allowed}`,
      );
    }
  }
}
