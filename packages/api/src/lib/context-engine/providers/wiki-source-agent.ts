...
export function createWikiSourceAgentContextProvider(
  options: {
...
function modelResultToHit(args: {
  ...
  return {
    ...
    snippet:
      args.result.summary || args.row.page.summary || args.row.page.title,
...
export function planWikiSourceAgentQueries(
  query: string,
): WikiSourceAgentPlanStep[] {
  ...
  if (
    repaired.query &&
    repaired.query.toLowerCase() !== original.toLowerCase()
  ) {
    steps.push({
  ...
  const focusedTerms = normalizeWikiSearchTerms(
    repaired.query || original,
  ).filter((term) => !["tell", "show", "find", "about"].includes(term));
...
function rankWikiSourceAgentResults(args: {
  searches: Array<{
    step: WikiSourceAgentPlanStep;
    rows: UserWikiSearchResult[];
  }>;
  ...
  return [...byPageId.values()]
    .sort(
      (a, b) =>
        b.score - a.score || a.row.page.title.localeCompare(b.row.page.title),
    )
    .slice(0, args.limit);
}
...
function countUniquePages(
  ...
  return new Set(
    searches.flatMap((search) => search.rows.map((row) => row.page.id)),
  ).size;
}
