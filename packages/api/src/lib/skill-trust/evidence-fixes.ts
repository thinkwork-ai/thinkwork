...
async function signSkill(
  ...
  if (!input.signer) {
    const artifact = {
      path: "skill.oms.sig",
      content: unsignedApprovalEnvelope({
        slug: input.slug,
        signedPayloadHash,
        now: input.now,
      }),
...
function unsignedApprovalEnvelope(input: {
  slug: string;
  signedPayloadHash: string;
  now?: Date;
}): Buffer {
    ...
    `${JSON.stringify(
      {
        ...
        note: "Generated by ThinkWork without signing configuration. Treat as operator approval evidence, not a cryptographic signature.",
      },
      null,
      2,
    )}\n`,
