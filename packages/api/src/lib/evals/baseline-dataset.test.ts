...
describe("buildBaselineDatasetCases", () => {
  it("converts all 195 bundled seed cases with stable ids equal to seed names", () => {
    ...
    expect(new Set(cases.map((c) => c.core.case_id)).size).toBe(195);
  });
...
describe("fresh tenant seed", () => {
  ...
  it("seeds the full 195-case bundled pack end-to-end", async () => {
    const result = await seedBaselineDataset(TENANT, deps());
    expect(result.inserted).toBe(195);
    expect(manifest().cases).toHaveLength(195);
    expect(fake.rows.filter((r) => r.source === "yaml-seed")).toHaveLength(195);
