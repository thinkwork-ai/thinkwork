...
const FUNCTION_NAME =
  process.env.SMOKE_FUNCTION_NAME || `thinkwork-${STAGE}-agentcore-pi`;
...
interface ScenarioOptions {
  /**
   * When true, assert `pi_retain.retained === true` in the response.
...
function fail(reason: string, context?: Record<string, unknown>): never {
  console.error(`[pi-smoke] FAIL: ${reason}`);
  if (context)
    console.error(`[pi-smoke] context:`, JSON.stringify(context, null, 2));
  process.exit(1);
}
...
async function invokePi(
  ...
  if (result.FunctionError) {
    const errorPayload = result.Payload
      ...
      : "<no payload>";
    fail(
      `[${scenario}] Lambda returned FunctionError=${result.FunctionError}`,
      {
  ...
  if (!result.Payload) {
    fail(
      `[${scenario}] Lambda returned no payload (LWA routing or Lambda crash)`,
      {
  ...
  if (response.runtime !== "pi") {
    fail(
      `[${scenario}] response.runtime is "${response.runtime}", expected "pi"`,
      {
        full_response: response,
      },
    );
  ...
  if (!expectedFingerprint.test(content)) {
    fail(`[${scenario}] response.content does not match expected fingerprint`, {
      content,
      expected: expectedFingerprint.source,
      duration_ms: durationMs,
    });
  }
  ...
  const retain = response.pi_retain;
  if (options.expectRetain) {
    if (!retain) {
      fail(
        `[${scenario}] response.pi_retain is missing — the runtime did not surface retain status (older container? response shape regression?)`,
        { full_response: response, duration_ms: durationMs },
      );
    }
    if (retain.retained !== true) {
      fail(
        `[${scenario}] response.pi_retain.retained=${retain.retained} despite use_memory=true — auto-retain dispatch did not fire`,
        {
          retain,
          duration_ms: durationMs,
          hint: "Check MEMORY_RETAIN_FN_NAME env var, IAM MemoryRetainInvoke policy, and Pi Lambda CloudWatch logs for memory_retain_failed events.",
        },
      );
    }
  }
...
async function main(): Promise<void> {
  console.log(
    `[pi-smoke] stage=${STAGE} function=${FUNCTION_NAME} agent=${AGENT_ID}`,
  );
  ...
  const multiTurn = await invokePi(
    client,
    "multi-turn-history",
    {
  ...
  // surfaces `pi_retain.retained === true`. The retain Lambda invoke
  ...
  const memoryBearing = await invokePi(
    ...
    // assertion is `pi_retain.retained === true` (handled by
    // `expectRetain` below).
  ...
  for (const r of [fresh, multiTurn, memoryBearing]) {
    console.log(
      `[pi-smoke] PASS [${r.scenario}]: model=${r.model} tokens=${r.tokens} duration_ms=${r.durationMs}`,
    );
    console.log(`[pi-smoke]   content: ${r.contentPreview}`);
    if (r.retain) {
      console.log(
        `[pi-smoke]   pi_retain: retained=${r.retain.retained}${r.retain.error ? ` error=${r.retain.error}` : ""}`,
      );
    }
  }
}
...
main().catch((err) => {
  console.error("[pi-smoke] uncaught error:", err);
  process.exit(1);
});
