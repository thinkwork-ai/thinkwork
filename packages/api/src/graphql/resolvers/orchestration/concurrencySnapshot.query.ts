...
export const concurrencySnapshot = async (
  _parent: any,
  args: any,
  ctx: GraphQLContext,
) => {
  const totalResult = await db.execute(sql`
	...
	`);
  const totalActive =
    ((totalResult.rows || [])[0] as { count: number } | undefined)?.count || 0;
  ...
  const byAgentResult = await db.execute(sql`
	...
	`);
  const byStatusRows = (byStatusResult.rows || []) as {
    status: string;
    count: number;
  }[];
  const byAgentRows = (byAgentResult.rows || []) as {
    agent_id: string;
    agent_name: string;
    count: number;
  }[];
