...
import { db, eq, and, sql, threads, threadToCamel } from "../../utils.js";

export const checkoutThread = async (
  _parent: any,
  args: any,
  ctx: GraphQLContext,
) => {
  ...
  const [row] = await db
    ...
    .where(
      and(eq(threads.id, args.id), sql`${threads.checkout_run_id} IS NULL`),
    )
    .returning();
