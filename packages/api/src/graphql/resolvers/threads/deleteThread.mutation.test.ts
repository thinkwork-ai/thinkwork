...
const {
  ...
  const db = {
    ...
    transaction: vi.fn(
      async (callback: (mockTx: typeof tx) => Promise<boolean>) => callback(tx),
    ),
  };
...
describe("deleteThread", () => {
  ...
  it("detaches and deletes dependent rows before deleting the thread", async () => {
    const result = await deleteThread(null, { id: "thread-1" }, {
  ...
  it("returns false without mutating when the thread does not exist", async () => {
    ...
    const result = await deleteThread(null, { id: "missing-thread" }, {
      auth: { authType: "cognito" },
    } as any);
  ...
  it("allows apikey callers without tenant membership lookup", async () => {
    await deleteThread(null, { id: "thread-1" }, {
