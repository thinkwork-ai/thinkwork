...
export const addThreadDependency = async (
  _parent: any,
  args: any,
  ctx: GraphQLContext,
) => {
  ...
  const [blocker] = await db
    .select({ tenant_id: threads.tenant_id })
    .from(threads)
    .where(eq(threads.id, blockedByThreadId));
  if (!blocker) throw new Error("Blocker thread not found");
  if (thread.tenant_id !== blocker.tenant_id)
    throw new Error("Threads must be in the same tenant");
  ...
  const cycleResult = await db.execute(sql`
	...
	`);
  const hasCycle = (
    (cycleResult.rows || [])[0] as { has_cycle: boolean } | undefined
  )?.has_cycle;
  if (hasCycle) {
