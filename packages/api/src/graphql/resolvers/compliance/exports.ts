...
function validateFilter(filter: ComplianceEventFilterInput): void {
  ...
  if (Buffer.byteLength(serialized, "utf8") > FILTER_BYTE_CAP) {
    throw new GraphQLError(`Filter exceeds the ${FILTER_BYTE_CAP}-byte cap.`, {
  ...
  if (filter.since && filter.until) {
    ...
    if (until <= since) {
      throw new GraphQLError("`until` must be strictly greater than `since`.", {
        extensions: { code: "BAD_USER_INPUT" },
      });
    }
...
export async function createComplianceExport(
  ...
  const tenantIdForRow =
    auth.effectiveTenantId ?? (auth.isOperator ? ALL_TENANTS_SENTINEL : null);
  if (!tenantIdForRow) {
