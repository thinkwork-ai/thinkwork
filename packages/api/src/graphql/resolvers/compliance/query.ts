...
export async function complianceEvents(
  ...
  const limit = Math.min(
    Math.max(
      typeof args.first === "number" ? args.first : DEFAULT_PAGE_SIZE,
      1,
    ),
    MAX_PAGE_SIZE,
  );
  ...
  const distinctTenants = Array.from(new Set(pageRows.map((r) => r.tenant_id)));
  const anchorEntries = await Promise.all(
    distinctTenants.map(
      async (tid) => [tid, await fetchAnchorState(client, tid)] as const,
    ),
  );
