...
import { bootstrapAgentWorkspace } from "../../../lib/workspace-bootstrap.js";
...
export async function createAgentFromTemplate(
  ...
  try {
    ...
    console.warn(
      `[createAgentFromTemplate] Failed to provision email capability:`,
      err,
    );
  }

  // 6. Materialize the agent's workspace. Per docs/plans/2026-04-27-003,
  // the runtime reads only the agent's S3 prefix — no overlay walk, no
  // composer fallback. Copy template + defaults files (with AGENT_NAME
  // / TENANT_NAME baked in) into the agent's prefix now so the first
  // invocation sees a populated workspace.
  //
  // Failure surfaces to the caller. The agent row is orphaned in that
  // case (no transaction wrapping this whole mutation today). An
  // operator can either delete the row or re-run the rematerialize
  // action; the inconsistency is visible because S3 list returns
  // nothing for the agent's prefix.
  ...
  try {
    ...
    console.warn(
      "[createAgentFromTemplate] workspace-map-generator not available:",
      err,
    );
  }
  ...
  if (i.teamId) {
    try {
      ...
      console.warn(
        `[createAgentFromTemplate] Failed to add agent to team:`,
        err,
      );
    }
  }
