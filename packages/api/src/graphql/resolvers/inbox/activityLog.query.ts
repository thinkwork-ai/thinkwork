...
export const activityLog_ = async (
  _parent: any,
  args: any,
  ctx: GraphQLContext,
) => {
  ...
  if (args.entityType)
    conditions.push(eq(activityLog.entity_type, args.entityType));
  if (args.entityId) conditions.push(eq(activityLog.entity_id, args.entityId));
  if (args.actorType)
    conditions.push(eq(activityLog.actor_type, args.actorType));
  ...
  if (args.after)
    conditions.push(gte(activityLog.created_at, new Date(args.after)));
  if (args.before)
    conditions.push(lte(activityLog.created_at, new Date(args.before)));
