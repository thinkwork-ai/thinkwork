...
export async function compositionFeedbackSummary(
  ...
  const rows = await db
    .select({
      ...
      positive:
        sql<number>`count(*) filter (where feedback_signal = 'positive')`.as(
          "positive",
        ),
      negative:
        sql<number>`count(*) filter (where feedback_signal = 'negative')`.as(
          "negative",
        ),
      total:
        sql<number>`count(*) filter (where feedback_signal is not null)`.as(
          "total",
        ),
    })
    .from(skillRuns)
