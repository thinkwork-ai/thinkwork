...
export async function updateAgent(
  ...
  if (humanPairIdChanging || nameProvided) {
    ...
    // invalidate. Runtimes pull the agent prefix on every invocation
    // and pick up identity / human-pair changes on the next turn.
    void cacheInvalidations;
  } else {
    const [updated] = await db
      .update(agents)
      .set(updates)
      .where(and(eq(agents.id, args.id), eq(agents.tenant_id, callerTenantId)))
      .returning();
    if (!updated) throw new Error("Agent not found");
    row = updated;
  }
