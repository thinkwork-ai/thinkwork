...
import {
	db, eq,
	agents, agentCapabilities, users,
	agentToCamel, generateSlug,
	invokeJobScheduleManager,
} from "../../utils.js";

export async function createAgent(_parent: any, args: any, ctx: GraphQLContext) {
	const i = args.input;
	...
	try {
		...
		console.warn(`[graphql-resolver] Failed to auto-provision email capability for agent ${row.id}:`, emailCapErr);
	}

	// Auto-create scheduled job for serverless agent heartbeat
	if (runtimeConfig?.heartbeat?.enabled && SERVERLESS_ADAPTERS.has(adapterType)) {
