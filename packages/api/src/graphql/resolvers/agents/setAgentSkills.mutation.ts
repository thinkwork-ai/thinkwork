...
import { db, eq, and, inArray, agents, agentSkills, snakeToCamel } from "../../utils.js";

export async function setAgentSkills(_parent: any, args: any, ctx: GraphQLContext) {
	// Safety: never delete all skills if the incoming list is empty.
	if (args.skills.length === 0) {
		console.warn(`[setAgentSkills] Ignoring empty skills list for agent ${args.agentId} — likely stale UI state`);
		const existing = await db.select().from(agentSkills).where(eq(agentSkills.agent_id, args.agentId));
	...
	for (const s of args.skills) {
		const config = s.config ? (typeof s.config === "string" ? JSON.parse(s.config) : s.config) : undefined;
		const permissions = s.permissions ? (typeof s.permissions === "string" ? JSON.parse(s.permissions) : s.permissions) : undefined;
	...
	const rows = await db.select().from(agentSkills).where(eq(agentSkills.agent_id, args.agentId));
	...
	try {
		...
		regenerateWorkspaceMap(args.agentId).catch((err: unknown) => {
			console.error("[setAgentSkills] Failed to regenerate workspace map:", err);
		});
	} catch (err) {
		console.warn("[setAgentSkills] workspace-map-generator not available:", err);
	}
