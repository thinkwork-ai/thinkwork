...
function extractInputPreview(inputBodyJson: any): string {
      ...
      if (typeof content === "string") {
        ...
        for (const b of content) {
          ...
          if (b.type === "tool_result")
            textParts.push(
              `[tool_result: ${JSON.stringify(b.content).slice(0, 200)}]`,
            );
  ...
  if (inputBodyJson.toolConfig?.tools) {
    const toolNames = inputBodyJson.toolConfig.tools
...
function extractOutputPreview(outputBodyJson: any): string {
  ...
  if (typeof outputBodyJson === "object" && !Array.isArray(outputBodyJson)) {
    const content =
      outputBodyJson.output?.message?.content || outputBodyJson.content || [];
    ...
    return response.events
      .map((event) => {
        try {
          ...
          for (const block of Array.isArray(outputContent)
            ? outputContent
            : []) {
          ...
          const hasToolResult = (input.inputBodyJson?.messages || []).some(
            (m: any) =>
