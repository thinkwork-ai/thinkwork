...
export const BUILT_IN_PROFILE_SEEDS: BuiltInProfileSeed[] = [
  ...
  },
  {
    ...
    execution_controls: {
      ...
      clarify: false,
      maxSubagentDepth: 0,
      // THINK-232: per-run dollar budget, enforced in the delegation loop
      // (token cost is charged from the run's final usage). Provisional
      // default; the signed sidecar budget wins when present
      // (resolve-agent-runtime-config KTD6 override).
      costBudgetUsd: 0.5,
      loopPolicy: defaultAgentLoopPolicy(),
    },
  },
