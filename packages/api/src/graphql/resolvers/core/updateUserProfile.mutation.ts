...
export const updateUserProfile = async (
  ...
  if (ctx.auth.authType === "apikey") {
    ...
    if (!ctx.auth.agentId) {
      throw new GraphQLError("Service-auth callers must present x-agent-id", {
    ...
    if (!agent || agent.human_pair_id !== args.userId) {
      throw new GraphQLError("Agent is not paired with the target user", {
        extensions: { code: "FORBIDDEN" },
      });
    }
  } else {
  ...
  if (i.operatingModel !== undefined)
    updates.operating_model =
      typeof i.operatingModel === "string"
        ? JSON.parse(i.operatingModel)
        : i.operatingModel;
  if (i.operatingModelHistory !== undefined)
    updates.operating_model_history = Array.isArray(i.operatingModelHistory)
      ? i.operatingModelHistory.map((entry: unknown) =>
  ...
  const affectsUserMd =
    ...
    i.family !== undefined ||
    i.context !== undefined ||
    i.operatingModel !== undefined ||
    i.operatingModelHistory !== undefined;
