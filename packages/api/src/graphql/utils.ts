...
export async function invokeComposition(
	...
	try {
		...
		if (!fnName) {
			return {
				ok: false,
				error: "agentcore-invoke Lambda name not configured (AGENTCORE_FUNCTION_NAME / SSM)",
			};
		}
		const { LambdaClient, InvokeCommand } = await import("@aws-sdk/client-lambda");
		const lambda = new LambdaClient({});
		...
		if (res.FunctionError) {
			...
			return { ok: false, error: `composition invoke threw: ${raw || res.FunctionError}` };
		}
		if (res.Payload) {
			try {
				...
				if (typeof parsed.statusCode === "number" && parsed.statusCode >= 400) {
					const inner = typeof parsed.body === "string" ? parsed.body : JSON.stringify(parsed.body);
					return { ok: false, error: `composition invoke returned ${parsed.statusCode}: ${inner}` };
				}
			} catch {
...
export function canonicalizeForHash(value: unknown): string {
	...
	const entries = keys.map((k) => `${JSON.stringify(k)}:${canonicalizeForHash(obj[k])}`);
	return `{${entries.join(",")}}`;
}

export function hashResolvedInputs(resolvedInputs: Record<string, unknown>): string {
