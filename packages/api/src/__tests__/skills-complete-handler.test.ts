...
const { mockSelect, mockUpdate, mockUpdateReturning } = vi.hoisted(() => ({
  mockSelect: vi.fn(),
  mockUpdate: vi.fn(),
...
const BODY = (overrides: Record<string, unknown> = {}) =>
  JSON.stringify({
    ...
    status: "failed",
    failureReason:
      "step 'gather' failed: skill 'crm_account_summary' not registered",
    ...overrides,
  });
    ...
    mockSelect.mockReturnValueOnce([
      {
        ...
        tenant_id: "T1",
        status: "running",
        completion_hmac_secret: HMAC_SECRET,
      },
    ]);
    mockUpdateReturning.mockReturnValueOnce([
      {
        id: "R1",
        status: "failed",
        finished_at: new Date("2026-04-22T13:00:00Z"),
      },
    ]);
...
describe("POST /api/skills/complete — validation", () => {
  ...
  it("400s when status is not complete and failureReason is missing", async () => {
    const res = await handler(
      EVENT({ body: BODY({ failureReason: undefined }) }),
    );
    expect(res.statusCode).toBe(400);
...
describe("POST /api/skills/complete — per-run HMAC", () => {
  ...
  it("401s when the signature header is missing", async () => {
    ...
    const res = await handler(
      EVENT({ headers: { authorization: "Bearer api-secret" } }),
    );
    expect(res.statusCode).toBe(401);
