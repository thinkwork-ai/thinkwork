...
describe("isPinnedFile", () => {
  it("returns true for guardrail-class files", () => {
    ...
    expect(isPinnedFile("expenses/GUARDRAILS.md")).toBe(true);
...
describe("applyPinAdvance", () => {
  it("writes the version store, bumps the pin, deletes the override, returns the updated row", async () => {
    ...
    pushDbRows([
      { id: "a1", agent_pinned_versions: { "GUARDRAILS.md": `sha256:${hex}` } },
    ]);
    ...
    expect(captured.agent_pinned_versions).toMatchObject({
      __sql: expect.stringContaining("jsonb_set"),
      values: expect.arrayContaining(["GUARDRAILS.md", `sha256:${hex}`]),
    });
  });

  it("still deletes the override when pin is already on the latest hash", async () => {
    const NEW = "# unchanged";
    const hex = sha(NEW);
  ...
  it("still deletes the nested override when a nested pin is already on the latest hash", async () => {
    const NEW = "# nested unchanged";
    const hex = sha(NEW);
    ...
    pushDbRows([
      {
        id: "a1",
        agent_pinned_versions: { "expenses/GUARDRAILS.md": `sha256:${hex}` },
      },
    ]);
    ...
    await applyPinAdvance({
      ...
      tenantSlug: "acme",
      templateSlug: "exec",
      filename: "expenses/GUARDRAILS.md",
      currentPins: { "expenses/GUARDRAILS.md": `sha256:${hex}` },
      latestContent: NEW,
      latestHex: hex,
    });
    ...
    expect(s3Mock.commandCalls(DeleteObjectCommand)[0].args[0].input.Key).toBe(
      "tenants/acme/agents/agent-a/workspace/expenses/GUARDRAILS.md",
    );
    expect(updateCaptured.length).toBe(0);
  });

  it("advances nested pins using the nested path for version and override keys", async () => {
    const NEW = "# nested content";
    const hex = sha(NEW);
    s3Mock.on(HeadObjectCommand).rejects(notFoundHead());
    s3Mock.on(PutObjectCommand).resolves({});
    s3Mock.on(DeleteObjectCommand).resolves({});
    ...
    expect(s3Mock.commandCalls(PutObjectCommand)[0].args[0].input.Key).toBe(
      `tenants/acme/agents/_catalog/exec/workspace-versions/expenses/GUARDRAILS.md@sha256:${hex}`,
    );
    expect(s3Mock.commandCalls(DeleteObjectCommand)[0].args[0].input.Key).toBe(
      "tenants/acme/agents/agent-a/workspace/expenses/GUARDRAILS.md",
    );
    expect(updateCaptured[0].agent_pinned_versions).toMatchObject({
...
describe("acceptTemplateUpdate", () => {
  it("rejects non-pinned filenames up front", async () => {
    await expect(
      acceptTemplateUpdate(
        null,
        { agentId: "a1", filename: "IDENTITY.md" },
        mockCtx(),
      ),
  ...
  it("throws NOT_FOUND when the agent is missing", async () => {
    ...
    await expect(
      acceptTemplateUpdate(
        null,
        { agentId: "a1", filename: "GUARDRAILS.md" },
        mockCtx(),
      ),
  ...
  it("accepts a nested path using inherited root template content when no nested base exists", async () => {
    const NEW = "# inherited root content";
    const hex = sha(NEW);
    pushDbRows([
      {
        id: "a1",
        ...
        tenant_id: "t1",
        template_id: "tmpl-1",
        agent_pinned_versions: { "GUARDRAILS.md": "sha256:old" },
      },
    ]);
    pushDbRows([{ role: "admin" }]);
    pushDbRows([{ slug: "acme" }]);
    pushDbRows([{ slug: "exec" }]);
    s3Mock
      .on(GetObjectCommand, {
    ...
    await acceptTemplateUpdate(
      null,
      { agentId: "a1", filename: "expenses/GUARDRAILS.md" },
      mockCtx(),
    );
...
describe("acceptTemplateUpdateBulk", () => {
  it("rejects non-pinned filenames up front", async () => {
    ...
    ).rejects.toThrow(/root pinned-class file/);
  });

  it("keeps bulk updates root-only", async () => {
    await expect(
      acceptTemplateUpdateBulk(
        null,
        {
    ...
    pushDbRows([
      {
        id: "agent-1",
        slug: "a1",
        agent_pinned_versions: { "GUARDRAILS.md": "sha256:old1" },
      },
      {
        id: "agent-2",
        slug: "a2",
        agent_pinned_versions: { "GUARDRAILS.md": "sha256:old2" },
      },
    ]);
