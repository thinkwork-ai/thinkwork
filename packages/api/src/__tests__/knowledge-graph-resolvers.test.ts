...
function evidence(overrides: Record<string, unknown> = {}) {
  return {
    ...
    char_start: 0,
    char_end: 9,
    evidence_source_kind: "thread_message",
    evidence_source_ref: "message-1",
      ...
      expect.objectContaining({
        ...
        evidence: [
          expect.objectContaining({
            messageId: "message-1",
            sourceKind: "THREAD",
            evidenceSourceKind: "THREAD_MESSAGE",
          }),
        ],
        relationships: [
          expect.objectContaining({
