...
describe("resolvePlaceForRecord", () => {
  ...
  it("falls back to metadata-only when the breaker is tripped", async () => {
    ...
    const out = await resolvePlaceForRecord(rec, {
      ...ctx(),
  ...
  it("falls back to postal_town when locality is absent (UK)", async () => {
    ...
    expect(placesCreated.find((p) => p.place_kind === "state")).toBeUndefined();
  });
  ...
  it("reuses an existing backing page when findExistingPageByTitleOrAlias hits", async () => {
    ...
    mockLookup.findExistingPageByTitleOrAlias.mockImplementation(
      async (args: any) => {
        if (args.title === "Paris") {
    ...
    const calls = (mockRepo.upsertPage as any).mock.calls.map(
      (c: any[]) => c[0],
    );
