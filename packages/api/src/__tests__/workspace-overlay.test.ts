...
function keys(
  tenantSlug = "acme",
  agentSlug = "marco",
  templateSlug = "exec-assistant",
) {
  return {
    agent: (p: string) =>
      `tenants/${tenantSlug}/agents/${agentSlug}/workspace/${p}`,
    template: (p: string) =>
...
describe("cross-tenant isolation", () => {
  it("throws AgentNotFoundError when ctx.tenantId does not match the agent's tenant", async () => {
    ...
    await expect(
      composeFile(ctx(TENANT_B), AGENT_MARCO, "IDENTITY.md"),
...
describe("composeFile — live class", () => {
  it("returns defaults content with substitution when agent has no override and template is empty", async () => {
    ...
    s3Mock
      .on(GetObjectCommand, { Key: k.agent("IDENTITY.md") })
      .rejects(noSuchKey());
    s3Mock
  ...
  it("renders {{HUMAN_*}} as em-dash pre-assignment", async () => {
    ...
    s3Mock
      .on(GetObjectCommand, { Key: k.agent("ROUTER.md") })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, { Key: k.template("ROUTER.md") })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, { Key: k.defaults("ROUTER.md") })
  ...
  it("throws FileNotFoundError when no layer has the path", async () => {
    ...
    await expect(
      composeFile(ctx(), AGENT_MARCO, "ghost.md"),
...
describe("composeFile — pinned class", () => {
  ...
  it("resolves against the pinned hash from the content-addressable store, not the latest base", async () => {
    ...
    const k = keys();
    s3Mock
      .on(GetObjectCommand, { Key: k.agent("GUARDRAILS.md") })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, {
  ...
  it("falls back to current template when versions store is empty but template hash matches pin", async () => {
    ...
    const k = keys();
    s3Mock
      .on(GetObjectCommand, { Key: k.agent("PLATFORM.md") })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, { Key: k.versions("PLATFORM.md", NEW_SHA) })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, { Key: k.template("PLATFORM.md") })
      .resolves(body(NEW_CONTENT));
  ...
  it("fails closed when pinned version cannot be resolved anywhere", async () => {
    ...
    s3Mock
      ...
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, { Key: k.versions("GUARDRAILS.md", OLD_SHA) })
      .rejects(noSuchKey());
    ...
    await expect(
      composeFile(ctx(), AGENT_MARCO, "GUARDRAILS.md"),
    ).rejects.toBeInstanceOf(PinnedVersionNotFoundError);
  });
  ...
  it("resolves a nested pinned path from its path-qualified pin before root", async () => {
    const subContent = "# Expenses guardrails\nPinned nested";
    const rootContent = "# Root guardrails\nPinned root";
    const subSha = sha256(subContent);
    const rootSha = sha256(rootContent);
    queueBaseContext({
      agent: agentRow({
        agent_pinned_versions: {
          "expenses/GUARDRAILS.md": `sha256:${subSha}`,
          "GUARDRAILS.md": `sha256:${rootSha}`,
        },
      }),
    });
    const k = keys();
    ...
    s3Mock
      .on(GetObjectCommand, {
        Key: k.versions("expenses/GUARDRAILS.md", subSha),
      })
      .resolves(body(subContent));
    s3Mock
      .on(GetObjectCommand, { Key: k.versions("GUARDRAILS.md", rootSha) })
      .resolves(body(rootContent));
    ...
    expect(result.path).toBe("expenses/GUARDRAILS.md");
    expect(result.source).toBe("template-pinned");
    expect(result.content).toBe(subContent);
    expect(result.sha256).toBe(subSha);
  });

  it("falls back to a root pin for nested pinned paths during the compatibility window", async () => {
    const rootContent = "# Root guardrails\nPinned root";
    const rootSha = sha256(rootContent);
    ...
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "using inherited pinned-version key GUARDRAILS.md",
      ),
    );
    warn.mockRestore();
  });
...
describe("composeFile — managed (USER.md)", () => {
  it("returns agent-scoped USER.md verbatim when one exists", async () => {
    ...
    s3Mock
      .on(GetObjectCommand, { Key: k.agent("USER.md") })
      .resolves(body(baked));
  ...
  it("falls through to template with substitution pre-assignment so admin preview is clean", async () => {
    ...
    s3Mock
      ...
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, { Key: k.template("USER.md") })
      .rejects(noSuchKey());
    s3Mock
...
describe("composeList", () => {
  it("returns the union of paths across layers with correct source labels and filters noise keys", async () => {
    ...
    const contentFor = (
      source: "agent" | "template" | "defaults",
      path: string,
    ...
    s3Mock
      .on(GetObjectCommand, { Key: k.template("IDENTITY.md") })
      .resolves(contentFor("template", "IDENTITY.md"));
    s3Mock
      .on(GetObjectCommand, { Key: k.defaults("SOUL.md") })
      .resolves(contentFor("defaults", "SOUL.md"));
    ...
    for (const path of CANONICAL_FALLBACKS) {
      ...
      s3Mock
        .on(GetObjectCommand, { Key: k.template(path) })
        .rejects(noSuchKey());
      s3Mock
  ...
  it("labels nested pinned metadata using path-aware pin lookup", async () => {
    const rootContent = "# Root guardrails";
    queueBaseContext({
      agent: agentRow({
        agent_pinned_versions: {
          "GUARDRAILS.md": `sha256:${sha256(rootContent)}`,
        },
      }),
    });
    const k = keys();
    ...
    s3Mock
      ...
      .resolves({
        Contents: [{ Key: k.template("expenses/GUARDRAILS.md") }],
      });
    s3Mock
      .on(ListObjectsV2Command, {
        Prefix: `tenants/acme/agents/_catalog/defaults/workspace/`,
    ...
    const listing = (await composeList(ctx(), AGENT_MARCO)) as Array<{
      path: string;
      source: string;
    }>;

    expect(
      listing.find((file) => file.path === "expenses/GUARDRAILS.md"),
    ).toMatchObject({
...
describe("composeFileCached + invalidateComposerCache", () => {
  ...
  it("re-fetches after invalidateComposerCache", async () => {
    ...
    expect(s3Mock.commandCalls(GetObjectCommand).length).toBeGreaterThanOrEqual(
      2,
    );
  });
});
...
describe("composeFile — recursive sub-agent paths", () => {
  ...
  it("falls through ancestor IDENTITY.md at agent layer when sub-agent has no override (AE2)", async () => {
    ...
    s3Mock
      .on(GetObjectCommand, { Key: k.agent("IDENTITY.md") })
      .resolves(body(rootIdentity));

    const result = await composeFile(
      ctx(),
      AGENT_MARCO,
      "expenses/IDENTITY.md",
    );
  ...
  it("3-level nested sub-agent resolves through three ancestors to template root", async () => {
    ...
    s3Mock
      .on(GetObjectCommand, {
        Key: k.agent("expenses/escalation/legal/GUARDRAILS.md"),
      })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, {
        Key: k.agent("expenses/escalation/GUARDRAILS.md"),
      })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, { Key: k.agent("expenses/GUARDRAILS.md") })
    ...
    s3Mock
      .on(GetObjectCommand, {
        Key: k.template("expenses/escalation/legal/GUARDRAILS.md"),
      })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, {
        Key: k.template("expenses/escalation/GUARDRAILS.md"),
      })
      .rejects(noSuchKey());
    s3Mock
  ...
  it("agent ancestor at depth 2 wins over template at requested depth", async () => {
    ...
    s3Mock
      .on(GetObjectCommand, { Key: k.agent("IDENTITY.md") })
      .resolves(body("agent root"));
    s3Mock
      .on(GetObjectCommand, {
        Key: k.template("expenses/escalation/IDENTITY.md"),
      })
    ...
    const result = await composeFile(
      ctx(),
      AGENT_MARCO,
      "expenses/escalation/IDENTITY.md",
    );
  ...
  it("rejects path traversal segments", async () => {
    ...
    ).rejects.toThrow(/invalid workspace path/i);
  });
  ...
  it("rejects absolute and empty-segment paths", async () => {
    await expect(
      composeFile(ctx(), AGENT_MARCO, "/GUARDRAILS.md"),
    ).rejects.toThrow(/invalid workspace path/i);
    await expect(
  ...
  it("does not inherit root pins through reserved folder segments", async () => {
    const rootContent = "# Root guardrails";
    queueBaseContext({
      agent: agentRow({
        agent_pinned_versions: {
    ...
    const k = keys();
    s3Mock
      .on(GetObjectCommand, { Key: k.agent("memory/GUARDRAILS.md") })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, { Key: k.template("memory/GUARDRAILS.md") })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, { Key: k.defaults("memory/GUARDRAILS.md") })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, {
        Key: k.versions("GUARDRAILS.md", sha256(rootContent)),
      })
      .resolves(body(rootContent));

    await expect(
      composeFile(ctx(), AGENT_MARCO, "memory/GUARDRAILS.md"),
    ).rejects.toBeInstanceOf(FileNotFoundError);
  });
  ...
  it("does not walk past skills/ reserved folder for nested SKILL.md", async () => {
    ...
    s3Mock
      .on(GetObjectCommand, {
        Key: k.agent("expenses/skills/approve-receipt/SKILL.md"),
      })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, {
        Key: k.template("expenses/skills/approve-receipt/SKILL.md"),
      })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, {
        Key: k.defaults("expenses/skills/approve-receipt/SKILL.md"),
      })
      .rejects(noSuchKey());
  ...
  it("substitutes placeholders during recursive resolution at template ancestor", async () => {
    ...
    s3Mock
      ...
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, { Key: k.template("expenses/IDENTITY.md") })
      .rejects(noSuchKey());
    s3Mock
      .on(GetObjectCommand, { Key: k.template("IDENTITY.md") })
      .resolves(body("Hi {{AGENT_NAME}}"));
...
describe("composeList — recursive enumeration", () => {
  it("includes sub-agent files in the union and labels them by source layer", async () => {
    ...
    expect(byPath.get("expenses/escalation/CONTEXT.md")?.source).toBe(
      "agent-override",
    );
...
describe("composeFileCached — recursive paths", () => {
  it("cache keys for root vs sub-agent paths do not collide", async () => {
    ...
    queueBaseContext();
    const sub = await composeFileCached(
      ctx(),
      AGENT_MARCO,
      "expenses/IDENTITY.md",
    );
