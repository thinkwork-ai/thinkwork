  ...
  return {
    db: {
        ...
        return {
          from: () => {
            if (which === 0) {
              ...
              return {
                where: () => ({
                  limit: () => Promise.resolve(mockTargetResult() as unknown[]),
                }),
              };
            }
            // source page rows — supports .where() awaited directly
            return {
              where: () => Promise.resolve(mockPageRowsResult() as unknown[]),
            };
          },
        };
      }),
      selectDistinct: vi.fn(() => ({
        from: () => ({
          where: () => Promise.resolve(mockSelectDistinctResult() as unknown[]),
        }),
      })),
    },
  };
...
describe("wikiBacklinks resolver dedup", () => {
  ...
  it("returns [] when the target page doesn't exist", async () => {
    ...
    const result = await wikiBacklinks(null, { pageId: "missing" }, makeCtx());
