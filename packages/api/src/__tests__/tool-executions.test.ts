...
const { mockEnforce } = vi.hoisted(() => ({
  mockEnforce: vi.fn().mockResolvedValue({ ok: true }),
}));
vi.mock("../lib/turn-assertion.js", () => ({
  enforceTurnAssertion: (args: unknown) => Promise.resolve(mockEnforce(args)),
}));
...
describe("POST /api/runtime/tool-executions", () => {
  ...
  it("passes the write-target binding to enforceTurnAssertion", async () => {
    mockEnforce.mockResolvedValueOnce({ ok: true });
    const res = await handler(ev(body()));
    expect(res.statusCode).toBe(200);
  ...
  it("returns 401 when enforcement refuses", async () => {
    mockEnforce.mockResolvedValueOnce({ ok: false, reason: "bad signature" });
    const res = await handler(ev(body()));
    expect(res.statusCode).toBe(401);
