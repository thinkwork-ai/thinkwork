/**
 * PR A — activity-timeline entries for webhook-driven external task updates.
 *
 * Focused on the `summarizeWebhookEvent()` branch + the system-message insert
 * it drives from inside `ingestExternalTaskEvent()`. Mocks the same moving
 * parts as `external-task-ingest-event.test.ts` so a single arrangement can
 * assert both "did we insert?" and "what did the content/metadata look like?".
 *
 * The goal is to lock the noise filter: `task.created` and property-only
 * noise (updated_at, viewed_at) must NOT insert a row, while real changes
 * (status, priority, due_at, assignee, comments) MUST produce a readable
 * summary line so the mobile timeline actually shows it.
 */
