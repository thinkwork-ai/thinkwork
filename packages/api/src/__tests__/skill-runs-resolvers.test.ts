...
import {
  startSkillRun,
  StartSkillRunError,
} from "../graphql/resolvers/skill-runs/startSkillRun.mutation.js";
import {
  cancelSkillRun,
  CancelSkillRunError,
} from "../graphql/resolvers/skill-runs/cancelSkillRun.mutation.js";
import {
  submitRunFeedback,
  SubmitRunFeedbackError,
} from "../graphql/resolvers/skill-runs/submitRunFeedback.mutation.js";
import {
  deleteRun,
  DeleteRunError,
} from "../graphql/resolvers/skill-runs/deleteRun.mutation.js";
...
describe("startSkillRun", () => {
  ...
  it("inserts a row, invokes AgentCore Event, and returns the run", async () => {
    ...
    const payload = mockInvokeSkillRun.mock.calls[0]![0] as Record<
      string,
      unknown
    >;
  ...
  it("rejects unauthorized caller", async () => {
    ...
    await expect(
      startSkillRun(
        null,
        { input: { skillId: "s", invocationSource: "chat" } },
        OK_CTX,
      ),
  ...
  it("rejects an invalid invocationSource", async () => {
    await expect(
      startSkillRun(
        null,
        { input: { skillId: "s", invocationSource: "chatroom" } },
        OK_CTX,
      ),
  ...
  it("transitions the row to failed when invokeSkillRun errors, then throws", async () => {
    ...
    mockUpdate.mockReturnValue([
      { ...insertedRow, status: "failed", failure_reason: "nope" },
    ]);
  ...
  it("requires skillId and invocationSource", async () => {
    await expect(
      startSkillRun(null, { input: {} as any }, OK_CTX),
...
describe("cancelSkillRun", () => {
  it("cancels a running row owned by the invoker", async () => {
    ...
    const out = (await cancelSkillRun(
      null,
      { runId: "run-1" },
      OK_CTX,
    )) as Record<string, unknown>;
  ...
  it("404s on cross-tenant run (existence must not leak)", async () => {
    ...
    await expect(
      cancelSkillRun(null, { runId: "run-x" }, OK_CTX),
  ...
  it("404s when caller is not the invoker", async () => {
    ...
    await expect(
      cancelSkillRun(null, { runId: "run-1" }, OK_CTX),
    ).rejects.toThrow(/run not found/);
  });
});
...
describe("submitRunFeedback", () => {
  it("writes positive feedback", async () => {
    ...
    const out = (await submitRunFeedback(
      null,
      {
  ...
  it("rejects unknown signals", async () => {
    ...
    ).rejects.toThrow(/positive|negative/);
  });
...
describe("deleteRun", () => {
  ...
  it("404s when caller is not the invoker", async () => {
    ...
    await expect(deleteRun(null, { runId: "run-1" }, OK_CTX)).rejects.toThrow(
      /run not found/,
    );
  });
});
...
describe("skillRuns list", () => {
  ...
  it("scopes to caller userId when no invokerUserId passed", async () => {
    mockSelect.mockReturnValue({
      rows: [{ id: "run-1", invoker_user_id: "U1" }],
    });
