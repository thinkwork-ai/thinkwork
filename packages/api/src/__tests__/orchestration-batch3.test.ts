  ...
  it("applies team override on top of tenant turn loop config", async () => {
    mockExecute.mockResolvedValueOnce({
      rows: [
        ...
        {
          turn_loop: { maxTurns: 10, continueOnToolUse: true },
          team_id: "team-1",
        },
      ],
    });
...
describe("resolveWorkflowConfig — workspace", () => {
  ...
  it("returns workspace defaults when no config exists", async () => {
    ...
    expect(config.workspace.prefixTemplate).toBe(
      "tenants/{tenantSlug}/agents/{agentSlug}/workspace/",
    );
  });
  ...
  it("applies custom prefix template from config", async () => {
    ...
    expect(config.workspace.prefixTemplate).toBe(
      "custom/{tenantSlug}/{agentSlug}/files/",
    );
  });
});
...
describe("workspace prefix generation", () => {
  ...
  function buildWorkspacePrefix(
    workflowConfig: {
      workspace: { isolateByThread: boolean; prefixTemplate: string };
    },
    tenantSlug: string,
    agentSlug: string,
    threadId?: string,
    ...
    const prefix = buildWorkspacePrefix(
      {
        workspace: {
          isolateByThread: false,
          prefixTemplate: "orgs/{tenantSlug}/bots/{agentSlug}/data/",
        },
      },
      "acme",
      "coder",
...
describe("turn loop gate conditions", () => {
  ...
  it("skips loop when signal is not continue", () => {
    expect(
      shouldEnterTurnLoop({ enabled: true, maxTurns: 5 }, "thread-1", "done"),
    ).toBe(false);
  });
