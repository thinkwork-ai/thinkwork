...
const { mockReturning, mockUpdate, mockWhere, mockSet, capturedWhere } =
  vi.hoisted(() => ({
    mockReturning: vi.fn(),
    mockUpdate: vi.fn(),
    mockWhere: vi.fn(),
...
describe("skill-runs-reconciler", () => {
  ...
  it("logs one line per reconciled row and a summary line", async () => {
    ...
    expect(
      messages.some(
        (m) => m.includes("row_reconciled") && m.includes("run_id=R1"),
      ),
    ).toBe(true);
    expect(
      messages.some(
        (m) => m.includes("reconciled=1") && m.includes("stale_after_min=15"),
      ),
    ).toBe(true);
    logSpy.mockRestore();
  });
