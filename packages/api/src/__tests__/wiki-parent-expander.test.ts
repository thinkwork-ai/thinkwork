...
describe("deriveParentCandidates", () => {
  ...
  it("resolves Mexican addresses with CDMX four-letter region code", () => {
    const out = deriveParentCandidates([
      makeRecord("r1", {
        place_address:
          "José María Izazaga 8, Centro, 06000 Ciudad de México, CDMX, Mexico",
      }),
      makeRecord("r2", {
        place_address:
          "Av. Insurgentes 100, 06000 Ciudad de México, CDMX, Mexico",
      }),
    ]);
  ...
  it("resolves Canadian 'B.C.' addresses", () => {
    const out = deriveParentCandidates([
      makeRecord("r1", {
        place_address: "100 Main St, Vancouver, B.C., Canada",
      }),
      makeRecord("r2", {
        place_address: "200 Main St, Vancouver, B.C., Canada",
      }),
    ]);
...
describe("deriveParentCandidatesFromPageSummaries", () => {
    ...
    const out = deriveParentCandidatesFromPageSummaries([
      {
        ...
        summary: "Korean-inspired restaurant in Toronto.",
      },
      { id: "p2", title: "Nana", summary: "Thai restaurant in Toronto." },
      {
        id: "p3",
        title: "Franklin Barbecue",
        summary: "BBQ joint in Austin, TX.",
      },
    ]);
  ...
  it("preserves accented city names in preposition matches", () => {
    ...
    expect(out.find((c) => c.parentTitle === "Bogotá")?.supportingCount).toBe(
      2,
    );
    expect(out.find((c) => c.parentTitle === "Montréal")?.supportingCount).toBe(
      2,
    );
  });
  ...
  it("strips trailing state codes from matches", () => {
    const out = deriveParentCandidatesFromPageSummaries([
      { id: "p1", title: "X", summary: "Located in Austin, TX and popular." },
      {
        id: "p2",
        title: "Y",
        summary: "A spot in Austin, TX with great views.",
      },
    ]);
    expect(out.find((c) => c.parentTitle === "Austin")?.supportingCount).toBe(
      2,
    ...
    const out = deriveParentCandidatesFromPageSummaries([
      {
        ...
        title: "Momofuku Daishō",
        summary:
          "Korean restaurant at 190 University Avenue, Toronto, ON M5H 0A3, Canada.",
      },
      {
        id: "p2",
        title: "Nana",
  ...
  it("prefers preposition match over address fallback", () => {
    ...
    const out = deriveParentCandidatesFromPageSummaries([
      ...
      },
      {
        ...
        title: "Bar",
        summary:
          "Restaurant in Austin, at 456 Other St, Austin, TX 78702, USA.",
      },
    ]);
