...
vi.mock("../lib/workspace-overlay.js", () => ({
	composeList: composeListMock,
}));
...
describe("deriveAgentSkills — happy paths", () => {
	it("unions skills across root + sub-agent AGENTS.md and dedups by slug", async () => {
		...
		const result = await deriveAgentSkills(ctx(), AGENT_ID);
...
`,
		...
		await expect(deriveAgentSkills(ctx(), AGENT_ID)).rejects.toThrow(
			/AGENTS\.md parse failed at broken\/AGENTS\.md:/,
		);
		expect(dbState.transactionInvocations).toBe(0);
		expect(dbState.insertCalls).toHaveLength(0);
	});
...
`,
	...
	it("throws when the agent doesn't exist in the database", async () => {
		...
		await expect(deriveAgentSkills(ctx(), "missing-agent")).rejects.toThrow(
