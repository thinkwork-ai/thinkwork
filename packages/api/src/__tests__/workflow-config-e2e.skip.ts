...
const CLUSTER_ARN =
  "arn:aws:rds:us-east-1:487219502366:cluster:thinkwork-ericodom-db";
const SECRET_ARN =
  "arn:aws:secretsmanager:us-east-1:487219502366:secret:thinkwork-ericodom-graphql-db-credentials-EMNbVe";
...
beforeAll(async () => {
  ...
  db = createDb(
    process.env.DATABASE_URL ||
      `postgresql://thinkwork_admin:password@localhost:5432/${DATABASE}`,
  );
...
describe("Workflow config resolution (e2e, real DB)", () => {
  it("queries workflow_configs table without error", async () => {
    const result = await db.execute(sql`
			SELECT
				...
				stall_detection, orchestration, prompt_template, team_id
  ...
  it("can insert and read back turn_loop + workspace config", async () => {
    ...
    const existing = await db.execute(sql`
			...
			WHERE tenant_id = ${sampleTenantId}::uuid AND team_id IS NULL
			LIMIT 1
		`);
    ...
    const turnLoop = (
      typeof rawTurnLoop === "string" ? JSON.parse(rawTurnLoop) : rawTurnLoop
    ...
    const workspace = (
      typeof rawWorkspace === "string" ? JSON.parse(rawWorkspace) : rawWorkspace
    ) as Record<string, unknown>;
