...
describe("initializePinnedVersions", () => {
	it("resolves template override when one exists and records the template hash", async () => {
		...
		s3Mock.on(GetObjectCommand, { Key: templateKey("GUARDRAILS.md") })
			.resolves(body(guardrails));
		// PLATFORM and CAPABILITIES fall through to defaults.
		s3Mock.on(GetObjectCommand, { Key: templateKey("PLATFORM.md") }).rejects(noSuchKey());
		s3Mock.on(GetObjectCommand, { Key: templateKey("CAPABILITIES.md") }).rejects(noSuchKey());
		s3Mock.on(GetObjectCommand, { Key: defaultsKey("PLATFORM.md") }).resolves(body("# Platform default"));
		s3Mock.on(GetObjectCommand, { Key: defaultsKey("CAPABILITIES.md") }).resolves(body("# Capabilities default"));
	...
	it("falls back to defaults when template has no override for the pinned file", async () => {
		s3Mock.on(GetObjectCommand, { Key: templateKey("GUARDRAILS.md") }).rejects(noSuchKey());
		s3Mock.on(GetObjectCommand, { Key: templateKey("PLATFORM.md") }).rejects(noSuchKey());
		s3Mock.on(GetObjectCommand, { Key: templateKey("CAPABILITIES.md") }).rejects(noSuchKey());
		const defaultGuardrails = "# Default guardrails";
		s3Mock.on(GetObjectCommand, { Key: defaultsKey("GUARDRAILS.md") }).resolves(body(defaultGuardrails));
		s3Mock.on(GetObjectCommand, { Key: defaultsKey("PLATFORM.md") }).resolves(body("# Platform"));
		s3Mock.on(GetObjectCommand, { Key: defaultsKey("CAPABILITIES.md") }).resolves(body("# Capabilities"));
		s3Mock.on(HeadObjectCommand).rejects(notFoundHead());
		s3Mock.on(PutObjectCommand).resolves({});
	...
	it("writes the content-addressable version store object so the composer can resolve by hash later", async () => {
		...
		s3Mock.on(GetObjectCommand, { Key: templateKey("GUARDRAILS.md") }).resolves(body(content));
		s3Mock.on(GetObjectCommand, { Key: templateKey("PLATFORM.md") }).rejects(noSuchKey());
		s3Mock.on(GetObjectCommand, { Key: templateKey("CAPABILITIES.md") }).rejects(noSuchKey());
		s3Mock.on(GetObjectCommand, { Key: defaultsKey("PLATFORM.md") }).rejects(noSuchKey());
		s3Mock.on(GetObjectCommand, { Key: defaultsKey("CAPABILITIES.md") }).rejects(noSuchKey());
		s3Mock.on(HeadObjectCommand).rejects(notFoundHead());
		s3Mock.on(PutObjectCommand).resolves({});

		await initializePinnedVersions({ tenantSlug: TENANT, templateSlug: TEMPLATE });
		...
		const guardrailsPut = puts.find((c) => c.args[0].input.Key === versionKey("GUARDRAILS.md", hex));
	...
	it("is idempotent — HEAD hit on the version key skips the PUT", async () => {
		...
		s3Mock.on(PutObjectCommand).rejects(new Error("unexpected PUT on idempotent re-run"));
