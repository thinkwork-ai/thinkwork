...
describe("assertTransition", () => {
  ...
  it("rejects invalid transitions", () => {
    ...
    expect(() => assertTransition("backlog", "in_review")).toThrow(
      "Invalid status transition",
    );
    expect(() => assertTransition("done", "cancelled")).toThrow(
      "Invalid status transition",
    );
    expect(() => assertTransition("done", "backlog")).toThrow(
      "Invalid status transition",
    );
    expect(() => assertTransition("cancelled", "done")).toThrow(
      "Invalid status transition",
    );
  ...
  it("rejects transition from unknown status", () => {
    expect(() => assertTransition("unknown", "todo")).toThrow(
      "Invalid status transition",
    );
  });

  it("rejects self-transition", () => {
    expect(() => assertTransition("backlog", "backlog")).toThrow(
      "Invalid status transition",
    );
