...
vi.mock("../lib/db.js", () => {
  ...
  const threadsBuilder: any = {
    ...
    then: (resolve: (rows: unknown[]) => unknown) => resolve(mockThreadRows()),
  };
...
describe("GET /api/threads/{tid}/attachments/{aid}/download", () => {
  ...
  it("returns 302 when Accept omits application/json (plain browser GET)", async () => {
    const res = await handler(event(undefined, { accept: "text/html,*/*" }));
    expect(res.statusCode).toBe(302);
  });
  ...
  it("returns 404 for unparseable threadId/attachmentId in path", async () => {
    const res = await handler(
      event("/api/threads/not-uuid/attachments/not-uuid/download"),
    );
    expect(res.statusCode).toBe(404);
  });
