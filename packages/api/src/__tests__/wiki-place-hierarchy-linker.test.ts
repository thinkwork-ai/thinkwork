...
describe("emitPlaceHierarchyLinks", () => {
  ...
  it("logs and skips when parent place has no backing page", async () => {
    ...
    const result = await emitPlaceHierarchyLinks({
      ...
      findPlaceById: vi
        .fn()
        .mockResolvedValue(placeRow({ parent_place_id: "orphan-parent" })),
  ...
  it("emits edges for multiple pages in one call (chain)", async () => {
    ...
    const result = await emitPlaceHierarchyLinks({
      ...
      findPlaceById: vi.fn().mockImplementation(async (args) => {
        if (args.id === "poi-1")
          return placeRow({ id: "poi-1", parent_place_id: "city-1" });
        if (args.id === "poi-2")
          return placeRow({ id: "poi-2", parent_place_id: "city-1" });
        if (args.id === "city-1")
          return placeRow({ id: "city-1", parent_place_id: "country-fr" });
        return null;
      }),
      findPageByPlaceId: vi.fn().mockImplementation(async (args) => {
        if (args.placeId === "city-1")
          return pageRow({ id: "page-city-paris" });
        if (args.placeId === "country-fr")
          return pageRow({ id: "page-france" });
        return null;
      }),
      writeLink,
    });
