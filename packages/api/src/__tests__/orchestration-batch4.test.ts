...
describe("renderPromptTemplate", () => {
  ...
  it("interpolates simple top-level variables", () => {
    const result = renderPromptTemplate("Source: {{source}}", {
      source: "chat_message",
    });
  ...
  it("interpolates nested dot-notation paths", () => {
    ...
    expect(result).toBe(
      "You are Research Agent working for acme. Current thread: Analyze data (in_progress)",
    );
  });
  ...
  it("handles whitespace in placeholder names", () => {
    const result = renderPromptTemplate("Agent: {{ agent.name }}", {
      agent: { name: "Alice" },
    });
  ...
  it("converts non-string values to string", () => {
    const result = renderPromptTemplate("Count: {{count}}", {
      count: 42,
    } as unknown as PromptTemplateContext);
    expect(result).toBe("Count: 42");
  });

  it("leaves null/undefined values as placeholder", () => {
    const result = renderPromptTemplate("ID: {{thread.identifier}}", {
