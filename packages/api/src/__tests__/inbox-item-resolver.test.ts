...
describe("assertInboxItemTransition", () => {
  it("allows approve from pending", () => {
    expect(() =>
      assertInboxItemTransition("pending", "approved"),
    ).not.toThrow();
  });

  it("allows reject from pending", () => {
    expect(() =>
      assertInboxItemTransition("pending", "rejected"),
    ).not.toThrow();
  });

  it("allows revision_requested from pending", () => {
    expect(() =>
      assertInboxItemTransition("pending", "revision_requested"),
    ).not.toThrow();
  });

  it("allows cancel from pending", () => {
    expect(() =>
      assertInboxItemTransition("pending", "cancelled"),
    ).not.toThrow();
  });

  it("allows resubmit (→ pending) from revision_requested", () => {
    expect(() =>
      assertInboxItemTransition("revision_requested", "pending"),
    ).not.toThrow();
  });

  it("allows cancel from revision_requested", () => {
    expect(() =>
      assertInboxItemTransition("revision_requested", "cancelled"),
    ).not.toThrow();
  });

  it("rejects transition from approved (terminal state)", () => {
    expect(() => assertInboxItemTransition("approved", "pending")).toThrow(
      "Invalid inbox item transition",
    );
    expect(() => assertInboxItemTransition("approved", "rejected")).toThrow(
      "Invalid inbox item transition",
    );
  });

  it("rejects transition from rejected (terminal state)", () => {
    expect(() => assertInboxItemTransition("rejected", "pending")).toThrow(
      "Invalid inbox item transition",
    );
    expect(() => assertInboxItemTransition("rejected", "approved")).toThrow(
      "Invalid inbox item transition",
    );
  });

  it("rejects transition from cancelled (terminal state)", () => {
    expect(() => assertInboxItemTransition("cancelled", "pending")).toThrow(
      "Invalid inbox item transition",
    );
  });

  it("rejects invalid target from pending", () => {
    expect(() => assertInboxItemTransition("pending", "pending")).toThrow(
      "Invalid inbox item transition",
    );
    expect(() => assertInboxItemTransition("pending", "expired")).toThrow(
      "Invalid inbox item transition",
    );
  });
  ...
  it("rejects approve/reject from revision_requested (must resubmit first)", () => {
    expect(() =>
      assertInboxItemTransition("revision_requested", "approved"),
    ).toThrow("Invalid inbox item transition");
    expect(() =>
      assertInboxItemTransition("revision_requested", "rejected"),
    ).toThrow("Invalid inbox item transition");
  });

  it("rejects unknown source status", () => {
    expect(() => assertInboxItemTransition("unknown", "approved")).toThrow(
      "Invalid inbox item transition",
    );
  });
