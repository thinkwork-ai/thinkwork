...
type Action = "revert-to-inherited" | "keep-as-override" | "materialize-sub-agent";
...
async function listAgentFiles(bkt: string, prefix: string): Promise<string[]> {
	...
	do {
		...
		for (const obj of resp.Contents ?? []) {
			...
			if (!rel || rel === "manifest.json" || rel === "_defaults_version") continue;
			out.push(rel);
		}
...
async function resolveAgentValues(agent: {
	...
	if (!agent.human_pair_id) return values;
	const [user] = await db()
...
export function classifyFatMigrationAction(
	...
	if (kind === "no-template" && path.includes("/")) return "materialize-sub-agent";
...
export function validateFatMigrationOptions(options: FatMigrationOptions): void {
	if (options.destructive && (!options.tenants || options.tenants.length === 0)) {
...
async function migrateOneAgent(
	...
	const values = await resolveAgentValues(row);
	...
	for (const path of files) {
		...
		if (options.destructive && action === "revert-to-inherited") {
			await s3.send(new DeleteObjectCommand({ Bucket: bkt, Key: `${prefix}${path}` }));
			deleted = true;
		}
		out.push({
			path,
