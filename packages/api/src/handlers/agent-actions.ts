...
async function updateProductivityConfig(
  ...
  const currentProd =
    (currentConfig.productivityConfig as Record<string, unknown>) || {};

  const enabled =
    body.emailTriageEnabled ??
    (currentProd.emailTriageEnabled as boolean) ??
    false;
  const intervalMin =
    body.emailTriageIntervalMin ??
    (currentProd.emailTriageIntervalMin as number) ??
    15;
  ...
  if (enabled) {
    ...
    console.log(
      `[agent-actions] Email triage enabled for agent ${agentId} at ${intervalMin}min intervals`,
    );
  } else {
