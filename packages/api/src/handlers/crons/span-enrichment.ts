...
> {
  try {
    ...
    if (code === "ResourceNotFoundException") {
      console.log(
        "[span-enrichment] Invocation log group not found — skipping",
      );
      return [];
    }
    throw err;
...
export async function handler(): Promise<void> {
  ...
  console.log(
    `[span-enrichment] Found ${estimatedEvents.length} estimated events to enrich`,
  );

  // 2. Query invocation logs for the full time window
  const invocations = await queryInvocationLogs(startTime, endTime);
  console.log(
    `[span-enrichment] Found ${invocations.length} invocation log entries`,
  );

  if (invocations.length === 0) {
    console.log(
      "[span-enrichment] No invocation logs available — will retry next run",
    );
    return;
  }
  ...
  for (const event of estimatedEvents) {
    ...
    if (bedrockRequestIds.length > 0) {
      ...
      if (matchedInvocations.length > 0) {
        console.log(
          `[span-enrichment] Matched event ${event.id} by ${matchedInvocations.length} request ID(s)`,
        );
      }
    }
    ...
    if (matchedInvocations.length === 0) {
      ...
      for (const inv of invocations) {
        ...
        if (
          event.model &&
          inv.modelId &&
          !inv.modelId.includes(event.model.replace(/^us\./, ""))
        )
          continue;
        if (delta < bestDelta) {
          bestDelta = delta;
      ...
      if (bestMatch) {
        ...
        console.log(
          `[span-enrichment] Matched event ${event.id} by timestamp fallback (delta=${bestDelta}ms)`,
        );
      }
    }
