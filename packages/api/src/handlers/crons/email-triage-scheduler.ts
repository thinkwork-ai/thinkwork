...
export async function handler(): Promise<{ enqueued: number }> {
  ...
  for (const agent of agents) {
    const prodConfig = (agent.runtime_config?.productivityConfig ||
    ...
    const lastFinished = (lastCompleted.rows || [])[0] as
      | { finished_at?: string }
      | undefined;
    if (lastFinished?.finished_at) {
      const sinceLastMs =
        Date.now() - new Date(lastFinished.finished_at).getTime();
    ...
    enqueued++;
    console.log(
      `[email-triage-scheduler] Enqueued triage for agent ${agent.id}`,
    );
  }
