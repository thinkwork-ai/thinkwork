...
import { routines, triggers, threadTurns } from "@thinkwork/database-pg/schema";
...
export async function handler(
  ...
  try {
    ...
    if (triggerDetailMatch) {
      ...
      if (method === "PUT") return updateTrigger(routineId, triggerId, event);
      if (method === "DELETE") return deleteTrigger(routineId, triggerId);
      return error("Method not allowed", 405);
    }

    // /api/routines/:id/triggers
    const triggersMatch = path.match(/^\/api\/routines\/([^/]+)\/triggers$/);
    if (triggersMatch) {
...
async function listRoutines(
  ...
  if (params.agent_id) conditions.push(eq(routines.agent_id, params.agent_id));
...
async function getRun(
  ...
  const [run] = await db
    ...
    .from(threadTurns)
    .where(
      and(eq(threadTurns.id, runId), eq(threadTurns.routine_id, routineId)),
    );
...
async function createTrigger(
  ...
  const [row] = await db
    ...
    .values({
      ...
      config: body.config,
      schedule_type: "cron",
      schedule_expression: cronExpr.startsWith("cron(")
        ? cronExpr
        : cronExpr
          ? `cron(${cronExpr} *)`
          : "",
...
async function updateTrigger(
  ...
  const [updated] = await db
    .update(triggers)
    .set(updates)
    .where(and(eq(triggers.id, triggerId), eq(triggers.routine_id, routineId)))
    .returning();
