...
async function getCurrentUser(
  ...
  const userId =
    event.headers["x-principal-id"] || event.queryStringParameters?.["userId"];

  if (!userId) {
    return error(
      "Cannot determine current user: missing x-principal-id header",
    );
  }
...
async function getUser(id: string): Promise<APIGatewayProxyStructuredResultV2> {
