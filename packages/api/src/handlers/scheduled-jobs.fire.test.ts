...
const mocks = vi.hoisted(() => ({
  ...
  selectFromAgentTemplates: vi.fn(),
...
vi.mock("../lib/db.js", () => {
    ...
    return {
      from: (table: unknown) => {
        ...
        const exec = async (): Promise<unknown[]> => {
          if (t === "scheduledJobs") return mocks.selectFromScheduledJobs();
          if (t === "computers") return mocks.selectFromComputers();
          if (t === "agents") return mocks.selectFromAgents();
          if (t === "agentTemplates") return mocks.selectFromAgentTemplates();
          return [];
        };
        return {
          where: (_cond: unknown) => ({
  ...
  const insert = (table: unknown) => {
    ...
    const dispatchInsert = (values: Record<string, unknown>) => {
      if (t === "messages") return mocks.insertIntoMessages({ values });
      if (t === "computerTasks")
