...
export async function handler(
	...
	try {
		...
		// /api/code-factory/repos/:id/jobs
		const repoJobsMatch = path.match(
			/^\/api\/code-factory\/repos\/([^/]+)\/jobs$/,
		);
		if (repoJobsMatch) {
		...
		// POST /api/code-factory/repos/provision — auto-create GitHub repo for tenant
		if (path === "/api/code-factory/repos/provision" && method === "POST") {
			return provisionRepo(event);
		}

		// /api/code-factory/repos
		if (path === "/api/code-factory/repos") {
		...
		console.error("Code factory handler error:", err);
