...
describe("mcp-oauth handler", () => {
  ...
  it("serves protected-resource metadata for the Codex resource path", async () => {
    const response = await handler(
      event("GET", "/.well-known/oauth-protected-resource/mcp/user-memory"),
    );
  ...
  it("registers Codex loopback redirect URIs and redirects authorize requests to Cognito", async () => {
    ...
    const { client_id } = JSON.parse(registration.body || "{}") as {
      client_id: string;
    };
  ...
  it("rejects unsupported scopes before redirecting to Cognito", async () => {
    ...
    expect(JSON.parse(authorize.body || "{}").error_description).toContain(
      "invalid_scope",
    );
  });
  ...
  it("does not allow an authorize-state token to be exchanged as an authorization code", async () => {
    ...
    expect(JSON.parse(token.body || "{}").error_description).toContain(
  ...
  it("exchanges a callback for an authorization code and then for a bearer token", async () => {
    ...
    expect(codexRedirect.origin + codexRedirect.pathname).toBe(
    ...
    expect(JSON.parse(replay.body || "{}").error_description).toContain(
      "already been used",
    );
  });
  ...
  it("accepts missing, invalid, duplicate, and expired token revocations idempotently", async () => {
    ...
    const expired = await handler(
      event(
        ...
        "/mcp/oauth/revoke",
        new URLSearchParams({
          token: signedAccessToken({ jti: "expired" }, -900),
        }).toString(),
        undefined,
        "application/x-www-form-urlencoded",
      ),
  ...
  it("rejects revoked tokens on subsequent MCP access-token verification", async () => {
    ...
    await expect(
      verifyMcpAccessToken(accessToken, resource, `https://${host}`),
