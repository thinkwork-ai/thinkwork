...
export async function handler(
  ...
  try {
    ...
    const historyMatch = path.match(/^\/api\/webhooks\/([^/]+)\/history$/);
    if (historyMatch) {
...
async function listWebhooks(
  ...
  if (spaceId) conditions.push(eq(webhooks.space_id, spaceId));
  ...
  const rows = await db
    .select({
      ...
      target_name: sql<
        string | null
      >`coalesce(${agents.name}, ${routines.name})`.as("target_name"),
    })
    .from(webhooks)
  ...
  if (targetType === "agent" && webhook.agent_id) {
    ...
    const [turn] = await db
      ...
      .values({
        ...
        trigger_detail: `webhook_test:${webhook.id}`,
        status: "queued",
        context_snapshot: { ...testPayload, spaceId: webhook.space_id },
      })
      .returning();
...
async function getHistory(
  ...
  const rows = await db
    ...
    .from(threadTurns)
    .where(
      and(eq(threadTurns.webhook_id, id), eq(threadTurns.tenant_id, tenantId)),
    )
