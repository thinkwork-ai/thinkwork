...
import type { PlaceholderValues } from "../lib/placeholder-substitution.js";
...
async function resolveAgentValues(
	agent: { id: string; name: string; tenant_id: string; human_pair_id: string | null },
	tenantName: string,
...
async function migrateOneAgent(
	...
	const values = await resolveAgentValues(agent, tenant.name);
...
async function loadCheckpoint(bkt: string, runId: string): Promise<Checkpoint | null> {
...
function summarize(results: PerAgentResult[]) {
	const s = {
		...
		agentsWithErrors: 0,
		files: { fork: 0, override: 0, "review-required": 0, "no-template": 0 } as Record<
...
export async function runMigration(opts: MigrateOptions): Promise<MigrateResult> {
	...
	while (true) {
		const rows = await db
			...
			.where(
				filterTenantId
					? and(eq(agents.tenant_id, filterTenantId))
					: undefined,
			)
			.limit(batchSize)
			.offset(cursor);
...
if (
	...
	(async () => {
		...
		console.log(`[migrate-overlay] starting mode=${opts.mode} tenant=${opts.tenantSlug ?? "(all)"} runId=${opts.runId ?? "(fresh)"}`);
		try {
			const out = await runMigration(opts);
			console.log(`[migrate-overlay] done — report: s3://${bucket()}/${out.reportKey}`);
			console.log(`[migrate-overlay] summary:`, JSON.stringify(out.summary, null, 2));
		} catch (err) {
