...
export async function handler(
  ...
  if (
    ...
    return error("actorType must be one of: user, system, agent", 400);
  }
  ...
  if (idempotencyKey && idempotencyKey !== body.event_id) {
    return error("Idempotency-Key header does not match body event_id", 400);
  }
  ...
  const source =
    typeof body.source === "string" && body.source ? body.source : "strands";
