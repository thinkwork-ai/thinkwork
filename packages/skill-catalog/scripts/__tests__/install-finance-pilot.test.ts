...
describe("parseArgs", () => {
  ...
  it("rejects --api-key without --tenant-id", () => {
    expect(() =>
      parseArgs(["--api-url=x", "--api-key=secret", "--agent-id=a"]),
    ).toThrow(/tenant-id/);
  });

  it("rejects passing neither --token nor --api-key", () => {
    expect(() => parseArgs(["--api-url=x", "--agent-id=a"])).toThrow(
      /token.*api-key|api-key.*token/,
    );
  });
...
describe("collectSkillFiles", () => {
  ...
  it("collects LICENSE-NOTES.md for the two lifted skills", async () => {
    for (const slug of ["finance-3-statement-model", "finance-audit-xls"]) {
      const records = await collectSkillFiles(slug);
      expect(
  ...
  it("throws when the skill directory is missing", async () => {
    await expect(collectSkillFiles("does-not-exist")).rejects.toThrow(
      /missing/,
    );
  });
});
