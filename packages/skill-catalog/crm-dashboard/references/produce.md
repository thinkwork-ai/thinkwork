...
- The only allowed CSS color tokens are the shadcn set propagated by the iframe host: `--background`, `--foreground`, `--card`, `--card-foreground`, `--popover`, `--popover-foreground`, `--primary`, `--primary-foreground`, `--secondary`, `--secondary-foreground`, `--muted`, `--muted-foreground`, `--accent`, `--accent-foreground`, `--destructive`, `--destructive-foreground`, `--border`, `--input`, `--ring`, `--chart-1` through `--chart-5`, and the `--sidebar*` family. Do NOT use `--info`, `--warning`, `--success`, `--color-info`, `--color-warning`, `--color-success`, or any other invented token — they resolve to undefined and the chart falls back to black. The same restriction applies to Tailwind utility classes built from those tokens (no `bg-info`, `text-warning`, `border-success`, etc.). For status/severity meaning, reuse `--primary`, `--destructive`, `--muted-foreground`, or a `--chart-N` slot.
