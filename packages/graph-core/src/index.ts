/**
 * @thinkwork/graph-core — renderer-agnostic graph logic shared by the web
 * (react-force-graph-2d / canvas) and mobile (react-native-skia) renderers.
 *
 * Everything here is pure and DOM-free (no `document`/`window`/canvas): community
 * detection, degree sizing, the community color palette, node classification,
 * neighborhood/focus math, layout anchoring, and label wrapping/zoom-gating.
 * The one web-only helper (`isDarkMode`, which reads the DOM) stays in
 * `@thinkwork/graph`'s `graph-utils.ts`. Consumed directly from `.ts` source
 * (see package.json `main`), so Metro and Vite transpile it in place — no build.
 */
 ...
 * `measure` is the text measurer for the already-set font (canvas
 * `measureText().width` on web, Skia glyph width on native).
 */
