...
/**
 * ThinkWork-owned primitive component *definitions* for the Thread json-render
 * catalog (THINK-116, D4 / U10).
 *
 * These replace the former `@json-render/shadcn/catalog` definitions. The type
 * names and prop schemas are copied verbatim from that catalog so every
 * previously-persisted spec keeps validating byte-for-byte — the migration is
 * schema-compatible, not a rewrite of the wire contract. The React
 * implementations that render these definitions live in the web app
 * (`apps/web/.../json-render/primitives`), built on `@base-ui/react` + styled
 * semantic HTML instead of the Radix-based shadcn layer.
 *
 * COMPAT INVARIANT: do not tighten or reorder these schemas. `className` stays
 * `.nullable()` (specs pass `className: null`, which the strict validator
 * accepts as a nullable forbidden-key). Enum member sets must remain supersets
 * of everything the shadcn catalog accepted.
 */
...
/**
 * Type for a primitive component definition (mirrors the former
 * `@json-render/shadcn` `ComponentDefinition`).
 */
export type ThreadJsonRenderPrimitiveComponentDefinition = {
  props: z.ZodType;
  slots?: string[];
  events?: string[];
  description: string;
...
/**
 * Infer the props type for a primitive component by name, derived directly from
 * its zod schema so React implementations stay in sync with the catalog.
 */
