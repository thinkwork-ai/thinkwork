...
import { EMPTY_TREE_ROOT, computeLeafHash, verifyProofPath } from "./merkle";
import {
...
import { enumerateAnchors, getJsonBody, isUnrecoverableS3Error } from "./s3";
import { checkRetention, type RetentionFailure } from "./retention";
import { walkTenantChain, type ChainFailure } from "./chain";
    ...
    limit(async () => {
      ...
      const sliceResults = await Promise.allSettled(
        parsedAnchor.proof_keys.map((sliceKey) =>
          limit(async () => {
            const sliceBody = await getJsonBody(s3, opts.bucket, sliceKey);
      ...
      for (let i = 0; i < sliceResults.length; i += 1) {
        ...
        if (r.status === "rejected") {
          if (r.reason instanceof SchemaVersionUnsupportedError) {
            ...
            throw r.reason;
          } else if (isS3NotFoundError(r.reason)) {
            merkleMismatches.push({
              cadence_id: parsedAnchor.cadence_id,
              key: sliceKey,
              tenant_id: null,
        ...
        const replayedRoot = verifyProofPath(slice.leaf_hash, slice.proof_path);
      ...
      if (opts.checkRetention) {
        await runRetentionCheck(s3, opts.bucket, anchor.key, retentionFailures);
      }
    }),
  );
