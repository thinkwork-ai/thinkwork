    ...
    for (const tenantId of tenants) {
      try {
        ...
        failures.push({
          ...
          reason: "query_error",
          expected_prev_hash: err instanceof Error ? err.message : String(err),
        });
      }
    }
  } finally {
...
interface PgClient {
  ...
  query(
    cursorOrText: PgCursor | string,
    values?: unknown[],
  ): PgCursor | Promise<{ rows: unknown[] }>;
  end(): Promise<void>;
}

type PgCursorCtor = new (sql: string, values?: unknown[]) => PgCursor;

interface PgCursor {
  read(
    rowCount: number,
    callback: (err: Error | null, rows: ChainRow[]) => void,
  ): void;
