...
class AgentCoreHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # Workflow-skill injection: when the thread's workflow ships
            # its own intake instructions/form (LastMile per-workflow skill),
            # render them as a `## Workflow Skill` block so the
            # lastmile-tasks skill takes its dynamic path instead of the
            # hardcoded 4-question fallback form.
