...
def test_execute_agent_turn_adds_computer_applet_contract(monkeypatch):
    ...
    # The save_app / preview_app rules now live in the workspace-resident
    # artifact-builder skill (packages/workspace-defaults/files/skills/
    # artifact-builder/SKILL.md) which the AgentSkills plugin handles via
    # progressive disclosure -- the agent loads the skill content on demand.
    # This test verifies the env-var protocol + the applet-build-request
    # classifier-driven suppress_app_build_helper_tools wiring; the contract
    # content is no longer injected into the system prompt directly.
