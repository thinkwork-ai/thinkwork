...
class TestValidateMemoryPath(unittest.TestCase):
    ...
    def test_parent_root_preferences(self):
        ...
        self.assertEqual(_validate_memory_path("memory/preferences.md"), "memory/preferences.md")

    def test_parent_root_contacts(self):
        from write_memory_tool import _validate_memory_path

        self.assertEqual(_validate_memory_path("memory/contacts.md"), "memory/contacts.md")
    ...
    def test_workspace_parent_layout(self):
        from write_memory_tool import _validate_memory_path

        self.assertEqual(
            _validate_memory_path("workspaces/expenses/memory/lessons.md"),
    ...
    def test_nfkc_fullwidth_collapses_to_canonical(self):
        ...
        self.assertEqual(_validate_memory_path("ｍemory/lessons.md"), "memory/lessons.md")
...
class TestWriteMemoryTool(unittest.TestCase):
    ...
    def test_sub_agent_path_posts_verbatim(self):
        """Workspace: write_memory("workspaces/expenses/memory/lessons.md", ...) lands at workspace scope."""
        from write_memory_tool import write_memory

        captured = []
        with patch("urllib.request.urlopen", self._fake_urlopen({"ok": True}, captured)):
            fn = _unwrap(write_memory)
            result = fn(path="workspaces/expenses/memory/lessons.md", content="x")

        self.assertIn("saved", result)
        body = captured[0]["body"]
        self.assertEqual(body["path"], "workspaces/expenses/memory/lessons.md")
        self.assertEqual(body["agentId"], "agent-marco")
...
class TestWriteMemoryToolSchema(unittest.TestCase):
    ...
    def test_tool_schema_uses_path_not_name(self):
        ...
        spec = getattr(write_memory, "tool_spec", None) or getattr(write_memory, "spec", None)
        if spec is None:
            self.skipTest(
