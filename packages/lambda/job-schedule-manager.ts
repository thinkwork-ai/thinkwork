...
async function ensureScheduleGroup(): Promise<void> {
  ...
  try {
    ...
    if ((err as { name?: string }).name !== "ConflictException") {
      console.warn(
        "[job-schedule-manager] Failed to create schedule group:",
        err,
      );
    }
  }
...
async function createJob(
  ...
  if (!targetArn || !roleArn)
    throw new Error("JOB_TRIGGER_ARN and JOB_TRIGGER_ROLE_ARN must be set");
  ...
  await schedulerClient.send(
    new CreateScheduleCommand({
      ...
      State:
        body.enabled !== false ? ScheduleState.ENABLED : ScheduleState.DISABLED,
  ...
  console.log(
    `[job-schedule-manager] Created EB schedule ${scheduleName} for trigger ${jobId} (${expression})`,
  );
...
async function updateJob(
  ...
  if (!isEnabled && wasEnabled && current.eb_schedule_name) {
    ...
    try {
      ...
      console.log(
        `[job-schedule-manager] Deleted EB schedule ${current.eb_schedule_name} (trigger disabled)`,
      );
    } catch (ebErr) {
      console.error(
        `[job-schedule-manager] Failed to delete EB schedule on disable:`,
        ebErr,
      );
    }
    ...
    await ensureScheduleGroup();
    const scheduleName =
      current.eb_schedule_name ||
      buildScheduleName(current.id, current.schedule_type === "at");
    const target: Target = {
      Arn: targetArn,
      RoleArn: roleArn,
      Input: JSON.stringify({
    ...
    try {
      ...
      console.log(
        `[job-schedule-manager] Recreated EB schedule ${scheduleName} (trigger re-enabled)`,
      );
    } catch (ebErr) {
      console.error(
        `[job-schedule-manager] Failed to recreate EB schedule on enable:`,
        ebErr,
      );
    }
    ...
    await ensureScheduleGroup();
    const scheduleName = buildScheduleName(
      current.id,
      current.schedule_type === "at",
    );
    const target: Target = {
    ...
    await db
      ...
      .where(eq(triggers.id, body.triggerId));
    console.log(
      `[job-schedule-manager] Repaired missing EB schedule ${scheduleName} for trigger ${current.id}`,
    );
  }
...
async function deleteJob(body: DeleteJobBody): Promise<void> {
  ...
  let scheduleName = (body as unknown as Record<string, unknown>)
    .ebScheduleName as string | undefined;
  ...
  if (scheduleName) {
    try {
      ...
      console.log(
        `[job-schedule-manager] Deleted EB schedule: ${scheduleName}`,
      );
    } catch (deleteErr) {
      console.warn(
        `[job-schedule-manager] Failed to delete EB schedule:`,
        deleteErr,
      );
    }
  }
...
async function listJobs(params: Record<string, string>): Promise<unknown[]> {
  ...
  if (params.spaceId) conditions.push(eq(triggers.space_id, params.spaceId));
  if (params.routineId)
    conditions.push(eq(triggers.routine_id, params.routineId));
  if (params.triggerType)
    conditions.push(eq(triggers.trigger_type, params.triggerType));
  if (params.enabled !== undefined)
    conditions.push(eq(triggers.enabled, params.enabled === "true"));
    ...
    if (method === "POST") {
      ...
      if (
        ...
        return {
          ...
          body: JSON.stringify({
            error:
              "tenantId, triggerType, name, scheduleType, and scheduleExpression are required",
          }),
          headers: JSON_HEADERS,
        };
      }
      ...
      return {
        statusCode: 201,
        body: JSON.stringify({ ok: true, job: result }),
        headers: JSON_HEADERS,
      };
    }
