...
import { auditEvents, tenantAnchorState } from "@thinkwork/database-pg/schema";
import {
  _anchor_fn_live,
  buildMerkleTree,
  computeLeafHash,
...
describe.skipIf(skip)("compliance-anchor: integration against dev DB", () => {
  ...
  it("multi-tenant fixture produces deterministic Merkle root across runs", async () => {
    ...
    const a = await seedAuditEvents(db, ANCHOR_TEST_TENANT_A, 3, baseSeq);
    const b = await seedAuditEvents(db, ANCHOR_TEST_TENANT_B, 2, baseSeq + 100);
    ...
    const tenantA = heads.find((h) => h.tenant_id === ANCHOR_TEST_TENANT_A);
    const tenantB = heads.find((h) => h.tenant_id === ANCHOR_TEST_TENANT_B);
