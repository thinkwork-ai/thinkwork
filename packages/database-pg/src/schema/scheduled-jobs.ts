...
export const scheduledJobs = pgTable(
  ...
  (table) => [
    index("idx_scheduled_jobs_tenant_type").on(
      table.tenant_id,
      table.trigger_type,
    ),
    index("idx_scheduled_jobs_space").on(table.tenant_id, table.space_id),
...
export const threadTurnsRelations = relations(threadTurns, ({ one, many }) => ({
  tenant: one(tenants, {
