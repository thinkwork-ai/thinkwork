...
- [knowledge_graph_ingest_runs](#knowledge-graph-ingest-runs)
- [knowledge_graph_observation_cursors](#knowledge-graph-observation-cursors)
...
- `brain_dream_actions.run_id` → `brain_dream_runs.id`
...
- `brain_dream_runs.tenant_id` → `tenants.id`
...
- `knowledge_graph_entities.thread_id` → `threads.id`
...
- `source_kind`: `thread`, `wiki`, `brain`, `observations`
- `evidence_source_kind`: `thread_message`, `wiki_page`, `wiki_section`, `brain_page`, `brain_section`, `hindsight_observation`, `graph_payload`, `normalizer`

Join hints:

- `knowledge_graph_evidence.entity_id` → `knowledge_graph_entities.id`
- `knowledge_graph_evidence.ingest_run_id` → `knowledge_graph_ingest_runs.id`
- `knowledge_graph_evidence.message_id` → `messages.id`
- `knowledge_graph_evidence.relationship_id` → `knowledge_graph_relationships.id`
- `knowledge_graph_evidence.tenant_id` → `tenants.id`
- `knowledge_graph_evidence.thread_id` → `threads.id`
...
- `source_kind`: `thread`, `wiki`, `brain`, `observations`
- `status`: `queued`, `running`, `succeeded`, `failed`, `canceled`, `stale_noop`
- `trigger`: `manual`, `scheduled`

Join hints:

- `knowledge_graph_ingest_runs.requested_by_user_id` → `users.id`
- `knowledge_graph_ingest_runs.tenant_id` → `tenants.id`
- `knowledge_graph_ingest_runs.thread_id` → `threads.id`
...
- `knowledge_graph_observation_cursors.tenant_id` → `tenants.id`
...
- `knowledge_graph_relationships.ingest_run_id` → `knowledge_graph_ingest_runs.id`
- `knowledge_graph_relationships.ontology_relationship_type_id` → `relationship_types.id`
- `knowledge_graph_relationships.source_entity_id` → `knowledge_graph_entities.id`
- `knowledge_graph_relationships.target_entity_id` → `knowledge_graph_entities.id`
- `knowledge_graph_relationships.tenant_id` → `tenants.id`
- `knowledge_graph_relationships.thread_id` → `threads.id`
...
- `status`: `queued`, `running`, `retained`, `failed_timeout`, `failed_backend`, `dead_lettered`

Join hints:

- `memory_retain_attempts.space_id` → `spaces.id`
- `memory_retain_attempts.tenant_id` → `tenants.id`
