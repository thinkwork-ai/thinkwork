...
export async function startStripeCheckout(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        status: "error",
        message:
          body.error || `Checkout could not be started (HTTP ${res.status})`,
      };
    }
    ...
    const message =
      err instanceof Error ? err.message : "Network error starting checkout";
  ...
  try {
    ...
    const message =
      err instanceof Error ? err.message : "Browser session failed to open";
    return { status: "error", message };
  }
}
