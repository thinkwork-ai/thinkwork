...
export function useHeartbeatRunDetail(id: string | undefined) {
  return useQuery({
    query: ThreadTurnDetailQuery,
    variables: { id: id! },
    pause: !id,
  });
}
export function useHeartbeatRunEvents(
  runId: string | undefined,
  opts?: { afterSeq?: number; limit?: number },
) {
  return useQuery({
    query: ThreadTurnEventsQuery,
    variables: { runId: runId!, ...opts },
    pause: !runId,
  });
}
export function useCancelHeartbeatRun() {
  return useMutation(CancelThreadTurnMutation);
}
export function useCreateWakeupRequest() {
  return useMutation(CreateWakeupRequestMutation);
}
