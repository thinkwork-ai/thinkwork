...
function InfoRow({
  ...
  return (
    <View
      className={`flex-row items-center justify-between py-3 ${isLast ? "" : "border-b border-neutral-200 dark:border-neutral-800"}`}
    >
      ...
      {valueComponent || (
        <View
          className={`ml-3 max-w-[55%] items-end ${valueContainerClassName || ""}`}
        >
          <Text
            className={`text-base text-right text-neutral-900 dark:text-neutral-100 ${valueClassName || ""}`}
          >
            {value}
          </Text>
        </View>
...
function normalizeGitHubRepoInput(input: string): string | null {
  ...
  if (direct)
    return `${direct[1]}/${direct[2].replace(/\.git$/i, "")}`.toLowerCase();
  const https = raw.match(
    /^https?:\/\/(?:www\.)?github\.com\/([^/\s]+)\/([^/\s#?]+)(?:[/?#].*)?$/i,
  );
  if (https)
    return `${https[1]}/${https[2].replace(/\.git$/i, "")}`.toLowerCase();
...
export function AgentDetailContent({
  ...
  const updateAgent = async (args: any) => {
    await executeUpdateAgent(args);
  };
  ...
  const agentInstanceId =
    (agentDetail as any)?.instanceId ?? (gateway as any)?.instanceId;
  ...
  const selectedModelLabel =
    selectedCatalogModel?.label ?? selectedModelId ?? "Kimi K2.5";
  const agentTypeLabel =
    gwAny.type === "ecs" ? "Cloud (Hosted)" : "BYOB (Self-hosted)";
  ...
  const displayVersion = versionLabel(
    gwAny.taskDefinitionArn as string | undefined,
    currentDigest,
    currentVersion,
  );
  const isRestartable =
  ...
  const isRelaunchable =
    gwAny.status === "failed" ||
    gwAny.status === "terminated" ||
    (gwAny.status === "stopped" && gwAny.connectionStatus !== "online");
  const isStartable = gwAny.status === "stopped" && !isRelaunchable;
  const hasDigestUpdate = Boolean(
    latestDigest && currentDigest && latestDigest !== currentDigest,
  );
  ...
  const lastHeartbeatLabel = gwAny.lastHeartbeatAt
    ? new Date(gwAny.lastHeartbeatAt).toLocaleString()
    : "—";
      ...
      <WebContent>
        <View className="mx-4 mt-3 rounded-lg border border-neutral-200 dark:border-neutral-800 px-4 overflow-hidden">
          ...
          {gwAny.type !== "local" && (agentInstanceId || gateway.baseUrl) && (
            <InfoRow
              ...
              valueComponent={
                <Pressable
                  onPress={async () => {
                    await Clipboard.setStringAsync(
                      agentInstanceId ?? gateway.baseUrl,
                    );
                    setCopiedUrl(true);
                ...
                >
                  <Text
                    className="text-neutral-900 dark:text-neutral-100 font-mono text-sm"
                    numberOfLines={1}
                  >
                    {agentInstanceId ??
                      gateway.baseUrl
                        .replace(/^https?:\/\//, "")
                        .replace(/\.thinkwork\.ai.*$/, "")}
                  </Text>
                  {copiedUrl ? (
          ...
          {(() => {
            ...
            const agentTypeLabel =
              AGENT_TYPE_OPTIONS.find((o) => o.value === gwAny.type)?.label ??
              gwAny.type ??
              "—";
            const isAgentTypeEditable =
              canEdit &&
              (gwAny.type === "claude" ||
                gwAny.type === "sdk" ||
                gwAny.type === "pi" ||
          ...
          {gateway.type !== "ecs" && (
            <InfoRow
              label="Default"
              labelIcon={<Star size={16} color="#0ea5e9" />}
              value={gateway.isDefault ? "Yes" : "No"}
            />
          )}
            ...
            <Pressable
              onPress={() => {
                if (gwAny.type === "local") {
                  if (Platform.OS === "web") {
                    window.alert(
                      "BYOB (self-hosted) agents are managed locally. User assignment is only available for cloud-hosted agents.",
                    );
                  } else {
                    Alert.alert(
                      "Not Available",
          ...
          <Pressable
            onPress={() => canEdit && router.push(`/agents/${id}/model`)}
          >
            <InfoRow
              label="Model"
          ...
          <View className="flex-1 justify-center items-center bg-black/60 px-6">
            ...
            >
              ...
              <Text className="text-sm text-center text-neutral-600 dark:text-neutral-400 mb-4">
                {isRelaunchable
                  ? "This agent is unreachable/terminated. Relaunch will request a fresh control-plane start. It may take a few minutes to come online."
                  : "This will launch a new container for this agent. It may take a few minutes to come online."}
              </Text>
              <Pressable
                onPress={async () => {
                  setStarting(true);
                  try {
                    ...
                    if (Platform.OS === "web") {
                      ...
                      Alert.alert(
                        "Error",
                        e.message || "Failed to start agent",
                      );
                    }
                  } finally {
                    setStarting(false);
            ...
            >
              ...
              {latestRelease?.changelog && (
                <View className="bg-neutral-100 dark:bg-neutral-800 rounded-lg px-3 py-2 mt-3 mb-1">
                  <Text className="text-xs text-neutral-500 dark:text-neutral-400 font-semibold mb-1">
                    What's new:
                  </Text>
                ...
                onPress={async () => {
                  ...
                  try {
                    ...
                    if (Platform.OS === "web") {
                      ...
                      Alert.alert(
                        "Error",
                        e.message || "Failed to update agent",
                      );
                    }
                  } finally {
                    setRestarting(false);
        ...
        >
          <View className="flex-1 justify-center items-center bg-black/60 px-6">
            ...
            >
              ...
              <Text className="text-sm text-center text-neutral-600 dark:text-neutral-400 mb-4">
                This will permanently delete this agent and all its data. This
              ...
              <Pressable
                ...
                style={{
                  backgroundColor:
                    deleteConfirmText === "DELETE" && !terminating
                      ? "#dc2626"
                      : "transparent",
                  borderWidth:
                    deleteConfirmText === "DELETE" && !terminating ? 0 : 1,
                  borderColor: colors.border,
                  paddingVertical: 14,
                  borderRadius: 10,
                  alignItems: "center",
