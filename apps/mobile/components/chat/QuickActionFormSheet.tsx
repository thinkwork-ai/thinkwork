...
export const QuickActionFormSheet = forwardRef<
  QuickActionFormSheetRef,
  QuickActionFormSheetProps
>(({ subAgents, onSave, onDelete }, ref) => {
  ...
  const isValid =
    formData.title.trim().length > 0 && formData.prompt.trim().length > 0;
  ...
  const selectedAgentName = formData.workspaceAgentId
    ? (subAgents.find((a) => (a.agentId ?? a.id) === formData.workspaceAgentId)
        ?.name ?? "Unknown")
    : "None";
  ...
  return (
    ...
    >
      ...
      >
        {/* Title */}
        <View className="mb-4">
          <Muted className="text-xs mb-1.5 font-medium uppercase tracking-wide">
            Title
          </Muted>
          <BottomSheetTextInput
        ...
        <View className="mb-4">
          ...
          <BottomSheetTextInput
            ...
            onChangeText={(t) =>
              setFormData((prev) => ({ ...prev, prompt: t }))
            }
                ...
                <Pressable
                  ...
                  style={{
                    ...
                    borderBottomColor: isDark
                      ...
                      : "rgba(0,0,0,0.06)",
                    backgroundColor:
                      formData.workspaceAgentId === null
                        ? isDark
                ...
                {subAgents.map((agent) => (
                  <Pressable
                    ...
                    onPress={() => {
                      setFormData((prev) => ({
                        ...prev,
                        workspaceAgentId: agent.agentId ?? agent.id,
                      }));
                      setShowPicker(false);
                    }}
                    className="px-3 py-2.5 active:opacity-70"
                  ...
                  >
                    ...
                    {agent.role && (
                      <Muted className="text-xs">{agent.role}</Muted>
                    )}
                  </Pressable>
                ))}
              </View>
