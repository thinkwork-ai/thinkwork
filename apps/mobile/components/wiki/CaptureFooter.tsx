...
export function CaptureFooter({
  ...
  const sendDisabled =
    !text.trim() || submitting || (mode === "add" && atHardLimit);
  ...
  const handleBrainModePress = useCallback(
    (nextMode: BrainMode) => {
  ...
  const handleCapture = useCallback(async () => {
    ...
    if (!agentId || !tenantId) {
      Alert.alert(
        "No agent selected",
        "Choose an agent before capturing a memory.",
      );
      return;
    }
    setSubmitting(true);
    try {
      const capturedVia: "text" | "dictation" = dictationUsedRef.current
        ? "dictation"
        : "text";
      const appVersion =
        Constants.expoConfig?.version ??
        (Constants as unknown as { nativeAppVersion?: string })
          .nativeAppVersion ??
        "unknown";
  ...
  const placeholder =
    mode === "search" ? searchPlaceholder : "Add new memory...";
            ...
            <View className="flex-row items-center gap-4">
              ...
              >
                <ArrowUp
                  ...
                  strokeWidth={2.5}
                  color={
                    sendDisabled ? (isDark ? "#737373" : "#a3a3a3") : "#ffffff"
                  }
                />
              </Pressable>
            </View>
