...
function lightenHex(hex: string, amount = 0.3): string {
  ...
  const r = Math.min(
    255,
    parseInt(h.substring(0, 2), 16) + Math.round(255 * amount),
  );
  const g = Math.min(
    255,
    parseInt(h.substring(2, 4), 16) + Math.round(255 * amount),
  );
  const b = Math.min(
    255,
    parseInt(h.substring(4, 6), 16) + Math.round(255 * amount),
  );
  return `rgb(${r},${g},${b})`;
}
...
export function PriorityBadge({ priority }: { priority?: string }) {
  ...
  return (
    ...
    >
      <Text size="xs" weight="medium" style={{ color }}>
        {priority}
      </Text>
    </View>
  );
...
export function personName(
  o?: { firstName?: string; lastName?: string } | null,
): string {
