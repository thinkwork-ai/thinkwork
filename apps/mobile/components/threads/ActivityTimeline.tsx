...
function mergeTimeline(
  ...
  for (const m of messages) {
    items.push({
      kind: "message",
      data: m,
      sortKey: new Date(m.createdAt).getTime(),
    });
  ...
  for (const t of turns) {
    ...
    if (t.status === "succeeded" && t.resultJson) {
      const finishTime = t.finishedAt
        ? new Date(t.finishedAt).getTime()
        : turnTime;
      const hasNearbyMessage = assistantTimes.some(
        (at) => Math.abs(at - finishTime) < 10000,
      );
      if (!hasNearbyMessage) {
  ...
  return (
    <View style={{ flexDirection: "row", gap: 12, paddingHorizontal: 16 }}>
      ...
      <View style={{ width: 32, alignItems: "center", alignSelf: "stretch" }}>
        ...
        {showLineAbove ? (
          <View
            style={{ width: 2.5, height: 12, backgroundColor: borderColor }}
          />
        ) : (
      ...
      <View style={{ flex: 1, gap: 2, paddingVertical: 8 }}>{children}</View>
    </View>
  );
}
...
function TurnContent({
  ...
  const statusLine = (
    <View className="flex-row items-center gap-2 flex-wrap">
      <Text className="text-xs capitalize" style={{ color: stColor }}>
        {item.status}
      </Text>
      {hasDuration && (
      ...
      {usage && (
        <Muted className="text-xs">{`${(usage.inp / 1000).toFixed(1)}K → ${usage.out}`}</Muted>
      )}
            ...
            >
              <View className="flex-row items-start gap-1.5">
                <AlertCircle
                  ...
                  style={{ marginTop: 2 }}
                />
                <Text className="text-xs flex-1" style={{ color: stColor }}>
                  {item.error}
                </Text>
              </View>
            </View>
...
function getGenuiIconConfig(type: string): { icon: any; color: string } {
  ...
  if (type === "crm_mutation_result")
    return { icon: RefreshCw, color: "#f59e0b" };
  if (type === "question_card")
    return { icon: ClipboardList, color: "#0ea5e9" }; // PRD-46
...
function genuiSummary(toolResult: Record<string, unknown>): string {
  ...
  if (type === "place_search_results") {
    ...
    const discovered = ((toolResult.discovered_places as unknown[]) || [])
      .length;
    const parts: string[] = [];
  ...
  if (type.endsWith("_list")) {
    ...
    const plural = entity.endsWith("y")
  ...
  // CRM detail
  if (type === "opportunity")
    return `${toolResult.title || ""}${toolResult.value ? ` — $${Number(toolResult.value).toLocaleString()}` : ""}`;
  if (type === "lead")
    return `${toolResult.title || ""}${toolResult.company ? ` — ${toolResult.company}` : ""}`;
  if (type === "task")
    return `#${toolResult.taskNumber || ""} ${toolResult.title || ""}`;
  if (type === "account") return String(toolResult.name || "");
  // Mutation result
  if (type === "crm_mutation_result")
    return String(
      toolResult.summary || (toolResult.success ? "Updated" : "Update failed"),
    );
  // PRD-46: Question Card
  if (type === "question_card") {
    const schema = toolResult.schema as
      | { title?: string; fields?: unknown[] }
      | undefined;
    const title = schema?.title || "Form";
    const count = Array.isArray(schema?.fields) ? schema!.fields!.length : 0;
    return count > 0
      ? `${title} — ${count} field${count !== 1 ? "s" : ""}`
      : String(title);
  }
  return "";
}
...
function extractToolInfo(
  ...
  metadata: any,
  toolIndex: number,
): { server: string; tool: string; params: Record<string, unknown> } | null {
  // 1. Try _source embedded in tool result (set by AgentCore)
  const source = toolResult._source as
  ...
  if (source?.tool) {
    ...
    else if (tool.startsWith("places_") || tool === "place_detail")
      server = "places";
...
function GenUIContent({
  ...
  const handleSaveRecipe = () => {
    console.log(
      "[GenUI] Save as Recipe pressed, onSaveRecipe:",
      !!onSaveRecipe,
      "messageId:",
      message.id,
      "tenantId:",
    ...
    const source = displayData._source as
      | { tool?: string; params?: Record<string, unknown> }
      | undefined;
  ...
  const hasToolInfo = !!extractToolInfo(
    displayData,
    message.metadata,
    toolIndex,
  );
          ...
          {refreshedAt &&
            (() => {
              const mins = Math.round(
                (Date.now() - new Date(refreshedAt).getTime()) / 60000,
              );
              const label =
                mins < 1
                  ? "just now"
...
export function ActivityTimeline({
  ...
  useEffect(() => {
    if (
      didInitialScroll.current &&
      timeline.length > prevCountRef.current &&
      timeline.length > 0
    ) {
      setTimeout(() => {
  ...
  const keyExtractor = useCallback((item: TimelineItem) => {
    if (item.kind === "fallback-response")
      return `fallback-${item.data.turnId}`;
