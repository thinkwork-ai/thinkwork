...
function InfoRow({
  ...
  return (
    <View
      className={`flex-row items-center justify-between py-2.5 ${last ? "" : "border-b border-neutral-100 dark:border-neutral-800"}`}
    >
      <Muted className="text-sm">{label}</Muted>
      <Text className="text-sm" style={color ? { color } : undefined}>
        {value}
      </Text>
    </View>
  );
...
export default function McpServerDetailScreen() {
  ...
  const handleAuthenticate = async () => {
    ...
    // Persistent ASWebAuthenticationSession jar so WorkOS remembers the
    // prior sign-in across reconnects — no need to re-enter credentials
    // every time a token expires. `force=true` still bypasses SSO
    // short-circuit server-side when the user explicitly re-authenticates.
          ...
          onPress: async () => {
            ...
            try {
              ...
              if (res.ok) {
                ...
                Alert.alert(
                  "Cleared",
                  "Authentication removed. Tap Authenticate to reconnect.",
                );
              } else {
              ...
              Alert.alert(
                "Error",
                "Failed to clear authentication. Check your connection.",
              );
            } finally {
              setClearing(false);
            }
  ...
  const isConnected =
    server.authStatus === "active" || server.authType === "none";
