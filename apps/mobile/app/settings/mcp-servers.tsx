...
export default function McpServersScreen() {
  ...
  const handleConnect = async (mcpServer: McpServerRow) => {
    ...
    //
    // Persistent ASWebAuthenticationSession cookie jar: WorkOS can reuse
    // its session cookie so subsequent reconnects skip the email/password
    // step. If the user ever needs to switch identities they can use the
    // "Clear authentication" action on the server detail screen, which
    // passes `force=true` to bypass SSO on the next authorize.
    //
