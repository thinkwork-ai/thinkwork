...
function buildCron(config: ScheduleConfig): string {
  ...
  switch (config.frequency) {
    ...
    case "every_month":
      return `0 ${hour24} ${config.dayOfMonth} * ? *`;
    default:
      return "0 * * * ? *";
  }
}
...
function buildLabel(config: ScheduleConfig): string {
  ...
  switch (config.frequency) {
    ...
    case "every_week": {
      const names = config.weekdays.map(
...
export default function AddScheduleScreen() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Not Implemented",
        "Schedule creation not yet migrated to GraphQL.",
      );
      router.back();
    } catch (err) {
                ...
                <View className="flex-row items-center justify-between px-4 py-3.5 border-b border-neutral-100 dark:border-neutral-800">
                  <View className="flex-1 mr-4">
                    ...
                    <Muted className="text-xs mt-0.5">
                      Skip runs outside this window
                    </Muted>
                  </View>
                  <Pressable
                    onPress={() =>
                      update({ activeHoursEnabled: !config.activeHoursEnabled })
                    }
                    className="w-12 h-6 rounded-full items-center justify-center"
                    style={{
                      backgroundColor: config.activeHoursEnabled
                        ? colors.primary
                        : colors.muted,
                    }}
                  >
                  ...
                  (Platform.OS === "web" ? (
                    <>
                      <View className="flex-row items-center justify-between px-4 py-3.5 border-b border-neutral-100 dark:border-neutral-800">
                        ...
                        <WebSelect
                          ...
                          onChange={(v) =>
                            update({ activeHoursStart: v as number })
                          }
                          colors={colors}
                        />
                      </View>
                      <View className="flex-row items-center justify-between px-4 py-3.5">
                        ...
                        <WebSelect
                          ...
                          options={hour24Options}
                          onChange={(v) =>
                            update({ activeHoursEnd: v as number })
                          }
                          colors={colors}
                        />
                      </View>
                    ...
                    <>
                      <PickerRow
                        label="Start hour"
                        value={
                          hour24Options[config.activeHoursStart]?.label ??
                          String(config.activeHoursStart)
                        }
                      ...
                      <PickerRow
                        label="End hour"
                        value={
                          hour24Options[config.activeHoursEnd]?.label ??
                          String(config.activeHoursEnd)
                        }
