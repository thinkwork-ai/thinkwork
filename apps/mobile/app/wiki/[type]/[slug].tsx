...
export default function WikiPageScreen() {
  ...
  const { page, loading, refetch } = useWikiPage({
    tenantId,
    userId,
    type,
    slug,
          ...
          <View style={{ paddingHorizontal: 24, paddingTop: 16, gap: 20 }}>
            {page.promotedFromSection ? (
              ...
              >
                ...
                >
                  {page.promotedFromSection.parentPage.title} →{" "}
                  {page.promotedFromSection.sectionHeading}
                </Text>
              </Pressable>
            ) : page.parent ? (
              <Pressable
                ...
                {page.children.map((child) => (
                  <Pressable
                    ...
                    onPress={() => {
                      ...
                      router.push(
                        userId
                          ? `${bp}?userId=${encodeURIComponent(userId)}`
                          : bp,
                      );
                    }}
                    style={({ pressed }) => ({
