...
export function ApiKeyDialog({
  ...
  const title = isRotation
    ? `Rotate API key: ${serverName}`
    : `Configure API key: ${serverName}`;
          ...
          <div className="space-y-4 py-2">
            ...
            >
              <div className="flex flex-col items-start">
                ...
                <span className="text-xs font-normal text-muted-foreground">
                  Creates a new{" "}
                  {serverName.toLowerCase().includes("admin") ? "tkm_" : ""}
                  token server-side. Recommended.
                </span>
              </div>
            </Button>
            ...
            <div className="rounded-md border border-border/60 bg-muted/30 p-3 text-xs">
              ...
              <div className="mt-2 text-muted-foreground">
                This mints a key, stores it in Secrets Manager, and registers it
                on this MCP server in one step. Then reload this page — you'll
                see the last 4 characters here.
              </div>
            </div>
          </div>
        )}
