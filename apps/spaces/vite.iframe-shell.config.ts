 ...
 *
 * Output:
 *   apps/spaces/dist/iframe-shell/iframe-shell.html
 *   apps/spaces/dist/iframe-shell/assets/*.js
 */
...
export default defineConfig(({ mode }) => {
  ...
  // .env.production (per scripts/build-spaces.sh) are honored
  // without re-exporting inline.
  const env = loadEnv(mode, new URL(".", import.meta.url).pathname, ["VITE_"]);

  const rawOrigins =
    env.VITE_ALLOWED_PARENT_ORIGINS ?? process.env.VITE_ALLOWED_PARENT_ORIGINS;
