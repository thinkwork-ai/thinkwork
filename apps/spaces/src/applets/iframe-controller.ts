 ...
 * `apps/spaces/src/iframe-shell/iframe-protocol.ts`.
 *
...
export class IframeAppletController {
    ...
    switch (envelope.kind) {
      ...
      case "state-read": {
        ...
        if (!getter) {
          this.postError(envelope.msgId, {
            code: "RUNTIME_ERROR",
            message:
              "state-read requested but no getState handler is registered",
          });
          return;
        }
        getter(payload.key)
          ...
          .catch((err: unknown) => {
            this.postError(envelope.msgId, {
              code: "RUNTIME_ERROR",
              message: err instanceof Error ? err.message : "state-read failed",
            });
          });
        return;
      }
      case "state-write": {
        ...
        if (!setter) {
          this.postError(envelope.msgId, {
            code: "RUNTIME_ERROR",
            message:
              "state-write requested but no setState handler is registered",
          });
          return;
        }
      ...
      case "state-write-ack": {
        ...
        if (envelope.kind === "state-read-ack") {
          pending.resolve((envelope.payload as StateReadAckPayload)?.value);
        } else {
          pending.resolve(
  ...
  private postError(replyTo: string | undefined, payload: ErrorPayload): void {
    const envelope = buildEnvelope("error", payload, this.channelId, replyTo);
    const target = this.element.contentWindow;
    if (!target) return;
    target.postMessage(envelope, "*");
  }
