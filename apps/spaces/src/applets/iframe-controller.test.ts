...
function postFromIframe(
  ...
  const envelope = buildEnvelope(kind, payload, controller.channelId, replyTo);
...
describe("IframeAppletController — __SANDBOX_IFRAME_SRC__ build-time wiring", () => {
  it("uses globalThis.__SANDBOX_IFRAME_SRC__ as the default iframe src when no override is passed", async () => {
    // Plan-012 U10/U11.5: scripts/build-spaces.sh writes
    // VITE_SANDBOX_IFRAME_SRC into apps/spaces/.env.production from
    // the Terraform `computer_sandbox_url` output. apps/spaces/
    ...
    const original = (globalThis as { __SANDBOX_IFRAME_SRC__?: string })
      .__SANDBOX_IFRAME_SRC__;
    (globalThis as { __SANDBOX_IFRAME_SRC__?: string }).__SANDBOX_IFRAME_SRC__ =
      stagedUrl;
    try {
      vi.resetModules();
      const { IframeAppletController: FreshController } =
        await import("./iframe-controller");
      ...
      if (original === undefined) {
        ...
        ).__SANDBOX_IFRAME_SRC__ = original;
      }
      vi.resetModules();
    }
  });
...
describe("IframeAppletController — element + handshake", () => {
  ...
  it("posts init on iframe `load` (resolves the chicken-and-egg without iframe knowing channelId)", () => {
    ...
    expect((init!.envelope as { payload: { tsx: string } }).payload.tsx).toBe(
      "<App />",
    );
    expect(
    ...
    expect(
      (
        init!.envelope as {
          payload: { themeOverrides?: Record<string, string> };
        }
...
describe("IframeAppletController — no-secrets-in-payload invariant (P0)", () => {
  ...
  it("sendCallback rejects a payload containing a credential field", () => {
    const { controller } = createController();
    expect(() => controller.sendCallback("submit", { token: "leak" })).toThrow(
      IframePayloadSecretLeakError,
    );
  });
  ...
  it("getState rejects a payload key reference that smuggles a credential field", async () => {
    ...
    await expect(
      ...
      ).requestReply("state-read", { apiKey: "leak" }),
