...
function handleInit(payload: InitPayload, msgId: string): void {
  ...
  applyShellRootOverflow(state.fitContentHeight);
  void compileAndMount(payload, msgId);
}

// When the host sizes the iframe to its reported content height
// (`fitContentHeight=true`), the surrounding panel owns scrolling and the
// shell root must NOT enforce its own `overflow-y: auto` — otherwise any
  ...
  window.__THINKWORK_IFRAME_STATE_PROXY__ = {
    async read(key: string): Promise<unknown> {
      return new Promise((resolve, reject) => {
        ...
        const envelope = buildEnvelope("state-read", { key }, state.channelId);
