...
export function registerThreadCommand(program: Command): void {
  const thread = program
    .command("thread")
    .alias("threads")
    .description("Create, list, update, and comment on threads in a tenant.");
  ...
  thread
    ...
    .option(
      "--assignee <id>",
      "Filter by assignee (user or agent ID). Use `me` to match the caller.",
    )
...
`,
  ...
  thread
    ...
    .description(
      "Create a new thread. Prompts for missing fields when running in a TTY.",
    )
    .option("-s, --stage <name>", "Deployment stage")
    .option("-t, --tenant <slug>", "Tenant slug")
...
`,
  ...
  thread
    ...
    .option(
      "--agent <id>",
      "Informational — checkout is tracked by the generated runId",
    )
    .addHelpText(
      "after",
      `
...
`,
  ...
  thread
    .command("release <id>")
    .description(
      "Release a checked-out thread (pass the run-id from checkout).",
    )
    .option("-s, --stage <name>", "Deployment stage")
    .option("-t, --tenant <slug>", "Tenant slug")
    .option(
      "--run-id <id>",
      "The runId returned by `thinkwork thread checkout`",
    )
    .addHelpText(
      "after",
      `
...
`,
  ...
  thread
    ...
    .description(
      "Add a comment to a thread. Prompts for content if omitted and TTY.",
    )
    .option("-s, --stage <name>", "Deployment stage")
    .option("-t, --tenant <slug>", "Tenant slug")
...
`,
  ...
  thread
    ...
    .description(
      "Attach or detach a label on a thread. Labels are managed via `thinkwork label`.",
    )
    .option("-s, --stage <name>", "Deployment stage")
    .option("-t, --tenant <slug>", "Tenant slug")
    .addHelpText(
...
`,
  ...
  thread
    .command("escalate <id>")
    .description(
      "Escalate a thread to another agent (carries context, records actor).",
    )
  ...
  thread
    .command("delegate <id>")
    .description(
      "Delegate ownership to another agent without the 'escalation' semantics.",
    )
  ...
  thread
    .command("delete <id>")
    .description(
      "Permanently delete a thread (not just close). Requires confirmation.",
    )
    .option("-s, --stage <name>", "Deployment stage")
    .option("-t, --tenant <slug>", "Tenant slug")
