...
const DOMAINS: DomainCase[] = [
  { domain: "thread", phase: 1, register: registerThreadCommand, expectedSubcommand: "list" },
  { domain: "message", phase: 1, register: registerMessageCommand, expectedSubcommand: "send" },
  { domain: "label", phase: 1, register: registerLabelCommand, expectedSubcommand: "create" },
  { domain: "inbox", phase: 1, register: registerInboxCommand, expectedSubcommand: "approve" },
  { domain: "agent", phase: 2, register: registerAgentCommand, expectedSubcommand: "list" },
  { domain: "template", phase: 2, register: registerTemplateCommand, expectedSubcommand: "sync-all" },
  { domain: "tenant", phase: 2, register: registerTenantCommand, expectedSubcommand: "create" },
  { domain: "member", phase: 2, register: registerMemberCommand, expectedSubcommand: "invite" },
  { domain: "team", phase: 2, register: registerTeamCommand, expectedSubcommand: "add-agent" },
  { domain: "kb", phase: 2, register: registerKbCommand, expectedSubcommand: "sync" },
  { domain: "routine", phase: 3, register: registerRoutineCommand, expectedSubcommand: "trigger" },
  { domain: "scheduled-job", phase: 3, register: registerScheduledJobCommand, expectedSubcommand: "run" },
  { domain: "turn", phase: 3, register: registerTurnCommand, expectedSubcommand: "cancel" },
  { domain: "wakeup", phase: 3, register: registerWakeupCommand, expectedSubcommand: "create" },
  { domain: "webhook", phase: 3, register: registerWebhookCommand, expectedSubcommand: "rotate" },
  { domain: "connector", phase: 3, register: registerConnectorCommand, expectedSubcommand: "enable" },
  { domain: "skill", phase: 3, register: registerSkillCommand, expectedSubcommand: "install" },
  { domain: "memory", phase: 4, register: registerMemoryCommand, expectedSubcommand: "search" },
  { domain: "recipe", phase: 4, register: registerRecipeCommand, expectedSubcommand: "create" },
  { domain: "artifact", phase: 4, register: registerArtifactCommand, expectedSubcommand: "list" },
  { domain: "cost", phase: 5, register: registerCostCommand, expectedSubcommand: "summary" },
  { domain: "budget", phase: 5, register: registerBudgetCommand, expectedSubcommand: "upsert" },
  { domain: "performance", phase: 5, register: registerPerformanceCommand, expectedSubcommand: "agents" },
  { domain: "trace", phase: 5, register: registerTraceCommand, expectedSubcommand: "thread" },
];
...
describe("stub registration (taxonomy smoke test)", () => {
  ...
  it("every stub action exits with code 2 when invoked", async () => {
    ...
    await program.parseAsync(["node", "thinkwork", "thread", "list"]).catch(() => undefined);
    await program.parseAsync(["node", "thinkwork", "agent", "list"]).catch(() => undefined);
    await program.parseAsync(["node", "thinkwork", "dashboard"]).catch(() => undefined);

    // Each stub should have called process.exit(2).
    expect(exitSpy).toHaveBeenCalledWith(2);

    const combinedStderr = stderrSpy.mock.calls.map((c) => String(c[0])).join("");
