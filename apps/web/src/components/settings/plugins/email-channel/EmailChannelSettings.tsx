...
export type EmailChannelSettingsProvider = "resend" | "sendgrid" | "all";

export function EmailChannelSettings({
  provider = "all",
}: {
  provider?: EmailChannelSettingsProvider;
}) {
  ...
  const showResendProvider = provider === "all" || provider === "resend";
  const showSendGridProvider = provider === "all" || provider === "sendgrid";
  ...
  return (
    <>
      <SettingsSection label="Provider setup">
        {showResendProvider ? (
          <SettingsRow
            label="Resend"
      ...
      <SettingsSection label="Email channel">
        {result.fetching && !summary ? (
          ...
          <SettingsRow
            ...
            description="Production sending opens after provider credentials, domain, receiving, and webhook checks pass. Provider events and loop evidence update after live traffic."
            layout="stacked"
          >
            <EmailReadinessPanel
              summary={summary}
