...
describe("TaskThreadView", () => {
  ...
  it("renders query_wiki_context invocations as wiki context thinking rows", () => {
    ...
    expect(rows[0]).toMatchObject({
      title: "Wiki returned 3 pages",
      kind: "source",
    });
    expect(rows[0]?.detail).toContain("Query: Acme renewal");
    expect(rows[0]?.detail).toContain("Top pages: Acme Renewal");
    expect(rows[0]?.detail).toContain('"retrieval_mode": "db"');
  });

  it("renders empty wiki context events instead of dropping them", () => {
    const rows = actionRowsForTurn(
      {
        id: "turn-empty-wiki-context",
        status: "succeeded",
        invocationSource: "chat_message",
        events: [
          {
    ...
    expect(rows[0]?.detail).toContain("Query: unknown customer");
