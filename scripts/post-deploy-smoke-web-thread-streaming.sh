...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *) echo "post-deploy-smoke-web-thread-streaming: unknown arg: $1" >&2; exit 2 ;;
  esac
done
...
if [[ -z "${API_URL:-}" || -z "${DATABASE_URL:-}" || -z "$APPSYNC_API_URL" || -z "$APPSYNC_REALTIME_URL" || -z "$APPSYNC_API_KEY" ]]; then
  echo "post-deploy-smoke-web-thread-streaming: failed to resolve deployed GraphQL/AppSync/database config" >&2
  exit 3
fi
