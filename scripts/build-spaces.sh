...
#
# apps/spaces reuses the existing ThinkworkAdmin Cognito client (same
...
#
# Usage:
#   bash scripts/build-spaces.sh <stage>
#
...
STAGE="${1:?Usage: build-spaces.sh <stage>}"
...
Provision var.computer_sandbox_domain before deploying apps/spaces.
EOF
  exit 1
fi
...
VITE_ALLOWED_PARENT_ORIGINS="${COMPUTER_SANDBOX_PARENT_ORIGINS}" \
VITE_MAPBOX_PUBLIC_TOKEN="${MAPBOX_PUBLIC_TOKEN}" \
  pnpm --filter @thinkwork/spaces build:iframe-shell

echo "▸ Syncing to S3 bucket: $APP_BUCKET ..."
aws s3 sync apps/spaces/dist/ "s3://${APP_BUCKET}/" \
  --delete \
...
aws s3 sync apps/spaces/dist/iframe-shell/ "s3://${COMPUTER_SANDBOX_BUCKET}/" \
  --delete \
  --region "$REGION"
