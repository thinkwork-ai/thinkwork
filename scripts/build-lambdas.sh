...
build_handler() {
  ...
  if [ "$name" = "graphql-http" ] || [ "$name" = "memory-retain" ] || [ "$name" = "brain-dream-state" ] || [ "$name" = "memory-stage-worker" ] || [ "$name" = "memory-retraction-drainer" ] || [ "$name" = "mcp-user-memory" ] || [ "$name" = "mcp-context-engine" ] || [ "$name" = "requester-memory-dreaming" ] || [ "$name" = "eval-runner" ] || [ "$name" = "eval-worker" ] || [ "$name" = "wiki-compile" ] || [ "$name" = "ontology-scan" ] || [ "$name" = "wiki-bootstrap-import" ] || [ "$name" = "routine-task-python" ] || [ "$name" = "routine-exec-git" ] || [ "$name" = "compliance-export-runner" ] || [ "$name" = "model-converse" ] || [ "$name" = "knowledge-graph-observations-ingest" ] || [ "$name" = "chat-agent-activity" ] || [ "$name" = "artifact-share" ] || [ "$name" = "document-conformance-judge" ] || [ "$name" = "analyst-query-broker" ] || [ "$name" = "analyst-connection-reconciler" ] || [ "$name" = "knowledge-base-manager" ] || [ "$name" = "harness-runner" ] || [ "$name" = "harness-code-interpreter-target" ] || [ "$name" = "agentcore-proof-oauth-provider" ]; then
...
# THINK-189 U5: conformance judge sweeper (scheduled, reserved concurrency 1).
build_handler "document-conformance-judge" \
  "$REPO_ROOT/packages/api/src/handlers/document-conformance-judge.ts"
...
build_handler "twenty-client-engagement" \
  "$REPO_ROOT/packages/api/src/handlers/twenty-client-engagement.ts"
...
# agents — projects the chat payload into AWS AgentCore Harness config,
# drives InvokeHarness, fulfills emit_document, finalizes the turn.
# In the bundled-SDK list: needs @aws-sdk/client-bedrock-agentcore(-control)
...
# 2026-07-03-004 U3). Pulls the tenant repo at branch HEAD, fixture-gates
# new SHAs, and executes run(input) in the AgentCore code interpreter via
# the shared routine-task-python core. Uses BUNDLED_AGENTCORE_ESBUILD_FLAGS
# for @aws-sdk/client-bedrock-agentcore.
build_handler "routine-exec-git" \
  "$REPO_ROOT/packages/lambda/routine-exec-git.ts"
