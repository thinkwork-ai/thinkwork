...
STAGE="${1:?Usage: build-web.sh <stage>}"
...
Provision var.computer_sandbox_domain before deploying apps/web.
EOF
  exit 1
fi
...
WEB_RELEASE_VERSION="${VITE_RELEASE_VERSION:-}"
if [[ -z "$WEB_RELEASE_VERSION" && -n "${VITE_APP_VERSION:-}" ]]; then
  WEB_RELEASE_VERSION="$VITE_APP_VERSION"
  if [[ "$WEB_RELEASE_VERSION" != v* ]]; then
    WEB_RELEASE_VERSION="v${WEB_RELEASE_VERSION}"
  fi
fi
...
VITE_RELEASE_VERSION=${WEB_RELEASE_VERSION}
