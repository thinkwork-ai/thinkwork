#!/usr/bin/env bash
# smoke-spaces — Spaces deployed-stage acceptance smoke.
#
# The COMPUTER_ENV_FILE, SMOKE_COMPUTER_ID, and COMPUTER_ID environment
# variables intentionally keep their legacy names for operator compatibility.
#
# Usage:
#   scripts/smoke-spaces.sh dev
#   scripts/smoke-spaces.sh --stage dev --region us-east-1
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --region)
      ...
      ;;
    -*)
      echo "smoke-spaces: unknown arg: $1" >&2
      exit 2
      ;;
    *)
      STAGE="$1"
...
bash "$(cd "$(dirname "$0")" && pwd)/post-deploy-smoke-spaces-thread-streaming.sh" \
  --stage "$STAGE" \
  --region "$REGION"
...
if [[ -z "${API_URL:-}" || -z "${DATABASE_URL:-}" || -z "$APPSYNC_API_URL" || -z "$APPSYNC_API_KEY" || -z "$COMPUTER_URL" ]]; then
  echo "smoke-spaces: failed to resolve deployed GraphQL/AppSync/database config" >&2
  exit 3
fi
...
node scripts/smoke/spaces-crm-dashboard-prompt-smoke.mjs
