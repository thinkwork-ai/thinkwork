...
4. **Persistence** — Aurora Postgres via Drizzle (`packages/database-pg`). Schema changes flow: edit `src/schema/*` → `db:generate` → PR the new `drizzle/NNNN_*.sql` → `db:push` after deploy. Skill assignment state is **workspace-native** (capability-mapping plan U9/U10, KTD-8): presence = the installed `skills/<slug>/SKILL.md` folder, per-assignment state (OAuth wiring refs, `permissions.operations`, enabled) = `skills/<slug>/.assignment.json` read via `packages/api/src/lib/skills/{assignment-state,workspace-skill-index}.ts`. The `agent_skills` table is retired (readers/writers removed; table DROP is a deferred follow-up) and `setAgentSkills` no longer exists.
5. **Agent runtime** — the Pi container image (`packages/agentcore-pi`) runs in **two intentional copies**, and both are load-bearing:
   - **Chat** (THINK-585/588) — GraphQL `sendMessage` → `chat-agent-invoke` Lambda (setup phase) → `resolveChatDispatchTarget` → `agentcore-runtime-dispatch` Lambda → **Bedrock AgentCore Runtime** (warm per-thread sessions). Enabled on `dev`, `tei-e2e`, and `mcpherson`. Exactly **one** AgentCore runtime per stage serves this path — `thinkwork_<stage>_pi`, whose ID is resolved from SSM `/thinkwork/<stage>/agentcore/runtime-id-pi` (the dispatcher's `AGENTCORE_PI_RUNTIME_SSM_NAME`). It is the only AgentCore runtime any stage should have; deploy tooling (`scripts/post-deploy.sh`, `deploy.yml`) only ever reconciles the `pi` runtime.
   - **Background** — wakeups, evals, and skill runs invoke the **Pi Lambda** (`thinkwork-<stage>-agentcore-pi`) directly via `AGENTCORE_PI_FUNCTION_NAME`. This copy is **deliberately kept** (THINK-588 decision): don't "consolidate" it onto the runtime or trim its provisioned concurrency as dead weight.

   Retired and gone — do not reintroduce: the Function URL invoker `packages/api/agentcore-invoke.ts`, the `thinkwork_<stage>_strands_*` and `thinkwork_<stage>_flue` runtimes with their `runtime-id-strands` / `runtime-id-flue` SSM params, and the THINK-324 `harness_Thinkwork_*` AgentCore Harness trial runtimes. The runtime loads installed skills from materialized workspace `skills/<slug>/` folders and keeps tenant catalog source files in S3 under `tenants/<tenant-slug>/skill-catalog/<skill-slug>/`; MCP tool servers use streamable HTTP. Memory is **Bedrock AgentCore Memory (managed)** — the only engine. `MEMORY_ENGINE` is effectively fixed to `agentcore`; legacy values are normalized at the env-parse boundary and the Terraform `enable_hindsight` / `memory_engine` inputs are deprecated no-ops.
