...
5. **Agent runtime** — Bedrock AgentCore hosts the **Pi** runtime (`packages/agentcore-pi`). Chat dispatch topology (THINK-585/588): GraphQL `sendMessage` → `chat-agent-invoke` Lambda (setup phase) → `resolveChatDispatchTarget` → `agentcore-runtime-dispatch` Lambda → AgentCore Runtime (warm sessions). Background paths (wakeups, evals, skill runs) still invoke the Pi Lambda directly via `AGENTCORE_PI_FUNCTION_NAME`. The legacy Function URL invoker (`packages/api/agentcore-invoke.ts`) is retired. The runtime loads installed skills from materialized workspace `skills/<slug>/` folders and keeps tenant catalog source files in S3 under `tenants/<tenant-slug>/skill-catalog/<skill-slug>/`; MCP tool servers use streamable HTTP. Memory is **Bedrock AgentCore Memory (managed)** — the only engine. `MEMORY_ENGINE` is effectively fixed to `agentcore`; legacy values are normalized at the env-parse boundary and the Terraform `enable_hindsight` / `memory_engine` inputs are deprecated no-ops.
