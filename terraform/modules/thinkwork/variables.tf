...
variable "microsoft_oauth_tenant" {
  description = "Microsoft Entra directory GUID for the deployment's default Microsoft login route. Cognito requires the exact tenant issuer; organizations/common authorities are not valid token issuers. Legacy authority aliases are tolerated only as vestigial WorkOS-era config while no Microsoft client id is set — an active route still requires a GUID, enforced by a precondition on the MicrosoftOrganizations IdP."
  type        = string
  default     = ""
