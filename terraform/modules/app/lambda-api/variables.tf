...
variable "kestra_url" {
  description = "Public Kestra URL (empty when not provisioned)."
  type        = string
  default     = ""
}

variable "kestra_alb_arn" {
  description = "Public Kestra ALB ARN (empty when not provisioned)."
  type        = string
  default     = ""
}

variable "kestra_target_group_arn" {
  description = "Kestra target group ARN (empty when not provisioned)."
  type        = string
  default     = ""
}

variable "kestra_cluster_arn" {
  description = "ECS cluster ARN for Kestra (empty when not provisioned)."
  type        = string
  default     = ""
}

variable "kestra_service_name" {
  description = "ECS service name for Kestra (empty when not provisioned)."
  type        = string
  default     = ""
}

variable "kestra_log_group_name" {
  description = "CloudWatch log group for Kestra (empty when not provisioned)."
  type        = string
  default     = ""
}

variable "kestra_storage_bucket_name" {
  description = "S3 bucket name backing Kestra internal storage (empty when not provisioned)."
  type        = string
  default     = ""
}

variable "kestra_database_name" {
  description = "Dedicated PostgreSQL database name used by Kestra (empty when not provisioned)."
  type        = string
  default     = ""
}

variable "kestra_basic_auth_secret_arn" {
  description = "Secrets Manager ARN for the Kestra UI/API basic-auth service credential (empty when not provisioned)."
  type        = string
  default     = ""
}
