...
This module is intentionally evidence-only in U2/U4. It provisions the durable
resources that later units use for managed-application plan/apply/destroy jobs:

- Step Functions state machine for deployment orchestration.
- CodeBuild project with a stub buildspec that records session-scoped evidence.
...
`sessions/<session>/<action>/deployment-evidence.json`, but it does not clone
source or run Terraform yet. U4 adds the deployment job domain and live
orchestration contract; U5 wires Cognee and Twenty adapters.
