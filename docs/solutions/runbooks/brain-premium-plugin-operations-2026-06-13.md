---
title: ThinkWork Brain premium plugin operations
date: 2026-06-13
...
severity: medium
applies_when:
  - Issuing a ThinkWork Brain install key
  - Running the ThinkWork Brain premium plugin smoke
  - Supporting a tenant through existing the retired graph substrate adoption
  - Configuring or removing the temporary dev/test backdoor key
tags: [brain, premium-plugins, install-keys, retired_graph_substrate, smoke]
---

# ThinkWork Brain premium plugin operations

## What owns what

ThinkWork Brain is the customer-facing premium plugin. the retired graph substrate is the internal
Brain substrate adapter behind the infrastructure component. Customer-facing
surfaces should point to:

- `/settings/plugins/brain` for premium entitlement, install, approval,
  and component status.
- `/settings/memory/knowledge-graph` for the working Memory / Ontology graph
  explorer.

Mention the retired graph substrate only when interpreting deployment evidence, Terraform outputs,
...
```graphql
mutation IssueBrainKey($input: IssuePremiumPluginInstallKeyInput!) {
...
Tenant admins normally paste the key into the ThinkWork Brain plugin detail page.
Programmatically, use the normal plugin install mutation:

```graphql
mutation InstallBrain($input: InstallPluginInput!) {
...
```json
{
  "input": {
    "pluginKey": "brain",
    "installKey": "<raw-key>",
    "idempotencyKey": "brain-install-<unique-id>"
  }
}
```
...
```graphql
mutation RevokeBrainKey($input: RevokePremiumPluginInstallKeyInput!) {
...
- `BRAIN_BACKDOOR_INSTALL_KEY_SECRET_ARN`: Secrets Manager ARN
  containing the temporary raw key.
- `BRAIN_BACKDOOR_INSTALL_KEY_STAGES`: comma-separated stage allowlist.
- `BRAIN_BACKDOOR_INSTALL_KEY`: local/runtime direct value path used
...
ThinkWork Brain's infrastructure component key is `brain-substrate`. Its
`handlerRef` links the plugin component to the managed-app deployment path.
Useful fields:

- `managedAppKey: "retired_graph_substrate"` means the internal adapter is the retired graph substrate.
...
- `adoptionRequiresNoChange: true` means the tenant had an existing the retired graph substrate
...
```sh
node plugins/brain/smoke/brain-plugin-smoke.mjs
```

Live with an existing issued or dev/test backdoor key:

```sh
SMOKE_ENABLE_BRAIN_PLUGIN=1 \
  SMOKE_TENANT_ID=<tenant-id> \
  SMOKE_ADMIN_USER_ID=<tenant-admin-user-id> \
  SMOKE_BRAIN_INSTALL_KEY=<issued-or-backdoor-key> \
  node plugins/brain/smoke/brain-plugin-smoke.mjs
```
...
ThinkWork Brain plugin detail and Memory / Ontology.
