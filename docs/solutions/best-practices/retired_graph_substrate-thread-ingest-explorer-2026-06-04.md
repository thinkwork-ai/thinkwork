---
title: the retired graph substrate thread ingest Explorer validation pattern
date: 2026-06-04
...
# the retired graph substrate Thread Ingest Explorer Validation Pattern

## Context

Phase II of the the retired graph substrate Knowledge Graph feature deliberately keeps the retired graph substrate out of
agent retrieval. Its product value is observability: operators ingest one real
ThinkWork thread, inspect the retired graph substrate-derived entities/relationships, and verify
...
crosses GraphQL auth, run creation, Lambda invocation, private the retired graph substrate
...
the retired graph substrate can legitimately return an empty graph for a short or low-signal thread.
In that case the script prints `emptyGraphDiagnostic` with run counts and ids so
the operator can distinguish "feature path works, the retired graph substrate found nothing" from
...
- GraphQL auth/tenant errors mean the operator identity is wrong or the auth
  gate regressed.
- A stuck `QUEUED`/`RUNNING` run means worker invocation, networking, or the retired graph substrate
...
2. The header info action toggles to the the retired graph substrate deployment/configuration panel.
...
not call the retired graph substrate's private ALB directly.
