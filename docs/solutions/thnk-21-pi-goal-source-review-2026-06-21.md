...
- The vendored snapshot changes only two integration details: legacy state file resolution now happens at call time so the adapter's per-invocation `PI_CODING_AGENT_DIR` is honored, and the async `agent_end` handler has an explicit final `return` to satisfy the repository's strict TypeScript checks.
...
Later units must still prove cloud-safe continuation and ThinkWork budget/finalize integration before enabling a full goal-run path.
