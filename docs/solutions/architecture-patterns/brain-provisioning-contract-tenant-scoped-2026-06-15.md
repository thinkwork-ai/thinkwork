---
title: ThinkWork Brain provisioning contracts must be tenant-scoped before production behavior
date: 2026-06-15
category: docs/solutions/architecture-patterns
module: ThinkWork Brain / Deployment Runner
...
# ThinkWork Brain provisioning contracts must be tenant-scoped before production behavior

## Context

THNK-18 added the provisioning contract that lets ThinkWork Brain create
tenant-scoped Brain substrate instances through the existing internal the retired graph substrate
managed-app path. Before this work, the the retired graph substrate module and runner path were
...
ThinkWork Brain product. ThinkWork Brain is the customer-facing layer; the retired graph substrate is an
...
migration without teaching product surfaces about the retired graph substrate environment variables,
...
surfacing raw provider variables in the plugin UI. For ThinkWork Brain, net-new
...
{
  "phase": "plan",
  "appKey": "retired_graph_substrate",
  "tenantId": "tenant-id",
...
return compactObject({
  retired_graph_substrate_brain_tenant_id: optionalString(
    desiredConfig,
    "brainTenantId",
  ),
  retired_graph_substrate_brain_instance_key: optionalString(
    desiredConfig,
    "brainInstanceKey",
  ),
  retired_graph_substrate_brain_storage_tier: brainStorageTier,
  retired_graph_substrate_private_substrate_mode:
...
production tier is a remote tier: the retired graph substrate-supported Neptune Analytics
...
keeping customer-facing copy on ThinkWork Brain.
...
The desired-config boundary matters for product clarity. ThinkWork Brain should
not expose the retired graph substrate, LanceDB, Kuzu, Neptune, S3 prefixes, or ECS settings as
...
Good ThinkWork Brain provisioning shape:

```text
Plugin install
  -> infrastructure handler seeds tenant Brain desiredConfig
  -> managed-app deployment runner maps desiredConfig to retired_graph_substrate_* variables
  -> Terraform derives names from brain_instance_key
  -> outputs report tier, providers, S3 roots, Neptune, EFS, endpoint, evidence
  -> product surfaces say ThinkWork Brain; operator evidence may mention the retired graph substrate
```
...
```text
Plugin install
  -> UI exposes raw the retired graph substrate env vars
...
- [ThinkWork Brain migrations keep reads on the active backend until validated cutover](./brain-migrations-keep-active-read-path-2026-06-15.md)
- [ThinkWork Brain active-substrate reads stay behind Context Engine](./brain-active-substrate-reads-through-context-engine-2026-06-15.md)
