...
THNK-23 exposed ThinkWork Brain to first-party Pi agents as a dedicated
`query_brain_context` tool. The tempting shortcut would have been to register a
raw Brain MCP server, couple runtime tool registration directly to ThinkWork Brain
...
ThinkWork Brain became a model-visible first-party affordance, but the transport
...
skipped because ThinkWork Brain was not installed for that tenant, so it did not
...
`ThinkWork Brain: ok (1 hits, 111ms)`, the hit id was
...
1. **Model affordance:** expose the smallest tool name and schema that helps the
   model choose correctly. For THNK-23, `query_brain_context` made ThinkWork Brain
...
2. **Policy transport:** keep the call routed through the established facade.
   For ThinkWork Brain, Pi forwards JSON-RPC `tools/call` to `/mcp/context-engine`
   with `params.name = "query_brain_context"`. It does not call the retired graph substrate,
...
3. **Provider eligibility:** let install/provisioning make the provider eligible
   behind the facade, not create a raw runtime transport. ThinkWork Brain plugin
...
Install coupling: ThinkWork Brain install makes Brain eligible behind Context Engine
Result shape: shortlist first, selected details by detailIds/detailIndexes
Live proof: CHAT-1119 returned ThinkWork Brain ok, 1 hit, 111ms
```
...
Pi tools:
- query_brain_context
- raw_retired_graph_substrate_search
- neptune_graph_query
...
- [THNK-23 plan](../../plans/2026-06-14-005-feat-brain-first-party-tool-plan.md)
...
- [Context Engine ThinkWork Brain reads plan](../../plans/2026-06-14-002-feat-context-engine-brain-reads-plan.md)
...
- [PR #2468: expose ThinkWork Brain context tool](https://github.com/thinkwork-ai/thinkwork/pull/2468)
