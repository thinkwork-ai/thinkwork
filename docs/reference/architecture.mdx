...
Memory is the context layer. **Bedrock AgentCore Memory** is the durable substrate and the only memory engine: the runtime hands each completed turn to the `memory-retain` Lambda and AgentCore extracts semantic facts, preferences, summaries, and episodes in the background. Context Engine remains the lazy-loaded external context layer for workspace files, compiled pages, and approved MCP tools.

In the current open source app, that mainly means:

- thread history selected for the context window
- authored documents mounted as workspace files
- long-term memory recall through AgentCore Memory
...
ThinkWork agent execution runs in AWS-managed AgentCore isolation; desktop and
mobile are clients. Pi is the active managed runtime on Bedrock AgentCore. The
surrounding harness remains ThinkWork's: managed does not mean vendor-hosted.
...
├─────────────────────────────────────────────────────────────┤
│  Data Tier                                                   │
│  Aurora Postgres (pgvector) · S3 (skills, docs, logs)       │
│  Secrets Manager                                             │
├─────────────────────────────────────────────────────────────┤
│  Foundation Tier                                             │
...
| Resource               | Purpose                                                                                                                                                |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Aurora Postgres        | Primary data store: agents, threads, messages, AgentLoops, Workflows, integrations, users |
| S3 — skill catalog     | `skills/catalog/*.md` — skill packs loaded at invoke time                                                                                              |
| S3 — knowledge docs    | Source documents for Brain Sources                                                                                                                     |
...
| Bedrock AgentCore Memory | Durable agent memory — retain, recall, episodes (the only memory engine)     |
...
3. Selected AgentCore runtime receives event
   ...
   └─ Uses the `recall` tool for durable memory
   └─ Uses Context Engine for compiled pages, workspace files, and approved MCP tools
   └─ After the turn completes, the platform hands the turn to AgentCore Memory for background extraction
   └─ Builds context: system prompt + selected history + recalled memory + lazy-loaded context + tool config
...
| Memories                  | Bedrock AgentCore Memory                    | AWS-managed encryption at rest                 |
...
<Aside type="note">
  ...
  restore brings back agents, threads, memories, **and** the vector index in
...
| AgentCore → S3      | IAM role (read skills, read source docs, write audit logs)   |
