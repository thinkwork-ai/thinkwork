...
The Inbox is the approval queue for agent-requested human-in-the-loop actions. Agents can be configured to request human approval before performing sensitive operations — configuration changes, document uploads, destructive tool calls — and those requests land here in `PENDING` state.
...
**Route:** `/inbox`  
**File:** `apps/web/src/routes/_authed/_tenant/inbox/index.tsx`
...
**File:** `apps/web/src/routes/_authed/_tenant/inbox/$inboxItemId.tsx`
...
Inbox decisions are operator-driven today. The underlying mutations (`approveInboxItem`, `rejectInboxItem`, `requestRevisionInboxItem`) and the workspace review actions they bridge to (`acceptAgentWorkspaceReview`, `cancelAgentWorkspaceReview`, `resumeAgentWorkspaceRun`) live on the GraphQL surface and can be called by trusted services with a tenant API key — but there is no first-class agent tool, MCP server, or skill that wraps them yet. An agent that wants to drain its tenant's system Inbox (a triage agent, for example) does so today by calling the GraphQL surface directly through the same authentication path used for any other tenant-scoped query. A vetted runtime tool for review resolution is plausible follow-up work.
...
- The underlying routing model — chain walk, mobile vs Inbox split, and folder-specialist review surfacing — is documented in [Workspace Orchestration → Human review flow](/concepts/agents/workspace-orchestration/#human-review-flow).
...
- [Threads](/applications/web/threads/) — inbox items usually link back to a parent thread
- [Tenant Agent](/applications/web/agents/) — configure platform-agent workspace and folder specialists
- [Spaces](/applications/web/spaces/) — configure Space context that can produce review work
- [Security Center](/applications/web/security-center/) — policy and guardrail enforcement
