...
**File:** `apps/web/src/routes/_authed/_tenant/agents/invites.tsx`
...
Pending rows have explicit Approve / Reject actions. Approving creates the agent in the tenant roster (visible in [Agents](/applications/web/agents/)) and issues the API key. Rejecting marks the invite spent without creating the agent.
...
9. The agent appears in [Agents](/applications/web/agents/) and the runtime receives its API key

### Revoke a rogue BYOB agent

1. Find the agent in the [Agents](/applications/web/agents/) list
...
- [Agents](/applications/web/agents/) — where approved BYOB agents appear
- [Authentication & Tenancy](/applications/web/authentication-and-tenancy/) — how the tenant context is resolved after a BYOB agent authenticates
