...
description: Reusable and legacy template bundles for agent authoring infrastructure. Tenant Agent and Spaces are the primary current operation surfaces.
---

import { Aside } from "@astrojs/starlight/components";

Templates are reusable configuration bundles for agent authoring infrastructure. They still exist for supported template flows, but they are no longer the primary place operators tune the managed-agent runtime. For day-to-day operation, configure tenant-wide defaults in [Tenant Agent](/applications/web/agents/) and local context or runtime policy in [Spaces](/applications/web/spaces/).

<Aside type="note">
  Older template docs described fleet-wide rotation across many top-level
  managed agents. The current model is one tenant platform agent plus Spaces.
  Use this page for template UI that still exists; use Tenant Agent and Spaces
  for current runtime behavior.
</Aside>

A template can define defaults such as model, built-in tool opt-ins, MCP server assignments, guardrail attachments, system prompt, budget caps, and workspace files where the template UI still owns that flow.
...
- **Configuration fields** (built-in tool opt-ins, MCP servers, guardrails, model, system prompt, budget) — propagated via the explicit **Sync** flow described later on this page. Operators review per-agent diffs and confirm.
- **Workspace files** (`AGENTS.md`, `CONTEXT.md`, `GUARDRAILS.md`, `TOOLS.md`, `skills/<slug>/SKILL.md`, etc.) — propagated via [workspace composition](/concepts/agents/workspace-composition/). Live-class files update on next invocation with no sync step; pinned guardrail files require a per-agent **Accept update** review; `USER.md` is rewritten server-side on human reassignment.
...
- **Web Extraction card** — opt this template into the tenant-configured `web_extract` built-in
...
The **Web Search** card controls whether agents using this template can receive the `web_search` built-in. This is separate from configuring the tenant's provider key under [Spaces -> Built-in Tools](/applications/web/builtin-tools/).
...
The **Web Extraction** card controls whether agents using this template can receive the `web_extract` built-in. This is separate from configuring the tenant's Firecrawl key under [Spaces -> Built-in Tools](/applications/web/builtin-tools/).
...
The template runtime is retained for template-backed flows. It is not the primary user-facing control for normal Space operation. See [Runtime Configuration](/concepts/agents/runtime-selection/) for tenant-agent defaults and Space overrides.

Template model choices are constrained by the tenant [Model Catalog](/applications/admin/model-catalog/). A globally known Bedrock model does not appear here unless the tenant catalog has an enabled row for it. Disabling a model in Model Catalog prevents new template/profile selections from using that model while preserving historical traces that already reference the raw Bedrock model ID.

## The Workspace tab

The template detail page has a **Workspace** tab alongside **Configuration** and **MCP Servers**. This is where the template's inheritance base for workspace files lives — `AGENTS.md`, `CONTEXT.md`, `GUARDRAILS.md`, `TOOLS.md`, `skills/<slug>/SKILL.md`, and so on.

Edits to this tab propagate to linked agents through [workspace composition](/concepts/agents/workspace-composition/) at read time, not through the Sync flow. Specifically:

- **Live files** (`AGENTS.md`, `CONTEXT.md`, `ROUTER.md`, `TOOLS.md`, `memory/*.md`, etc.) — edits are visible to every linked agent on the **next invocation**. No per-agent sync, no explicit propagation step. If an agent overrode the file, its override still wins; otherwise it picks up the template's new bytes.
- **Pinned files** (`GUARDRAILS.md`) — edits do **not** flow automatically. Every linked agent surfaces a **Template update available** badge on that file. Operators review and accept per-agent (or bulk per-template) from the agent's Workspace tab. This is the safety lane for anything that could meaningfully change agent behavior.
...
<Aside type="note">
  The template's Workspace tab does **not** show a per-file source badge,
  because by definition every file listed here is at the template layer. The
  operator-facing distinction matters on the _agent's_ Workspace tab, where the
  same file could come from the agent override, the template, or the defaults.
</Aside>
...
<Aside type="note">
  The explicit sync step is a deliberate design choice. Auto-propagating
  template changes would make fleet-wide mistakes trivially easy to ship — "I
  tweaked the system prompt and now 47 agents misbehave." Making operators
  review and confirm keeps fleet operations reviewable.
</Aside>
...
1. Install the new skill via [Skills Catalog](/applications/web/skills-catalog/)
...
3. Save the template
4. Verify any tenant-level prerequisite on [Built-in Tools](/applications/web/builtin-tools/), such as the Web Search provider key, Firecrawl Web Extraction key, or Code Sandbox policy
...
3. Save
4. (Optional) Pick one linked agent, open its [Agents](/applications/web/agents/) detail page, and run it through a manual turn to sanity-check
...
Template history is not currently versioned — rollback means editing forward. Per-agent config history **is** versioned (see the Rollback button on the [Agents](/applications/web/agents/) detail page).
...
- **Don't install built-ins as skills.** `web_search`, `web_extract`, `send_email`, `browser_automation`, and `execute_code` are injected runtime tools controlled by Space, platform-agent, template, and tenant policy.
...
- [Tenant Agent](/applications/web/agents/) — tenant-wide platform-agent defaults
- [Spaces](/applications/web/spaces/) — Space-specific context, triggers, settings, and members
- [Runtime Configuration](/concepts/agents/runtime-selection/) — tenant-agent defaults and Space overrides
- [Agent Invites](/applications/web/agent-invites/) — BYOB agent registration
- [Skills Catalog](/applications/web/skills-catalog/) — the skills added to templates
- [Tenant MCP Servers](/applications/web/mcp-servers/) — the MCP servers assigned to templates
- [Security Center](/applications/web/security-center/) — guardrails assigned to templates
- [Agents (concept)](/concepts/agents/) — the current tenant platform-agent model
