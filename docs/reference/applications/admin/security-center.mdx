...
**Route:** `/security`  
**File:** `apps/web/src/routes/_authed/_tenant/security/index.tsx`
...
4. Cross-reference in [Threads](/applications/web/threads/) to see the full conversation that triggered the block
...
- **Approvals and Audit tabs are Coming Soon.** Approvals will integrate the [Inbox](/applications/web/inbox/) approval flows; Audit will expose the full `guardrail_blocks` table with filtering. Today, use the Dashboard's Recent Blocks list as the closest approximation.
...
- [Inbox](/applications/web/inbox/) — the human-in-the-loop approval queue (Approvals tab will eventually unify with this)
- [Agent Templates](/applications/web/agent-templates/) — where guardrails are assigned
- [Threads](/applications/web/threads/) — where to look for the conversation that triggered a block
