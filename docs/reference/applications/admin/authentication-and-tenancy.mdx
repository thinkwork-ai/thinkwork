...
When email/password sign-in is configured, the page also exposes **Reset
password**. This uses Cognito's forgot-password flow: the user requests a reset
code by email, confirms the code with a new password, and returns to sign-in.
The request step avoids casual account enumeration while still surfacing
configuration, delivery, rate-limit, code, and password-policy failures. This is
the credential-establishment path for manually added password users; operators
do not create or distribute passwords.

The sign-in page itself is small — a logo, the sign-in entry points, and any
setup/recovery context. Human tenant setup is managed from Settings -> Users;
BYOB agent invites are handled separately at `/invite/:token` and flow through
adapter-selection -> join-request -> API-key-claim before landing at sign-in.
...
**File:** `apps/web/src/routes/_authed/_tenant.tsx`
...
**File:** `apps/web/src/lib/TenantContext.tsx`
...
<Aside type="note">
  If the app sits idle long enough for the refresh token to expire (hours, not
  minutes), the next request returns 401 and the app forces a sign-out. There is
  no mid-request retry-with-refresh flow.
</Aside>
...
- [Admin Overview](/applications/web/) — the full map of pages
