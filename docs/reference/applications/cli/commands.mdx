...
Run the full deploy pipeline: preflight checks → terraform apply →
release-artifact resolution → schema migrations → post-deploy verification.
Rerunning after any partial failure converges — partial state is a normal
condition, never a stranded error.
...
| `-s, --stage <name>`      | Stage name (required)                                    |
| `-c, --component <tier>`  | `foundation`, `data`, `app`, or `all` (default: `all`)   |
| `-y, --yes`               | Skip confirmation (CI). Required for non-TTY sessions.   |
| `--skip-preflight`        | Skip preflight account checks (not recommended)          |
| `--release-version <tag>` | Pin a specific ThinkWork release for artifact resolution |
| `-p, --profile <name>`    | AWS profile override                                     |

Pipeline stages on init-scaffolded environments:

1. **Preflight** — runs the `doctor` check registry (credentials + expiry
   margin, Bedrock invocation, Lambda concurrency quota, state-backend
   reachability, domain NS delegation) and reports **every** blocker before
   any resource is created. SES production access is warn-tier: reported and
   tracked, never blocking.
2. **State backend** — provisions or verifies a per-account S3 state bucket
   (versioned, encrypted, public-access-blocked) + DynamoDB lock table. A
   stale lock from an interrupted run gets guided recovery on rerun.
3. **Apply** — terraform apply per tier, streamed with a captured transcript.
4. **Release artifacts** — pins a ThinkWork release, seeds its Lambda zips,
   threads the Pi image, and publishes prebuilt web assets. Placeholder-mode
   deploys fail loudly.
5. **Schema** — applies the bundled journaled Drizzle migrations idempotently
   via the Aurora Data API.
6. **Verify** — the [`verify`](#verify) probes run; blocking failures fail
   the deploy.
...
List published ThinkWork platform releases and install one into an already
deployed external environment through that environment's deployment controller.

Use this command for operator recovery, support automation, or scripted
upgrades. For day-to-day customer operations, prefer Settings -> General ->
Releases when the deployed web app is available.

```bash
thinkwork release list
thinkwork release deploy [version] -s <stage> [options]
```

| Flag                 | Description                                     |
| -------------------- | ----------------------------------------------- |
| `-s, --stage <name>` | Existing deployment stage                       |
| `-y, --yes`          | Skip confirmation                               |
| `--web-only`         | Sync only the web application static bundle     |
| `--no-wait`          | Start the controller execution and return early |
...
Use a full upgrade when the release includes infrastructure, Lambda, database,
runtime image, AgentCore, overlay, or managed-application changes.
...
`--web-only` verifies the release manifest, downloads only the pinned `web`
static-site artifact, reads the existing web bucket and CloudFront distribution
from Terraform state outputs, syncs the web bundle, writes runtime config,
invalidates CloudFront, and records deployment evidence. It does not run
Terraform apply, copy Lambda artifacts, run database work, copy runtime images,
update AgentCore, apply overlays, or install managed applications.

The command requires a previous successful controller deployment for the stage;
first-time installs still go through `thinkwork deploy --bootstrap` or the
enterprise bootstrap flow.
...
Check AWS account prerequisites for a Thinkwork deployment: AWS + Terraform
CLIs, credentials and their expiry margin, a live Bedrock model invocation,
the Lambda concurrency quota, and (warn-tier) SES production access. `deploy`
runs the same registry as its preflight.
...
Prove a deployed stage actually works — `terraform apply` exiting 0 is not
evidence. Runs automatically at the end of every deploy and on demand.

```bash
thinkwork verify -s <stage>
```
...
| `--api-auth-secret <value>` | Service bearer for the authenticated probe |

Probes: GraphQL API answers, an authenticated API call succeeds, the web app
serves HTML, the database schema is applied (Data API),
workspace seeding, and deployed-artifact evidence (Lambda env + code size vs
placeholder). Pending external approvals — SES production access, domain DNS
delegation — render as a tracked checklist and never fail verification.
...
Local destroys tear down to a **redeployable clean slate**: stage buckets are
pre-emptied (including all object versions), each tier gets a bounded retry
for dependency-order errors, lingering Secrets Manager entries are
force-deleted (no 7-day `AlreadyExists` window on redeploy), and a final
orphan scan reports anything still carrying the stage prefix.

Stages marked with `thinkwork config graduate <stage>` get the strongest
guard: `--yes` requires an explicit `--stage`, and the recorded AWS account
...
With `-s`, prints the detailed view for one stage — Lambda counts, AgentCore, AgentCore Memory, etc.
...
Use [Spaces -> Built-in Tools](/applications/web/builtin-tools/) to inspect the full built-in catalog, configure Firecrawl Web Extraction, test credentials, and confirm the tenant platform agent has access to `execute_code`, `browser_automation`, `web_search`, `web_extract`, and `send_email`.
...
Agent templates — reusable configs you spawn agents from. Use templates to bake in model, prompt files, default capabilities, skills, and sandbox/browser/web-search opt-in. Linked agents do **not** auto-sync when a template changes — use `template sync-agent` / `template sync-all` deliberately.
...
- `--from-agent <id>` clones name + role + linked-template model from an existing agent. The CLI composes the `CreateAgentTemplateInput` client-side; the template's deep config (skills, workspace files) is not cloned — edit those in the admin UI after create if needed.
...
Side-by-side compare on the top-level fields the CLI tracks (name, description / role, runtime, model). Deeper config (skills / workspace files) is not compared at the CLI level — use the admin UI for a full diff.
...
- `--feature key=value` is repeatable. Values are auto-parsed: `true`/`false` → bool, integers → number, otherwise string. Example: `--feature beta_tool=false --feature enable_voice_dictation=true`.
...
- [Tenant Agent](/applications/web/agents/) — tenant-wide platform-agent defaults, workspace, and folder specialists.
- [Spaces](/applications/web/spaces/) — Space workspace, tools, memory, triggers, settings, and members.
...
Do not use retired subcommands for new automation. Space email is configured from [Admin — Spaces](/applications/web/spaces/), not through `thinkwork agent email`.
...
| 2 — Agents & workspace        | [`member`](#member), [`team`](#team), [`template`](#template), [`tenant`](#tenant), [`agent`](#agent)                                             |
...
Inspect, search, and edit an agent's Bedrock AgentCore Memory records.

```bash
thinkwork memory list --agent <id> [--namespace semantic|preferences|summaries|episodes]
...
`memory graph`, `memory promote`, and `memory bank` are **retired** along with the memory graph view and Tenant Bank promotion.
