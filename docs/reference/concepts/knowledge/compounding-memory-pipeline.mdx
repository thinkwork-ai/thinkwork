...
<Aside type="caution">
  **Status: paused on AgentCore.** The cursor-driven compile pipeline described
  here was built against the Hindsight memory engine's
  `listRecordsUpdatedSince` API. That engine was removed (THINK-406) and the
  AgentCore adapter does not implement the cursor method, so automatic compiles
  do not currently run. This page stays as the design record for the pipeline
  and for the pages already compiled; treat the code paths below as the state
  before the engine removal.
</Aside>
...
Then it reads the next 50 memories from the memory adapter's `listRecordsUpdatedSince` cursor API where `updated_at > cursor`. Raw memory is the canonical substrate; the wiki is a derived store.
...
- **Post-retain** — `maybeEnqueuePostTurnCompile` in `packages/api/src/lib/wiki/enqueue.ts`. Checks `tenants.wiki_compile_enabled`, confirms the adapter implements the cursor API, inserts a job row with dedupe key `${tenantId}:${ownerId}:${floor(epoch_s/300)}`, fires `Lambda.invoke` with `InvocationType: "Event"` and `{jobId}` payload.
...
```ts
// the Hindsight adapter's cursor query, kept as the reference implementation
