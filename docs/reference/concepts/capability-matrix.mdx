...
| MCP servers             | Grant — registry row + enablement               | Grant — subset via `tool_policy.mcpServers` + per-server tool allowlist | Restrict only — blocked/allowed servers                   | Self-serve auth only — per-user OAuth tokens or personal API keys |
