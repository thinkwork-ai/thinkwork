...
The guardrail is evaluated _inside_ the Bedrock call, not by a wrapper ThinkWork layers on top. That matters for correctness: the boundary can't be bypassed by a skill pack calling Bedrock directly, because every call that goes through the agent's configured template includes the guardrail id.
...
From the agent's side, a blocked response is handled like any other non-success. The runtime surfaces the guardrail intervention to the agent loop, and AgentCore's standard fallback (a canned "I can't help with that" response, configurable per template) is what the user sees.
...
- **Contextual grounding** — when the agent is answering from retrieved context, require the response to be grounded in those sources.
...
<Aside type="tip">
  Guardrail changes take effect on the next agent invocation. In the current
  Space model, test stricter guardrails by setting a Space override before
  making the policy a tenant-wide default.
</Aside>
...
- **No guardrail evaluation on connected (BYOB) agents.** ThinkWork only enforces guardrails for _managed_ agents running through AgentCore + Bedrock. A connected agent running its own runtime bypasses this path; any guardrails on its own side are its own concern.
...
- [Concepts: Agents](/concepts/agents/) — the tenant platform-agent model
- [Spaces](/concepts/spaces/) — where Space runtime policy can override guardrails
- [Admin: Security Center](/applications/admin/security-center/) — the operator surface for reviewing guardrail hits
- [Admin: Spaces](/applications/admin/spaces/) — where Space context and policy are configured
...
Inside ThinkWork, the runtime resolves the guardrail from the current Space override or tenant-agent default, then passes it into every Bedrock `converse` call. Guardrail activations are recorded on the turn and surfaced both in the admin thread timeline (see [Admin: Threads](/applications/admin/threads/)) and in the audit log.
