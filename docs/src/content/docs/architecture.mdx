---
title: Architecture
description: How the ThinkWork Agent Harness fits together — the PPAF agent loop, the four R.E.S.T. anchors, the components, and the AWS deployment topology.
---
...
1. The **harness mechanics** — the agent loop, the four anchors the runtime enforces, the metaphor that ties it together
...
### The four R.E.S.T. anchors

Beyond the loop, the harness has four operational properties it commits to delivering on every turn — the **R.E.S.T. anchors**. These are the four anchors a buyer or operator can grab onto and the four dimensions an evaluator can measure:
...
The five governance controls in [Control](/concepts/control/) map 1:N to these four anchors — for example, "Runs in your AWS" implements Security and Traceability; "Cost control and analysis" implements Efficiency. The R.E.S.T. anchors are the *language*; the controls are the *implementation*.

### The conceptual model maps onto the harness

The conceptual model below — Threads, Memory, Agents, Connectors, Automations, Control — is what most of the rest of this site documents. Reading the components against the harness mechanics keeps the picture coherent: every component is solving a piece of the harness problem, not floating in isolation.
