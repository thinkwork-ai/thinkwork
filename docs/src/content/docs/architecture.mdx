---
title: Architecture
description: How the ThinkWork Agent Harness fits together — the PPAF agent loop, the four REST anchors, the components, and the AWS deployment topology.
---
...
### The four REST anchors

Beyond the loop, the harness has four operational properties it commits to delivering on every turn — the **REST anchors**. These are the four anchors a buyer or operator can grab onto and the four dimensions an evaluator can measure:
...
The five governance controls in [Control](/concepts/control/) map 1:N to these four anchors — for example, "Runs in your AWS" implements Security and Traceability; "Cost control and analysis" implements Efficiency. The REST anchors are the *language*; the controls are the *implementation*.
