---
title: Architecture
description: How Thinkwork fits together across threads, memory, agents, and connectors, and where that maps onto the AWS deployment tiers.
---
...
Memory is the context layer. It determines what gets surfaced into the current turn beyond the latest message.
...
In managed mode, this execution happens in AgentCore.
...
2. AgentCore Lambda receives event
   ...
   └─ Reads long-term memories through the default AgentCore LongTerm path (`memory_engine = "managed"`) or Hindsight (ECS)
...
<Aside type="note">
The only state that is NOT in Aurora is the vector index (OpenSearch), binary assets (S3), and a small amount of managed identity or secret state in AWS services such as Cognito and Secrets Manager. If you restore from an Aurora snapshot, agents, threads, and memories are largely recovered. You would still need to restore any non-Aurora configuration and re-trigger a Knowledge Base sync to rebuild the vector index.
</Aside>
