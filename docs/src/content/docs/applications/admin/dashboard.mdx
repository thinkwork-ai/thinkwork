...
<Aside type="note">
  The activity feed is currently constructed client-side by merging runs and
  thread updates, sorting by timestamp, and trimming to a page. A dedicated
  activity-feed resolver is a known TODO — the file carries a `// TODO: Wire to
  a real activity feed query when available` comment. Until that lands, the feed
  is computed from two queries rather than one.
</Aside>
...
- **No per-metric drill-in.** Clicking platform-agent metrics does not navigate anywhere. The drill-in for tenant agent configuration is [Tenant Agent](/applications/admin/agents/); for Space-scoped work it's [Spaces](/applications/admin/spaces/); for threads it's [Threads](/applications/admin/threads/); for spend it's [Analytics](/applications/admin/analytics/).

## Related pages

- [Threads](/applications/admin/threads/) — the full thread list and detail views
- [Tenant Agent](/applications/admin/agents/) — the platform-agent baseline
- [Spaces](/applications/admin/spaces/) — Space-scoped workrooms
