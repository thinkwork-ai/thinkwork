...
This is how template-backed authoring flows stay reusable. It is separate from Space runtime overrides, which are the current way to tune model, guardrail, budget, paused state, and sandbox for a workroom.
...
| Action      | Computer Templates link to Computers; Agent Templates expose supported legacy or authoring actions |

The backing query is `AgentTemplatesListQuery`. Computer creation and migration are handled from the Computers surface and migration CLI. Where an Agent Template still exposes a create-agent action, treat it as legacy or authoring infrastructure rather than the primary path for new managed-agent specialization.
...
- [Spaces](/applications/admin/spaces/) — Space-specific context and runtime overrides
