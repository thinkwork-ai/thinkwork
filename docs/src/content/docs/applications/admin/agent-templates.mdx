...
Agent templates are reusable configuration bundles that multiple agents can share. A template defines the default runtime, model, default skill assignments, default knowledge bases, MCP server assignments, guardrail attachments, system prompt, budget caps, and a workspace file set.
...
- **Code Sandbox card** — opt this template into the `execute_code` tool and pick a network mode (see below)
...
4. Add the skill to the Skills table
5. Save the template
