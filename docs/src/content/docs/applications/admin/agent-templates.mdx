...
The **Web Search** card controls whether agents using this template can receive the `web_search` built-in. This is separate from configuring the tenant's provider key under [Spaces -> Built-in Tools](/applications/admin/builtin-tools/).
...
The **Web Extraction** card controls whether agents using this template can receive the `web_extract` built-in. This is separate from configuring the tenant's Firecrawl key under [Spaces -> Built-in Tools](/applications/admin/builtin-tools/).
...
3. Save the template
4. Verify any tenant-level prerequisite on [Built-in Tools](/applications/admin/builtin-tools/), such as the Web Search provider key, Firecrawl Web Extraction key, or Code Sandbox policy
...
- [Tenant Agent](/applications/admin/agents/) — tenant-wide platform-agent defaults
- [Spaces](/applications/admin/spaces/) — Space-specific context, triggers, settings, and members
