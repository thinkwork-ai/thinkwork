---
title: Spaces — Built-in Tools
description: Standalone Spaces Settings surface for platform-owned tools that ship with ThinkWork, including credentialed provider tools and policy-gated AgentCore tools.
---
...
**File:** `apps/spaces/src/components/settings/SettingsTools.tsx`

<Aside type="note">
  Admin's Agent -> Tools tab is being sunset. Spaces Settings -> Built-in Tools
  is the current standalone operator surface for the same catalog,
  credentials, tests, platform status, and agent access state.
</Aside>
