...
- **Template or Space opt-in** - older template-backed flows still point to **Agent Template -> Configuration**; Space-aware flows should be controlled by the current Space surfaces and runtime policy.
...
6. Set Enabled = true
7. Save
8. Do not look for a current Space Studio Tools tab. If a deployment uses Space-level tool policy, inspect or update it through the Space tools API or the rollout-specific surface that owns that policy.
...
- [Spaces](/applications/admin/spaces/) — current Space Studio tabs and local policy context
