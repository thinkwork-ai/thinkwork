...
- **Import starter pack** — calls the `seedEvalTestCases` mutation to idempotently import the 210-test ThinkWork red-team and performance pack across 7 categories. Re-runs are safe (unique index on `(tenant_id, name)` for `source='yaml-seed'` rows).
...
The Studio auto-seeds the 210-case starter pack on a tenant's first visit (the `evalTestCases` query checks for any `source='yaml-seed'` rows and imports if zero). The seed is idempotent so re-visits don't duplicate.
