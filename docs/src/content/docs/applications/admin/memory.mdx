...
The Memory tab is the raw retained-record view inside [Admin — Memory](/applications/web/knowledge/). It covers both AgentCore managed memory and the optional Hindsight add-on. Use it when you need to inspect what was retained before it is routed into `query_context` or compiled into pages.
