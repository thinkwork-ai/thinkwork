...
The Memory tab is the raw retained-record view inside [Admin — Memory](/applications/admin/knowledge/). It covers both AgentCore managed memory and the optional Hindsight add-on. Use it when you need to inspect what was retained before it is routed into `query_context` or compiled into pages.
...
<Aside type="note">
The graph view is a memory-backend inspection tool. It is not the full compiled-memory graph: compiled pages, aliases, links, and source citations live in the Pages surface.
</Aside>
...
- [Admin — Memory](/applications/admin/knowledge/) — the combined operator surface
