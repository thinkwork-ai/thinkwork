...
| Plane      | Plane plugin       | Per-user Plane PAT + workspace slug headers       | Users activate Plane from Settings -> Plugins; repair by reinstalling or repairing the Plane plugin |
...
`x-api-key` and `x-workspace-slug`. Do not configure a tenant-wide Plane API key
or a second manual Plane MCP row.
