...
| Kestra Control | Kestra managed app | Tenant API key to ThinkWork `/mcp/kestra`, backed by Secrets Manager secret refs | Repair the row from Settings -> Applications -> Kestra; do not paste credentials |
...
Kestra deserves one extra distinction: public Kestra catalog or docs MCP
servers are authoring context only. They can help agents discover plugins,
blueprints, and examples, but they do not authenticate to the customer's
managed Kestra runtime. The **Kestra Control** row points at ThinkWork's
`/mcp/kestra` endpoint and is the only v1 path for agents to validate, upsert,
execute, and inspect customer Kestra flows.
...
- **Managed rows are repaired from app pages.** System-managed rows such as
  Twenty CRM and Kestra Control are reconciled by their owning managed app, not
  by manual delete/recreate flows.
...
- [Admin — Kestra](/applications/admin/kestra/) — Kestra control MCP lifecycle and verification
