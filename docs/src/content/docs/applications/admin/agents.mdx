...
Cold-contact delivery is accepted only when email triggers are enabled for that Space and the sender email matches a registered tenant user. Private Spaces additionally require that user to be a Space member. Token-bearing replies to agent-initiated emails continue to work regardless of the toggle so existing conversations can continue.
