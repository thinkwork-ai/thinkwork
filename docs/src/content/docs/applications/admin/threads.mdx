...
**Route:** `/threads`  
**File:** `apps/web/src/routes/_authed/_tenant/threads/index.tsx`
...
<Aside type="caution">
  Because assignee filtering happens after pagination, a filtered page can show
  fewer rows than the page size if the assignee matches only some of the rows on
  that page. This is a known quirk, not a bug — the total-count field still
  reflects the pre-assignee-filter count.
</Aside>
...
**File:** `apps/web/src/routes/_authed/_tenant/threads/$threadId.tsx`
...
5. If the thread needs a human decision that the agent is waiting on, approve it from the [Inbox](/applications/web/inbox/)
...
- [Inbox](/applications/web/inbox/) — the approval queue for agent-requested human-in-the-loop actions
- [Tenant Agent](/applications/web/agents/) — the platform agent that runs managed turns
- [Spaces](/applications/web/spaces/) — the workrooms that scope thread context
- [Artifacts](/applications/web/artifacts/) — tenant-wide view of thread-generated artifacts
- [Analytics](/applications/web/analytics/) — aggregate metrics over threads
