...
Optional infrastructure toggles such as Twenty CRM and n8n live in
...
| Feature flags      | JSON object with any tenant-level feature flags (e.g. `enable_voice_dictation`)                     |
...
| Managed memory   | Enabled / Disabled — Bedrock AgentCore Memory             |
...
2. Check `Managed memory: Enabled` — AgentCore Memory is the only memory engine, so if this is Disabled nothing will retain
3. If it is Disabled, the tenant config doesn't match expectations and a Terraform update + redeploy is needed
4. If it is Enabled but records are still missing, move to `/settings/memory` and check retain attempts (see [Admin — Memory](/applications/admin/memory/))
...
- [Managed Applications](/applications/admin/managed-applications/) — optional Twenty CRM and n8n deployment controls
