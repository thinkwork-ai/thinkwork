...
Use [Spaces -> Built-in Tools](/applications/admin/builtin-tools/) to inspect the full built-in catalog, configure Firecrawl Web Extraction, test credentials, and confirm the tenant platform agent has access to `execute_code`, `browser_automation`, `web_search`, `web_extract`, and `send_email`.
...
- [Tenant Agent](/applications/admin/agents/) — tenant-wide platform-agent defaults, workspace, and folder specialists.
- [Spaces](/applications/admin/spaces/) — Space workspace, tools, memory, triggers, settings, and members.
