...
Mobile uses the same shared `@thinkwork/thread-json-render` validator as web
and the API. It reads persisted `data-json-render` parts only; assistant
markdown, fenced JSON, `_type` blocks, and legacy `{ component, props }` objects
remain ordinary message content or unsupported legacy data.
