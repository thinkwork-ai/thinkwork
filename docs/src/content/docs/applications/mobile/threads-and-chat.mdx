...
- `agentWorkspaceReviews` lists pending reviews, filtered by `status: "awaiting_review"` for the Threads tab.
...
The mobile app does not hide HITL state locally as a workaround. The list and detail screens derive visibility from `agentWorkspaceReviews(status: "awaiting_review")`; once the backend run leaves that state, the amber row treatment and Threads tab HITL count clear naturally.
