...
description: Self-serve per-user OAuth for connectors and MCP servers in the ThinkWork mobile app — Google Productivity, Microsoft 365, and any MCP server registered at the tenant level.
---

import { Aside } from '@astrojs/starlight/components';

The mobile app owns the **user side** of connector and MCP configuration: self-serve OAuth flows, per-user token storage, and the "Needs Connection" prompts that surface when something needs to be granted. Tenant-level registration of MCP servers lives in the [Admin Web App](/applications/admin/mcp-servers/).
