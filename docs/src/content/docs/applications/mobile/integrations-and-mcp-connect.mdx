...
The MCP Servers screen is the user side of [MCP Tool Connectors](/concepts/connectors/mcp-tools/). It lists every MCP server registered at the tenant level that is assigned to any agent the user has access to, plus the user's per-server auth status.
...
- [MCP Tool Connectors](/concepts/connectors/mcp-tools/) — the concept page with the full MCP model and token lifecycle
