...
For integrations that route inbound work, the integration row can expose a **target picker**. The default target is a Computer, which gives the user a stable always-on orchestrator while still allowing the Computer to delegate to managed Agents or routines. The selection is stored per-user-per-provider when the provider supports user-level routing.
