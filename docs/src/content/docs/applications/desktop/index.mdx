...
Operator-only Spaces Settings also include [Managed Applications](/applications/admin/managed-applications/), where optional deployed applications such as Cognee Knowledge Graph, Twenty CRM, and n8n can be queued through the normal deploy workflow. App-specific settings pages appear only after deployment status reports that runtime as running.
...
- **Managed Applications are operator-only.** Cognee, CRM, and n8n settings are hidden from members and remain hidden until deployment status reports the managed runtime as enabled.
