...
Operator-only Spaces Settings also include [Managed Applications](/applications/admin/managed-applications/), where optional deployed applications such as Cognee Knowledge Graph and Twenty CRM can be queued through the normal deploy workflow. App-specific settings pages appear only after deployment status reports that runtime as running.
...
- **Managed Applications are operator-only.** Cognee and CRM settings are hidden from members and remain hidden until deployment status reports the managed runtime as enabled.
