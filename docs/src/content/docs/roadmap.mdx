...
| Agent Templates                                                        | Stable |
| Skill packs (SKILL.md → S3 → invoke-time loading)                      | Stable |
| Threads + channels (CHAT, AUTO, EMAIL, GITHUB)                         | Stable |
| Hindsight Brain memory (`enable_hindsight = true`, ECS Fargate)        | Beta   |
| AgentCore managed memory compatibility                                 | Stable |
| Brain Sources (Hindsight document ingest)                              | Beta   |
| Legacy Knowledge Bases (Bedrock KB + document upload)                  | Stable |
...
A Postgres-backed knowledge graph with entity extraction, relationship modeling, and an ontology editor UI. This would complement Hindsight Brain Sources and legacy document RAG with structured entity graphs for precise, relationship-aware retrieval.

**Not in v1 because:** Hindsight Brain Sources and legacy document retrieval cover the common authored-document path. The graph data model needs more design work before we commit to a schema.
...
### Slack extended surfaces and distribution

Conversational Slack presence is shipped: `@ThinkWork` mentions and bot DMs run turns as the explicitly linked ThinkWork user, with final responses delivered back to the originating thread (see [Slack Workspace App](/integrations/slack/)). Slash commands, message shortcuts, interactive components, and Slack Marketplace distribution are not in v1.

**Not in v1 because:** The tracer-bullet scope is mentions/DMs with exactly-once delivery. Extended surfaces and commercial distribution follow customer demand — Microsoft Teams presence is the current channel priority.

## What's next (active development)

These are the highest-priority items being actively worked on:

1. **Microsoft Teams presence** — customer-facing Teams mention/DM/thread loop on the same ThinkWork seam as Slack
2. **Spaces ingestion** — Email and webhook ingress flowing into per-Space rendered workspaces
3. **Integration platform expansion** — Linear is the current proof; future integration ordering is still being planned
4. **Cost tracking** — Aggregated cost reports by tenant, Space, Agent, and time period
