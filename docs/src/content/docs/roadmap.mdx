...
ThinkWork v1 is a working, production-deployable Agent Harness. It is not a finished product. This page is honest about what's in scope for v1 across all three deployment tiers (ThinkWork open / ThinkWork for Business / ThinkWork Enterprise), what's explicitly out of scope, and what the roadmap looks like. The harness mechanics — PPAF agent loop, R.E.S.T. anchors, six-component model — are stable; what continues to evolve is the depth of each component.
...
1. **Cost tracking** — Aggregated cost reports by tenant, agent, and time period
2. **Places module** — Location intelligence as an optional Terraform module
3. **AutoResearch** — Step Functions-based iterative research loop
4. **End-user web client** — White-label React app for end users
5. **Eval agent with browser automation** — Nova Act or similar for UI-driven agent tests
