...
# enable_hindsight = true                # default Hindsight Brain memory
```

Hindsight is the default hosted Brain memory substrate. It provides durable
user memory, Space memory, retained Brain Sources, recall/reflect, and evidence.
Set `memory_engine = "agentcore"` only for an explicit low-cost/development
managed-memory compatibility deployment.
