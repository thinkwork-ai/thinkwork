...
4. Click **Start Evaluation**. The run appears in Recent Runs with `status=pending` and transitions to `running → completed` as the Lambda works through the pack. The run targets the tenant's [platform agent](/applications/web/evaluations/#agent-target).
...
Every run targets the tenant's [platform agent](/applications/web/evaluations/#agent-target); there is no `--agent`/`--agent-template` flag.
