...
<Aside type="note" title="Desktop Pi target">
  In the Desktop app, Settings -> Evaluations can also run the RedTeam catalog
  against local Desktop Pi. Desktop Pi runs use the same run list, result detail,
  deterministic assertions, and persisted `eval_runs` / `eval_results` rows, but
  execution happens inside the local Electron sidecar instead of the cloud
  eval-worker. See the repo runbook
  `docs/runbooks/desktop-pi-redteam-evals.md` for the operator checklist.
</Aside>

## How a run works

Cloud/backend runs follow the eval-runner path:
...
Desktop Pi runs follow the local sidecar path:

```
User clicks "Run Evaluation" in Desktop Settings
  ↓ Selects target: Desktop Pi
  ↓ Electron bridge: pi.startEvalRun(...)
  ↓ API: POST /api/desktop/eval-runs
  ↓ eval_runs row + planned result rows are created
  ↓ Electron main starts a local sidecar eval run
  ↓ sidecar runs each case through Desktop-local Pi
  ↓ sidecar scores output with @thinkwork/evals-core
  ↓ API callback writes each eval_results row
  ↓ normal run detail UI updates and finalizes
```
...
4. Click **Start Evaluation**. The run appears in Recent Runs with `status=pending` and transitions to `running → completed` as the Lambda works through the pack. The run targets the tenant's [platform agent](/applications/admin/evaluations/#agent-target).
...
Desktop adds a local target option:

1. Open Desktop -> **Settings** -> **Evaluations**.
2. Click **Run Evaluation**.
3. Choose **Desktop Pi** when the target is available.
4. Pick one case, a category, or the full catalog.
5. Start the run and open the normal run detail page.

Desktop Pi is disabled when the local sidecar is unavailable or already busy.
The run still appears in the standard Recent Runs list and detail page, with
Desktop Pi provenance so operators can distinguish local runs from cloud runs.
...
Every run targets the tenant's [platform agent](/applications/admin/evaluations/#agent-target); there is no `--agent`/`--agent-template` flag.
...
| Desktop eval API        | `packages/api/src/handlers/desktop-eval-runs.ts`                        |
| Desktop local runner    | `apps/desktop/src/sidecar/eval-runner.ts`                               |
