...
thinkwork eval run --stage dev \
  --agent-template tpl-ops \
  --test-case tc-email-calendar-01 \
  --watch
```
...
Bedrock model access is granted per-account — new accounts don't have Haiku access until someone toggles it on. The judge fall-back means a fresh deploy still produces pass/fail data instead of hard-erroring every rubric test. The Run Results panel shows `Heuristic rubric check passed: …` so you can tell the difference and grant model access when ready.
</Aside>
