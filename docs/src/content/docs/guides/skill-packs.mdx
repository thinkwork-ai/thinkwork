...
Skill packs are the primary way to extend managed agents with new capabilities. A skill pack is a single `SKILL.md` file that defines tools (Python functions the agent can call), instructions (additions to the system prompt), and static context (reference data injected into every turn).

Skill packs are stored in S3 and loaded by AgentCore at invoke time. You don't need to rebuild or redeploy the AgentCore Lambda to add new skills — just upload a new `SKILL.md` and assign it to an agent.
...
A `SKILL.md` file has three sections, each separated by a level-2 heading.

### Minimal example

A `SKILL.md` file has up to three sections. Here's a complete example for a Jira skill:

**Header and description** (`jira.md`, line 1–3):

```text
# jira

A skill for reading and writing Jira issues.
```
