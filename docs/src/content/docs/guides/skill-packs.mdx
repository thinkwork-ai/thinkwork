...
<Aside type="tip">
  The [Agent Skills spec](https://agentskills.io/specification) defines the
  `SKILL.md` format, naming conventions, and directory structure. ThinkWork
  follows this standard, so skills you write are portable across any platform
...
<Aside type="note">
  The `## Tools` section must be a fenced Python code block (` ```python ... ```
  `). Functions without docstrings are treated as helpers and are not exposed to
  the agent as tools. Type hints in the function signature are used to generate
  the tool schema.
</Aside>
...
<Aside type="caution">
  Catalog presence and installation are not enough for the skill to reach the
  model. Every skill must also pass the trust pipeline (trust report, spec
  check, content scan, signature) before the runtime injects it — and any edit
  to the pack's content invalidates its existing trust report. An installed
  skill with stale trust is **silently skipped** at invoke time. See [Skills:
  the trust pipeline](/concepts/agents/skills/#the-trust-pipeline).
</Aside>
...
4. Registers tools with the AgentCore agent loop
...
<Aside type="tip">
  Keep skill files small. The S3 download and parse happens in the Lambda hot
  path. If a skill's Python code is complex, consider splitting it into multiple
  focused skills rather than one large file.
</Aside>
