...
| ThinkWork Brain  | distilled learning from reviewed source material                                  | Completed work should improve future work.                                       |

At runtime, Goal and task projections are mounted under `Thread/` as generated files such as `Thread/GOAL.md`, `Thread/PROGRESS.md`, and `Thread/TASKS.md`. `Thread/PROGRESS.md` is not a second task database. Treat it as the readable briefing that helps humans and agents understand the structured state; task changes should go through the UI or `set_task_status`.
...
8. Reviewed work can become better source material for ThinkWork Brain.
