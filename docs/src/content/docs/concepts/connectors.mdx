...
External task connectors turn work items from task systems like LastMile — and, via the same adapter seam, future targets like Linear — into native ThinkWork threads. The task renders as a mobile GenUI card with realtime activity and round-trip edits, not a markdown transcript and not an iframe-wrapped remote UI. The external system remains the source of truth for status, assignment, and audit history; ThinkWork owns the experience.

Use external task connectors when a task system outside ThinkWork should feel native inside it.
...
- See [External Tasks](/concepts/connectors/external-tasks/) for the adapter seam, webhook ingestion pipeline, and task-card GenUI model
...
If you want agents to call remote tools through MCP, see [MCP Tools](/concepts/connectors/mcp-tools/).
...
<Aside type="note">
Need agents to use external tools instead of inbound channels? See [MCP Tools](/concepts/connectors/mcp-tools/) for the ThinkWork pattern for connecting remote tool servers through the Connectors layer. MCP stays under Connectors in the docs because it is one integration path, not a separate platform pillar.
</Aside>
