...
Automations start from one of two entry points: a schedule fires at a predetermined time, or an event arrives and triggers a run. Both end up in the same Step Functions state machine and the same `AUTO-` thread. The difference is all at the front.
...
- External task events (`LastMile: task assigned to me`).
...
- [Admin: Scheduled Jobs](/applications/admin/scheduled-jobs/) — the UI
...
- **Aurora tables.** `scheduled_jobs`, `event_triggers`, `job_runs`.
- **Provisioning Lambda.** `packages/api/src/handlers/job-schedule-manager.ts` — called on every `scheduled_jobs` insert/update/delete to reconcile with AWS Scheduler.
- **Trigger Lambda.** `packages/api/src/handlers/job-trigger.ts` — fired by both AWS Scheduler (scheduled path) and EventBridge rules (event-driven path); normalizes both to the same Step Functions input.
- **State machine.** `terraform/modules/automations/step-function-automation.tf` — defines the states, retries, dead-letter handling.
- **AWS Scheduler target.** Every schedule targets the `job-trigger` Lambda directly (not the state machine), because the Lambda normalizes input and resolves the current `scheduled_jobs` row at fire time. This decouples the fire-time payload from Scheduler's snapshot.
- **EventBridge custom bus.** `thinkwork-<stage>-events`. Rule patterns live in `terraform/modules/automations/triggers.tf`.
