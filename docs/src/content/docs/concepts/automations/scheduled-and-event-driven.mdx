...
description: How ThinkWork schedule and event plumbing reaches AgentLoops and Workflows through AWS Scheduler, EventBridge, and scheduled_jobs.
---
...
goal-driven agent work should surface as an **AgentLoop**. Explicit multi-step
...
- A **target** — which AgentLoop, Workflow, or internal job the schedule should wake.
- A **start and optional end window.**
- A **tenant + Space scope** — schedules are tenant-scoped and usually run in a specific Space.

AgentLoop schedules are created through Settings -> AgentLoops. Lower-level
...
Event-driven AgentLoops are later-phase work. When they arrive, EventBridge or
...
```
User creates scheduled AgentLoop or schedule-backed Workflow
...
AWS Scheduler → job-trigger Lambda
        │  (resolves the job row; starts an AgentLoopRun OR
        │   starts a Workflow SFN execution)
        ▼
For AgentLoop targets: AgentLoopRun -> worker wakeup -> AgentCore
...
1. **Does an AgentLoopRun or Workflow run exist?** If yes, debug the product
   run first.
...
4. **Did it find the `scheduled_jobs` row?** If the Aurora row was deleted but
   ...
   matching job."
5. **Did AgentCore or Step Functions run?** AgentLoops dispatch an AgentCore
...
- [Admin: Automations](/applications/admin/automations/) - legacy/operator schedule and webhook surfaces during transition
