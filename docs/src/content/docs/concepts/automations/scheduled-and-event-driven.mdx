...
AWS Scheduler → job-trigger Lambda
        │  (resolves the job row; starts a routine SFN execution OR
        │   wakes the agent for single-turn work)
        ▼
For routine targets: triggerRoutineRun → SFN.StartExecution → routine_executions
For agent targets:   AgentCore invocation → AUTO- thread → thread_turns
```
