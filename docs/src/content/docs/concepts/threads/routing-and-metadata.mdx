...
```json
{
  ...
  "toAddress": "acme.support@agents.thinkwork.ai",
  "spaceId": "sp_123",
...
SES inbound lookup first resolves the Space from `<tenant-slug>.<space-slug>@agents.thinkwork.ai`, then uses `inReplyTo` and `references` to stitch replies into existing threads via standard email threading rules. A reply to a closed thread reopens it.
...
Routing is configured per integration in the relevant admin surface, such as [Webhooks](/applications/admin/webhooks/) for custom inbound events. The thread side of routing — which Space owns a new thread, what channel a thread starts in, and what metadata follows it — is downstream of the integration configuration.
...
8. **Reply or delegation.** The platform agent handles the work or delegates to a sub-agent folder. The outbound Slack handler reads `threadTs` from thread metadata and posts the reply in the same Slack thread.
...
- **No cross-channel routing rules.** A rule like "route any GitHub issue mentioning `@on-call` to the ops Space _and_ the PagerDuty integration" requires two separate routing setups today.
...
- [Spaces](/concepts/spaces/) — Space-scoped context and runtime policy
