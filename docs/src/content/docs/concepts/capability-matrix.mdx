...
| Knowledge Bases         | Grant — `agent_knowledge_bases` binding         | Never                                                                   | Grant — `space_knowledge_bases` binding                   | Never                                                             |
...
| Knowledge Bases         | `bound_knowledge_bases` in the invoke payload → Pi `search_knowledge` tool                                                                       | Direct agent/Space binding queries only (no tenant-wide fallback); tool assembles only when the payload carries at least one bound KB. A binding whose `search_config.retrieval` names an MCP knowledge server resolves that server (service-credential auth from Secrets Manager) and the payload carries a ready-to-call `{url, toolName}` target instead of — or alongside — the AWS KB id; unresolvable directives fall back to the binding's `aws_kb_id` when present                                                                                                                                                                           |
