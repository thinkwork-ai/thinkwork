...
Context assembly happens inside the AgentCore runtime (`packages/agentcore/agent-container/`) before any Bedrock call. The four sources are queried in parallel, token-counted, and merged into the final input. Budget knobs (retrieval `top_k`, memory recall count, max thread-history window) live on the agent template and can be tuned without a redeploy.
