...
**Agents can traverse the page graph when it is explicitly enabled.** The [OKF Wiki Navigator](/concepts/knowledge/okf-wiki-navigator/) projects compiled pages into a read-only linked markdown filesystem so a Pi runtime can search, read, and follow links with trace evidence. That projection is rebuildable output, not the source of truth.
...
Beyond conversations, compounding memory can ingest **external memory sources** — four V1 source families: Twenty CRM (`twenty`), web pages via Firecrawl (`firecrawl`), Gmail behind a provider-neutral adapter (`email`), and Bedrock knowledge-base documents (`bedrock_kb`). Each item flows through one lifecycle:
...
- **Extract (claims).** Deterministic extraction reduces the snapshot to ontology-shaped **claims** (subject, predicate, value, effective interval) supported by claim-evidence edges. New editions supersede stale single-valued claims; facts removed at the source lose support and close their interval.
- **Project + retain (bank).** Active claims render into a deterministic markdown projection with per-line claim provenance and retain into the right Hindsight bank — the user's private bank for personal processors, the Space/Tenant bank for shared ones. **Derivations** record exactly which evidence produced which document so retraction can undo it.
...
- [OKF Wiki Navigator](/concepts/knowledge/okf-wiki-navigator/) — read-only filesystem traversal over compiled pages
