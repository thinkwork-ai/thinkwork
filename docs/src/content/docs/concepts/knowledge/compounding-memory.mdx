...
description: What happens when an agent keeps learning — scattered memories become durable pages about the people, places, topics, and decisions that matter.
---
...
With compiled Memory pages, the agent maintains a real page about that place. You mention Austin over a hundred chats and trips; the agent writes it all into one page titled "Austin, Texas" — with sections like an overview, your visits, and related activities. The page grows as you do. Over time, your agent builds a small encyclopedia about the things it knows you care about. This is what turns raw recall into a durable knowledge asset: the answer to "what does this agent know about X" is a stable page, not a different summary each turn.
...
**Decision** pages record choices — what was decided, why, and what followed. "Decision: standardize on AgentCore Memory." "Decision: book the hotel in Bairro Alto instead of Chiado."
...
Step 6 is what makes memory _compound_ instead of just piling up. As you mention Austin restaurants over and over, the Austin page's rollup section grows. When it's dense enough, it becomes its own page — and that page can then grow its own sections which eventually promote again. Recursive, grounded, self-organizing.
...
## External memory sources (THINK-193)

Beyond conversations, compounding memory can ingest **external memory sources** — three V1 source families: Twenty CRM (`twenty`), web pages via Firecrawl (`firecrawl`), and Gmail behind a provider-neutral adapter (`email`). Each item flows through one lifecycle:

**source → evidence → claims → projection → bank → canonical identity → Wiki.**

- **Acquire (evidence).** An adapter reads inside an explicitly granted boundary and records each item edition as an evidence row: source id, content-sensitive version, bounded snapshot (raw payloads live in encrypted S3 with a 7–90 day TTL, default 30). Replays are no-ops; the source stays authoritative — evidence is never a shadow mailbox or CRM.
- **Extract (claims).** Deterministic extraction reduces the snapshot to **claims** (subject, predicate, value, effective interval) supported by claim-evidence edges. New editions supersede stale single-valued claims; facts removed at the source lose support and close their interval.
- **Project + retain.** Active claims render into a deterministic markdown projection with per-line claim provenance and retain into the requester's memory. **Derivations** record exactly which evidence produced which document so retraction can undo it.
- **Resolve (canonical identity).** Subjects resolve to **canonical entities** (exact source mappings win; strong natural keys like domains corroborate; ambiguity defers to the operator Resolution Queue instead of guessing).
- **Compile (Wiki).** Tenant-scope Entity pages materialize one canonical entity each — canonical ID is the key, title and slug are presentation — so Twenty, a scraped website, a shared email thread, and a policy PDF that all describe "Acme" land on ONE page with cross-source citations.

Two modes run this pipeline: a **personal** processor per user (opt-in sources, manual runs show an editable preflight plan, writes only your private bank — nothing personal enters shared memory) and operator-owned **shared** processors per Space/Tenant (read only explicitly granted shared sources). Deleting a source is an **erase epoch**: provider documents are retracted, snapshots destroyed, and the evidence ledger hard-deleted, so re-onboarding the same source later starts clean.

## Related pages

- [Memory Compile Pipeline](/concepts/knowledge/compounding-memory-pipeline/) — how a memory actually becomes a page
- [Memory Page Model](/concepts/knowledge/compounding-memory-pages/) — the full data model
- [Operating Compiled Memory Pages](/guides/compounding-memory-operations/) — turning it on, troubleshooting
- [Compiled Pages API](/api/compounding-memory/) — GraphQL reference
- [Retained Memory](/concepts/knowledge/memory/) — the raw memory compiled pages sit on top of
...
Raw long-term memory in AgentCore stays the canonical record. The wiki is a derived, rebuildable view — every page can be reconstructed by replaying the compile pipeline over the same memory records.

The OKF filesystem view adds one more derived layer. S3 holds versioned OKF
bundles and current manifests for audit; EFS holds the currently mounted,
read-only projection for Pi traversal. Neither layer should be edited directly.
...
The compile pipeline is cursor-driven: it needs a memory adapter that implements `listRecordsUpdatedSince`. Only the removed Hindsight adapter did. The AgentCore adapter does not, so automatic compiles do not run — the enqueue path short-circuits with `skipped_adapter`. Restoring compiles means implementing a cursor read over AgentCore memory records.
