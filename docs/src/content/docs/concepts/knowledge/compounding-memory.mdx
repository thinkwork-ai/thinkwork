...
## Scope: personal and tenant-shared pages

Compiled pages began as owner-scoped agent wikis. That still exists: an agent can keep its own entity, topic, and decision pages without bleeding into another agent's context.

Tenant-shared pages now exist for governed business memory as well. Those pages are shaped by Business Ontology definitions and Company Brain materialization, then read through Context Engine policy. They are not a free-for-all shared wiki; redaction, provenance, tenant scope, and provider status still decide what can be read.

OKF Wiki Navigator adds a third view over the same compiled knowledge: a generated markdown projection for read-only traversal. It does not make markdown canonical storage. It lets operators and agents inspect an Open Knowledge Format bundle derived from governed pages, Brain artifacts, graph evidence, ontology versions, and redacted provenance.
...
- [OKF Wiki Navigator](/concepts/knowledge/okf-wiki-navigator/) — generated markdown projection and traversal boundary
...
Raw long-term memory, approved ontology definitions, Brain materialization state, graph evidence, provenance, ACLs, and operational Postgres indexes stay canonical. Compiled pages are derived, rebuildable views. OKF bundles are another derived projection over those views, optimized for portable markdown inspection and progressive traversal.

### Where scope lives

Every `wiki_*` table row carries `tenant_id`. Owner-scoped agent pages carry an agent `owner_id`; tenant-shared compiled pages use `owner_id IS NULL` and must stay behind tenant policy and Context Engine routing. Read paths should treat owner scope and tenant scope deliberately rather than assuming every page is visible to every agent.

OKF bundle metadata repeats that boundary in `x-thinkwork`: tenant scope, page kind, ontology version, lifecycle status, provenance references, and redaction posture are part of the generated page profile.
