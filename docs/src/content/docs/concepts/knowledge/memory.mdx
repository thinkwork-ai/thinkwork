...
description: The adapter-agnostic retained-memory layer — why ThinkWork owns the contract while Hindsight or AgentCore Memory stay pluggable.
---
...
- The wiki. The wiki is _derived_ from memory — it's a layer above, not memory itself.
...
<Aside type="tip">
  The contract is the stable part. The engine is pluggable. When you're thinking
  about memory in ThinkWork, think in terms of the contract operations first —
  then figure out which engine your deployment happens to use.
</Aside>
...
- **Recall quality depends on the engine.** The contract guarantees the _interface_, not equivalent _results_. Queries that return great matches on Hindsight may return less-relevant results on AgentCore Memory; tune per adapter.

## Related pages

- [Memory](/concepts/knowledge/) — the umbrella
- [Sources and Knowledge Bases](/concepts/knowledge/document-knowledge/) — what memory _isn't_
- [Compiled Memory Pages](/concepts/knowledge/compounding-memory/) — what's built on top of the memory layer
- [Source Routing](/concepts/knowledge/retrieval-and-context/) — where memory sits in turn assembly
- [Operating Compiled Memory Pages](/guides/compounding-memory-operations/) — recipes that depend on the memory contract
...
- **Retention event.** Emitted from the AgentCore runtime after every turn to whichever adapter is active.
