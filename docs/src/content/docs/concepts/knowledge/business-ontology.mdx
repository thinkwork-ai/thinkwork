...
## Brain Graph Boundary

ThinkWork Brain now uses a Postgres-backed graph mirror for ontology-shaped
entities, relationships, evidence, diagnostics, and wiki graph rendering. That
mirror is generated from approved ontology definitions and cited source
material; it is not a separate operator-provisioned application.

The graph pipeline does not by itself approve ontology change sets, rewrite
Hindsight memories, or expose raw graph storage to end users. Operators still
use the ontology approval and reprocess-job flow to change the business model,
and agent-facing retrieval continues through Context Engine and Brain APIs.
