...
## Cognee substrate boundary

ThinkWork can optionally provision Cognee as an AWS-native ontology and knowledge-graph substrate. This is infrastructure, not an automatic product migration.

When `enable_cognee = false`, existing tenants do not get Cognee resources and the ontology loop continues to use the current approved-definition and reprocess-job path. When `enable_cognee = true`, Terraform creates an internal ECS/Fargate service, internal ALB, EFS-backed dogfood storage or remote graph/vector configuration, CloudWatch logs, IAM, and Secrets Manager wiring for provider credentials.

That substrate does not by itself:

- migrate retained Hindsight memories;
- rewrite compiled Wiki or Brain pages;
- approve or apply ontology change sets;
- change agent context retrieval;
- expose Cognee directly to end users.

Operators should treat Cognee as the backing substrate for future ontology/KG capabilities. The product contract still requires approved definitions, tenant-scoped reprocess jobs, provenance, and agent-facing retrieval APIs before Cognee output becomes Brain context.

<Aside type="note">
  Terraform success means the substrate deployed. It does not prove that Cognee
  can reach every provider, graph store, vector store, or database, and it does
  not mean any tenant ontology has changed.
</Aside>
...
- [Memory](/concepts/knowledge/) — the product boundary around memory, pages, knowledge bases, workspace files, and source routing
- [Memory Compile Pipeline](/concepts/knowledge/compounding-memory-pipeline/) — how retained memories become compiled pages
- [Memory Graph Direction](/concepts/knowledge/knowledge-graph/) — how typed relationship semantics fit with existing page links
- [Memory Context API](/api/context-engine/) — the provider contract used by agents and MCP clients
