...
Control is the harness's governance layer — the component that turns ThinkWork from a demo into a production system, and the load-bearing implementation of the four REST anchors ([Reliability, Efficiency, Security, Traceability](/architecture/#the-four-rest-anchors)). It answers two questions that every real deployment has to answer:
...
## REST anchors implemented

The five governance controls below map 1:N to the four REST anchors. The anchors are the *language* a buyer or operator can grab onto; the controls are the *implementation* that ships in the admin web. Every anchor has at least one shipping control behind it.
