...
description: Per-turn cost capture via OTel spans, tenant and Space budget enforcement, and the append-only audit log that makes incident reconstruction possible.
---
...
- [Admin: Analytics](/applications/admin/analytics/) — the operator view of tenant, Space, and agent spend where available
...
- **Pause mechanism.** Space-level pause is stored with Space runtime overrides; tenant-level pause is stored with the platform-agent budget state. Admin and mobile surfaces react to the change via real-time subscriptions.
