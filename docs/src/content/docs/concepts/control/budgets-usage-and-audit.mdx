...
The second half of Control is about knowing what happened and bounding how much it cost. Three intertwined concerns:

- **Usage tracking** — per-turn cost, broken down by input tokens, output tokens, and tool call overhead.
- **Budgets** — tenant and per-agent spend limits that actually *suspend* agents when hit, not just warn.
- **Audit** — the durable, append-only record of every invocation that makes "what did the agent see, decide, and do?" answerable months later.
...
- Tenant configuration changes (template edits, agent creation, connector updates).
...
- **Cost capture.** `apps/agent-core/src/otel.py` instruments every turn; the CloudWatch log group emits spans to a cost reducer Lambda in `packages/api/src/lib/billing/cost-reducer.ts`.
- **Cost storage.** `thread_turn_costs` table in Aurora — one row per turn, foreign-keyed to `thread_turns`. Index on `(tenant_id, created_at)` for the common month-to-date query.
- **Budget reconciler.** `packages/api/src/handlers/budget-reconciler.ts`. EventBridge rule `budget-reconciler-15min`, runs every 15 minutes per stage.
- **Pause mechanism.** Reconciler issues `updateAgent` mutation with `status = 'PAUSED'`; admin app and mobile surfaces reflect this immediately via `OnAgentStatusChangedSubscription`.
- **Audit bucket.** `s3://thinkwork-<stage>-audit/` with `s3:DeleteObject` denied on the Lambda role in IAM. Lifecycle policy is configured in Terraform per deployment.
- **NDJSON record format.** `packages/api/src/lib/audit/record.ts` defines the schema; `thinkwork audit replay` in the CLI reads the bucket and re-plays records into a local SQLite DB for analysis.
