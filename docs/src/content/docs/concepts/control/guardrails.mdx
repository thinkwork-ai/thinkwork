...
- **Backing service.** Amazon Bedrock Guardrails. Reference: [Bedrock Guardrails docs](https://docs.aws.amazon.com/bedrock/latest/userguide/guardrails.html).
- **Invocation path.** `apps/agent-core/src/bedrock.py` — the `invokeModel` wrapper passes `guardrailIdentifier` and `guardrailVersion` from the agent's template into the `converse` call.
- **Template reference.** `agent_templates.guardrail_id` and `agent_templates.guardrail_version` columns in Aurora.
- **Event propagation.** Guardrail-activated events are written as turn trace rows (`thread_turn_events` table with `event_type = 'guardrail_activated'`) and included in the audit log record.
- **Admin rendering.** `apps/admin/src/components/ThreadTraces/GuardrailEvent.tsx` renders the inline guardrail events; the detail panel shows the full evaluator output.
- **Terraform starter.** `terraform/modules/thinkwork/guardrails.tf` ships the `gr-default-safe` resource; tenants can fork or add their own.
