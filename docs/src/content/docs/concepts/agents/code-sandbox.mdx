...
2. **Pre-flight** — policy check + interpreter-ready check.
...
6. **Session start** — `StartCodeInterpreterSession` against the tenant's interpreter, followed by the readiness preamble.
7. **User code** — `executeCode(user_code)` runs the agent's Python.
