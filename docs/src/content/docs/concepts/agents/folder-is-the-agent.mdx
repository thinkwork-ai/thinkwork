...
A folder specialist is not a separate top-level managed agent. It does not need its own budget, model, thread identity, or deployment. Those belong to the tenant platform agent and, when needed, Space runtime overrides.
