...
Operators handle these in the admin app at **Workspace Reviews**. The page lists every `awaiting_review` run for the current tenant and shows the source review object, target folder, agent, and requested time.

Available decisions:

| Decision | Effect                                                                                                                           |
| -------- | -------------------------------------------------------------------------------------------------------------------------------- |
| Accept   | Records `review.responded`, moves the run back to `pending`, and enqueues a `workspace_event` wakeup for the same agent.         |
| Resume   | Same wakeup behavior as accept, for cases where the operator wants the run to continue without approving the content.            |
| Cancel   | Records `review.responded` with a cancellation decision and marks the run `cancelled`. It does not enqueue another agent wakeup. |

Directly deleting a review object is not a cancellation mechanism. Deletions are recorded as `event.rejected` with `review_deleted_directly`; use the admin cancel action so run state and audit history remain coherent.
...
| Review deletion does not resume a run                | Direct S3 deletion is recorded as `event.rejected`. Use Workspace Reviews in admin so the platform can write `review.responded` and update run state.                                                               |
| A review is not listed in admin                      | Confirm the review filename starts with a UUID run id, the run belongs to the current tenant, and the run status is `awaiting_review`.                                                                              |
