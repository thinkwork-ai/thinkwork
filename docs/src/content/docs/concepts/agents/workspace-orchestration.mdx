...
The important idea is that the **folder is the routing boundary**. The root
agent is addressed as `.`, and specialist folders like `workspaces/expenses/`
or `workspaces/customer-support/workspaces/escalations/` are addressed by their
relative path. That keeps async work aligned with the same workspace model used
by [workspace composition](/concepts/agents/workspace-composition/) and
[Folder Is the Agent](/concepts/agents/folder-is-the-agent/).

<Aside type="note">
  Workspace orchestration is not a general DAG engine. It is a durable event
  primitive for agent work: write a request, record a canonical event, wake the
  right runtime, and let the agent manage its own plan, fan-out, and completion
  files.
</Aside>
...
- **Async specialist work** — a parent asks `workspaces/legal/` or
  `workspaces/expenses/` to handle work without blocking the current turn.
...
- **Long-running background work** — scheduled jobs and integration events can write requests into the workspace instead of inventing a parallel queue.
...
<Aside type="caution">
  In local-zip Terraform mode, the dispatcher resources only become active when
  `lambda_zips_dir` points at built Lambda artifacts. Build with `bash
  scripts/build-lambdas.sh workspace-event-dispatcher` before applying a stack
  that should process workspace events.
</Aside>
...
| ------------------------ | -------------------------------------- | ------------------------------------------------ | --------------- |
| Expense policy questions | workspaces/expenses/                   | workspaces/expenses/CONTEXT.md                   | expense-review  |
| Legal review             | workspaces/legal/workspaces/contracts/ | workspaces/legal/workspaces/contracts/CONTEXT.md | contract-review |
```
...
| ------------------------------------------------- | --------------------------------------------------------- |
| `.`                                               | The root agent.                                           |
| `workspaces/<slug>/` segments                     | Specialist folders.                                       |
| Lowercase, numbers, hyphens, slash separators     | Valid folder syntax.                                      |
| Max nested specialist depth 4                     | Deeper trees are rejected.                                |
| No `memory` or `skills` specialist segment        | Those names are reserved resources at every folder depth. |
| No absolute paths, `..`, backslashes, `?`, or `#` | Prevents path traversal and ambiguous S3 keys.            |

### 5. Deploy a runtime with `wake_workspace`

Managed AgentCore runtimes register the `wake_workspace` tool when the environment has:
...
### Routing: who resolves the review
...
- **Paired** reviews surface only on the responsible human's **mobile** Threads. Folder-specialist reviews follow the parent chain: if a specialist folder pauses for review, it appears in your mobile threads even though you are not a participant in that specialist run directly. The mobile thread row carries a `Sub-agent <name> needs your input on <target>` label so the routing is visible.
...
These mutations live on the GraphQL surface and can be called by trusted services, but no first-class agent tool wraps them today — review resolution is admin-UI-driven (or mobile-driven for paired-human reviews). A vetted runtime tool for review resolution would let an agent (e.g. a triage agent) drain its tenant's queue programmatically; that's plausible follow-up work, not part of v1.
...
2. Apply the database migrations and enable one tenant:
...
3. Add a route to the agent's `AGENTS.md`:
...
4. Call `wake_workspace(target="smoke-test", request_md="...", wait_for_result=false)`.

5. Confirm a canonical event landed:
...
6. Confirm the wakeup request exists:
...
7. Confirm the target agent was invoked by checking the wakeup processor logs and the agent run output.

8. Confirm protected generic writes are still refused by attempting a generic workspace file write to `smoke-test/work/inbox/manual.md`. The API should return `403` with `use orchestration writer`.
...
| A paired-human review is not visible on mobile       | Confirm the responsible human is the agent's `human_pair_id` or the folder-specialist run's first paired ancestor. Confirm the user is signed into the right tenant on mobile.                                                                                                                                                                                                              |

## Related pages

- [Workspace Composition](/concepts/agents/workspace-composition/) — how workspace files compose from agent folders, Spaces, templates, and defaults.
- [Folder Is the Agent](/concepts/agents/folder-is-the-agent/) — why folder paths are the unit of specialization.
