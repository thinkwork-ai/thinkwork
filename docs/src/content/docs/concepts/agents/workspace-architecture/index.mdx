...
The runtime does not show the same top-level source folders. It uses the Agent source as the root, keeps User context explicit under `User/`, hydrates only the active Space under `Spaces/<active-space>/`, and renders Thread status under `Thread/`:
