...
The **factory** is the editable source side of the workspace. It contains one canonical Agent source, Space sources, and User sources. Operators edit these sources in the workspace UI or through platform actions.
...
4. **Reconcile.** The runtime sends changed files to the API. The API checks provenance, lanes, access, secret scanning, and object freshness before writing source files back to S3.
