...
`GOAL.md` and `PROGRESS.md` are mounted read-only because they describe database state. Narrative files such as `DECISIONS.md`, `ARTIFACTS.md`, and `HANDOFFS.md` are writable Thread Goal files.
...
<Aside type="caution">
  Do not ask an agent to change task status by editing `PROGRESS.md`. Ask it to
  use the task-status tool. The file will re-render from the database.
</Aside>
