...
- managed-app descriptors for ThinkWork Brain/Cognee, Twenty, and n8n
- smoke contracts
...
publisher builds the ThinkWork-owned Cognee image for ThinkWork Brain graph
infrastructure and reads pinned Twenty and n8n image URIs from repository
variables so the controller can fail before Terraform when an optional app
cannot be deployed from the selected release.

The default release manifest currently advertises:

| Managed app            | Required runtime image | Smoke contract                                             |
| ---------------------- | ---------------------- | ---------------------------------------------------------- |
| ThinkWork Brain/Cognee | `cognee`               | `plugins/company-brain/smoke/cognee-managed-app-smoke.mjs` |
| Twenty CRM             | `twenty`               | `plugins/twenty/smoke/twenty-managed-app-smoke.mjs`        |
| n8n                    | `n8n`                  | `plugins/n8n/smoke/n8n-managed-app-smoke.mjs`              |
...
- managed-app smoke results for enabled apps when they are provisioned or parked
