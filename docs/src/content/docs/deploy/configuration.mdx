...
| `memory_engine` | string | `"managed"` | `managed` (the default AWS AgentCore LongTerm memory path used by the open source app) or `hindsight` (ECS Fargate alternative with a fuller explicit memory lifecycle). |
...
| Variable | Type | Default | Description |
|----------|------|---------|-------------|
| `memory_engine` | string | `"managed"` | `managed` or `hindsight`. `managed` is the config surface for the default AWS AgentCore LongTerm memory integration. |
...
With `memory_engine = "managed"`, Thinkwork uses AWS AgentCore LongTerm memory by default. The documented default setup includes four built-in strategies: semantic, summarization, user-preference, and episodic.
