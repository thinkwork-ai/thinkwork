...
The Hindsight memory engine and all of its infrastructure were removed in
THINK-407. Two inputs are still **declared and ignored** so existing tfvars and
`-var` passthroughs keep applying; new stacks should omit both.

| Variable           | Status                                                                                   |
| ------------------ | ---------------------------------------------------------------------------------------- |
| `enable_hindsight` | Declared no-op. Accepted, never read. Omit on new stacks.                                |
| `memory_engine`    | Declared no-op. Legacy values (`hindsight`, `managed`) still validate and are discarded. |

The `hindsight_image_tag`, `hindsight_database_name`,
`hindsight_enable_auto_consolidation`, `hindsight_consolidation_dedup_threshold`,
and `hindsight_observations_mission` inputs were **removed** — a tfvars file
that still sets them fails with "Value for undeclared variable".
