...
This guide walks through a full ThinkWork deployment into a fresh AWS account with no existing infrastructure. At the end, you'll have ~260 AWS resources running across three tiers, accessible via the web app and GraphQL API.
...
- Aurora Postgres (serverless v2 by default, 0.5–16 ACUs) with `pgvector`
...
- Integration Lambda functions (Slack, GitHub, Google)
- Step Functions state machines (automations, Workflow execution adapter)
- CloudFront distributions (web app)
- Hindsight on ECS Fargate (hosted Brain memory for user and Space recall)
- Bedrock AgentCore Memory (explicit compatibility mode when selected)
...
<Aside type="tip">
  Enable model access before running deploy. If models aren't enabled, the
  deploy will succeed but agent invokes will fail with a Bedrock
...
5. **Edit terraform.tfvars**
   ...
   # Memory
   # Hindsight Brain memory is enabled by default.
...
8. **Get your endpoints**
   ...
   Web App:       https://d1234.cloudfront.net
...
10. **Sign in to the web app**

    Navigate to the Web App URL, sign in with your Cognito user, and create your first agent.
...
| S3              | 8        | Skill catalog, source docs, audit logs, assets, CF logs, Lambda artifacts |
| Lambda          | 18       | AgentCore, integration handlers, auth triggers, migration runner      |
...
| Step Functions  | 4        | State machines for automations and the Workflow execution adapter     |
| EventBridge     | 3        | Scheduled automation rules, event bus                                 |
| SES             | 5        | Identity, config set, event destination, suppression list             |
| Secrets Manager | 4        | DB credentials, Cognito secrets, integration secrets                  |
...
After `terraform apply` finishes, `thinkwork deploy` runs a non-fatal probe of the Pi AgentCore runtime (`scripts/post-deploy.sh`) and prints `ok` / `WARN`. It verifies that the runtime is `READY`, the `DEFAULT` endpoint has no pending `targetVersion`, and the endpoint's `liveVersion` matches the runtime's current version.

The probe does **not** force a warm-container flush — AgentCore rejects `UpdateAgentRuntimeEndpoint` on DEFAULT endpoints with "managed through agent updates". When a deploy changes runtime env vars (e.g. a new `THINKWORK_API_URL`), warm containers booted before the update continue serving until AgentCore's **15-minute internal reconciler** cycles them out. The probe's role is early-warning visibility, not mitigation.

Run the probe on its own any time with:

```bash
bash scripts/post-deploy.sh --stage dev                # warn-only
bash scripts/post-deploy.sh --stage dev --strict       # exit 1 on drift (CI)
bash scripts/post-deploy.sh --stage dev --json         # machine-readable
```

If a runtime persistently shows `endpoint liveVersion != runtime version` for more than ~20 minutes after a deploy, the endpoint is stuck — check the runtime's CloudWatch logs for a container crashloop and raise a support ticket if needed.
