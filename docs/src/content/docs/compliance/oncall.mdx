---
title: On-call notes
description: "Alarm-to-playbook quick reference plus procedures for anchor/exports DLQs, watchdog heartbeat missing, anchor gap, drift gate, audit_outbox runaway, and runtime REST emit failures."
---
...
| (No alarm; surface via runtime errors) audit emits silently failing     | [Runtime REST emit silent failure](#runtime-rest-emit-silent-failure)   |
...
## Runtime REST emit silent failure

**Symptom:** Audit events emitted through `/api/compliance/events` stop appearing in `audit_events` while Yoga-emitted events continue normally. Runtime logs show no obvious errors.

**Likely cause:** Env-var drift or an API secret mismatch on the runtime caller. The runtime should snapshot `THINKWORK_API_URL` + `API_AUTH_SECRET` at invocation entry; a stale value will fail bearer validation at the REST handler.
...
1. Check `compliance-events` REST handler logs for failed bearer validations. If you see auth failures from runtime callers, compare the deployed `API_AUTH_SECRET` and runtime env snapshot.
2. Confirm the AgentCore endpoint has rolled to the expected runtime version. The DEFAULT endpoint does not expose a flush API ([project_agentcore_default_endpoint_no_flush](`memory/project_agentcore_default_endpoint_no_flush.md`)) — the reconciler is the only flush mechanism.
3. Verify runtime callers snapshot env at invocation entry, not on each REST call. Re-reading mutable env mid-handler can silently break audit emits after an AgentCore deploy.

Runtime emit reference: [developer-guide.md → Runtime REST emit path](/compliance/developer-guide/#runtime-rest-emit-path).
