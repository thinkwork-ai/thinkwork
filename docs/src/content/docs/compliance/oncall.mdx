...
**Symptom:** Audit events from Strands runtime (folder-specialist operations, tool invocations from Python containers) stop appearing in `audit_events` while Yoga-emitted events continue normally. Strands logs show no obvious errors.
