---
title: SOC2 Type 1 walkthrough
description: "10-step narrative for an auditor sitting next to an operator: sign-in → events list → walk-back → export → CLI verifier → S3 retention check."
---
...
For the architecture this walkthrough exercises, see [architecture](/compliance/architecture/). For the operator-side procedures referenced inline, see [operator-runbook](/compliance/operator-runbook/).
...
- For a production engagement, the anchor bucket's Object Lock mode is COMPLIANCE (irreversible). For a non-prod rehearsal, GOVERNANCE is acceptable. See [operator-runbook.md → GOVERNANCE → COMPLIANCE flip](/compliance/operator-runbook/#flip-s3-object-lock-governance--compliance-for-an-audit-engagement).
- The `audit-verifier` CLI is installed locally on the auditor's laptop or a shared workstation. Install: `npm install -g @thinkwork/audit-verifier`. CLI README: [`packages/audit-verifier/README.md`](https://github.com/thinkwork-ai/thinkwork/blob/main/packages/audit-verifier/README.md).
...
**See:** Web app loaded; sidebar shows **Compliance** between Settings and Billing.

**Why it matters:** The Compliance entry is operator-tier only; non-operators do not see it. Permission gating is enforced server-side by `requireComplianceReader` ([`packages/api/src/lib/compliance/resolver-auth.ts`](https://github.com/thinkwork-ai/thinkwork/blob/main/packages/api/src/lib/compliance/resolver-auth.ts)) — the sidebar is the visible signal.

### 2. Open `/compliance`

**See:** The events list with a default 7-day window and the 14-event filter dropdown.

**Why it matters:** The 14 event types are the SOC2 starter slate the platform commits to recording. Source: [`packages/database-pg/src/schema/compliance.ts`](https://github.com/thinkwork-ai/thinkwork/blob/main/packages/database-pg/src/schema/compliance.ts) `COMPLIANCE_EVENT_TYPES`. The list of types is in [architecture](/compliance/architecture/#event-type-slate); reference it during the explanation.
...
**Why it matters:** Visual proof of append-only history. Each backward step uses `complianceEventByHash` ([`packages/api/src/graphql/resolvers/compliance/query.ts`](https://github.com/thinkwork-ai/thinkwork/blob/main/packages/api/src/graphql/resolvers/compliance/query.ts)) which is hash-keyed — no way to "skip" an event without changing its hash.
...
CLI source + README: [`packages/audit-verifier/`](https://github.com/thinkwork-ai/thinkwork/blob/main/packages/audit-verifier/).
...
The leading byte (`0x00` for leaves, `0x01` for inner nodes) prevents second-preimage forgery on the Merkle proof path. Reference: RFC 6962 §2.1. Implementation: [`packages/lambda/compliance-anchor.ts`](https://github.com/thinkwork-ai/thinkwork/blob/main/packages/lambda/compliance-anchor.ts) (`computeLeafHash`, `combineNodes`, `buildMerkleTree`).

## Event-type slate

The 14 Phase 3 events the platform commits to recording are listed in [architecture.md → Event-type slate](/compliance/architecture/#event-type-slate). 5 Phase 6 reservations exist for future policy events.

## Redaction allow-list

Each event type has an explicit allow-list of payload fields that survive into the audit row; everything else is redacted at write time by the `emitAuditEvent` helper before the INSERT into `audit_outbox`. A new event type cannot land without a corresponding redaction rule — the allow-list registry is the gate. Source: [`packages/api/src/lib/compliance/redaction.ts`](https://github.com/thinkwork-ai/thinkwork/blob/main/packages/api/src/lib/compliance/redaction.ts).
...
1. **Yoga TypeScript resolvers** — call `emitAuditEvent(tx, ...)` inside their existing `db.transaction` block. Audit failure rolls back the originating mutation (control-evidence tier). Source: [`packages/api/src/lib/compliance/emit.ts`](https://github.com/thinkwork-ai/thinkwork/blob/main/packages/api/src/lib/compliance/emit.ts).

2. **AgentCore runtime REST emit** — runtime callers POST `/api/compliance/events` with bearer `API_AUTH_SECRET` and a UUIDv7 `event_id` generated client-side. The REST handler ([`packages/api/src/handlers/compliance.ts`](https://github.com/thinkwork-ai/thinkwork/blob/main/packages/api/src/handlers/compliance.ts)) validates and inserts into the outbox. The `audit_outbox.uq_audit_outbox_event_id` unique constraint makes retries idempotent — if the network drops mid-call, the runtime caller retries with the same `event_id` and the second insert is a no-op.
...
The verifier CLI is the auditor's independent verification tool — it does not read from the platform's GraphQL API; it reads from S3 (anchor bucket) + Aurora (chain) directly using read-only credentials. Full CLI reference: [`packages/audit-verifier/README.md`](https://github.com/thinkwork-ai/thinkwork/blob/main/packages/audit-verifier/README.md).
