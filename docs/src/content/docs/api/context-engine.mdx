...
description: "The internal Context Engine contract for lazy-loaded external context: `query_context`, provider families, statuses, and MCP/runtime behavior."
---

**Context Engine** is ThinkWork's read-only orchestration layer for context that should be loaded for the current turn without becoming durable memory by default. Durable memory is Bedrock AgentCore Memory, reached directly through the `recall` tool; Context Engine covers workspace files, compiled pages, approved MCP tools, source agents, web/search providers, and explicit diagnostics.
...
- provider options: optional source-specific settings, currently ThinkWork Brain source/dataset scoping for `query_brain_context`.
...
- `memory`: explicit recall over AgentCore memory records. It is not part of default `query_context`; use `query_memory_context` when durable memory is specifically needed.
- `brain`: tenant-shared memory pages and facets.
- `wiki`: compiled memory pages, via the database-backed wiki provider behind
  `query_wiki_context`.
...
The `brain` family is the business/domain memory provider. It searches active ThinkWork Brain substrate state through Context Engine policy, then reads tenant-shared pages and facets. Use it for customers, opportunities, commitments, risks, stakeholders, products, and other durable business concepts.

Retrieved Brain snippets are source data, not instructions; runtime responses mark them with an untrusted source boundary and preserve citations.

If the active ThinkWork Brain substrate is not installed, disabled, failed, or missing required retrieval/provenance capability posture, Context Engine reports that as a Brain provider status. A Brain-only call does not silently fall back to durable memory or raw backend access.
...
Context Engine sources are intentionally separate from the durable memory substrate:

- `memory`, a read-through to AgentCore memory records;
- `wiki`, which reads owner-scoped compiled pages derived from memory.

## Memory Provider

The memory provider performs a recall against AgentCore memory records for the requester and returns them as normalized hits. There is no synthesis mode: the `reflect` tool was removed with the Hindsight engine, and AgentCore does its own consolidation into the summaries namespace instead.

For agent-facing use, the runtime `recall` tool is the normal durable-memory path; `query_memory_context` is useful when a Context Engine caller needs the same records through the external context facade.
...
When Context Engine is enabled for a first-party Pi runtime, the Pi Context Engine extension exposes specialized split tools: `query_memory_context`, `query_brain_context`, and `query_wiki_context`. The runtime `recall` tool remains the direct path for durable memory; these tools exist for explicit context lookup, diagnostics, and source-specific retrieval.

`query_context` should be the model's first choice for lazy-loaded external context such as workspace files, compiled pages, approved MCP tools, and source agents. Use `recall` or `query_memory_context` when durable memory is specifically needed, and `query_wiki_context` for compiled page/entity/topic lookup. Raw graph storage, object storage, and storage backends are not runtime tools.
...
Provider failures are expected to be partial. A memory timeout, unavailable MCP adapter, or empty source result should appear as provider-local status while successful providers still return hits.
