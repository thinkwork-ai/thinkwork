...
Provider failures are expected to be partial. A Hindsight timeout, unavailable MCP adapter, or empty source result should appear as provider-local status while successful providers still return hits.
