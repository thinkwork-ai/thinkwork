---
title: Compounding Memory (API)
description: Programmatic access to the wiki — reading pages, searching, compiling on demand, bulk-importing.
---
...
- Ranked wiki search for mobile — runs the same semantic recall your agent uses, then shows you the pages those memories feed into.
...
```graphql
query MobileSearch($agentId: ID!, $query: String!, $limit: Int = 20) {
...
This is different. It doesn't do its own search over the wiki pages — it asks the memory engine (Hindsight) to find the most relevant memories for the query, then returns the pages those memories cite.

**The ranking trusts Hindsight's order.** If Hindsight's #1 hit cites page A, page A wins. If Hindsight's #5 hit cites page B, page B ranks lower. The `score` is a reciprocal-rank value (higher is better, not comparable across queries).

`matchingMemoryIds` tells you which memories caused each page to surface, in Hindsight's rank order. You can use this to render a "why this matched" explanation in the UI.

<Aside type="tip">
If you're building a user-facing search, this is probably what you want. The lexical search is better for exact title lookups; this one is better when users ask in natural language.
</Aside>
...
- [Compounding Memory: Pipeline](/concepts/knowledge/compounding-memory-pipeline/) — what compiles do
...
// 1. Hindsight recall returns memories in rank order (rank 0 = best).
// 2. Reverse-join through wiki_section_sources to candidate pages.
// 3. For each page:
//    bestRank  = min(rank of any memory citing the page)
//    matchCount = count of distinct hits citing the page
// 4. Sort by bestRank ASC, then matchCount DESC, then lastCompiledAt DESC.
// 5. Return score = 1 / (bestRank + 60)  (reciprocal rank surfacing score)
```
