---
title: Rename the retired graph substrate Cluster Dev Cutover Verification
issue: THNK-30
date: 2026-06-15
status: pending-main-cd
---

# Rename the retired graph substrate Cluster Dev Cutover Verification

## Scope

This artifact records the TEI ThinkWork dev cutover proof for renaming the
legacy stage-wide ThinkWork Brain ECS cluster from
`thinkwork-dev-retired_graph_substrate-cluster` to `thinkwork-dev-brain-cluster`.

the retired graph substrate remains the implementation key, ECS service name, task family, container,
...
- Allowed resource changes:
  ...
  - Acceptable if unavoidable: the retired graph substrate ECS service relocation/replacement to the
...
- AWS ECS active Brain cluster:
  - Status: pending
  - Redacted evidence:
- the retired graph substrate service `thinkwork-dev-retired_graph_substrate` active under Brain cluster:
  - Status: pending
  - Desired/running/pending:
- ALB target group healthy for desired count:
  - Status: pending
  - Redacted evidence:
- Terraform `retired_graph_substrate_cluster_arn`, GraphQL `deploymentStatus.retired_graph_substrateClusterArn`,
  the retired graph substrate managed app `clusterArn`, ECS service identity, and
  `knowledgeGraphHealthCheck` agree on Brain cluster:
  - Status: pending
  - Redacted evidence:
- `GRAPH_CLUSTER_ARN` absent or Brain-named:
  - Status: pending
  - Redacted evidence:
- Old cluster `thinkwork-dev-retired_graph_substrate-cluster` absent, or decommission owner/date:
  - Status: pending
  - Redacted evidence:
...
- Terraform plan artifact from GitHub Actions or deployment controller using the
  shared dev backend.
- `plugins/brain/smoke/retired_graph_substrate-managed-app-smoke.mjs` with live smoke enabled.
- Read-only ECS describe checks for both the Brain cluster and legacy the retired graph substrate
  cluster.
