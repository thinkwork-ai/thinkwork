...
related:
  ...
  - docs/plans/2026-04-30-001-refactor-brain-nav-docs-plan.md
  - docs/plans/2026-04-30-002-feat-brain-hybrid-retrieval-plan.md
---

# Mobile ThinkWork Brain Search

## Problem Frame

The current mobile Wiki surface is still page-first: users can search compiled pages, browse recent pages, and switch into the force graph. Admin is moving to **ThinkWork Brain** as the product-facing umbrella for memory, compiled pages, knowledge bases, and source adapters. Mobile should make the same conceptual move, but with a different job: it is the user's surface plane for asking the Brain, enriching it, and reviewing what should become durable context for agents.
...
- A4. ThinkWork Brain page: the durable topic/entity/decision surface being enriched, browsed, converted, or reviewed.
- A5. Tenant admin: governs which sources are eligible for ThinkWork Brain search and enrichment.
...
- F1. Search-first Brain query
  ...
  - **Actors:** A1, A3
  - **Steps:** The surface runs a ThinkWork Brain search across default-safe sources. Results show source family, title, summary/snippet, citation/provenance, and provider status. The user can filter results by source family or switch to Pages/graph browsing for page-shaped results.
...
- R1. The mobile top-level Wiki label is replaced or reframed as ThinkWork Brain/Brain, with search as the primary interaction.
- R2. Mobile Brain search returns a unified ranked result set across multiple ThinkWork Brain source families instead of only compiled wiki/page search.
...
- This does not introduce external system writes. v0 writes only to ThinkWork Brain pages/review state.
...
- The ThinkWork Brain source/router work in `docs/plans/2026-04-30-002-feat-brain-hybrid-retrieval-plan.md` provides the retrieval substrate for Brain/Page and source-agent results.
- Admin source governance from the ThinkWork Brain admin plans determines which source families mobile may show by default or allow as opt-in.
...
- [Affects R2-R4][Technical] Whether mobile calls the existing ThinkWork Brain HTTP/MCP facade directly or uses a GraphQL wrapper for query/search and provider status.
