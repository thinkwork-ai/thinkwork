...
- **R13.** **Invariant:** token values never reach a persisted log, dashboard, or any record surfaced outside the session they were fetched for. This applies to env-var names, stdout, stderr, executed code, outbound request payloads observed in APPLICATION_LOGS, and any dispatcher-side trace. The scrubbing mechanism (value-based redaction using the session-scoped set of actual token strings and their base64 / URL-encoded / hex forms, not key-name pattern matching) is a planning concern; the invariant is binding.
...
- No token value appears in any ThinkWork-persisted log, dashboard, or audit record under normal operation, enforced structurally by per-tenant IAM scoping (R1) + path-only preamble (R9/R12) + value-based redaction invariant (R13). Deliberate exfil by agent code under prompt injection is the named unmitigated threat T1 and is tracked for v2 hardening.
