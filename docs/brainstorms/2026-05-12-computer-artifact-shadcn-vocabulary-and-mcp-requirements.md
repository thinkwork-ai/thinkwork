...
- The Computer agent's existing `save-app` tool is the right hook to evolve into the tightened save-artifact validator. The constrained-import-surface validator that the morning brainstorm planned to delete is *retained* and *tightened*, not rebuilt from scratch.
