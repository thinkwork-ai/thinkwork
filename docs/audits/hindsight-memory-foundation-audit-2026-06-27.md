...
- W6. Mobile user-posted `OBSERVATION` captures should not be treated as engine-synthesized observations for the retired graph substrate/Brain promotion; the existing observation promotion code already uses `source_memory_ids` for this reason.
...
| the retired graph substrate/Brain observation promotion | `packages/api/src/lib/knowledge-graph/observations-source.ts`                                                                                                | Reads engine observations with populated `source_memory_ids`, gates before promotion                          | Good safety posture. Continue requiring synthesis provenance rather than `fact_type=observation` alone.                                                                                      |
...
- R4. Source-fact evidence exists in Hindsight but is not yet a first-class evidence chain through memory APIs, admin, Context Engine detail, Wiki, or the retired graph substrate promotion surfaces.
...
| Entity labels                      | Not set in sampled banks                                             | Candidate for substrate quality and Wiki/the retired graph substrate alignment, but needs product vocabulary review.      |
...
- Context Engine remains valuable as a governed routing, policy, and operator-verification surface across Memory, Wiki, Brain/the retired graph substrate, workspace files, knowledge bases, and MCP context tools.
...
- ThinkWork Brain/the retired graph substrate remains the governed tenant-shared business/domain substrate.
- Wiki remains a compiled, reviewable projection over retained memory and other source families.
- Raw the retired graph substrate, Neptune, and S3 internals should not become ordinary runtime/product APIs. Hindsight is different: it is the chosen memory substrate, so its memory-domain concepts should be first-class even if the HTTP/database internals remain encapsulated.
...
| HIN-9  | Add operator evidence chain surfaces                            | Admin Memory, Context Engine detail, Wiki/the retired graph substrate provenance | Memory-derived claims become auditable                                     | Admin/source detail can show observation ID, source IDs, and redacted source metadata.                    |
...
- Aggressive entity-label tuning for Wiki/the retired graph substrate until the business ontology vocabulary is stable.
...
| Operator detail view can trace a memory-derived Wiki/the retired graph substrate claim to Hindsight observation/source identifiers | Admin verification      | AE4, HIN-9        |
