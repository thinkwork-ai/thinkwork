...
- **Expose a separate `wake_workspace` tool that shares target-resolution with `delegate_to_workspace`.** `delegate_to_workspace` does not exist in main yet — only a generic `delegate(task, context)` tool registered in `server.py:1370`. The path-targeting `delegate_to_workspace` is planned in `docs/plans/2026-04-24-008-feat-fat-folder-sub-agents-and-agent-builder-plan.md` U9. To avoid sequencing risk, this plan extracts the **shared target-resolution helper** (validate path, reject `..`, reject reserved `memory/`/`skills/`, enforce depth cap) up front (new U10), and `wake_workspace` adopts it from day one. Plan 008 U9 then adopts the same helper when it lands.
...
- **Single `wait_for` per `events/blocked.json` + N+1 wakes for N children, parent-managed.** Schema has single nullable `wait_for_run_id`; the platform does NOT manage fan-in. Fan-out pattern: parent issues N `wake_workspace` calls (writes N inbox files + initial `events/blocked.json` pointing at the FIRST child's runId), each with the child's runId tracked in `runs/{parentRunId}/status.json`. When child #1 completes, dispatcher wakes parent. Parent reads `status.json`, sees N-1 children still pending, writes new `events/blocked.json` with `wait_for` pointing at child #2 (or terminates if all done). Net wakes for N children: N+1. The wake-amplification cost is real and bounded by the depth-4 + 10-inbox-per-run quotas. Brainstorm Success Criterion #3 ("resume only when each specialist completes") describes platform-managed fan-in — flagged as Outstanding Question for brainstorm addendum; v1 ships agent-managed fan-out via `status.json`. Future v2 may add an array `wait_for` schema with platform fan-in semantics; v1 stays primitive.
...
- **Workspace-files API rejects PUT to eventful prefixes.** The existing `packages/api/workspace-files.ts` handler (used by the Strands runtime for memory writes and by the admin SPA for workspace edits) is modified in U5 to refuse PUT operations targeting `work/inbox/`, `review/`, `work/runs/*/events/`, `events/intents/`, or `events/audit/` with HTTP 403 `error: "use orchestration writer"`. Same handler's presigned-URL branch denies minting URLs for those prefixes. Orchestration writes go EXCLUSIVELY through the new `write-api.ts` orchestration writer (U5), which enforces F2 race ordering (parent blocked.json BEFORE child inbox), single-`wait_for` shape, and tenant identity from service auth or Cognito. This makes the F2 ordering and HITL conflict guarantees structural, not just tool-level discipline — closes both the agent-bypass-via-direct-API and admin-presigned-URL-bypass attack vectors.
...
- Cross-tenant boundary: enforced at S3 IAM (Strands runtime role scoped to its own tenant prefix) as the primary line; dispatcher prefix-derived tenant check is defense-in-depth. Workspace-files API rejects PUT to eventful prefixes (and presigned URL generator denies minting URLs for them), closing the agent-bypass and admin-bypass attack vectors structurally.
...
- `delegate_to_workspace` dependency: not in main yet — extract a shared target-resolution helper here (new U10) and have plan 008 U9 adopt it when it lands. No hard sequencing dependency.
...
- Whether suppression uses S3 object metadata, object tags, or both. The dispatcher interface should support both; implementation can choose the cheaper available API after SDK prototyping.
...
- **[Identity bet, defer to v2 docs polish] Workspace-native orchestration as deliberate platform identity** — Adopting S3-as-orchestration-substrate with 8-event vocabulary, sweepers, depth/quota guards, and ETag conflict handling is a real positioning statement: ThinkWork bets agent orchestration is platform-owned + folder-native, not a thin layer over AgentCore + Scheduler + Lambda. Maintenance surface is ongoing. Future contributors should be able to challenge or reaffirm this bet — surface as a one-paragraph framing in U9 docs once both sync (`delegate_to_workspace`) and async (`wake_workspace`) primitives have shipped and operators have used them.
...
sequenceDiagram
  ...
  AC->>S3: read work/runs/{runId}/, write result/event intent/review (via /api/workspaces/* — eventful prefixes go through orchestration writer ONLY)
...
- **Schema work for U7's per-tenant rollout** (referenced from U7 file list): `tenants.workspace_orchestration_enabled boolean default false` column added. The U1 migration includes this column.
...
- Add **one EventBridge rule per eventful prefix family** (`work/inbox/`, `work/runs/*/events/`, `work/outbox/`, `memory/`, `review/`, `errors/`, `events/audit/`, `events/intents/`) using the `wildcard` operator on `object.key` (e.g., `"key": [{ "wildcard": "tenants/*/agents/*/workspace/work/inbox/*.md" }]`). Per-rule cardinality is easier to reason about, costs the same, and lets each rule wire to a different SQS or DLQ later. The `review/` rule subscribes to BOTH `Object Created` and `Object Removed` event types (review-file deletion handling).
...
- **Dispatcher Lambda IAM** scoped to: S3 `GetObject`+`HeadObject` on workspace bucket (read source + suppress-tag check), S3 `PutObject` on `events/audit/*` ONLY (not on `work/*`, `review/*`, or other operator-facing prefixes), SQS receive/delete on inbox queues, SQS send on DLQs, DB access through existing `DATABASE_SECRET_ARN`.
- **Strands runtime IAM** (out of this unit's file list but specified here for the U2/U3 contract): scoped to `tenants/${tenantSlug}/agents/${agentSlug}/*` write only; **explicit Deny on `s3:PutObjectTagging` and on `s3:PutObject` with `Condition` on `s3:RequestObjectTag/x-amz-meta-thinkwork-suppress-event`** so the runtime cannot forge the suppression tag. The runtime role spec lives in `terraform/modules/app/agentcore-runtime/` and is updated as part of this unit.
...
- Security: Strands runtime IAM role attempting `PutObjectTagging` with `x-amz-meta-thinkwork-suppress-event` on its own object → IAM Deny → CloudTrail records the failed call.

**Verification:**
- Terraform plan includes S3 EventBridge enablement, per-prefix EventBridge rules (review with both Created+Removed), per-rule SQS queues with `max_receive_count=1` redrive + DLQs with SSE-KMS + scoped resource policy + 14d retention, dispatcher Lambda with `function_response_types=["ReportBatchItemFailures"]` ESM, dispatcher IAM scoped to `events/audit/*` writes only, Strands runtime IAM with explicit Deny on suppression-tag write, importer principals scoped per-tenant, and no route/API Gateway exposure.
...
**Approach:**
- **Workspace-files API rejects PUT to eventful prefixes — structural enforcement of F2 ordering and HITL conflict guarantees.** The existing `packages/api/workspace-files.ts` POST handler is modified to reject PUT operations whose key matches `work/inbox/`, `review/`, `work/runs/*/events/`, `events/intents/`, or `events/audit/` with HTTP 403 `error: "use orchestration writer"`. The same handler's presigned-URL branch denies minting URLs for those prefixes (otherwise admin SPA could mint a presigned PUT and bypass everything). Memory writes (`memory/*.md`) and ordinary workspace edits remain allowed via this API; eventful prefixes go EXCLUSIVELY through `write-api.ts`.
- Add the `write-api.ts` orchestration writer — narrow surface that creates work requests under `work/inbox/`, lifecycle intents under `work/runs/{runId}/events/`, and review files under `review/`. The writer validates tenant/agent identity from service auth or Cognito, never from agent-supplied tenant slug, then performs the S3 PUT(s).
- Add a Strands tool `wake_workspace(target, request_md, reason?, idempotency_key?, wait_for_result: bool = False)` that calls `write-api.ts` and returns the source object key. **Path validation goes through `workspace_target.py` from U10** — same helper that the planned `delegate_to_workspace` (plan 008 U9) will use, so both tools enforce identical reserved-name and depth rules from day one.
...
- Update default workspace docs so agents know when to use synchronous `delegate_to_workspace` (planned in plan 008 U9, not yet shipped) versus asynchronous `wake_workspace` (this plan). Three coexisting primitives in v1 (existing generic `delegate(task, context)`, planned sync `delegate_to_workspace`, async `wake_workspace`) — see U9 docs for the discrimination.
...
- **Security: workspace-files API rejection** — PUT to `work/inbox/foo.md`, `review/run_xxx.needs-human.md`, `work/runs/run_xxx/events/started.json`, `events/intents/x.json`, `events/audit/x.json` all return HTTP 403 with `error: "use orchestration writer"`. PUT to `memory/lessons.md` and `IDENTITY.md` remain allowed (existing memory and workspace-edit paths preserved).
- **Security: presigned URL gating** — `POST /api/workspaces/files` with `action=presignPut, key="work/inbox/foo.md"` returns HTTP 403; same for the other eventful prefixes. Presigned PUT for `memory/lessons.md` still succeeds (memory write path preserved).

**Verification:**
- Strands unit tests show agents cannot write arbitrary workspace paths through the orchestration tools.
- workspace-files API integration tests confirm eventful prefixes are blocked at HTTP 403 for both direct PUT and presigned URL minting.
...
- **Stale review-file GC**: when a run terminates (completed/failed/cancelled), the dispatcher synchronously deletes any lingering `review/{runId}.needs-human.md` (with suppress-tag) so a future edit cannot re-wake a completed run.
...
- Modify: `packages/database-pg/src/schema/tenants.ts` (add `workspace_orchestration_enabled boolean default false` column for per-tenant rollout — kept per finding 5 user choice)
- Create: `packages/database-pg/drizzle/NNNN_workspace_orchestration_enabled.sql` (additive migration with `-- creates-column: public.tenants.workspace_orchestration_enabled` marker)
- Modify: `packages/api/src/handlers/workspace-event-dispatcher.ts` (gate canonical-event enqueue on `tenant.workspace_orchestration_enabled` — when false, dispatcher logs the event but does not enqueue wakeups; when true, full path runs)
...
- **Per-tenant rollout via `tenants.workspace_orchestration_enabled`** (kept per finding 5 user choice — provides canary-tenant safety for the orchestration primitive going live, despite added schema work). Default `false` in v1; flipped per-tenant after canary smoke. Dispatcher reads this flag and SKIPS wakeup-enqueue for tenants where it's false (canonical event still records for observability — operators can see what would have fired). The Terraform variable `enable_workspace_orchestration` at the module level controls whether the EventBridge rules + SQS infrastructure exist at all (defaults `false` in v1, flipped at stage level once Phase 2 ships).
...
- `awaitingReviewBacklog(operatorId)` query returns the count + list of `awaiting_review` runs across all agents the operator can access (for the sidebar badge and the `_tenant/review/` index).
- `saveReviewFile` mutation: accepts `runId`, `etag` (`If-Match`), `body`, optional `force: bool`. Steps: (1) `resolveCallerTenantId(ctx)`; (2) `SELECT agent_workspace_runs WHERE id=$runId AND tenant_id=$callerTenantId` — return `NOT_FOUND` on mismatch (non-leakage; this is the run-ownership cross-check); (3) verify run status is `awaiting_review` — error if not (operator's view is stale); (4) S3 PUT with `If-Match: $etag` unless `force=true`; (5) on `If-Match` mismatch, return `REVIEW_CONFLICT` GraphQL error with current ETag + body. Mutation never bypasses ETag without explicit `force=true`.
- `cancelReviewRequest` mutation (file in U6): tenant-checked; transitions `awaiting_review` → `cancelled`; idempotent on second call (returns first-canceller attribution).
...
- **Run-detail page at `/analytics/agent-runs/$runId`** — `WorkspaceRunTimeline` component shows event-by-event timeline with state-differentiated rendering (`RunStateBanner` per state):
  ...
  - `awaiting_subrun`: shows the awaited child agent slug + child runId + child's current status (queryable inline since it's a sibling row); "Waiting since N hours" + `Cancel wait` button
...
- U9. **Document and evaluate the new primitive (split into U9a + U9b to manage the plan-008 dependency)**

**Goal:** Make the primitive legible to agents, operators, and future implementers — without referencing tools that don't yet exist.

**Requirements:** R17 plus all success criteria.

**Dependencies:** U9a depends on U5, U8. U9b additionally depends on plan 008 U9 (`delegate_to_workspace`) shipping.

### U9a — `wake_workspace`-only docs and evals (ships with this plan)

**Files:**
- Modify: `packages/workspace-defaults/files/AGENTS.md` (mention `wake_workspace`; do NOT yet describe `delegate_to_workspace` since plan 008 may not have shipped)
- Modify: `packages/workspace-defaults/files/TOOLS.md`
- Modify: `packages/workspace-defaults/files/CAPABILITIES.md`
...
- Add eval cases for choosing `wake_workspace` for long-running/HITL work, refusing unrelated peer writes, preserving stateless resume semantics, and the multi-child fan-out pattern (worked example: parent uses `status.json` to track 3 children; each child completion produces one parent wake).
...
### U9b — Sync-vs-async comparison docs (ships AFTER plan 008 U9 lands)

**Files:**
- Modify: `packages/workspace-defaults/files/AGENTS.md` (full discrimination doc once both tools exist)
- Modify: `packages/agent-tools/eval/datasets/workspace-orchestration.yaml` (add eval cases comparing sync vs async)

**Approach:**
- Once plan 008 U9 has shipped `delegate_to_workspace`, expand AGENTS.md and TOOLS.md with the full discrimination — when to pick sync vs async, with worked examples. Add eval cases comparing the two for borderline scenarios (e.g., "specialist task that may take 30 seconds" — sync if under turn budget, async if not).
- Reaffirm the "files-and-events, not journal-replay" framing now that operators have used both primitives — surface the workspace-native-orchestration identity bet from Open Questions if the team has settled on it.

**Patterns to follow:**
- Existing `packages/agent-tools/eval/datasets/workspace-routing.yaml` and `workspace-memory.yaml`.
- The future `/docs/agent-design/` section from the fat-folder requirements if it lands first.

**Verification:**
- U9a docs and evals encode the same mental model as the implementation and origin requirements.
- U9b ships only after plan 008 U9 confirmed merged; at that point, no doc references a non-existent tool.

---

- U10. **Shared workspace target-resolution helper (TS + Python parity)**

**Goal:** Extract one canonical "is this a valid workspace target path" helper used by both `wake_workspace` (this plan) and the planned `delegate_to_workspace` (plan 008 U9), so both tools enforce identical path safety, reserved-folder rejection, and depth-cap rules from day one.
...
- Validation rules (must match across TS and Python byte-for-byte semantically):
  ...
  6. Reject if normalized depth exceeds platform cap (default 4 — matches R15 wake-chain depth limit; this is the per-target depth, not the chain depth).
...
- Error path: `parse_target("a/b/c/d/e", routes)` with chain depth 5 rejected with `reason: "depth_exceeded"`.
...
**Verification:**
- Both `wake_workspace` (U5) and the future `delegate_to_workspace` (plan 008 U9) call into the same helper without re-implementing path validation; CI parity test passes for ≥20 cases.
...
- **Cross-tenant blast surface:** Three layers of defense — (1) S3 IAM scopes Strands runtime role to its own tenant prefix (primary), (2) workspace-files API rejects PUT to eventful prefixes for both direct PUT and presigned URL minting (closes the agent-bypass-via-direct-API and admin-presigned-URL-bypass attack vectors structurally), (3) dispatcher's prefix-derived tenant check + cross-agent runId ownership check on event intents (defense-in-depth at canonicalization). Admin SPA writes to inbox/review go through `saveReviewFile` and `cancelReviewRequest` GraphQL mutations with explicit tenant cross-check before any S3 write.
...
- **Unchanged invariants:** Existing chat, email, webhook, sync `delegate` (and future `delegate_to_workspace` from plan 008 U9), budget, skill, KB, and cost paths still go through current wakeup processor behavior unless source is `workspace_event`. The `wakeup-processor` payload-construction branch on `source === "workspace_event"` is additive — existing source branches (`chat_message`, `trigger`, `thread_assignment`) are unchanged. Synchronous `delegate` returns its result in-context and produces no canonical workspace event; that audit-timeline gap is the only known cost of the primitive being additive rather than replacing.
...
| Bulk imports accidentally trigger memory/run work | HeadObject metadata check at the dispatcher (since neither `x-amz-meta-*` nor `detail.requester` are filterable at EventBridge). Bulk writers tag objects with `x-amz-meta-thinkwork-suppress-event: true`; runtime IAM-denied from setting that tag (U2). |
...
| Admin UI exposes cross-tenant run data | Resolver tests use tenant mismatch fixtures; mutations gate before S3 writes; `resolveCallerTenantId` everywhere; `saveReviewFile` and `cancelReviewRequest` perform explicit run-ownership cross-check before any S3 write or DB transition. |
...
| **Cross-tenant attack via runtime IAM** (compromised agent writes to victim tenant's prefix) | Runtime IAM scoped to `tenants/${tenantSlug}/agents/${agentSlug}/*` only — write itself fails at S3 IAM. Workspace-files API rejects PUT to eventful prefixes (closes agent + admin bypass paths). Dispatcher prefix-derived check + cross-agent runId ownership check are defense-in-depth. `API_AUTH_SECRET` cannot bypass tenant scope because the dispatcher derives tenant from the S3 prefix, not the API key. |
...
| **Coexistence with `delegate_to_workspace`** (audit timeline gap — sync delegate produces no canonical events) | Documented as accepted gap in U9a; v1 audit viewer reads transcripts to surface sync delegations. Future v2 can add synthetic `delegate.invoked`/`delegate.completed` events if the gap is observably painful. |
| **U9 docs reference vapor `delegate_to_workspace`** (plan 008 U9 may not have shipped) | U9 split into U9a (`wake_workspace`-only docs, ships now without referencing the unbuilt sibling) + U9b (full sync-vs-async comparison, blocked on plan 008 U9 landing). |
...
- **U10** shared workspace target-resolution helper (TS + Python parity) — lands first; required by U3 and U5.
- **U1** schema (`agent_workspace_runs`, `agent_workspace_events`, `agent_workspace_waits`, plus `wakeup_retry_count` column) — can land in parallel with U10.
...
- **U5** Strands runtime tools (`wake_workspace`) using U10 helper; workspace-files API rejects PUT to eventful prefixes; presigned URL generator denies same; F2-race ordering enforced atomically in `write-api.ts`; env snapshot at coroutine entry.
...
- **U7** scheduler conversion (`job-trigger.ts` writes inbox files) + memory pipeline same-PR cutover (`wiki-compile.ts` becomes `memory.changed` event subscriber, `memory-retain.ts` stops calling `maybeEnqueuePostTurnCompile` directly, idempotency via `wiki_compile_jobs` dedup) + tenant-flag schema work.
- **U8** admin GraphQL + UI: run/audit viewer at `analytics/agent-runs/` (tenant-wide with agent filter chip), HITL backlog at `review/` with sidebar nav badge, `saveReviewFile` mutation with ETag-conditional + run-ownership cross-check, `cancelReviewRequest` mutation, state-differentiated rendering, causeEventId chain inline rendering.
- **U9a** `wake_workspace`-only docs + evals; "files-and-events, not journal-replay" framing; three-primitive landscape.
- **U9b** sync-vs-async comparison docs (BLOCKED on plan 008 U9 landing — ships separately once `delegate_to_workspace` is in main).
