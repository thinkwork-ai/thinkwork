---
title: "feat: ThinkWork Brain remaining substrate"
type: feat
status: active
date: 2026-06-14
origin: Linear THNK-6 / ThinkWork Brain physical substrate plan
linear: THNK-6
---

# feat: ThinkWork Brain remaining substrate

## Overview

THNK-6 already shipped the first dogfood proof milestone through child issues
THNK-17, THNK-18, THNK-19, and THNK-20. Those units provide the ThinkWork Brain
plugin shell prerequisite, substrate status contract, tenant-scoped the retired graph substrate
provisioning contract, canonical S3 artifacts/manifests, and first-party
Context Engine `query_brain_context` reads.

This plan covers the remaining parent-scope ThinkWork Brain substrate work:
default-to-production migration orchestration, migration-aware Brain reads,
Brain operations UI, and docs/smoke closure. ThinkWork Brain remains the
customer-facing product. the retired graph substrate is internal substrate machinery and may appear
...
ThinkWork Brain should become a premium, tenant-scoped substrate rather than a
...
- R4. Migration-aware `query_brain_context` reads report active, shadow,
  fallback, and vault provenance without exposing the retired graph substrate, Neptune, S3 keys, or
...
- R7. Documentation and smoke coverage verify ThinkWork Brain through GraphQL,
...
- Do not expose the retired graph substrate as a customer-visible product, plugin, license,
...
- Do not make OpenSearch a direct ThinkWork Brain production vector store.
- Do not treat EFS or markdown vault projections as canonical ThinkWork Brain
  storage.
...
- `packages/api/src/graphql/resolvers/brain/brainStatus.query.ts`
  projects redacted tenant status and operator evidence for substrate and
  migration state.
- `packages/api/src/lib/context-engine/providers/brain.ts` gates
...
- `packages/deployment-runner/src/apps/retired_graph_substrate.ts` and
  `terraform/modules/app/retired_graph_substrate` normalize tenant-scoped Brain desired config,
...
- `apps/web/src/components/settings/plugins/PluginDetail.tsx` already links
  ThinkWork Brain plugin detail to the Memory / Ontology area; Brain operations
...
- `docs/plans/autopilot/THNK-17-status.md`: explicit substrate state is
  authoritative over legacy environment-derived the retired graph substrate status and tenant
...
- `docs/solutions/best-practices/retired_graph_substrate-thread-ingest-explorer-2026-06-04.md`:
  validate the retired graph substrate/Brain behavior through ThinkWork GraphQL and Context Engine
  paths, not direct private backend access.

### External References

- the retired graph substrate docs confirm the storage split: relational metadata/state,
...
- AWS Neptune Database full-text search with OpenSearch Serverless is a
  ...
  OpenSearch vector storage to ThinkWork Brain production.
...
- Treat S3 manifest replay enumeration and validation as pure, testable
  helpers. Live replay into the retired graph substrate/Neptune remains deploy-time/runtime work;
...
- Modify: `packages/api/src/graphql/resolvers/brain/brainStatus.query.ts`
- Modify: `packages/api/src/graphql/resolvers/brain/index.ts`
- Create: `packages/api/src/graphql/resolvers/brain/brainMigration.mutation.ts`
- Create: `packages/api/src/graphql/resolvers/brain/brainMigration.test.ts`
- Create or modify: `packages/api/src/lib/brain/migration.ts`
- Test: `packages/database-pg/__tests__/migration-0166-brain-substrate.test.ts`
...
- Admin/service authorization from `brainStatus.query.ts` and existing
  settings/managed-application mutations.
- Manual migration marker discipline from
  `packages/database-pg/__tests__/migration-0166-brain-substrate.test.ts`
...
- Modify: `packages/api/src/lib/context-engine/providers/brain.test.ts`
- Modify: `packages/api/src/handlers/mcp-context-engine.requester-context.test.ts`
- Modify: `plugins/brain/smoke/brain-context-engine-smoke.mjs`
- Modify: `docs/plans/autopilot/THNK-6-status.md`
...
- Provider-local status handling from current ThinkWork Brain provider and
...
- Render a dense operations page with a top status/action band and sections for
  ...
  evidence.
- Use the existing `brainStatus` query and U4 mutations; hide
...
- Error path: failed/degraded substrate promotes blocking action and failure
  text.
- Error path: non-operator view does not render the retired graph substrate endpoints, S3 roots,
  Neptune ids/endpoints, or EFS ids.
- Integration: plugin detail links ThinkWork Brain users to Brain operations
...
**Goal:** Update docs and smoke coverage so ThinkWork Brain install, migration,
...
- Modify: `plugins/brain/smoke/brain-context-engine-smoke.mjs`
- Create or modify: `plugins/brain/smoke/brain-operations-smoke.mjs`
- Modify: `docs/plans/autopilot/THNK-6-status.md`
...
- Integration: docs link the Brain operations and Context Engine access paths
  without presenting the retired graph substrate as customer-facing product.

**Verification:**

- A maintainer can follow docs/smoke to validate ThinkWork Brain value through
...
| Tenant callers see backend evidence                                 | Centralize projection/redaction in GraphQL and test absence of S3/Neptune/the retired graph substrate details                |
...
| Docs imply OpenSearch or the retired graph substrate is the product | Keep customer-facing language on ThinkWork Brain / Context Engine; reserve the retired graph substrate for internal evidence |
...
- Linear blocker: THNK-15, "ThinkWork Brain Plugin Shell"
- Completed child issues: THNK-17, THNK-18, THNK-19, THNK-20
- Linear docs: "Implementation plan: ThinkWork Brain physical substrate",
  "ThinkWork Brain physical substrate requirements", "OKF considered and
  deferred for ThinkWork Brain"
- Status ledgers:
