...
| U8 Documentation and full regression matrix           | `codex/workspace-contract-u8` | [#1959] | Merged | `pnpm install --frozen-lockfile`; `pnpm exec tsx --test scripts/release/__tests__/workspace-contract-docs.test.ts`; stale-doc `rg` scan over contract docs; `pnpm dlx prettier@3.8.2 --write` touched docs/test files; `pnpm --filter @thinkwork/docs build`; `pnpm test:release`; focused U1-U7 regression matrix across API, desktop, mobile, AgentCore Pi, and Spaces; `pnpm -r --if-present typecheck`; `pnpm -r --if-present lint`; `git diff --check`; touched-file `pnpm dlx prettier@3.8.2 --check`; `pnpm --filter @thinkwork/desktop test`; `pnpm --filter @thinkwork/api test`; `pnpm --filter @thinkwork/spaces test`; GitHub checks passed (`cla`, `lint`, `test`, `typecheck`, `verify`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Squash merged into `main` as `d87ff303272d12c8c0904547a0d83aad63124dc6`; remote and local branch cleaned up. Updates runbooks/docs to describe the v1 runtime tree, adds a stale-doc regression verifier, and records the full regression matrix. Root `pnpm test` was interrupted by a local Electron lazy-install race in `apps/desktop` (`EEXIST` while creating the Electron Framework symlink); after reinstalling Electron, the full desktop package test passed, and full API/Spaces package tests were rerun successfully because the original root run had aborted before they completed. |
...
- 2026-06-01: Opened PR #1959 for U8.
- 2026-06-01: PR #1959 passed all required checks, squash merged into `main`
  as `d87ff303272d12c8c0904547a0d83aad63124dc6`, removed the remote/local
  branch and U8 worktree, and fast-forwarded the main checkout.
- 2026-06-01: All eight Workspace Contract v1 hardening implementation units
  are merged.
...
[#1959]: https://github.com/thinkwork-ai/thinkwork/pull/1959
