...
| U7 Hard S3 migration and legacy compatibility removal | `codex/workspace-contract-u7` | Pending | Local verified | `pnpm install --frozen-lockfile`; `pnpm --filter @thinkwork/api test -- migrate-workspace-layout workspace-files-handler`; `pnpm --filter @thinkwork/desktop test -- test/sidecar/workspace-cache.test.ts test/sidecar/local-turn-runner.test.ts`; `pnpm --filter @thinkwork/mobile test -- lib/agent/workspace-cache.test.ts lib/agent/extensions/__tests__/local-bash-extension.test.ts`; `pnpm --filter @thinkwork/agentcore-pi test -- tests/bootstrap-workspace.test.ts tests/workspace-diff.test.ts`; `pnpm --filter @thinkwork/spaces test -- consolidated-workspace-client`; `pnpm --filter @thinkwork/api typecheck`; `pnpm --filter @thinkwork/desktop typecheck`; `pnpm --filter @thinkwork/agentcore-pi typecheck`; `pnpm --filter @thinkwork/spaces typecheck`; touched-file Prettier write; `git diff --check`                                                                                                                                                                                                                                                                              | Removes legacy `source/`, `workspace/`, `workspace-archives/`, rendered-user, root `USER.md`, and singular `Space/` compatibility from API, Spaces, desktop, mobile, and AgentCore runtime surfaces while retaining the dry-run/apply migration path for legacy S3 copies/deletes.                                                                                                           |
