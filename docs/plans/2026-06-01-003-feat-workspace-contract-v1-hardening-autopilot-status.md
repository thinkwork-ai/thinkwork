...
| U6 Workspace sync telemetry                           | `codex/workspace-contract-u6` | Pending | Local verified | `pnpm install --frozen-lockfile`; `pnpm --filter @thinkwork/desktop test -- test/sidecar/local-turn-runner.test.ts`; `pnpm --filter @thinkwork/api test -- process-finalize`; `pnpm --filter @thinkwork/spaces test -- TaskThreadView`; `pnpm --filter @thinkwork/desktop typecheck`; `pnpm --filter @thinkwork/api typecheck`; `pnpm --filter @thinkwork/spaces typecheck`; `pnpm --filter @thinkwork/api test -- process-finalize chat-agent-finalize`; touched-file Prettier write; `git diff --check`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Adds normalized `workspace_diagnostics` to desktop local finalize payloads, persists reconcile write-back duration/counts in API turn usage diagnostics, and renders a Workspace sync thinking row in Spaces with backward-compatible legacy timing fallback.                                                                                                                                |
