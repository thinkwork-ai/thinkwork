...
| U8 Documentation and full regression matrix           | `codex/workspace-contract-u8` | [#1959] | PR open | `pnpm install --frozen-lockfile`; `pnpm exec tsx --test scripts/release/__tests__/workspace-contract-docs.test.ts`; stale-doc `rg` scan over contract docs; `pnpm dlx prettier@3.8.2 --write` touched docs/test files; `pnpm --filter @thinkwork/docs build`; `pnpm test:release`; focused U1-U7 regression matrix across API, desktop, mobile, AgentCore Pi, and Spaces; `pnpm -r --if-present typecheck`; `pnpm -r --if-present lint`; `git diff --check`; touched-file `pnpm dlx prettier@3.8.2 --check`; `pnpm --filter @thinkwork/desktop test`; `pnpm --filter @thinkwork/api test`; `pnpm --filter @thinkwork/spaces test`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Updates runbooks/docs to describe the v1 runtime tree, adds a stale-doc regression verifier, and records the full regression matrix. Root `pnpm test` was interrupted by a local Electron lazy-install race in `apps/desktop` (`EEXIST` while creating the Electron Framework symlink); after reinstalling Electron, the full desktop package test passed, and full API/Spaces package tests were rerun successfully because the original root run had aborted before they completed.                                                                                                        |
