...
- **Stop conditions:** surface a blocker instead of guessing if the Open Question on built-in-tool policy and profile space assignments cannot be resolved without semantic loss before U7 unroutes the chip multiselects.
...
- R6. The Profiles sheet is a list → detail editor. Detail has a Basic section — Profile (name, model, enabled, clarify-before-work) and Instructions (description, routing guidance, instructions) — and an Advanced accordion — Loop/Review (closed loop, mode, max iterations, review gate, external reviewer, max review loops, failure behavior) and Execution (max runtime, max tokens, thinking).
...
- R8. The profile editor carries no capability fields; the Spaces/Tools/MCP/Skills chip multiselects are removed. Profile capability shaping happens on the tree with the profile selected: profile state renders as an overlay, and attach/detach fires the unified mutations at AgentProfile scope.
...
- KTD-2. **Profile capability truth stays the policy JSON; the unified mutations are its only writers.** `grantCapability`/`detachCapability` at AgentProfile scope are already implemented and already write `agent_profiles.skill_policy.skillSlugs` / `tool_policy.mcpServers` — that JSON is the unified substrate at profile scope, not a legacy store. No data migration, no read-projection, no parity apparatus. The change is write-path exclusivity: the chip multiselects' `updateAgentProfile` policy writes retire (U11) once U7 unroutes the chips, and the deferred drop covers only the GraphQL input fields — the columns stay.
...
- **Deferred to implementation (resolve in U5, before U7 unroutes the chips):** the chip multiselects also write built-in-tool policy (`toolPolicy.builtInTools`) and profile space assignments (`spaceAssignments`) — neither the tree overlay (skills + MCP) nor the Profiles sheet (no capability fields) currently claims them. U5's write-surface audit must pin their post-chip home: unified-mutation coverage, a Profiles-sheet field, or an explicit deferral confirmed with Eric. Non-blocking for plan execution start; blocking for U7.
...
- **Goal:** `updateAgentProfile` stops writing `toolPolicy`/`skillPolicy`; the unified mutations become the policy JSON's only writers.
...
- The Open Question (built-in-tool policy and profile space assignments' post-chip home) resolved and reflected in the shipped surface before U7.
