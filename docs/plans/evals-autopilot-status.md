...
- **Admin active-run polling fallback**
...
| 2026-05-16 | Admin active-run polling fallback | `codex/evals-admin-live-poll` | pending | Local verification complete | `pnpm --filter @thinkwork/admin build`; `pnpm --filter @thinkwork/admin test`; `git diff --check` | Adds a 3s network refetch fallback for the evaluations list while pending/running rows exist, so progress and pass rate update even if subscription events are missed. |
