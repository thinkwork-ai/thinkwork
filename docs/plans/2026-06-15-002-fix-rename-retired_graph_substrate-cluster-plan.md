---
title: "fix: Rename legacy the retired graph substrate ECS cluster to Brain"
type: fix
status: active
date: 2026-06-15
origin: docs/brainstorms/2026-06-15-rename-retired_graph_substrate-cluster-requirements.md
---

# fix: Rename legacy the retired graph substrate ECS cluster to Brain

## Overview

Rename the legacy stage-wide ThinkWork Brain ECS cluster from
`thinkwork-${stage}-retired_graph_substrate-cluster` to `thinkwork-${stage}-brain-cluster`
while keeping the retired graph substrate as the internal implementation app key, service name,
task family, container name, container image, database, log group, and
managed-app contract.

The implementation should isolate cluster identity from the broader the retired graph substrate
...
The dev ThinkWork Brain substrate is healthy but operator-facing AWS ECS still
shows the legacy cluster name `thinkwork-dev-retired_graph_substrate-cluster`. This leaks
...
the existing the retired graph substrate service dependencies are preserved or intentionally
reattached (see origin: `docs/brainstorms/2026-06-15-rename-retired_graph_substrate-cluster-requirements.md`).

---

## Requirements Trace

- R1. The live dev ThinkWork Brain ECS cluster ends as `thinkwork-dev-brain-cluster`.
- R2. Future legacy stage-wide the retired graph substrate substrate deployments derive ECS cluster
...
- R3. The managed application key, service name, task family, container name,
  ...
  remain the retired graph substrate unless a concrete dependency requires otherwise.
- R4. The plan accounts for ECS cluster name immutability and does not assume an
  AWS CLI in-place rename.
- R5. The dev cutover preserves or intentionally reattaches the existing the retired graph substrate
...
- R8. Verification proves dev is healthy under the Brain name and future deploys
  will not recreate or revert to the the retired graph substrate cluster name.
...
service health; AE2 covers fresh future deploy naming and `retired_graph_substrate_cluster_arn`
...
- Do not rename the the retired graph substrate managed application key, database, container name,
  container image, API routes, GraphQL fields, Knowledge Graph product surface,
  or user-facing feature language.
- Do not migrate ThinkWork Brain data, alter the retired graph substrate ingestion behavior, or change
  Hindsight/memory semantics.
- Do not generalize all the retired graph substrate resource names. THNK-30 changes only the legacy
...
- `terraform/modules/app/retired_graph_substrate/main.tf` currently uses
  `local.legacy_name = "thinkwork-${var.stage}-retired_graph_substrate"` and
...
- `terraform/modules/app/retired_graph_substrate/outputs.tf` and
  `terraform/modules/thinkwork/outputs.tf` expose `retired_graph_substrate_cluster_arn`.
...
- `terraform/modules/thinkwork/main.tf` forwards `retired_graph_substrate_cluster_arn` into the
  Lambda API module, but `terraform/modules/app/lambda-api/handlers.tf` does
  not set `GRAPH_CLUSTER_ARN` because the GraphQL Lambda env block is close to
...
- `packages/api/src/graphql/resolvers/core/deploymentStatus.query.ts`,
  ...
  each reconstruct the old `thinkwork-${stage}-retired_graph_substrate-cluster` fallback today.
...
- `docs/solutions/architecture-patterns/brain-provisioning-contract-tenant-scoped-2026-06-15.md`:
  ThinkWork Brain/the retired graph substrate infrastructure should expose operator evidence through
...
- `docs/solutions/runtime-errors/ecs-exec-existing-tasks-need-force-new-deployment-2026-05-13.md`:
  ...
  launched tasks.
- `docs/plans/retired_graph_substrate-terraform-infrastructure-autopilot-status.md`:
  accepted the retired graph substrate dev evidence includes ECS service desired/running `1/1`,
...
- Isolate cluster identity in a dedicated Terraform local rather than renaming
  `local.name`: this changes only the ECS cluster while preserving the retired graph substrate
...
- Keep `retired_graph_substrate_cluster_arn` as the durable Terraform source of truth and
  `GRAPH_CLUSTER_ARN` only as an optional compatibility override when already
...
- Treat implementation merge as the dev cutover trigger: because `main` CD
  ...
  operator is ready to accept a short the retired graph substrate service replacement window.
...
- Gate non-dev rollout explicitly: this plan authorizes dev proof only. Any
  production/customer stage with legacy the retired graph substrate enabled must stay pinned or
...
- Safest dev cutover sequence: use Terraform-owned cluster/service replacement
  ...
  dependencies.
- Status/health source of truth: keep Terraform `retired_graph_substrate_cluster_arn` as the
  durable applied identity, treat `GRAPH_CLUSTER_ARN` as an optional legacy
...
```mermaid
flowchart TD
  A["Terraform the retired graph substrate module"] --> B["Dedicated cluster-name local"]
  B --> C["aws_ecs_cluster.main = thinkwork-stage-brain-cluster"]
  A --> D["local.name remains thinkwork-stage-retired_graph_substrate"]
  D --> E["the retired graph substrate service, task family, ALB/TG, EFS, IAM, logs unchanged"]
  C --> F["retired_graph_substrate_cluster_arn output points to Brain cluster"]
...
preserving existing the retired graph substrate implementation resource names.
...
- Modify: `terraform/modules/app/retired_graph_substrate/README.md`
...
- Keep `local.name`, `local.resource_short_name`, service name, task family,
  ...
  the retired graph substrate-derived values.
- Keep `retired_graph_substrate_cluster_arn` output names for compatibility, but update
  descriptions/docs to clarify the ARN is the ThinkWork Brain ECS cluster for the
  the retired graph substrate service.
- Do not change tenant-scoped Brain instance naming.

**Patterns to follow:**

- Existing tenant-scoped naming in `terraform/modules/app/retired_graph_substrate/main.tf`.
...
- Happy path: service/task/log/ALB/EFS/IAM naming assertions continue to match
  the retired graph substrate-derived names, proving THNK-30 did not become a global rebrand.
- Happy path: container name and container image references remain the retired graph substrate-based
...
- Integration: composite `retired_graph_substrate_cluster_arn` output remains present and points
  through the the retired graph substrate module output.

**Verification:**

- Static Terraform fixture coverage proves only cluster identity changes.
- Terraform module validation succeeds for both the the retired graph substrate app module and the
  composite Thinkwork module.

---

- U2. **Centralize the retired graph substrate/Brain cluster identity in API status**
...
- Create or modify: `packages/api/src/graphql/resolvers/core/retired_graph_substrateClusterIdentity.ts`
...
- Implement one small helper that resolves the cluster ARN and cluster name from
  `GRAPH_CLUSTER_ARN` when present, otherwise derives the Brain fallback from
  stage, region, and account ID.
- Use the helper in all three current fallback sites instead of duplicating
  string literals.
- Preserve the compact `GRAPH = backend|endpoint` env contract and avoid adding
  a new GraphQL Lambda env var.

**Patterns to follow:**

- Existing `readthe retired graph substrateStatus()` env compatibility behavior in
...
- Happy path: with `GRAPH` enabled, `STAGE=dev`, region/account present, and no
  `GRAPH_CLUSTER_ARN`, `deploymentStatus.retired_graph_substrateClusterArn` contains
  `cluster/thinkwork-dev-brain-cluster`.
- Happy path: managed application status for key `retired_graph_substrate` reports the same
...
- Edge case: when `GRAPH_CLUSTER_ARN` is set, all status/health paths use it
...
- Extend the the retired graph substrate managed-app smoke to include the Terraform
  `retired_graph_substrate_cluster_arn` output, GraphQL `deploymentStatus.retired_graph_substrateClusterArn`,
  the the retired graph substrate managed application `clusterArn`, and
...
- Add a non-secret assertion that enabled legacy stage-wide deployments should
  report a Brain cluster name. If an explicit `GRAPH_CLUSTER_ARN` override is
  present and points at the old the retired graph substrate cluster, smoke/readiness must fail unless
  the evidence names a break-glass recovery reason and owner.
- Update operator docs to state that the cluster is Brain-named while the retired graph substrate
...
- Use an explicit smoke assertion matrix:
  - Disabled the retired graph substrate: skip cluster identity assertions and report skipped.
  - Enabled the retired graph substrate: Terraform `retired_graph_substrate_cluster_arn`,
    `deploymentStatus.retired_graph_substrateClusterArn`, the retired graph substrate managed app `clusterArn`, ECS
  ...
  - Explicit `GRAPH_CLUSTER_ARN`: accept only when it is Brain-named or when a
...
- Existing smoke evidence shape in `plugins/brain/smoke/retired_graph_substrate-managed-app-smoke.mjs`.
- Existing smoke expectations in `terraform/modules/app/retired_graph_substrate/README.md`.
...
- Edge case: disabled the retired graph substrate deployments remain skippable and do not fail on a
  missing cluster ARN.
...
- Create: `docs/verification/rename-retired_graph_substrate-cluster-dev-cutover.md`
...
- Inventory enabled non-dev stages before rollout. If any production/customer
  stage has the legacy stage-wide the retired graph substrate substrate enabled, do not deploy this
...
- After main CD settles, capture read-only AWS and GraphQL evidence showing the
  Brain cluster is active, the the retired graph substrate service is steady, the ALB target is
  healthy, Terraform output matches API status, and
  `thinkwork-dev-retired_graph_substrate-cluster` is absent or explicitly marked for
...
- Evidence style from `docs/plans/retired_graph_substrate-terraform-infrastructure-autopilot-status.md`.
...
- **Interaction graph:** Terraform creates the ECS cluster and service binding;
  Lambda API status reads compact the retired graph substrate env plus fallback identity; web/admin
...
- **State lifecycle risks:** ECS cluster identity is immutable, so replacement
  may briefly interrupt the dev the retired graph substrate service. Persistent state should remain
  in EFS, database, secrets, IAM, and ALB/TG resources that are not renamed.
- **API surface parity:** GraphQL field names remain `retired_graph_substrateClusterArn` and
  managed application key remains `retired_graph_substrate`; only returned values/fallbacks
  change.
...
- **Unchanged invariants:** the retired graph substrate remains private/internal-only, disabled by
  default for new deployments unless explicitly enabled, and still uses the
  compact GraphQL Lambda `GRAPH` env projection.
...
| Risk                                                                                                       | Mitigation                                                                                                                                                                                                 |
| ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Terraform replaces more than the ECS cluster/service binding because `local.name` was renamed too broadly. | U1 isolates cluster naming and adds fixture tests proving service, ALB/TG, EFS, IAM, task family, and logs stay the retired graph substrate-named.                                                         |
...
| GraphQL Lambda env grows past the 4 KB limit.                                                              | Do not add `BRAIN_CLUSTER_ARN`; keep compact `GRAPH` and use Brain fallback derivation.                                                                                                                    |
...
| A production/customer stage already has legacy the retired graph substrate enabled.                        | Inventory enabled stages before rollout and do not deploy the implementation to non-dev stages without an explicit maintenance window.                                                                     |
| The old empty cluster remains visible and keeps confusing operators.                                       | Verification must prove `thinkwork-dev-retired_graph_substrate-cluster` is absent/decommissioned, or record a time-boxed owner/date if AWS deletion is temporarily blocked.                                |
| An explicit old `GRAPH_CLUSTER_ARN` override masks the fallback fix.                                       | Smoke/readiness must fail for legacy stage-wide deployments when the override points at `thinkwork-${stage}-retired_graph_substrate-cluster`, unless break-glass recovery evidence is present.             |
...
- Non-dev rollout gate: customer/prod stacks must remain on their current
  ...
  non-dev stack has no legacy the retired graph substrate substrate enabled, record that inventory
  ...
  "no non-dev legacy the retired graph substrate enabled" or the blocked stage list with owner/date.
- The implementation PR should call out that merging to `main` will trigger the
  shared dev deploy and may briefly interrupt the the retired graph substrate service while ECS
...
- Safe proof path for dev:
  1. Capture baseline read-only evidence for the old dev cluster: cluster
     exists, the retired graph substrate service desired/running counts, target health, EFS id,
  ...
  5. Capture final evidence in `docs/verification/rename-retired_graph_substrate-cluster-dev-cutover.md`.
- Terraform plan allowlist for the dev cutover:
  - Expected: `aws_ecs_cluster.main` replacement to
    `thinkwork-dev-brain-cluster`.
  - Acceptable if unavoidable: `aws_ecs_service.retired_graph_substrate` replacement/relocation
...
- Verification must explicitly confirm:
  ...
  - the retired graph substrate ECS service `thinkwork-dev-retired_graph_substrate` is active under the Brain cluster.
  ...
  - `deploymentStatus.retired_graph_substrateClusterArn` and the the retired graph substrate managed application
  ...
  - `GRAPH_CLUSTER_ARN` is absent from the GraphQL Lambda env or points to the
    Brain cluster; an old-cluster override is not readiness-acceptable outside a
    named break-glass recovery note.
  - `thinkwork-dev-retired_graph_substrate-cluster` is absent from ECS, or explicitly documented
    as a temporary decommission item with owner/date because AWS deletion is
    blocked.
  - Read-only ECS checks were performed against both
    `thinkwork-dev-brain-cluster` and `thinkwork-dev-retired_graph_substrate-cluster`.
  - A fresh legacy stage-wide plan would create
    `thinkwork-${stage}-brain-cluster`, not the old the retired graph substrate cluster name.
...
- Origin document:
  `docs/brainstorms/2026-06-15-rename-retired_graph_substrate-cluster-requirements.md`
- Linear issue: `THNK-30`
- Linear requirements document:
  https://linear.app/thinkworkai/document/requirements-rename-retired_graph_substrate-cluster-2989a9f5db00
- Requirements PR: https://github.com/thinkwork-ai/thinkwork/pull/2515
- Related code: `terraform/modules/app/retired_graph_substrate/main.tf`
- Related code: `terraform/modules/app/retired_graph_substrate/outputs.tf`
...
- Related code:
  `packages/api/src/graphql/resolvers/core/knowledgeGraphHealthCheck.query.ts`
- Related tests: `apps/cli/__tests__/terraform-retired_graph_substrate-fixture.test.ts`
- Related tests:
  `packages/api/src/graphql/resolvers/core/general-reads-authz.test.ts`
- Related tests:
