---
title: "feat: ThinkWork Brain artifact manifests"
type: feat
status: active
date: 2026-06-14
origin: docs/plans/2026-06-13-003-feat-brain-physical-substrate-plan.md
linear: THNK-19
---

# feat: ThinkWork Brain artifact manifests

## Overview

THNK-19 implements the U3 slice from the ThinkWork Brain physical substrate plan: make S3 the durable replay and projection layer for ThinkWork Brain. The work adds canonical Brain artifact storage separate from short-lived wiki exports, persists replayable ingest manifests, records vault projections as projections rather than graph source of truth, and keeps credential and S3 identifier leakage out of tenant-visible surfaces.

---

## Problem Frame

Today the Knowledge Graph ingest pipeline can queue runs, send rendered source documents to the retired graph substrate, normalize graph snapshots into Aurora, and export wiki/vault markdown into a short-lived `wiki_exports` bucket. That proves graph observability, but it does not yet create a durable replay layer for ThinkWork Brain. Production migration and provenance need exact source artifacts, metadata, checksums, ontology and embedding evidence, and S3 object references that can be enumerated later without treating the markdown vault as canonical storage.
...
- Do not surface raw S3 keys, source ids, the retired graph substrate internals, or credentials in end-user payloads.
- Do not introduce a new customer-visible the retired graph substrate product surface.
...
- `packages/api/src/handlers/knowledge-graph-thread-ingest.ts` centralizes thread/wiki source bundles before the retired graph substrate ingest.
...
- `docs/solutions/best-practices/retired_graph_substrate-thread-ingest-explorer-2026-06-04.md`: deployed verification should go through ThinkWork GraphQL/smoke paths, not direct the retired graph substrate ALB access.
...
- Upstream THNK-6 plan already reviewed the retired graph substrate and AWS storage documentation. No new external dependency is required for this bounded implementation.
...
- Write source artifacts and ingestion manifests after source validation and before the retired graph substrate ingest so the source batch is replayable even if downstream indexing fails. Failure paths should record safe failure metadata without exposing raw keys to client errors.
...
- Migration filename/ordinal resolved as `0167_brain_artifact_manifest_runtime.sql`.
...
- Test: `apps/cli/__tests__/terraform-retired_graph_substrate-fixture.test.ts` or nearest Terraform fixture test if the module snapshot covers lambda-api resources
...
- Create: `packages/database-pg/drizzle/0167_brain_artifact_manifest_runtime.sql`
...
- Test: `packages/database-pg/__tests__/migration-0167-brain-artifact-manifests.test.ts`
...
- For source ingest, write the rendered source document and an ingestion manifest to the Brain artifacts bucket after source validation and before the retired graph substrate ingest.
...
- Docs: ThinkWork Brain, not the retired graph substrate, is the customer-facing product language.
...
- **Knowledge Graph ingest:** Adds source artifact writes before the retired graph substrate ingest while preserving current the retired graph substrate dataset behavior.
...
- Origin plan: `docs/plans/2026-06-13-003-feat-brain-physical-substrate-plan.md`
- Origin requirements: `docs/brainstorms/2026-06-13-brain-physical-substrate-requirements.md`
- THNK-19 Linear issue
- Related prerequisite plan: `docs/plans/2026-06-13-002-feat-brain-premium-plugin-plan.md`
