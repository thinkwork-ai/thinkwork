---
title: "feat: Context Engine ThinkWork Brain reads"
type: feat
status: completed
date: 2026-06-14
linear: THNK-20
...
# feat: Context Engine ThinkWork Brain reads

## Overview

THNK-20 delivers the U5a dogfood proof for ThinkWork Brain: first-party agents can call `query_brain_context` through Context Engine, read the active/default ThinkWork Brain substrate, receive bounded source data with cited provenance, and avoid falling back to less-governed memory paths when required Brain capabilities are unavailable.
...
The existing MCP handler already exposes `query_brain_context`, but the route currently selects the generic `brain` provider family rather than binding retrieval to the ThinkWork Brain substrate state and artifact/provenance contracts from THNK-17 and THNK-19. The dogfood proof needs Context Engine to be the policy boundary: first-party agents should not call the retired graph substrate, Neptune, or S3 directly, and retrieved content should be treated as untrusted source material rather than instructions.

---

## Requirements Trace

- R1. `query_brain_context` returns ThinkWork Brain context with cited provenance for first-party callers.
...
- R5. Disabled/degraded the retired graph substrate capabilities produce explicit Context Engine provider status rather than silent fallback.
- R6. the retired graph substrate recall/search options for the proof are represented as provider options or metadata: source/dataset scoping, NodeSet/source filters where available, top-k/depth, and only-context behavior.
- R7. A dogfood smoke fixture demonstrates better ThinkWork Brain retrieval than the memory-only path for one named workflow.

---

## Scope Boundaries

- No direct the retired graph substrate, Neptune, or S3 access from first-party agent runtime callers; all reads stay behind Context Engine.
...
- No raw the retired graph substrate write/delete tools or Brain write-back behavior.
- No customer-facing the retired graph substrate branding; the retired graph substrate may appear only as internal provider/status evidence.
...
- `packages/api/src/graphql/resolvers/brain/brainStatus.query.ts` projects THNK-17 substrate status and redacts operator evidence from tenant callers.
...
- `docs/solutions/best-practices/retired_graph_substrate-thread-ingest-explorer-2026-06-04.md`: the retired graph substrate validation should use ThinkWork GraphQL/Context Engine paths rather than direct private backend access.
...
- THNK-6 parent plan and comments, including the product evidence gate and ThinkWork Brain branding rule.
- THNK-17 merged substrate contract and status doc.
- THNK-19 merged artifact manifest contract and status doc.
- THNK-15 completed plugin-shell plan/brainstorm, including the the retired graph substrate-as-internal-component rule.

---

## Key Technical Decisions

- Add a concrete ThinkWork Brain Context Engine provider rather than changing MCP callers to use raw GraphQL, the retired graph substrate, S3, or Neptune. Context Engine remains the policy facade.
...
- Represent disabled the retired graph substrate capabilities as provider-local `skipped`/`stale`/`error` statuses with metadata, not as silent fallback to memory/wiki providers.
...
- U1. **Add ThinkWork Brain provider substrate gate**
...
- Test: `packages/api/src/lib/context-engine/providers/brain.test.ts`

**Approach:**

- Follow provider descriptor patterns from `wiki.ts`.
- Inject database/search dependencies for tests.
- Treat `active_backend` values `default`, `production`, and `legacy_retired_graph_substrate` as provider metadata but only use direct local Brain-page retrieval for U5a.
...
- Modify: `packages/api/src/lib/context-engine/providers/brain.ts`
- Test: `packages/api/src/lib/context-engine/providers/brain.test.ts`
...
- Mark source content with `instructionBoundary: "untrusted_source_data"` and `retrievalSurface: "brain_active_backend"`.
...
**Goal:** Add a lightweight smoke fixture/documentation that demonstrates one named workflow where ThinkWork Brain context beats memory-only retrieval.
...
- Create: `plugins/brain/smoke/brain-context-engine-smoke.mjs`
...
- Docs mention ThinkWork Brain/Context Engine boundary and avoid customer-facing the retired graph substrate language.
...
- **Interaction graph:** first-party MCP tool call -> Context Engine service -> ThinkWork Brain provider -> Brain substrate/page/artifact tables -> MCP structured/text response.
...
- **Unchanged invariants:** no external Brain MCP profile activation, no raw the retired graph substrate/Neptune/S3 access, and no migration cutover routing.
...
- Update Context Engine docs to clarify ThinkWork Brain is the product/source and Context Engine is the runtime policy layer.
...
- Parent issue/docs: THNK-6, "Implementation plan: ThinkWork Brain physical substrate", "ThinkWork Brain physical substrate requirements"
