---
title: "feat: ThinkWork Brain first-party context tool"
type: feat
status: active
date: 2026-06-14
...
# feat: ThinkWork Brain first-party context tool

## Overview

Expose ThinkWork Brain to first-party Pi agents as a dedicated `query_brain_context` tool registered by the existing Context Engine extension. The tool remains a Context Engine forwarder: Pi does not connect to raw the retired graph substrate, Neptune, S3, ontology admin APIs, or a separate Brain MCP server.
...
The API MCP facade already exposes `query_brain_context`, and THNK-20 delivered active/default ThinkWork Brain reads behind Context Engine. First-party Pi agents still expose only `query_context`, `query_memory_context`, and `query_wiki_context`, so models do not get a clear Brain-specific runtime affordance for tenant-shared business/domain context.

This work closes that runtime access gap without reopening ThinkWork Brain substrate, migration, operations UI, or external Brain MCP scope. ThinkWork Brain remains the customer-facing product, Context Engine remains the runtime policy facade, and raw backends remain internal.
...
- R9. Describe the tool as tenant-shared ThinkWork Brain/domain context, distinct from raw Hindsight Memory and compiled Wiki lookup.
- R10. Treat ThinkWork Brain plugin install as provider eligibility behind Context Engine, not raw runtime transport.
...
**Origin actors:** A1 first-party Pi agent, A2 Pi Context Engine extension, A3 AgentCore Pi runtime, A4 Context Engine API MCP facade, A5 ThinkWork Brain plugin install, A6 implementing agent.
...
- Do not expose raw the retired graph substrate, Neptune, S3, ontology admin, Brain storage, or Brain write-back tools to first-party agents.
...
- Bundled ThinkWork Brain skill/workspace guidance if tool descriptions and docs do not prove sufficient to route models toward `query_brain_context`.
...
- `docs/plans/2026-06-14-004-feat-brain-remaining-substrate-plan.md` keeps migration-aware reads and operations UI in THNK-6 follow-up scope.
...
- Do not auto-enable Context Engine from ThinkWork Brain plugin install. Tool registration remains controlled by `context_engine_enabled`.
...
- Source-data policy metadata from the ThinkWork Brain provider.
...
- **Interaction graph:** Pi model -> Context Engine extension -> `/mcp/context-engine` -> Context Engine service -> ThinkWork Brain provider -> progressive MCP response.
...
- **Unchanged invariants:** Context Engine remains the policy boundary; ThinkWork Brain plugin install does not create raw runtime transport; no raw the retired graph substrate/Neptune/S3/admin APIs are exposed.
...
- Linear document: `Requirements: ThinkWork Brain first-party agent context tool`.
- Linear document: `Plan: ThinkWork Brain first-party context tool`.
- Related Linear issues: THNK-6, THNK-20.
- Related plan: `docs/plans/2026-06-14-002-feat-context-engine-brain-reads-plan.md`.
- Related plan: `docs/plans/2026-06-14-004-feat-brain-remaining-substrate-plan.md`.
