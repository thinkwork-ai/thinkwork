---
title: "feat: Add the retired graph substrate thread ingest and explorer"
type: feat
status: active
date: 2026-06-04
origin:
...
related:
  - docs/plans/2026-06-04-001-feat-retired_graph_substrate-terraform-infrastructure-plan.md
...
# feat: Add the retired graph substrate Thread Ingest and Explorer

## Overview

Add Phase II of the the retired graph substrate Knowledge Graph feature: tenant operators can ingest
complete ThinkWork thread message histories into the retired graph substrate, then inspect the
...
This plan intentionally keeps the retired graph substrate out of agent runtime retrieval. Phase II is
an observability and validation layer: prove what the retired graph substrate produces, preserve
...
The first the retired graph substrate plan made the service deployable and visible in Spaces. That
still leaves the product unable to answer the important question: what graph did
the retired graph substrate build from real ThinkWork work?
...
as a deterministic ingest event, and operators should see the resulting the retired graph substrate
...
- R11-R16. Table defaults to entities with entity, type, grounding status,
  ...
  same filtered the retired graph substrate dataset and distinguish trusted, diagnostic, and
...
- R21-R22. No agent retrieval through the retired graph substrate and no graph/ontology editing in
  Phase II.

**Origin actors:** A1 tenant operator/admin, A2 requester/user, A3 thread
message source, A4 approved ontology layer, A5 the retired graph substrate service, A6 ThinkWork
...
- No incremental, scheduled, automatic, or post-turn the retired graph substrate ingest.
...
- No migration from Wiki Memory, Hindsight, ThinkWork Brain, or Bedrock Knowledge
  Bases to the retired graph substrate.
- No browser or GraphQL live calls to the retired graph substrate's private ALB for Explorer reads.
  The UI reads ThinkWork-normalized data from GraphQL.

### Deferred Follow-Up Work

- Agent retrieval and context-engine adapters backed by validated the retired graph substrate data.
...
- Dedicated eval harness comparing the retired graph substrate output to Wiki Memory.
...
- `packages/database-pg/src/schema/ontology.ts` and
  ...
  and relationship definitions that should ground trusted the retired graph substrate output.
- `packages/api/src/graphql/resolvers/core/knowledgeGraphHealthCheck.query.ts`
  ...
  private the retired graph substrate ALB directly, so private the retired graph substrate calls need a runtime with the
  right VPC path.
...
- the retired graph substrate API reference: `/api/v1` includes `add`, `cognify`, `remember`,
  `search`, and dataset management. See
  <https://docs.retired_graph_substrate.ai/api-reference/introduction>.
- the retired graph substrate `remember` combines ingest and graph construction in one call and
  accepts dataset name/id, node sets, background mode, and custom prompt. See
  <https://docs.retired_graph_substrate.ai/api-reference/remember/remember>.
- the retired graph substrate `cognify` processes datasets into graph output and accepts
  `ontologyKey`, `customPrompt`, chunk size, and background mode. See
  <https://docs.retired_graph_substrate.ai/api-reference/cognify/cognify>.
- the retired graph substrate dataset graph retrieval returns nodes with `id`, `label`, `type`,
  `properties` and edges with `source`, `target`, and `label`. See
  <https://docs.retired_graph_substrate.ai/api-reference/datasets/get-dataset-graph>.
- the retired graph substrate ontology management supports uploading ontology files by
  `ontology_key`; ontologies constrain which entity types and relationships
  the retired graph substrate should extract during cognify. See
  <https://docs.retired_graph_substrate.ai/retired_graph_substrate-cloud/functionality/configuration-and-ontologies>.
...
- **Use a dedicated private ingest worker.** GraphQL creates an ingest run row
  ...
  GraphQL resolver path, calls the retired graph substrate from a network context that can reach the
  private the retired graph substrate service.
- **Persist a ThinkWork-normalized graph snapshot in Aurora.** the retired graph substrate is the
...
- **Use one the retired graph substrate dataset per tenant-thread.** Dataset names should be stable,
  e.g. `thinkwork:<tenantId>:thread:<threadId>`, with the the retired graph substrate dataset id
...
- **Represent trust explicitly, not by omission.** The graph should keep
  the retired graph substrate-produced ungrounded entities and weak-provenance edges as diagnostic
...
- **Ontology is a constraint and classifier in Phase II.** The worker exports or
  maps approved ontology entity/relationship definitions into the retired graph substrate ingest and
  then classifies the retired graph substrate output as approved-grounded, unapproved/diagnostic, or
...
- **Explorer reads GraphQL only.** The Spaces UI calls ThinkWork GraphQL queries
  ...
  the retired graph substrate directly.
...
Add a dedicated schema module for the retired graph substrate-derived graph state, preferably
...
- `knowledge_graph_ingest_runs`
  ...
  - `status`: `queued`, `running`, `succeeded`, `failed`, `canceled`,
    `stale_noop`
  - `trigger`: `manual`
  - `retired_graph_substrate_dataset_name`, `retired_graph_substrate_dataset_id`
...
- `knowledge_graph_entities`
  ...
  - `retired_graph_substrate_node_id`, `label`, `normalized_label`, `type_label`
...
- `knowledge_graph_relationships`
  - `id`, `tenant_id`, `thread_id`, `ingest_run_id`
  - `retired_graph_substrate_edge_id`, `source_entity_id`, `target_entity_id`, `label`
...
- `knowledge_graph_evidence`
  ...
  - `source_kind`: `thread_message`, `retired_graph_substrate_payload`, `normalizer`
...
worker without doing long the retired graph substrate work in the GraphQL request.
...
### U4. the retired graph substrate Thread Ingest Worker and Normalizer

**Goal:** Process one thread into the retired graph substrate, fetch the retired graph substrate graph output, normalize
...
- Worker exports approved ontology entity and relationship definitions and
  passes them to the retired graph substrate through the selected supported mechanism
  (`ontologyKey` upload/reuse, `graphModel`, or custom prompt), recording which
  mechanism was used.
- Worker uses the retired graph substrate `remember` when supported for one-step ingest, or
  `add` + `cognify` + dataset graph retrieval when that is more reliable for
  the deployed the retired graph substrate version.
- Worker fetches the retired graph substrate dataset graph and normalizes nodes/edges into
...
- Failed the retired graph substrate calls update run status, error, duration, and partial counts
  without throwing unhandled Lambda errors.

### U5. Worker Infrastructure, IAM, and Private the retired graph substrate Access

**Goal:** Deploy the ingest worker with the right environment, timeout, IAM,
and network path to call the private the retired graph substrate service.
...
- Worker receives `GRAPH_ENDPOINT`, stage, database, and worker function-name
  env vars.
- Worker has Lambda invoke permissions where needed, database access, and only
  scoped Secrets Manager/SSM access for the retired graph substrate auth/config.
- Worker can reach the private the retired graph substrate endpoint. If Lambda VPC attachment is
  required, add it explicitly with security-group ingress to the retired graph substrate's internal
...
**Goal:** Reuse Wiki graph ergonomics while supporting the retired graph substrate trust styling and
...
- `docs/plans/retired_graph_substrate-terraform-infrastructure-autopilot-status.md` if
...
- Smoke script verifies at least one graph query result or records an explicit
  empty-graph diagnostic when the retired graph substrate returns no nodes.
...
3. U5 makes the worker deployable and able to reach private the retired graph substrate.
...
- **the retired graph substrate API version drift.** The worker should isolate the retired graph substrate calls in
  `retired_graph_substrate-client.ts` and record the selected ingestion mode in run metadata.
  Tests should mock both `remember` and `add` + `cognify` paths.
- **Private networking mismatch.** Do not assume GraphQL can call the retired graph substrate because
  health checks currently use AWS control-plane probes. U5 must validate actual
  worker-to-the retired graph substrate connectivity.
- **Evidence gaps from the retired graph substrate graph output.** If the retired graph substrate's graph endpoint lacks
...
- **Long-running ingest.** The GraphQL mutation should enqueue and return;
  the retired graph substrate work happens async with visible run status and errors.

---

## Acceptance Test Map

- AE1: U3/U4/U5/U8 prove selected complete-thread ingest through the retired graph substrate and run
  history recording.
...
- **Single thread or requester batch?** Start with single-thread ingest.
  ...
  is deferred.
- **Worker shape?** Use a dedicated async worker with private the retired graph substrate access.
  Prefer Lambda if VPC/security-group wiring is clean; otherwise use an ECS
  one-off task worker rather than exposing the retired graph substrate.
- **Normalized representation?** Store ThinkWork-normalized runs, entities,
  relationships, and evidence in Aurora.
- **Graph payload source?** GraphQL returns the normalized payload. the retired graph substrate's
...
- **Relationship evidence gaps?** Preserve diagnostic relationships and mark
  provenance `weak` or `missing` when the retired graph substrate does not provide enough source
  data.
...
- Exact the retired graph substrate ingestion mode for the deployed image/version:
  `remember` versus `add` + `cognify`.
- Exact ontology handoff format: uploaded OWL ontology, the retired graph substrate `graphModel`, or
  custom prompt. The worker should pick the most reliable option for the
  deployed the retired graph substrate version and record it in metadata.
- Whether Lambda VPC attachment is sufficient for the retired graph substrate access or an ECS worker
