---
title: "feat: Add the retired graph substrate infrastructure to Terraform"
type: feat
status: active
date: 2026-06-04
origin:
...
# feat: Add the retired graph substrate Infrastructure to Terraform

## Overview

Add the retired graph substrate as an optional AWS-native knowledge-graph service in the Thinkwork
Terraform stack. the retired graph substrate should support the product direction of replacing the
...
wiring. It does not replace ThinkWork Brain, migrate wiki data, or route runtime
agent context through the retired graph substrate yet. The goal is to make a deployable the retired graph substrate
...
ontology is meaning, and ThinkWork Brain/Wiki is a materialized view. The current
conversation updates the technical direction: the retired graph substrate is now the leading
...
variables. the retired graph substrate should follow that pattern rather than becoming ad hoc
...
- R1. the retired graph substrate is provisioned as an optional infrastructure add-on and is disabled
  by default for existing deployments.
- R2. Hindsight remains the agent-memory substrate; the retired graph substrate is named and wired as
  ontology/knowledge graph infrastructure, not a replacement memory engine.
- R3. Terraform provisions a the retired graph substrate API runtime with durable logs, stable
  endpoint output, health checks, IAM, and security-group boundaries.
- R4. the retired graph substrate storage backends are configurable enough to support a low-cost
  dogfood mode and a production-ready remote graph/vector mode.
- R5. Sensitive the retired graph substrate credentials and API tokens are stored in Secrets Manager
  or SSM SecureString by ARN/reference, not broadcast as plaintext Lambda or ECS
  environment values.
- R6. The the retired graph substrate service can use AWS Bedrock for LLM and embedding calls through
  IAM where supported by the retired graph substrate's provider configuration.
...
- R8. Operators can inspect the retired graph substrate's deployed endpoint, log group, enabled
...
Hindsight memory layer, A4 ontology suggestion engine, A5 ThinkWork Brain
...
ontology-shaped ThinkWork Brain context.

**Origin acceptance examples:** AE1-AE5 remain product acceptance examples for
the later ontology integration. This Terraform plan creates the the retired graph substrate substrate
...
- No ThinkWork Brain or Wiki migration to the retired graph substrate in this plan.
- No user-facing ontology UI changes in this plan.
- No agent runtime/provider changes that query the retired graph substrate in this plan.
- No decision that the retired graph substrate replaces Hindsight.
...
- No automatic import of existing wiki pages, Hindsight memory, or PDFs into
  the retired graph substrate during Terraform apply.
- No adoption of non-AWS managed SaaS as the default production backend.

### Deferred to Follow-Up Work

- the retired graph substrate ingestion jobs for existing wiki pages, PDFs, catalogs, and Hindsight
  observations.
- API wrappers that expose the retired graph substrate through Thinkwork auth/tenant boundaries.
- Operator UI for the retired graph substrate dataset status, ontology proposals, graph inspection,
  and materialized entity pages.
- Eval harness comparing the retired graph substrate-backed ontology retrieval against the existing
...
- the retired graph substrate Docker deployment docs: self-hosted the retired graph substrate exposes an API on port 8000,
  supports Docker deployment, and uses environment variables for LLM, relational
  DB, vector store, graph store, auth, and CORS.
- the retired graph substrate architecture docs: the retired graph substrate combines relational, vector, and graph stores;
...
- the retired graph substrate graph-store docs: supported graph stores include Kuzu, Kuzu remote,
  Neo4j, Neptune, and Neptune Analytics; local Kuzu is not suitable for
  concurrent multi-agent/process use.
- the retired graph substrate vector-store docs: supported vector stores include LanceDB, PGVector,
  ChromaDB, and Neptune Analytics.
- the retired graph substrate ontology quickstart: the retired graph substrate can ground entities and relationships to OWL
...
- **the retired graph substrate is an optional app module.** Add `terraform/modules/app/retired_graph_substrate` and
  enable it from `terraform/modules/thinkwork` with `enable_retired_graph_substrate`, mirroring
  the add-on pattern rather than modifying Hindsight.
- **Use ECS Fargate as the first runtime shape.** the retired graph substrate is a containerized API
...
- **Use a VPC-local endpoint for phase 1.** the retired graph substrate should expose only an
  ...
  caller-network access. The the retired graph substrate module should not expose an internet-facing
  endpoint mode, CORS controls, or the retired graph substrate-native auth controls in this phase;
...
- **Support two backend modes.** Provide a low-cost dogfood mode using shared
  Aurora/pgvector plus persistent ECS storage for the retired graph substrate's local data/system
  directories, and a production mode that can point the retired graph substrate at remote graph/vector
  backends such as Neptune/Neptune Analytics or future dedicated services.
- **Do not hardcode local file stores as the production truth.** the retired graph substrate's Kuzu
...
- **Prefer Bedrock provider configuration over third-party API keys.** Since
  Thinkwork is AWS-native, the the retired graph substrate task role should get Bedrock invoke
  permissions and the retired graph substrate should be configured for Bedrock LLM/embedding models
...
- **Expose stable outputs, not implementation internals.** Downstream
  application work should consume `retired_graph_substrate_enabled`, endpoint, log group, and
...
- **Should the retired graph substrate replace Hindsight?** No. The plan names the retired graph substrate as ontology and
...
- **Should this use AWS GraphRAG Toolkit instead?** No for this plan. The chosen
  direction is the retired graph substrate as ontology substrate; AWS GraphRAG Toolkit remains a
...
- **What is the phase-1 endpoint shape?** Use an internal ALB with ECS tasks in
  ...
  the the retired graph substrate API off the public internet while avoiding NAT/VPC-endpoint work
  until a fully private runtime path is explicitly planned.

### Deferred to Implementation

- **Exact the retired graph substrate image and extras:** Implementation must verify whether the
...
- **Exact Bedrock env values:** the retired graph substrate docs list Bedrock support, but the
  ...
  the selected the retired graph substrate version.
...
- **Aurora/pgvector readiness:** Implementation must verify whether the selected
  ...
  the existing Hindsight setup before the retired graph substrate dogfood mode is enabled.
...
## Output Structure
    ...
    apps/cli/
      ...
      __tests__/terraform-retired_graph_substrate-fixture.test.ts
    .github/workflows/
      verify.yml
      deploy.yml
...
```mermaid
flowchart TD
  TF["terraform/modules/thinkwork"] --> C["terraform/modules/app/retired_graph_substrate"]
  C --> ECS["ECS Fargate service<br/>the retired graph substrate API :8000"]
...
```mermaid
flowchart TD
  OFF["enable_retired_graph_substrate = false"] --> NOOP["No the retired graph substrate resources<br/>outputs null/empty"]
  ON["enable_retired_graph_substrate = true"] --> MODE{"backend mode"}
...
- U1. **Create the the retired graph substrate Terraform app module**

**Goal:** Add a reusable app submodule that provisions the the retired graph substrate runtime
...
- Create: `terraform/modules/app/retired_graph_substrate/main.tf`
- Create: `terraform/modules/app/retired_graph_substrate/variables.tf`
- Create: `terraform/modules/app/retired_graph_substrate/outputs.tf`
- Create: `terraform/modules/app/retired_graph_substrate/README.md`
...
- Model the module after `terraform/modules/app/hindsight-memory`, but name the
  service, IAM roles, security groups, log group, and endpoint around the retired graph substrate.
...
- Expose variables for image URI/tag, CPU/memory, desired count,
  ...
  networks. Do not expose public endpoint mode, CORS, or the retired graph substrate-native auth
...
- Provision an internal ALB and run ECS tasks in the existing public subnets
  ...
  internet-facing ALB for the retired graph substrate in phase 1.
- Restrict network ingress so the the retired graph substrate task accepts traffic only from the
  internal ALB, and the internal ALB accepts traffic only from VPC-local or
  explicitly configured caller-network sources.
- Mount persistent writable paths for the retired graph substrate's data and system directories,
  because the retired graph substrate requires writable `DATA_ROOT_DIRECTORY` and
...
- Verify the selected Aurora/PGVector path includes the required extension and
  ...
  satisfies the retired graph substrate's relational/vector requirements.
- Add CloudWatch log retention and health checks for the the retired graph substrate API endpoint.
...
- Happy path: with `enable_retired_graph_substrate` handled by the parent module and valid module
...
- Edge case: with remote backend variables empty in dogfood mode, the task
  ...
  ephemeral container storage for required the retired graph substrate directories.
- Error path: invalid backend mode/provider combinations are rejected by
  ...
  contradictory the retired graph substrate env vars.
- Error path: any attempt to configure an internet-facing the retired graph substrate endpoint in
...
- The module documents required the retired graph substrate env/provider assumptions and links to
  the retired graph substrate deployment/configuration docs.
- The the retired graph substrate ALB is internal-only, the task security group has no direct public
...
- U2. **Wire the retired graph substrate through the composite Thinkwork module**

**Goal:** Make the retired graph substrate available from `terraform/modules/thinkwork` without
breaking existing deployments or conflating the retired graph substrate with memory-engine selection.
...
- Add `enable_retired_graph_substrate` and the retired graph substrate-specific configuration variables to the
  composite root.
- Instantiate `module "retired_graph_substrate"` behind `count = var.enable_retired_graph_substrate ? 1 : 0`.
- Pass VPC, subnet, database endpoint/secret, database security group, region,
  account id, and selected model/backend config into the the retired graph substrate module.
- Add database security-group ingress only through the the retired graph substrate module path, not a
  broad global rule.
- Expose outputs such as `retired_graph_substrate_enabled`, `retired_graph_substrate_endpoint`,
  `retired_graph_substrate_log_group_name`, and `retired_graph_substrate_task_role_arn`.
...
- Happy path: default `enable_retired_graph_substrate = false` leaves the composite module output
  values null/empty and creates no the retired graph substrate resources.
- Happy path: `enable_retired_graph_substrate = true` wires module inputs from existing VPC,
  database, and region/account variables.
- Edge case: setting both `enable_hindsight = true` and `enable_retired_graph_substrate = true`
  is valid and does not change `memory_engine`.
- Error path: invalid the retired graph substrate backend mode/provider values are rejected by
  validation.
...
- the retired graph substrate outputs are stable names for downstream application work.

---

- U3. **Add the retired graph substrate secrets and configuration hygiene**

**Goal:** Keep sensitive the retired graph substrate/provider credentials out of tfstate-adjacent
...
- Create or modify: `terraform/modules/app/retired_graph_substrate/README.md`
- Test: `apps/cli/__tests__/terraform-retired_graph_substrate-fixture.test.ts`
...
- Add IAM policy statements narrowly scoped to the the retired graph substrate task role.
- Document any unavoidable the retired graph substrate env var that contains a secret so reviewers can
  decide whether the selected the retired graph substrate version requires a wrapper/entrypoint to
...
- Terraform plan does not expose the retired graph substrate provider secrets in outputs.
- the retired graph substrate task role has only the permissions required for selected runtime paths.
- The README explains secret rotation and operator-owned secret-value behavior.

---

- U4. **Propagate the retired graph substrate through examples, CLI templates, and CI workflows**

**Goal:** Keep every deployment surface aligned so the retired graph substrate can be enabled in real
...
- Add the retired graph substrate variables to the greenfield example with safe disabled defaults.
...
- Update CI Terraform plan/apply variable lists with explicit disabled/default
  ...
  style.
- Keep the retired graph substrate off in CI by default unless explicitly enabled for a stage.
...
- Happy path: generated greenfield HCL includes `enable_retired_graph_substrate = false` by
  default and passes the value through to `module.thinkwork`.
- Happy path: enterprise deploy template exposes the retired graph substrate variables with safe
  defaults.
- Edge case: CI verify/deploy still passes explicit values for every required
  root variable when the retired graph substrate is disabled.
- Error path: enterprise secret validation does not require the retired graph substrate provider
...
- CI plan/apply surfaces cannot accidentally enable the retired graph substrate without an explicit
  variable change.
...
**Goal:** Make the retired graph substrate deployable by operators without confusing Terraform
...
- Document how to enable the retired graph substrate, expected monthly cost drivers, selected backend
  mode, secret setup, health endpoint, log group, and rollback behavior.
- Document that the retired graph substrate infrastructure alone does not migrate the Wiki/Brain or
  change agent context.
- Add a post-deploy smoke expectation: health endpoint is reachable from the
  intended caller network, the retired graph substrate reports configured providers, and logs show no
...
- Edge case: docs explain disabled/default state so existing deployments do not
  expect the retired graph substrate resources.
...
- Operators can enable the retired graph substrate and understand what Terraform does and does not
  prove.
- Docs preserve the product boundary: the retired graph substrate substrate first, ontology migration
  later.
...
- **State lifecycle risks:** Persistent the retired graph substrate stores must not be accidentally
...
- **Integration coverage:** Terraform validation proves syntax and dependencies;
  a post-deploy smoke check is still needed to prove the retired graph substrate boots, reaches its
  backends, and can call Bedrock/provider APIs.
- **Unchanged invariants:** Hindsight remains the memory engine. Existing
  Bedrock KBs remain document retrieval. the retired graph substrate starts as ontology/knowledge
...
- **Use the retired graph substrate Cloud instead of self-hosted Terraform:** Rejected as the default
...
- **Use AWS GraphRAG Toolkit instead of the retired graph substrate:** Rejected for this plan because
  the current strategic need is ontology/knowledge-graph substrate, not only
  document-to-graph retrieval over PDFs.
- **Deploy the retired graph substrate on EC2:** Rejected as the first path because the repo already
...
- **Use embedded Kuzu/LanceDB only:** Useful for a dogfood mode, but rejected as
  the only infrastructure path because the retired graph substrate docs warn local Kuzu is not fit
  for concurrent multi-process use and local stores are fragile for enterprise
  runtime expectations.
- **Provision Neptune Analytics immediately:** Deferred. the retired graph substrate can be designed
...
- the retired graph substrate image/version selection must be pinned before implementation lands.
- AWS Bedrock model access must be enabled in target accounts for the selected
  the retired graph substrate LLM and embedding models.
- If production mode uses remote graph/vector stores, those services and network
  routes must exist before enabling the retired graph substrate against them.
...
| ----------------------------------------------------------------------------------------- | ---------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| the retired graph substrate public image lacks required provider/PDF extras               | Medium     | Medium | Make image URI/tag configurable and document a custom ECR image path as an implementation-time decision.                                                                               |
...
| Secrets leak through task environment or tfstate                                          | Medium     | High   | Prefer Bedrock IAM; use Secrets Manager/SSM indirection for non-Bedrock keys; document any unavoidable the retired graph substrate limitation.                                         |
...
| Terraform apply succeeds but the retired graph substrate cannot start                     | Medium     | Medium | Add health checks, logs, and post-deploy smoke guidance; do not treat Terraform success as runtime proof.                                                                              |
...
| Dogfood mode assumes PGVector is available when the selected database lacks the extension | Medium     | Medium | Verify extension/migration ownership in U1 and fail validation or docs handoff before enabling PGVector-backed the retired graph substrate storage.                                    |
| the retired graph substrate blurs memory and ontology boundaries                          | Medium     | High   | Keep naming, outputs, docs, and deferred work explicit: Hindsight is memory; the retired graph substrate is ontology/knowledge graph substrate.                                        |
...
- Land optional the retired graph substrate Terraform module, composite wiring, safe disabled default,
  and docs.
- Use dogfood backend mode unless implementation verifies a production remote
  backend is cheap and ready.
- Do not route app traffic to the retired graph substrate yet.
...
- Decide whether production backend should be Neptune, Neptune Analytics, or a
  dedicated graph service based on measured the retired graph substrate behavior.
...
- Existing deployments with default variables produce no the retired graph substrate resources and no
  behavior change.
- Enabling the retired graph substrate provisions a healthy service with stable endpoint/log outputs.
- the retired graph substrate can call the selected LLM/embedding provider and connect to configured
  relational/vector/graph storage.
- CI, greenfield examples, and CLI-generated templates share the same the retired graph substrate
  variable names and defaults.
- Operators can explain where the retired graph substrate fits without confusing it with Hindsight or
  Bedrock KBs.
...
- The the retired graph substrate module README should be written as an operator handoff: enablement,
  backend modes, secrets, logs, health, rollback, and cleanup.
- Product docs should state that the retired graph substrate is infrastructure for future
...
- External docs: [the retired graph substrate Docker deployment](https://docs.retired_graph_substrate.ai/how-to-guides/retired_graph_substrate-sdk/deployment/docker)
- External docs: [the retired graph substrate architecture](https://docs.retired_graph_substrate.ai/core-concepts/architecture)
- External docs: [the retired graph substrate graph stores](https://docs.retired_graph_substrate.ai/setup-configuration/graph-stores)
- External docs: [the retired graph substrate vector stores](https://docs.retired_graph_substrate.ai/setup-configuration/vector-stores)
- External docs: [the retired graph substrate ontology quickstart](https://docs.retired_graph_substrate.ai/guides/ontology-support)
