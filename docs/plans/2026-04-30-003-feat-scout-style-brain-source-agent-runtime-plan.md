---
title: "feat: Scout-style ThinkWork Brain source agent runtime"
type: feat
status: active
date: 2026-04-30
origin:
...
# feat: Scout-style ThinkWork Brain source agent runtime

## Overview

Build the first real Scout-style ThinkWork Brain source agent runtime: a bounded
...
The current ThinkWork Brain source-agent substrate is useful but not agentic. The
...
This plan focuses on the compiled ThinkWork Brain page source first. Operational
source agents for ERP, CRM, support, and catalog remain planned until their
connectors and read tools exist.

## Problem Frame

ThinkWork Brain currently says "source agent" in places where the runtime is still
...
- R1. The ThinkWork Brain Page Agent can run in a real model-backed mode that calls
...
- R3. The runtime exposes a narrow allowlist of typed tools, initially
  `brain.pages.search` and `brain.pages.read`.
...
- A live ThinkWork Brain Page Agent model-backed path using the compiled wiki.
...
- Browser verification on the Admin ThinkWork Brain Sources page.
...
- `packages/api/src/lib/context-engine/providers/wiki-source-agent.ts` owns the
  current deterministic ThinkWork Brain Page Agent and should become the first
...
2. **Tools are source-local functions, not arbitrary MCP calls.** The first
   ...
   `brain.pages.search` and `brain.pages.read`. The runtime
...
- Parse model responses as JSON actions:
  - `{"tool_calls":[{"id":"call-1","tool":"brain.pages.search","input":{...}}]}`
...
- A fake model calls `brain.pages.search`, then
  `brain.pages.read`, then returns a cited final result.
...
### Unit 2: ThinkWork Brain page source tools
...
- Implement `brain.pages.search` as a typed wrapper over
  `searchWikiForUser`.
- Implement `brain.pages.read` as a typed page reader that can only read
...
### Unit 3: Model-backed ThinkWork Brain Page Agent provider
...
- Add or modify: `packages/api/test/integration/context-engine/brain-source-agent.e2e.test.ts`
...
- Existing ThinkWork Brain retrieval tests still pass.
