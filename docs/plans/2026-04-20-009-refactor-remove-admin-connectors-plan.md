...
Retiring the *current* surface is not the same as closing the door on inbound partner-initiated task ingestion forever. If a LastMile-class partner re-enters the picture later, the right home is mobile self-serve (per memory: `feedback_user_opt_in_over_admin_config`) built on the preserved `connect_providers`/`connections`/`credentials` schema. This plan leaves that door open — no design work is committed here.
...
- **R10** — Mobile dead-code cleanup (LastMile tile + `hasTaskConnector`/`activeTaskConnectors` branches + nav cleanup)
...
- **Out of scope:** archived plans under `plans/archived/` and historical docs under `docs/plans/` and `docs/brainstorms/`.
...
- **Out of scope: the LastMile MCP server.** The retirement targets *LastMile-as-task-connector* (the deprecated surface this PR removes). *LastMile-as-MCP-server* is a separate, supported integration and stays wherever it lives — MCP server catalog entries, `apps/mobile/app/settings/mcp-server-detail.tsx`, MCP-side OAuth flows, and any provider-name references in MCP code. Unit 6's tolerated-match list below names these paths explicitly.
- **Out of scope:** `LASTMILE_CLIENT_ID` / `LASTMILE_CLIENT_SECRET` / `LASTMILE_TASKS_API_URL` revocation at the provider and env removal (captured in Documentation / Operational Notes as a tracked follow-up).
...
- **LASTMILE_*** env/secret cleanup and provider-side revocation** — separate PR, see Documentation / Operational Notes.
...
- `apps/mobile/app/settings/integrations.tsx` — LastMile surface is wider than a catch-all captures (verified via grep). Enumerated in Unit 4
...
11. `docs/src/content/docs/roadmap.mdx` (line 27: prose bullet "External task connectors (LastMile adapter + generic adapter seam) | Beta" — no dead link, but the prose needs updating)
...
- **Surgical edit of `apps/mobile/app/settings/integrations.tsx`, not file delete.** The file still owns the Google + Microsoft integration UI. Unit 4 enumerates every verified LastMile branch (research-grep confirmed at least 8 distinct locations, not the 3 the brainstorm catch-all implied); Unit 6's grep gate includes `lastmile` (case-insensitive) with targeted tolerated-match comments.
...
- **R5 grep expanded beyond origin spec.** Unit 6 adds `handleConnectLastmile` and `lastmile` (case-insensitive) to catch literals that escape the origin's symbol-only pattern. Tolerated matches: `plans/archived/`, `docs/plans/`, `docs/brainstorms/`, `seeds/`, and **MCP-side LastMile references** (the LastMile MCP server is a supported integration — see Scope Boundaries). Concrete tolerated paths: `apps/mobile/app/settings/mcp-server-detail.tsx`, `apps/mobile/app/settings/mcp-servers.tsx`, any `packages/api/src/handlers/mcp-*.ts`, and MCP server catalog seed data.
...
- **Does the LastMile MCP server also retire?** No. LastMile-as-task-connector retires (this PR); LastMile-as-MCP-server stays as a supported integration. Unit 6's tolerated-match list names the concrete MCP paths where `lastmile` literals are legitimate.
...
- **Should `handleReconnect` (integrations.tsx:173–180) keep a `lastmile` branch after the task-connector UI is gone?** Resolved via Unit 4: the `else if (providerName === "lastmile")` branch is removed because the Integrations screen no longer surfaces a LastMile tile — there's nothing to "reconnect" from this screen. Users reconnect LastMile-as-MCP-server from the MCP Servers screen.
...
- Edge case: Deep link `/connectors/lastmile` also yields no matched route
...
- Modify (cross-link sweep — verified list from grep):
  ...
  11. `docs/src/content/docs/roadmap.mdx` (prose edit: update the "External task connectors (LastMile adapter + generic adapter seam) | Beta" row to reflect retirement, or delete it if the table contract allows)
...
- Integration: `roadmap.mdx` no longer lists `External task connectors (LastMile adapter + generic adapter seam)` as an active/beta item (either removed or marked retired)
...
- Modify: `apps/mobile/lib/hooks/use-connections.ts`
  ...
  - Update the **`ConnectionRow.provider_type` docstring** at lines 32–39 — drop the `"task" for LastMile Tasks` example and the "Surfaced by the server join... decides whether the inbox should show the Tasks tab" rationale
...
- Modify (surgical, enumerated): `apps/mobile/app/settings/integrations.tsx`
  ...
  - Line 21: `PROVIDER_ICONS.lastmile` entry
  - Line 47: agent-pairing comment referencing LastMile
  - Lines 88–94: MCP explanation block referencing "LastMile Tasks uses OAuth through the MCP Servers screen…" — this block is task-connector guidance (how to get LastMile *task data* into ThinkWork) and goes with the rest of the task-connector surface. The LastMile MCP server itself stays; users connect to it via the MCP Servers screen directly
  - Lines 91–96: `handleConnectLastmile` declaration (originally called out)
  - Lines 173–180: `handleReconnect` else-if branch for `providerName === "lastmile"`
  - Lines 284–357: per-user default-agent picker block gated on `conn.provider_name === "lastmile"` (includes the `handleSelectDefaultAgent(conn, "lastmile", …)` callsites at 320 and 337, plus surrounding metadata reads at 289, the ChevronDown/ChevronUp picker UI, and the "Attached automatically to new LastMile task threads" footer at 351)
  - Lines 411–430: the unconnected-LastMile "Available Connectors" tile Pressable
  - Lines 433–439: the "All available integrations connected" guard — drop `connectedProviders.has("lastmile")` from the three-provider AND check so the guard correctly fires when Google + Microsoft are both connected
  - Line 435: the `connectedProviders.has("lastmile") && (...)` connected-LastMile render block (if not already swept by the 411–430 removal)
  - After removal, audit `AgentsQuery` (lines ~49–65) and the `myAgents` useMemo — if their only remaining consumer was the LastMile picker, remove them so the integrations screen stops firing an unused GraphQL query on every load
...
- Happy path: Settings → Integrations renders Google + Microsoft tiles, no LastMile tile, no "All available integrations connected" dead state. When Google + Microsoft are both connected, the Muted "All available integrations connected" empty-state message shows (previously unreachable because lastmile was in the AND)
- Happy path: Integrations → Reconnect on a Google or Microsoft row still works; no crash from the missing `handleConnectLastmile`
...
**Verification:**
- `grep -rEi 'hasTaskConnector|activeTaskConnectors|handleConnectLastmile|PROVIDER_ICONS\.lastmile|settings/connectors' apps/mobile/` returns zero matches
- `grep -rEi '[\"'\''\`]lastmile[\"'\''\`]' apps/mobile/app/ apps/mobile/lib/` returns only MCP-server-catalog matches: `apps/mobile/app/settings/mcp-server-detail.tsx`, `apps/mobile/app/settings/mcp-servers.tsx`, and any MCP seed/catalog code. Matches anywhere else (settings/integrations.tsx, hooks/use-connections.ts, inbox, tabs/index.tsx, etc.) are unfinished Unit 4 scope
...
**Approach:**
- **Primary grep** (expanded from origin R5 to cover the LastMile literal surface Unit 4 requires): `grep -rEi 'task-connectors|task_connectors|hasTaskConnector|activeTaskConnectors|handleConnectLastmile' apps/ packages/ scripts/ terraform/ docs/src/`
  - Expected: zero matches in live code
  - Tolerated: matches in `plans/archived/`, `docs/plans/`, `docs/brainstorms/`, `seeds/` (scenario fixtures, explicitly out of scope)
- **Secondary grep** (literal surface, mobile-scoped): `grep -rEi "[\\\"'\\\`]lastmile[\\\"'\\\`]" apps/mobile/app/ apps/mobile/lib/`
  - Expected: zero matches
  - Tolerated: matches under `apps/mobile/app/mcp-servers/` or MCP-catalog code if the LastMile MCP server is staying (see Open Questions — confirm at implementation time and document tolerated path)
...
- **Unchanged invariants:**
  ...
  - All mobile non-LastMile integration UI (Google, Microsoft branches)
  - LastMile MCP server catalog entry (if present) — out of scope; if it stays, Unit 6's tolerated-match list covers its `lastmile` literals
...
| Unit 4 misses a LastMile branch not yet surfaced by grep (e.g., string-concatenated provider names) | Unit 6 secondary grep for `lastmile` literal in `apps/mobile/app/` + `apps/mobile/lib/` catches any survivor; enumerated list in Unit 4 Files covers the 8+ verified locations |
...
| `LASTMILE_CLIENT_ID` / `LASTMILE_CLIENT_SECRET` / `LASTMILE_TASKS_API_URL` stay injected into Lambda common_env | Tracked follow-up in Documentation / Operational Notes. Not an in-scope security issue given the API Gateway 404 and zero active LastMile partners |
...
- **No customer communication needed** — origin's dev-only LastMile assumption; even if wrong, external callers already 404 at API Gateway today.
...
- **Tracked follow-ups (separate tasks, not blocking this PR):**
  - **LastMile credentials cleanup.** `terraform/examples/greenfield/terraform.tfvars` injects `LASTMILE_TASKS_API_URL`; `oauth-authorize.ts` and `oauth-callback.ts` read `LASTMILE_CLIENT_ID` / `LASTMILE_CLIENT_SECRET` from `process.env`. With the admin UI gone and no live partner, these credentials should be zeroed in tfvars/SSM and the OAuth client revoked at the provider. Per `project_tfvars_secrets_hygiene` memory, migration to SSM is already planned for prod — fold this cleanup into that effort.
