...
- **R5 grep expanded beyond origin spec.** Unit 6 adds `handleConnectLastmile` and `lastmile` (case-insensitive) to catch literals that escape the origin's symbol-only pattern. Tolerated matches: `docs/plans/archived/`, `docs/plans/`, `docs/brainstorms/`, `seeds/`, and **MCP-side LastMile references** (the LastMile MCP server is a supported integration — see Scope Boundaries). Concrete tolerated paths: `apps/mobile/app/settings/mcp-server-detail.tsx`, `apps/mobile/app/settings/mcp-servers.tsx`, any `packages/api/src/handlers/mcp-*.ts`, and MCP server catalog seed data.
