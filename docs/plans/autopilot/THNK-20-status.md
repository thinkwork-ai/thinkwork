...
- Fetched THNK-15 documents: ThinkWork Brain premium plugin plan and brainstorm.
- Fetched THNK-6 documents: ThinkWork Brain physical substrate plan, requirements, and OKF decision.
...
- Searched repo for THNK-20, U5a, `query_brain_context`, ThinkWork Brain dogfood proof, THNK-6/15/17/19, Context Engine, and Brain artifact/provenance terms.

## Important Context

- THNK-20 acceptance focuses on first-party active/default Brain reads, not migration-aware U5b.
- ThinkWork Brain is the customer/product term. Context Engine is the runtime policy layer. the retired graph substrate is internal substrate evidence only.
...
- Keep reads behind Context Engine; do not add direct the retired graph substrate, Neptune, or S3 caller paths.
...
- 2026-06-14: Searched repo-local docs and code for THNK-20/U5a/ThinkWork Brain/Context Engine references.
...
- 2026-06-14: Added ThinkWork Brain Context Engine provider, Brain retrieval option parsing, untrusted source-data formatting, provider tests, MCP boundary tests, and read-only smoke/docs updates.
...
- 2026-06-14: `pnpm --filter @thinkwork/api test -- src/lib/context-engine/providers/brain.test.ts src/lib/context-engine/__tests__/service.test.ts src/handlers/mcp-context-engine.requester-context.test.ts` passed: 3 files, 14 tests.
...
- 2026-06-14: `node plugins/brain/smoke/brain-context-engine-smoke.mjs` passed dry-run.
