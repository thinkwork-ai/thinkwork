...
Issue: THNK-17 - U1: Define ThinkWork Brain substrate contract and storage-tier model
...
- Read repo-local planning/requirements docs from the main checkout:
  - docs/plans/2026-06-13-003-feat-brain-physical-substrate-plan.md
  - docs/brainstorms/2026-06-13-brain-physical-substrate-requirements.md
...
- Searched the repository for THNK-17, THNK-6, THNK-15, ThinkWork Brain, Brain substrate, storage tier, redaction, the retired graph substrate status, and related filenames.
...
U1 objective: add the durable API/data contract for ThinkWork Brain substrate status, storage tier, migration state, operational counters, and the retired graph substrate capability posture, with tenant-safe redaction and operator evidence boundaries.
...
2. GraphQL contract and resolvers
   ...
   - Ensure legacy env-derived the retired graph substrate status cannot override explicit Brain substrate rows.
...
- 2026-06-14: Added GraphQL `brainStatus` contract, resolver, settings query, and focused resolver tests.
- 2026-06-14: Ran `pnpm --filter @thinkwork/database-pg db:generate`; drizzle-kit reached an interactive schema-conflict prompt in the non-TTY shell, so added manual migration `0166_brain_substrate_contract.sql` with drift markers.
...
- 2026-06-14: Verification passed:
  - `pnpm --filter @thinkwork/api exec vitest run src/graphql/resolvers/brain/brainStatus.query.test.ts`
  ...
  - `pnpm schema:build`
  - `bash scripts/db-migrate-manual.sh --dry-run packages/database-pg/drizzle/0166_brain_substrate_contract.sql`
...
- Use the Linear-attached ThinkWork Brain physical substrate plan as the primary plan. The repo-local plan in the main checkout matches the referenced plan and contains the detailed U1 implementation guidance.
