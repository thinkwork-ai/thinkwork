...
| 2026-05-20 | U5    | `codex/spaces-workrooms-u5`    | [#1491](https://github.com/thinkwork-ai/thinkwork/pull/1491) | CI pending | `pnpm install`; focused Template migration/default Space/workspace bootstrap/workspace files tests; `pnpm --filter @thinkwork/database-pg test`; `pnpm --filter @thinkwork/api test`; `pnpm --filter @thinkwork/database-pg typecheck`; `pnpm --filter @thinkwork/api typecheck`; `pnpm typecheck`; scoped migration dry-run for `0114_migrate_templates_to_agents_and_spaces.sql`; touched-file Prettier check; `git diff --check`.                                                                                                                                                     | Added the Template-to-Agent/Space backfill migration, Space source migration helper, tenant default context Space helper, and Template-free workspace bootstrap. Physical Template table/API removal is deferred to U7 because live resolvers still reference that surface.                              |
