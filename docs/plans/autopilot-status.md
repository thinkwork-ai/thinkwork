...
| 2026-05-18 | U5   | `codex/enterprise-ci-workflow-u5`       | pending                                                      | Local pass | `pnpm --filter thinkwork-cli test -- enterprise-template.test.ts enterprise-workflow-template.test.ts no-required-options.test.ts`; `pnpm --filter thinkwork-cli typecheck`; `pnpm --filter thinkwork-cli build`; rendered workflow YAML parse; `pnpm -r --if-present typecheck`; `pnpm -r --if-present lint`; `pnpm test`; touched-file Prettier check; `git diff --check`                                                                                            | Expanded the generated deployment workflow for release manifest verification, Lambda/static artifact prep, Terraform apply, customer ECR image copy, AgentCore runtime refresh with endpoint freshness verification, overlay record, smokes, and summary artifacts.         |
