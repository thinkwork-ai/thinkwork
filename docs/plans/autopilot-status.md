...
- Active unit: U7 - Remove Template GraphQL/API/resolver surface
- Active branch: `codex/spaces-workrooms-u7`
- Active worktree: `.Codex/worktrees/spaces-workrooms-u7`
- Started: 2026-05-20
- PR: [#1493](https://github.com/thinkwork-ai/thinkwork/pull/1493)
- CI: pending
...
| 2026-05-20 | U7    | `codex/spaces-workrooms-u7`    | [#1493](https://github.com/thinkwork-ai/thinkwork/pull/1493) | In CI  | `pnpm schema:build`; CLI/admin/mobile codegen; focused API GraphQL contract/runtime/admin resolver tests; focused admin workspace/agent/computer route tests; focused CLI registration/enterprise overlay tests; `pnpm --filter @thinkwork/api typecheck`; `pnpm --filter @thinkwork/admin-ops typecheck`; `pnpm --filter @thinkwork/lambda typecheck`; `pnpm --filter thinkwork-cli typecheck`; `pnpm --filter @thinkwork/admin build`; `pnpm --filter @thinkwork/database-pg typecheck`; `pnpm typecheck`; `pnpm --filter @thinkwork/api test`; `pnpm --filter @thinkwork/admin test`; `pnpm --filter thinkwork-cli test`; `pnpm --filter @thinkwork/lambda test -- __tests__/admin-ops-mcp.test.ts`; touched-file Prettier check; `git diff --check`. | Removed the public Template GraphQL schema/resolver surface, admin-ops Template tools, CLI Template commands/version rollback, generated client Template documents, and admin Template routes/sync UI. Lower-level legacy Template storage for Computers/evals remains for U10 cleanup. Full CLI test initially failed on stale Template/version registration expectations; updated those tests and reran green. CI test initially failed on stale `templates_*` admin-ops MCP expectations; updated the Lambda test and reran green locally. |
