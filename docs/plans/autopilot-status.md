...
| 2026-05-21 | U2   | `codex/u2-space-detail-simplify` | Pending                                                      | Locally verified | Focused Spaces route test; full `pnpm --filter @thinkwork/admin test`; `pnpm --filter @thinkwork/admin build`; Chrome smoke check of Spaces list, Configuration, Memory, and Automations; `git diff --check`. | Simplified Spaces list columns, made Configuration the default detail route, replaced old Connected Data/MCP/Settings tabs with Configuration/Workspace/Tools/Memory/Automations, and removed raw/internal configuration panels from the visible Space detail UI. |
