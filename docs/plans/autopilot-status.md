...
| 2026-05-20 | U2   | `codex/computer-artifact-side-panel-u2` | [#1501](https://github.com/thinkwork-ai/thinkwork/pull/1501) | CI pending | Focused `GeneratedArtifactCard`, `TaskThreadView`, and `ComputerThreadDetailRoute` tests; `pnpm --filter @thinkwork/computer typecheck`; touched-file Prettier check via `pnpm dlx prettier@3.5.3`; `git diff --check`; full `pnpm --filter @thinkwork/computer test`; `pnpm --filter @thinkwork/computer build`. Follow-up validation fix: focused tests, typecheck, touched-file Prettier check, and `git diff --check`. | Started from `origin/main` after U1 merge. Replaced the inline transcript artifact embed with a compact card affordance while preserving the full artifact preview renderer for the upcoming side panel. Added dynamic transcript bottom inset measurement after local validation showed the runbook composer could cover the bottom of the feed. |
