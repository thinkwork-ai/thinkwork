---
title: "feat: ThinkWork Brain hybrid retrieval and source agents"
type: feat
status: active
date: 2026-04-30
origin:
...
This is intentionally narrower than the full ThinkWork Brain v0 plan. It strengthens retrieval quality and the source-agent seam without adding embeddings, a unified vector index, or write-capable operational adapters.

## Problem Frame

The current ThinkWork Brain operator test can make the product look broken even when useful knowledge exists. Hindsight may return a single memory, while the compiled wiki either misses or under-ranks the relevant pages. Worse, a simple misspelling can drop wiki results entirely even though semantic/compiled retrieval should absorb that kind of user input.
...
- E2E and integration coverage for ThinkWork Brain query quality.
...
- `docs/plans/2026-04-29-004-feat-brain-v0-plan.md` defines ThinkWork Brain as an agent-augmentation substrate and explicitly calls out sub-agent provider shape, source citations, provider statuses, and operator verification.
...
4. **Bridge hits should appear as wiki-family hits.** Even if the enrichment happens while processing memory results, the user should see compiled pages returned as compiled wiki/ThinkWork Brain pages, with metadata showing that the page was boosted because Hindsight recalled a cited memory.
5. **Make one live source-agent adapter observable.** The existing inert provider metadata is useful, but it does not answer the user's concern. This PR should include a bounded ThinkWork Brain wiki source-agent adapter or source-agent seam test that proves source-specific retrieval logic was invoked and normalized into Context Engine hits.
...
- Add a live ThinkWork Brain/wiki source-agent demonstration that invokes the wiki retrieval helper through an injected source retrieval seam, or harden the existing source-agent test so it proves the retrieval seam ran and produced normalized hits.
...
### Unit 4: ThinkWork Brain E2E retrieval coverage
...
- Add or modify: `packages/api/test/integration/context-engine/brain-context.e2e.test.ts`
...
- Keep provider selection explicit so optional MCP/source failures do not hide core ThinkWork Brain regressions.
...
**Goal:** Let an operator see whether ThinkWork Brain source agents are live, inert, or degraded.
