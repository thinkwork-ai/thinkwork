---
title: "feat: ThinkWork Brain premium plugin"
type: feat
status: active
date: 2026-06-13
origin: docs/brainstorms/2026-06-13-brain-premium-plugin-requirements.md
deepened: 2026-06-13
linear: THNK-15
---

# feat: ThinkWork Brain premium plugin

## Summary

Ship ThinkWork Brain as the first premium Application Plugin. V1 keeps the scope
focused on the premium plugin shell: ThinkWork Brain is always discoverable in the
...
ownership of the internal the retired graph substrate-powered substrate component through the
...
status copy should say ThinkWork Brain, Brain substrate, knowledge graph, Memory /
Ontology, or Brain operations. the retired graph substrate may appear in operator-only evidence,
...
the retired graph substrate is already a managed application and the technical base for ThinkWork's
knowledge graph direction, but it must stop being treated as a customer-facing
thing to install. THNK-15 should make the ThinkWork Brain packaging, entitlement,
and lifecycle model real while treating the retired graph substrate as an internal substrate
...
reinstalls and upgrades, an adoption path for tenants that already have the retired graph substrate,
...
From origin `docs/brainstorms/2026-06-13-brain-premium-plugin-requirements.md`:

- R1. ThinkWork Brain is always visible in the Plugins catalog.
- R2. Catalog and plugin detail mark ThinkWork Brain as premium and key-gated.
...
- R8. ThinkWork Brain v1 is an Application Plugin that owns the retired graph substrate as an internal
  infrastructure component, not as a customer-visible plugin or license.
- R9. The internal substrate component reuses the existing the retired graph substrate managed-app
  adapter and deployment-runner plan/approval/apply path where appropriate.
- R10. Existing internal the retired graph substrate deployments are adopted after entitlement when a
...
- R12. New entitled tenants provision ThinkWork Brain's internal substrate through
  the normal infrastructure component flow.
- R13. Destructive actions preserve internal substrate data-impact disclosure
  and approval.
- R14. ThinkWork Brain plugin detail becomes the install, entitlement, and status
  home.
- R15. Existing Memory / Ontology remains the working graph explorer in v1.
- R16. Legacy the retired graph substrate / managed-application entry points are reduced only after
  ThinkWork Brain plugin detail covers the same lifecycle needs.
...
existing the retired graph substrate adoption, F4 new tenant install, F5 manage graph workspace.

**Origin acceptance examples:** AE1 catalog/key prompt, AE2 one-time key and
persistent entitlement, AE3 temporary backdoor key, AE4 no-change the retired graph substrate
...
- ThinkWork Brain catalog entry and manifest metadata.
- Premium entitlement and install-key infrastructure for ThinkWork Brain.
...
- Internal the retired graph substrate-powered substrate adoption/provisioning as the ThinkWork Brain
  infrastructure component.
- Plugin detail UI for premium status, key entry, lifecycle status, and the
  Memory / Ontology link.
- Redirect or reduction of legacy the retired graph substrate entry points once the ThinkWork Brain
  plugin detail path covers install/status needs.
- Smoke coverage for catalog visibility, key redemption, entitlement
  persistence, the retired graph substrate adoption failure evidence, and graph explorer continuity.
...
- Wiki materialization from the retired graph substrate.
...
- Automatic entitlement for tenants merely because legacy the retired graph substrate exists.
- Duplicate internal substrate deployments for tenants that already run the
  legacy the retired graph substrate deployment.
...
- `packages/deployment-runner/src/apps/registry.ts` has the closed managed-app
  adapter contract for `retired_graph_substrate`, `twenty`, and `kestra`. ThinkWork Brain should
  use the the retired graph substrate adapter rather than a new Terraform path.
- `packages/deployment-runner/src/apps/retired_graph_substrate.ts` already declares the retired graph substrate's
...
- `packages/api/src/graphql/resolvers/core/managedApplications.ts` still has
  environment-derived the retired graph substrate status readers. This becomes compatibility glue
...
- `apps/web/src/components/settings/Settingsthe retired graph substrateApplication.tsx`,
  ...
  the retired graph substrate status/action and Memory / Ontology integration points.
...
- `docs/solutions/architecture-patterns/managed-app-mcp-oauth-lifecycle-2026-06-06.md`
  ...
  state machines. THNK-15 should keep ThinkWork Brain entitlement, the retired graph substrate
...
- `docs/solutions/best-practices/retired_graph_substrate-thread-ingest-explorer-2026-06-04.md`
  confirms the retired graph substrate verification should use ThinkWork GraphQL/smoke paths rather
...
- **ThinkWork Brain is a normal Application Plugin with premium metadata.**
...
- **Install gating happens before infrastructure ownership changes.** Company
  Brain install may not adopt or provision the retired graph substrate until the tenant already has
  an entitlement or successfully redeems a key. Existing the retired graph substrate alone is not a
  premium entitlement.

- **Adoption is a no-change infrastructure plan, then ownership flip.** For an
  existing the retired graph substrate tenant, the infra handler creates an adoption/upgrade plan
  through the existing the retired graph substrate adapter and treats "no changes" as the safety
  signal. Only after that evidence exists should the managed app/component be
  marked plugin-owned. If no-change verification fails, leave the legacy the retired graph substrate
  state untouched and surface evidence.

- **New tenant provisioning follows the normal infra component path.** If no
  legacy the retired graph substrate deployment exists, ThinkWork Brain invokes the existing the retired graph substrate
...
- **Legacy the retired graph substrate routes redirect only after parity.** Keep compatibility until
  plugin detail can show entitlement, install status, adoption failure evidence,
  and lifecycle actions. Then redirect `settings.applications.retired_graph_substrate` to the
  ThinkWork Brain plugin detail and hide/reduce the retired graph substrate in the generic managed-app
  list.
...
sequenceDiagram
    ...
    participant Infra as the retired graph substrate infra handler
    participant Runner as Deployment runner

    Admin->>UI: Open Plugins
    UI->>API: Query catalog + tenant plugin state
    API-->>UI: ThinkWork Brain visible, premium, installKeyRequired
    Admin->>UI: Install with key
    UI->>API: installPlugin(brain, installKey)
    API->>Ent: Validate/redeem key or confirm existing entitlement
    Ent-->>API: Active ThinkWork Brain entitlement
    API->>Engine: Start install / component orchestration
    Engine->>Infra: Adopt or provision the retired graph substrate component
...
```mermaid
stateDiagram-v2
    [*] --> visible: catalog includes ThinkWork Brain
    ...
    installing --> awaiting_approval: the retired graph substrate provision plan needs approval
    installing --> adopted: no-change legacy the retired graph substrate adoption succeeds
...
tenant/plugin state, install is plugin lifecycle state, and the retired graph substrate infrastructure
...
```text
packages/plugin-catalog/src/plugins/brain/manifest.ts
packages/plugin-catalog/src/plugins/brain/manifest.test.ts

packages/database-pg/src/schema/plugins.ts
packages/database-pg/graphql/types/plugins.graphql
packages/database-pg/drizzle/<migration>_brain_premium_entitlements.sql
...
apps/web/src/components/settings/plugins/BrainPluginDetail.tsx
apps/web/src/components/settings/plugins/InstallKeyDialog.tsx
apps/web/src/components/settings/plugins/*.test.tsx
apps/web/src/routes/_authed/settings.plugins.brain.tsx
apps/web/src/routes/_authed/settings.applications.retired_graph_substrate.tsx
...
### U1. Add ThinkWork Brain manifest and premium catalog metadata

**Purpose:** Make ThinkWork Brain always discoverable as a premium plugin while
declaring the retired graph substrate as its infrastructure component.

**Requirements covered:** R1, R2, R8, R17. **Flows:** F1. **Acceptance:** AE1.

**Approach:**

- Add a `brain` manifest to the plugin catalog.
...
- Declare a the retired graph substrate infrastructure component using the Application Plugins infra
  component contract rather than adding custom ThinkWork Brain deployment logic.
...
- Catalog query returns ThinkWork Brain for an unentitled tenant.
- Manifest validation fails if `premium` is true but install-key metadata is
  missing.
- Manifest contains exactly the the retired graph substrate infrastructure component for v1 and no
...
- Unentitled tenant sees ThinkWork Brain but install returns `installKeyRequired`
  without a key.
- Valid key starts install and creates entitlement.
- Existing entitlement allows install/update without `installKey`.
- Invalid key does not start the retired graph substrate adoption/provisioning.
- Retrying the same install request is idempotent.

---

### U5. Add the retired graph substrate adoption and provisioning as the ThinkWork Brain infra component

**Purpose:** Move the retired graph substrate under ThinkWork Brain plugin ownership without duplicate
...
- Extend the Application Plugins infra handler so the ThinkWork Brain component
  resolves to the existing the retired graph substrate `ManagedAppAdapter`.
- For tenants with an existing the retired graph substrate managed application, create an adoption
...
- For new tenants, call the normal the retired graph substrate plan/approval/apply path.
- Preserve the retired graph substrate data-impact disclosure for destructive actions and expose the
...
- Existing the retired graph substrate plus entitlement creates a no-change adoption plan and then
  marks the component plugin-owned.
- Adoption failure leaves legacy the retired graph substrate ownership/status untouched and surfaces
  evidence to the plugin detail query.
- New tenant install creates a normal the retired graph substrate plan with existing data-impact
  disclosure.
- Destroy/park actions require the same approval/data-impact treatment as the
  managed-app the retired graph substrate path.
- Deployment job idempotency prevents duplicate plan jobs on user retry.

---

### U6. Build ThinkWork Brain plugin detail and key-entry UX
...
- Add ThinkWork Brain plugin detail UI within the Plugins settings surface from
  THNK-1.
- Show premium state, entitlement state, install status, the retired graph substrate component
...
- Keep copy product-facing: "ThinkWork Brain" is the visible product; the retired graph substrate
...
- Unentitled tenant can see ThinkWork Brain in catalog and detail.
...
### U7. Redirect/reduce legacy the retired graph substrate managed-application surfaces

**Purpose:** Avoid two competing product homes after ThinkWork Brain can own the
...
- Audit current the retired graph substrate entry points and only change each one after U6 exposes
  equivalent status/action affordances.
- Redirect the legacy the retired graph substrate route to ThinkWork Brain plugin detail, with query
  or toast context only if useful.
- Hide or reduce the retired graph substrate in the generic managed-applications list once plugin
  detail is the lifecycle home.
- Keep Memory / Ontology gating compatible during rollout: it can read the
  plugin-owned the retired graph substrate component status first and fall back to legacy
...
- Visiting the old the retired graph substrate settings route lands on or points to ThinkWork Brain
  plugin detail after parity.
- Managed Applications no longer presents the retired graph substrate as an independent install home.
- Memory / Ontology remains visible for tenants with plugin-owned the retired graph substrate.
...
- Add a smoke script that exercises the GraphQL/API path and, where practical,
  ...
  - existing the retired graph substrate adoption failure/success evidence is readable.
...
- `plugins/brain/smoke/brain-plugin-smoke.mjs`
- `docs/solutions/<optional-after-implementation>.md`
- `docs/plans/2026-06-13-002-feat-brain-premium-plugin-plan.md`
...
- Adoption failure has enough evidence for support to decide whether to retry,
  fix infrastructure drift, or leave legacy the retired graph substrate untouched.
...
- **Deployment runner:** the retired graph substrate remains the underlying adapter. Adoption may
  require a new runner status/evidence field if current plan summaries cannot
  clearly assert no-change.
- **Web:** Plugins becomes the ThinkWork Brain home. Memory / Ontology retains
...
- **Future premium plugins:** Entitlement/key work should be generic enough for
  the next premium plugin, not hardcoded to ThinkWork Brain except in manifest
  data and UI copy.
...
- **Risk: Existing the retired graph substrate adoption accidentally changes infrastructure.**
  Mitigation: require no-change evidence before ownership migration and leave
  legacy the retired graph substrate untouched on any drift/error.

- **Risk: UI shows both the retired graph substrate and ThinkWork Brain as separate products.**
...
2. Add ThinkWork Brain manifest and catalog visibility.
3. Validate key generation/redemption in dev with the backdoor and at least one
   generated operator key.
4. Validate new-tenant the retired graph substrate provision path reaches the normal plan approval.
5. Validate existing-tenant the retired graph substrate adoption against a dev tenant with the retired graph substrate
   already running.
6. Enable ThinkWork Brain plugin detail as the primary status home.
7. Redirect/reduce legacy the retired graph substrate settings entry points after parity.
...
- **Existing the retired graph substrate migration:** Entitlement/key first, then no-change adoption
...
- Update operator docs or runbooks with:
  - how ThinkWork generates one-time ThinkWork Brain install keys.
  ...
  - how to direct users from legacy the retired graph substrate routes to ThinkWork Brain plugin
    detail.
...
- ThinkWork Brain is visible in Plugins for an unentitled tenant and clearly
  marked premium/key-gated.
- A valid generated key and the configured dev/test backdoor key both create
  persistent ThinkWork Brain entitlements.
- A redeemed key cannot be reused.
- Existing entitlement allows reinstall/update without another key.
- Install cannot adopt or provision the retired graph substrate before entitlement.
- Existing the retired graph substrate adoption requires no-change evidence and leaves legacy the retired graph substrate
  untouched on failure.
- New tenant ThinkWork Brain install provisions the retired graph substrate through the existing
  managed-app adapter path.
- Plugin detail shows entitlement/install/component status and links to
  Memory / Ontology.
- Legacy the retired graph substrate entry points are redirected or reduced only after plugin detail
  parity.
