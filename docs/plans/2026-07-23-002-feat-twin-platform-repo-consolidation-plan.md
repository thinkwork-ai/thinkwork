...
**KTD amendment (U12, recorded 2026-07-23):** Seam validation verdict — the `twin-mapping/v1` export is sufficient **as-is** to feed ontology description and query compilation (describe = pure render of the export, proven byte-identical by `packages/api/src/lib/twin/describe-ontology.export-seam.test.ts`; the query compiler reads zero Postgres). No format extension needed; fail-loud gates confirmed both sides. **MCP key auth for the moved server:** product-minted `tkt_` keys verified platform-side against a hashed-key manifest published to S3 (`twin-mcp-keys/<tenant>/latest.json`, format-gated, ≤60s cache + S3-event refresh; rotation publishes both hashes before revoking). The OAuth bearer lane stays product-side only — the product `/mcp/twin` endpoint remains an OAuth-terminating thin proxy. **Projector feed (KTD-8 refinement):** platform projector becomes a convergent full-apply of the identity snapshot with a platform-owned cursor, S3-event trigger + hourly reconciliation; bulk-writer debounce lives product-side in the snapshot cutter. Full writeup: thinkwork#4076.
