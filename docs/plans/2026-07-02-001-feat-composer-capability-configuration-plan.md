...
- **The result tree is read-only with jump-to-cause.** The rendered workspace is derived; editing it in place would blur derived-vs-source — the confusion that produced today's scatter. Every tree node deep-links to its owning surface instead: a skill folder to its attach control, a space mount to the space source editor, generated AGENTS.md to the template that produced it.
...
- R2. The tree is read-only; every node carries a jump-to-cause link to the surface that owns it (attach control, space source editor, user tree, or the producing template).
