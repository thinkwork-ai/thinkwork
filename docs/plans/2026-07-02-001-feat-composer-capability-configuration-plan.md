...
### U9. MCP servers as workspace folders

- **Goal:** MCP-server attachment becomes workspace-native, mirroring skills: an `mcp/<server-slug>/` folder in the AGENT workspace source = attached, with a server-managed `mcp/<slug>/.assignment.json` manifest beside it. Attachment is visible in the Composer tree and honestly attributed in the inspector; the runtime repoint and registry demotion follow the same arc skills took (capability-mapping U9/U10, KTD-8).
- **Product decision (settled):** The `mcp/<slug>/` folder's presence in the agent source is the attachment marker — the exact parallel of `skills/<slug>/`. The manifest carries the server name/slug, the registry row id as an endpoint **reference**, the auth pattern name, a Secrets Manager `secretRef` ARN (a reference, never the value), and the enabled tool names — **references only, NEVER tokens or secrets**. Attachment ≠ injection: per-user OAuth stays the render/turn-time injection gate exactly as today (fail-closed, `oauth_missing`-style reasons). The registry page keeps inventory/trust/credentials. `agent_skills`→workspace-file retirement (U9/U10, KTD-8) is the precedent.
- **Sub-units:**
  - **U9a — dual-write + visibility [this PR]:** The MCP_SERVER-class agent grant additionally materializes `mcp/<slug>/.assignment.json` in the agent workspace source (S3, the same bucket/prefix machinery skills use); detach removes the `mcp/<slug>/` prefix. The `agent_mcp_servers` row stays **authoritative for the runtime** — the file is a ship-inert dual-write (`packages/api/src/lib/mcp/assignment-state.ts`). A `buildMcpAssignmentState` pure builder extracts an explicit allowlist of keys from the registry row's `auth_config` (never a spread) so no token-like field can land in the file. An idempotent write-if-absent backfill (`reconcileMcpAssignmentFolders`) runs on every MCP grant/detach for the whole agent — the least-new-machinery hook, riding the config change that already touches the workspace — and is exported for an explicit ops backfill of a pre-existing agent (e.g. the dev platform agent's 5 servers). The `mcp/` folder + manifests flow into the rendered hydrate manifest for free (agent-source files are included generically by prefix; a compose-tuple test pins it). `capabilityInspector` mcp rows report `agent: workspace folder (mcp/<slug>/)` provenance when the file exists, mirroring skill rows — assignability and gating semantics unchanged.
  - **U9b — runtime repoint + `mcp_configs` demotion:** The Pi runtime resolves attached MCP servers from the `mcp/<slug>/.assignment.json` files (via the registry-id reference + per-user OAuth at turn time) instead of `agent_mcp_servers`; the DB read is demoted to a fallback, then retired. Cutover only after the file path is observed serving a live turn (don't cut over before the replacement is proven).
  - **U9c — Composer tree Add/Detach menus:** The Composer workspace tree gains Add/Detach affordances on the `mcp/` folder (grant/detach through the existing `grantCapability`/`detachCapability` mutations), so MCP attachment is operated from the same tree as skills.
- **Files (U9a):** `packages/api/src/lib/mcp/assignment-state.ts` (+ `.test.ts`); `packages/api/src/graphql/resolvers/capabilities/capabilityAssignment.mutations.ts` (+ test); `packages/api/src/graphql/resolvers/capabilities/capabilityInspector.query.ts` (+ test); `packages/api/src/lib/workspace-renderer/compose-tuple.test.ts` (visibility assertion).
- **Test scenarios (U9a):** grant MCP_SERVER materializes `.assignment.json` with the reference-only shape; detach removes the prefix; idempotent re-grant is a no-op file-wise; backfill materializes files for existing attachments and no-ops on second run; rendered manifest includes `mcp/` entries as owner=agent; inspector mcp row provenance names the folder when present; a no-secrets assertion proves no token-like field reaches the file.
- **Verification:** full `pnpm --filter @thinkwork/api test` + typecheck (separate gate) + lint + format:check; `matrix-no-change:` dual-write visibility only — assignability and gating semantics unchanged.
