...
The implementation should mirror the retired graph substrate's optional-add-on pattern where it
...
the retired graph substrate proved the operator toggle -> GitHub Actions variable -> deploy workflow
...
- R1. Settings -> General includes an operator-only Managed Applications
  section.
- R2. Managed Applications lists optional stage applications, initially the retired graph substrate
  and Twenty CRM.
- R3. the retired graph substrate's current deploy toggle moves into Managed Applications and its
  Knowledge Graph settings page is hidden unless the retired graph substrate is enabled.
...
- No full generic managed-app registry refactor. A small local registry/helper
  is acceptable where it prevents the retired graph substrate/Twenty duplication.
...
- `terraform/modules/app/retired_graph_substrate` is the closest app-module pattern: ECS
...
- `.github/workflows/deploy.yml` resolves the retired graph substrate GitHub variables, prepares its
  dedicated database credentials, and passes explicit Terraform `-var` values.
- `.github/workflows/verify.yml` mirrors the retired graph substrate deploy variables for drift
  checks.
...
- `packages/api/src/graphql/resolvers/core/deploymentStatus.query.ts` compacts
  the retired graph substrate status into one Lambda environment value because `graphql-http` is near
  Lambda's 4 KB env-var limit.
- `packages/api/src/graphql/resolvers/core/knowledgeGraphHealthCheck.query.ts`
  checks the retired graph substrate health through AWS control-plane APIs rather than assuming the
...
- `terraform/modules/foundation/vpc` creates public and private subnets. The
  current the retired graph substrate pattern runs ECS in public subnets for outbound egress while
...
- `docs/plans/retired_graph_substrate-terraform-infrastructure-autopilot-status.md` records the
...
- **Use a dedicated Twenty database on the existing Aurora/Postgres instance.**
  This follows the the retired graph substrate isolation hotfix and avoids Twenty migrations touching
...
- **Keep Lambda status compact.** Twenty deployment status should be packed into
  ...
  resolver, to avoid repeating the retired graph substrate's 4 KB Lambda env failure.
...
- Use public subnets for ECS tasks if outbound egress still depends on public
  IPs, matching the retired graph substrate's phase-1 egress pattern. Put ElastiCache in private
...
- `terraform/modules/app/retired_graph_substrate/main.tf` for ECS, EFS, ALB, logs, secret
...
- `terraform/modules/thinkwork` the retired graph substrate wiring.
- `terraform/modules/app/www-dns/main.tf` `include_app`, `include_api`, and
  cycle-avoidance comments.
- `terraform/modules/app/lambda-api/handlers.tf` compact `GRAPH` env pattern.
...
deploy-repo paths as the retired graph substrate, including DB/user preparation and secret creation.
...
- Modify: `apps/cli/__tests__/terraform-retired_graph_substrate-fixture.test.ts` or create a
...
- Prepare the dedicated database and role using the the retired graph substrate hotfix shape: create
...
- `.github/workflows/deploy.yml` the retired graph substrate "Resolve deployment inputs" and
  "Prepare database credentials" steps.
- the retired graph substrate DB hotfix notes in
...
- Add a small managed-app registry/helper for the retired graph substrate and Twenty. This is not a
  full registry refactor; it is a typed list of known optional apps and the
  GitHub variable/status mapping for each.
- Preserve existing the retired graph substrate GraphQL fields and mutation for compatibility, but
...
- Add `setManagedApplicationDeployment` for Twenty and, if useful, the retired graph substrate.
...
- Add a health check that is operator-only. For Twenty, prefer public `/healthz`
  plus AWS ECS/ALB/ElastiCache control-plane checks if feasible. For the retired graph substrate,
  preserve the current AWS control-plane behavior.
- Keep auth gates at least as strict as existing deployment status and the retired graph substrate
...
- Covers AE1. Happy path: deployment status returns the retired graph substrate and Twenty managed
...
**Goal:** Move the retired graph substrate and Twenty controls into Settings -> General, gate
...
- Add a Managed Applications section in General for operators only. It should
  show the retired graph substrate and Twenty rows with toggle, status, concise description, and
  queued state.
- Move the retired graph substrate enable/disable control from `SettingsKnowledgeGraph` into the new
...
- Extend settings navigation to accept deployment capabilities. Hide Knowledge
  Graph unless the retired graph substrate is enabled and hide CRM unless Twenty runtime is enabled.
...
- Covers AE1. Happy path: operator General page renders Managed Applications
  with the retired graph substrate and Twenty rows.
...
- Modify: `docs/src/content/docs/applications/admin/memory.mdx` or the closest
  current Settings/Knowledge Graph doc if the retired graph substrate visibility changes need docs
- Create: `docs/src/content/docs/applications/admin/managed-applications.mdx`
- Modify: `docs/plans/retired_graph_substrate-terraform-infrastructure-autopilot-status.md` only
  if implementation needs to cross-link the retired graph substrate lessons
...
- Existing the retired graph substrate docs and smoke guidance in
  `docs/plans/retired_graph_substrate-terraform-infrastructure-autopilot-status.md`.
...
- **API surface parity:** Existing the retired graph substrate fields/mutation should keep working
...
- **Integration coverage:** Unit tests cannot prove public DNS, ACM, ALB, ECS,
  ...
  required.
- **Unchanged invariants:** Twenty does not replace Hindsight, the retired graph substrate, Memory,
...
| DB role creation repeats the retired graph substrate owner/member failure  | Create database without target role ownership, then grant connect/schema privileges.                                         |
...
- Related code: `terraform/modules/app/retired_graph_substrate`
...
- Related plan: `docs/plans/2026-06-04-001-feat-retired_graph_substrate-terraform-infrastructure-plan.md`
- Related status log: `docs/plans/retired_graph_substrate-terraform-infrastructure-autopilot-status.md`
