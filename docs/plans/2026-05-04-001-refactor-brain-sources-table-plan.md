---
title: "refactor: Clean up ThinkWork Brain Sources tab"
type: refactor
status: completed
date: 2026-05-04
...
# refactor: Clean up ThinkWork Brain Sources tab

## Overview

ThinkWork Brain -> Sources currently mixes actionable source configuration with inert/planned adapters and a redundant source-agent section. The next slice should make the tab feel like the rest of Admin: one compact, scannable source table that only shows sources an operator can actually use or configure today.
...
The ThinkWork Brain v0 requirements introduced sub-agent providers and inert seams so future ERP, CRM, support, and catalog adapters could be activated without reshaping the Context Engine contract. That was useful implementation scaffolding, but the current Admin UI exposes those planned seams as cards with amber "planned" states. This makes the source list look unfinished and dilutes the live operator workflow.

The user request is explicit: remove planned adapters from the visible Sources tab, remove the redundant "ThinkWork Brain source agents" panel, and replace source cards with the shared datatable pattern used across Admin.
...
- R3. The redundant `ThinkWork Brain source agents` panel is removed from the page.
...
- Do not change mobile ThinkWork Brain or runtime provider selection behavior.
...
- `docs/plans/2026-04-29-004-feat-brain-v0-plan.md` intentionally shipped inert seams for future adapters; this plan should hide them from Admin rather than delete the seam contract.
...
- Hide inert source-agent seams in Admin, do not delete them: this satisfies the requested UX cleanup while preserving the ThinkWork Brain v0 seam-swap architecture.
...
- Should planned adapters be deleted from backend provider registration? No. The prior ThinkWork Brain v0 plan intentionally introduced inert seams. This cleanup should make them invisible in Admin operator lists while leaving the backend seam contract untouched.
...
**Requirements:** R1, R2, R5, R6; origin R7-R11 and R31 from `docs/brainstorms/2026-04-29-brain-v0-requirements.md`.
...
**Goal:** Render the ThinkWork Brain source catalog as a shared Admin datatable instead of a card grid.
...
**Goal:** Delete the lower "ThinkWork Brain source agents" section and any dead imports/components created solely for it.
...
- The phrase `ThinkWork Brain source agents` no longer appears in the rendered Sources route or source files.
...
- **Origin document:** [docs/brainstorms/2026-04-29-brain-v0-requirements.md](../brainstorms/2026-04-29-brain-v0-requirements.md)
...
- Related plan: [docs/plans/2026-04-29-004-feat-brain-v0-plan.md](2026-04-29-004-feat-brain-v0-plan.md)
