---
title: "fix: Reorder component nav and rename ThinkWork Brain to Memory"
type: fix
status: completed
date: 2026-05-26
---

# fix: Reorder component nav and rename ThinkWork Brain to Memory

## Overview

Update the docs Components navigation so it reads Agents, Spaces, Threads, then Memory. Keep existing `/concepts/knowledge/` routes stable, but change visible labels and nearby explanatory copy so readers see Memory as the component name instead of ThinkWork Brain.

---

## Problem Frame

The Components sidebar currently presents Threads, Spaces, Agents, then ThinkWork Brain. The requested reader path is Agents first, then Spaces, Threads, and Memory. The naming is also drifting: the concept pages still teach "ThinkWork Brain" as the component label, while current positioning prefers Memory as the visible term for the context layer that carries retained facts, compiled pages, knowledge bases, and source routing.
...
- R2. The visible Components label "ThinkWork Brain" must become "Memory" while preserving current docs URLs.
...
- R4. Adjacent docs and landing-page links should avoid sending readers back to "ThinkWork Brain" as the primary component name.
...
- `docs/astro.config.mjs` statically defines the Starlight sidebar. The Components group currently orders Threads, Spaces, Agents, ThinkWork Brain.
- `docs/src/content/docs/concepts/knowledge.mdx` is the Components overview page for the current ThinkWork Brain label.
...
- Use "Memory" as the component label: The sidebar and overview should teach Memory as the component. "ThinkWork Brain" can remain only where needed as legacy/compatibility vocabulary or where a historical pipeline name is still precise.
- Keep the rename targeted: Update high-signal pages and related links rather than mechanically replacing every historical "ThinkWork Brain" mention inside deep technical docs where the phrase names a specific existing page/pipeline concept.
...
- Should "ThinkWork Brain" disappear everywhere? No. Use Memory as the primary component term, but leave precise historical or compatibility mentions when they explain existing route names or compiled-page terminology.
...
- Rename the ThinkWork Brain group label to Memory without changing its slug values.
- Keep all existing Memory child items unless a child label still over-emphasizes ThinkWork Brain as primary navigation language.
...
- Change frontmatter title from ThinkWork Brain to Memory.
- Rewrite the lead to define Memory as the harness-owned context layer.
- Keep a short compatibility note that older docs and routes may still say ThinkWork Brain or `/concepts/knowledge/`.
...
- Modify targeted files under `docs/src/content/docs/concepts/knowledge/` where "ThinkWork Brain" is used as the current primary component label.
...
- Update related-page labels from ThinkWork Brain to Memory.
- Leave "ThinkWork Brain" in precise historical/compatibility statements only when a sentence would otherwise become misleading.
...
- `rg "ThinkWork Brain" docs/src/content/docs/index.mdx docs/src/content/docs/applications/admin docs/src/content/docs/concepts/knowledge` returns only intentional compatibility or historical/technical mentions.
...
| Risk                                                         | Mitigation                                                                                   |
| ------------------------------------------------------------ | -------------------------------------------------------------------------------------------- |
| Over-renaming breaks historical or precise technical meaning | Keep ThinkWork Brain only where it is explicitly compatibility or historical terminology     |
