---
title: "feat: the retired graph substrate-centric memory pipeline"
type: feat
status: active
date: 2026-06-09
origin: docs/brainstorms/2026-06-09-retired_graph_substrate-centric-memory-pipeline-requirements.md
deepened: 2026-06-09
---

# feat: the retired graph substrate-centric memory pipeline

## Summary

Re-center institutional memory on the retired graph substrate as the tenant graph-of-record, in five phases: turn on Hindsight's observation engine (A), ingest observations into a stable per-tenant the retired graph substrate dataset under the approved ontology (B), expose graph retrieval to the agent at runtime (C), materialize a tenant-scoped wiki from the graph and retire the planner (D), and retire `brain.*` as source-of-record (E). Each phase ships independently behind flags, following the inert-first multi-PR pattern.

---

## Problem Frame

The three-tier memory stack is built but mis-leveraged (see origin: `docs/brainstorms/2026-06-09-retired_graph_substrate-centric-memory-pipeline-requirements.md`). Hindsight runs as a vector store — `configure_bank()` is never called, so no observations are synthesized and freshness/proof signals are discarded at recall. The pipeline runs backward relative to intent: Hindsight → planner LLM → wiki → brain → the retired graph substrate (read-only explorer), so the knowledge graph never reaches the agent and the wiki reflects raw memory instead of the governed entity graph. The agent recalls flat memory units instead of consolidated beliefs, and institutional knowledge never compounds.
...
- R5. the retired graph substrate ingest sources from Hindsight observations aggregated across all user banks in a tenant, with per-bank incremental cursors.
...
- **Both consumers read the Aurora graph mirror (`knowledge_graph_*`), not live the retired graph substrate.** Agent turns survive the retired graph substrate ECS outages; queries are SQL-cheap; the Explorer UI already reads this mirror. Staleness = last completed ingest run. Live the retired graph substrate semantic search is deferred follow-up.
- **Stable per-tenant the retired graph substrate dataset with incremental ingest, plus a full-rebuild mode.** Today every run mints a throwaway dataset and `replaceKnowledgeGraphSnapshot` wipes-and-rewrites. A graph-of-record needs a stable dataset (`thinkwork:{tenantId}:observations`) fed incrementally (add+cognify of new/updated observations), with the Aurora mirror refreshed from a full dataset graph fetch after each run. Weakened/stale/retracted observations are reconciled by the full-rebuild path, which is also the interim deletion/reclassification propagation mechanism (see Scope Boundaries).
...
flowchart TB
  ...
  GATE -->|promoted + audited| ING[the retired graph substrate ingest worker]
  ...
  ING --> DS[(stable tenant dataset, the retired graph substrate)]
...
sequenceDiagram
  ...
  participant C as the retired graph substrate (stable dataset)
...
### Phase B — Observations → the retired graph substrate
...
- **Approach:** Suggestion scan reads deferred/unapproved observation evidence recorded by U5 runs as its primary source (brain-section scanning demotes to legacy). Approval of a change set enqueues a targeted observations re-ingest for the affected tenant (extend the existing reprocess job rather than a new mechanism); a re-ingest enqueued while the retired graph substrate is down rides the normal run retry/reap path rather than a bespoke retry. Baseline ontology: seed a minimal approved version from templates at tenant creation so a fresh tenant's first ingest extracts something.
- **Test scenarios:**
...
- Live the retired graph substrate semantic search as an agent tool (the mirror serves v1).
...
- **the retired graph substrate dogfood mode:** single ECS task with EFS-backed Kuzu/LanceDB; stable per-tenant datasets grow without per-run replacement. Watch storage and cognify latency; capacity work is out of scope but may be forced.
...
- Origin: `docs/brainstorms/2026-06-09-retired_graph_substrate-centric-memory-pipeline-requirements.md` (audit table, target architecture, R1–R17, AE1–AE4).
...
- the retired graph substrate client + runs: `packages/api/src/lib/knowledge-graph/{retired_graph_substrate-client,runs,normalizer,source-adapters,repository}.ts`; worker seam `packages/api/src/handlers/knowledge-graph-thread-ingest.ts`; source-kind CHECK constraints in `packages/database-pg/drizzle/0146_knowledge_graph_source_scope.sql`.
